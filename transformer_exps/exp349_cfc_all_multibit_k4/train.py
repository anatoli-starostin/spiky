...
def _make_qk(seed_offset):
    return MultiBitPermutationLUT(
        ...
        tph=cfg['qk_tph'],
        bit_width=int(cfg['qk_bit_width']),
        random_seed=cfg['random_seed'] + seed_offset,
        initial_weights_noise=cfg['bit_lut_latent_init_std'],
        pre_quant_temperature=float(cfg.get('pre_quant_temperature', 0.1)),
        device=DEVICE,
    )
...
def _make_out(seed_offset):
    # Hybrid: q/k/v stay BitPermLUT; out_proj becomes MultiBitPermutationLUT
    # (K-bit packed, rational-pre-quant, bf16 latent, soft backward).
    # Output is pair-dominance [B, H=1, P=496] — consumed by DominanceToVector
    # in the LUTBlock (Borda + LN), matching the exp329 pattern.
    return MultiBitPermutationLUT(
        n_inputs=H * d_v, n_outputs=E, n_heads=1,
        input_nap=cfg['out_input_nap'], output_nap=cfg['out_output_nap'],
        tph=cfg['out_tph'],
        bit_width=int(cfg['out_bit_width']),
        random_seed=cfg['random_seed'] + seed_offset,
        initial_weights_noise=cfg['bit_lut_latent_init_std'],
        pre_quant_temperature=float(cfg.get('out_pre_quant_temperature', 0.1)),
        device=DEVICE,
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        # MultiBit out_proj emits pair-dominance [B, 1, P_out]; apply
        # DominanceToVector(E) (Borda + LN) as in exp329.
    ...
    def forward(self, x, pos_emb):
        ...
        # Out: concat heads -> MultiBit dominance -> Borda+LN to E.
...
for layer in model.layers:
    # All-MultiBit: q/k/v and out_proj all use MultiBitPermutationLUT.
    multi_bit_luts += [layer.q_perm, layer.k_perm, layer.v_perm, layer.out_proj]

adam_params = list(model.parameters())
print(f'multi-bit LUTs: {len(multi_bit_luts)} modules')
...
print(f'Q/K MultiBit:  in_nap={cfg["qk_input_nap"]} out_nap={cfg["qk_output_nap"]} tph={cfg["qk_tph"]} bit_width={cfg["qk_bit_width"]} d_qk={d_qk} P_qk={D_QK_P}')
print(f'V MultiBit:    in_nap={cfg["v_input_nap"]} out_nap={cfg["v_output_nap"]} tph={cfg["v_tph"]} bit_width={cfg["v_bit_width"]} d_v={d_v} P_v={D_V_P}')
print(f'Out MultiBit:  in_nap={cfg["out_input_nap"]} out_nap={cfg["out_output_nap"]} tph={cfg["out_tph"]} bit_width={cfg["out_bit_width"]}')
...
summary = {
    ...
    'n_multi_bit_luts': len(multi_bit_luts),
