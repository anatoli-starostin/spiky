"""
exp150_hard_attention — Fork of exp149.

Hard attention via STE in RankAttention (smooth_mode=False).
hard_temperature annealed 1.0 -> 0.01 linearly over first 50% of training.
LayerNorm after each LUT output. Quant reg D=256. 50k steps, bs=128.
"""
...
# ── Hard temperature annealing ─────────────────────────────────────────────────

def get_hard_temperature(step, n_steps, t_start=1.0, t_end=0.01, anneal_fraction=0.5):
    anneal_steps = int(anneal_fraction * n_steps)
    if step >= anneal_steps:
        return t_end
    return t_start + (t_end - t_start) * step / max(anneal_steps, 1)


def set_hard_temperature(model, temp):
    for m in model.modules():
        if isinstance(m, RankAttention):
            m.hard_temperature = temp
...
csv_w.writerow(['step', 'train_loss', 'qreg_loss', 'lambda', 'hard_temp', 'val_loss', 'val_loss_snapped'])
...
for step in range(cfg['n_steps']):
    hard_temp = get_hard_temperature(step, cfg['n_steps'])
    set_hard_temperature(model, hard_temp)
    ...
    if step % 100 == 0:
        ...
        print(f'step {step:6d} | train_loss={ema:.4f} | qreg={ema_qreg:.4f} | lambda={lam:.5f} | hard_temp={hard_temp:.4f} | lr={lr:.2e} ({phase})')
    ...
    if step % cfg['test_every'] == 0:
        ...
        print(f'[VAL] step {step}: val_loss={val_loss:.4f} | snapped={val_loss_snapped:.4f} | gap={val_loss_snapped - val_loss:.4f}')
        train_losses.append(ema)
        val_losses.append(val_loss)
        steps_log.append(step)
        csv_w.writerow([step, f'{ema:.6f}', f'{ema_qreg:.6f}', f'{lam:.6f}', f'{hard_temp:.6f}', f'{val_loss:.6f}', f'{val_loss_snapped:.6f}'])
        csv_f.flush()
