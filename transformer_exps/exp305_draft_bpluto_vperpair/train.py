"""exp305_draft_bpluto_vperpair -- fork of exp304 with DraftBPLUTOVPerPair.

Replaces BitPermutationLUTOptimizer with DraftBPLUTOVPerPair:
  - m per-weight fp8 (per-pair amax scale)
  - v per-pair scalar fp32 (EMA of mean(g^2) over pair group)
  - latent_scale per-pair fp32
  - AdamW-style weight decay on latent (config: bit_lut_weight_decay)
  - Safety clamp ±256 (vs ±10 in production optimizer)

Pure-PyTorch draft — no CUDA kernel dispatches. Slower per step than the
production BitPermutationLUTOptimizer, but identical autograd interface.
"""
...
_INIT_NOISE = cfg.get('bit_lut_latent_init_std', 0.001)
_SOFT_BACKWARD = cfg.get('bit_lut_soft_backward', False)
...
def _make_qk(seed_offset):
    return BitPermutationLUT(
        ...
        initial_weights_noise=_INIT_NOISE,
        soft_backward=_SOFT_BACKWARD,
        device=DEVICE,
    )
...
print(f'bit_lut_weight_decay={cfg.get("bit_lut_weight_decay", 0.01)}')
