...
class Model(nn.Module):
    def __init__(self):
        ...
        # Single BitMultiHeadLUT(concat_dim -> VOCAB_SIZE) maps the transformer
        # output directly to logits — flat output channel space, no Borda /
        # pair-dominance interpretation. partition_sets restricts input
        # anchor pairs to within-layer comparisons (same as exp410).
        fcfg = cfg['final_lut']
        offset = 0
        partition_sets = []
        for pe in _PER_LAYER_E:
            ...
            offset += pe
        self.final_lut = BitMultiHeadLUT(
        ...
        # Log-space learnable scale: α = exp(log_logit_scale) is always
        # positive regardless of Adam updates, and log-space updates
        # correspond to multiplicative changes in α — cleaner optimization
        # than learning α directly with a positivity constraint. Init
        # log_logit_scale = log(0.0511) ≈ -2.974 so initial α matches the
        # CLT-target init that brings logit std to ~1.0.
        self.log_logit_scale = nn.Parameter(torch.tensor(
            float(cfg.get('log_logit_scale_init', -2.974))
        ))
        # Per-token additive bias (zero-init): captures the unigram token
        # distribution without burning bit-LUT vote capacity on it.
    ...
    def forward(self, tokens):
        ...
        B, T, _ = concat.shape
        logits = self.final_lut(concat.reshape(B * T, -1)).squeeze(1)  # [B*T, V]
        logits = logits * self.log_logit_scale.exp() + self.logit_bias
