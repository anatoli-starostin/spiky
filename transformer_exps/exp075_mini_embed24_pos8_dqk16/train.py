"""
exp075_mini_embed24_pos8_dqk16 — embedding_dim=24, positional_dim=8, d_qk=16, d_v=6.
Positional encodings are kept separate and concatenated to x inside each layer before
q/k/v LUTs (input_dim=32). out_proj and ffn operate on content only (24-dim).
tph=256, nap=4, FULL_COVERAGE. 50k steps.
"""
import sys, os, json
import torch
...
class LUTBlock(nn.Module):
    """
    Transformer block with separate content and positional streams.
    q/k/v LUTs receive cat(x, pos) as input; out_proj and ffn use content only.
    """
    ...
    def forward(self, x, pos):
        ...
        x_pos = torch.cat([x, pos], dim=-1)          # [B, T, 32]
        x_pos_flat = x_pos.reshape(-1, E + pos.shape[-1])

        q = self.q_lut(x_pos_flat).permute(1, 0, 2)  # [h, B*T, d_qk]
        k = self.k_lut(x_pos_flat).permute(1, 0, 2)
        v = self.v_lut(x_pos_flat).permute(1, 0, 2)  # [h, B*T, d_v]

        q = q.reshape(self.n_heads, B, T, self.d_qk).permute(1, 0, 2, 3)  # [B, h, T, d_qk]
        k = k.reshape(self.n_heads, B, T, self.d_qk).permute(1, 0, 2, 3)
        v = v.reshape(self.n_heads, B, T, self.d_v).permute(1, 0, 2, 3)   # [B, h, T, d_v]

        attn = self.rank_attn(q, k, v, is_causal=True)          # [B, h, T, d_v]
        attn = attn.permute(0, 2, 1, 3).reshape(B * T, E)       # [B*T, h*d_v=d=24]
...
class LUTTransformerV2(nn.Module):
    ...
    def forward(self, tokens):
        B, T = tokens.shape
        x = self.token_embedder(tokens)                   # [B, T, 24]
        pos = self.pos_emb[:, :T].expand(B, -1, -1)      # [B, T, 8]
