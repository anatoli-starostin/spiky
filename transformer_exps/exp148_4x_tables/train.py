"""
exp145_quant_reg — Fork of exp135.

Quantization regularization: L_quant = lambda * mean(sin^2(pi * w * D))
D=64 → grid spacing 1/64 ≈ 0.0156.
Lambda annealed 0 -> 0.01 linearly over last 80% of training (steps 10k-50k).

At each checkpoint also evaluates val_loss after snapping weights to the grid.
"""
...
for step in range(cfg['n_steps']):
    ...
    if step % cfg['test_every'] == 0:
        ...
        csv_w.writerow([step, f'{ema:.6f}', f'{ema_qreg:.6f}', f'{lam:.6f}', f'{val_loss:.6f}', f'{val_loss_snapped:.6f}'])
        csv_f.flush()
