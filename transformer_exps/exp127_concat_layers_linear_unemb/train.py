"""
exp127_concat_layers_linear_unemb — Fork of exp091.

Changes from exp091:
- No residual connections in LUTBlock (pure attention output per layer)
- Intermediate outputs from every layer are collected (including initial embedding)
- All (num_layers+1) outputs concatenated along feature dim before unembedder
- Unembedder is nn.Linear((num_layers+1)*d, vocab_size) instead of MultiHeadLut
"""
import sys, os, json
import torch
...
class LUTBlock(nn.Module):
    """Attention block without residual connection — returns pure attention output."""
    def __init__(self, cfg, layer_idx):
        ...
        d_qk    = cfg['d_qk']                       # 16
        d_v     = cfg['d_v']                        # 16
        tph_qkv = cfg['qkv_tables_per_head']        # 384
    ...
    def forward(self, x, pos):
        ...
        # No residual: return pure attention output
        return self.out_proj(attn)[:, 0, :].reshape(B, T, E)


class LUTTransformerConcat(nn.Module):
    ...
    def __init__(self, cfg, maxlen=CONTEXT_SIZE):
        ...
        # Linear unembedder: input is concat of initial embedding + all layer outputs
        self.unembedder = nn.Linear((n + 1) * d, cfg['vocab_size'], bias=False)
    ...
    def forward(self, tokens):
        ...
        # Concatenate all intermediates: [B, T, (n_layers+1)*d]
        concat = torch.cat(intermediates, dim=-1)
        return self.unembedder(concat)


# ── Run ────────────────────────────────────────────────────────────────────────
sampler = make_sampler(DEVICE, random_seed=1)
model = LUTTransformerConcat(cfg).to(DEVICE)
