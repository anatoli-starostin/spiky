"""
exp122_hierarchical_d64_dqk16_nap4 — Fork of exp119.

Changes from exp119:
- embedding_dim=64, positional_dim=64 → interleaved input_dim=128 (auto->1984 tph)
- d_qk=d_v=16, out_proj input_dim=64 (auto->480 tph)
- nap=4
"""
import sys, os, json
import torch
...
class LUTBlock(nn.Module):
    def __init__(self, cfg, layer_idx):
        super().__init__()
        d       = cfg['embedding_dim']              # 64
        p       = cfg['positional_dim']             # 64
        h       = cfg['num_heads']                  # 4
        d_qk    = cfg['d_qk']                       # 16
        d_v     = cfg['d_v']                        # 16
        tph_qkv = cfg['qkv_tables_per_head']        # 4096 (auto->1984)
        tph_op  = cfg['out_proj_tables_per_head']   # 4096 (auto->480)
        lut_input_dim = d + p                       # 128 (interleaved)
        s = layer_idx * 10
    ...
    def forward(self, x, pos):
        B, T, E = x.shape
        # Interleave: [e1,p1,e2,p2,...,e64,p64] -> [B*T, 128]
