...
class Model(nn.Module):
    def __init__(self):
        ...
        # stored as [V, E] fp32 vectors. Input is used directly as the
        # continuous layer-0 input. Output is used via standard dot product
        # against a D2V-projected E-dim vector from the final_lut (no V2D /
        # Kendall-tau on the output side).
        ...
        # D2V (Borda + LN) to compress ±1 pair-dominance back to an E-dim
        # vector. Logits = pair_vec @ token_embedder_out.weight.T (standard
        # dot-product classifier).
        self.final_d2v = DominanceToVector(E)
        self.log_logit_scale = nn.Parameter(torch.zeros(1))
    ...
    def forward(self, tokens):
        ...
        pair = self.final_canon(pair)                              # [B*T, 1, P_E] ±1
        pair_vec = self.final_d2v(pair).squeeze(1)                 # [B*T, E] Borda+LN
        # MSE-based logits: logit_v = -||pair_vec - emb[v]||². Direct
        # broadcast form — the softmax picks the nearest prototype in
        # Euclidean space. Embedding norm is a natural regularizer.
        emb = self.token_embedder_out.weight                       # [V, E]
        dist_sq = (pair_vec.unsqueeze(1) - emb.unsqueeze(0)).pow(2).sum(dim=-1)  # [B*T, V]
        logits = self.log_logit_scale.exp() * (-dist_sq) + self.token_bias
