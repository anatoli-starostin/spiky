"""
exp282_fullperm_sumpos — exp280 but positional embeddings summed (pos_dim=E=32).
Full perm Q/K/V/out_proj + RankAttention + dominance V. ctx=32, 25K bs=64.
"""
...
class FullPermBlock(nn.Module):
    ...
    def forward(self, x, pos_emb):
        B, T, _E = x.shape
        # Sum positional embeddings for Q/K
        xp = x + pos_emb.unsqueeze(0)  # [B, T, E]

        q = self.q_perm(xp.reshape(B * T, _E))  # [B*T, H, d_qk]
        k = self.k_perm(xp.reshape(B * T, _E))
