...
class Model(nn.Module):
    ...
    def forward(self, tokens):
        ...
        # CLIP-style linear logits: dot product / sqrt(P_E) with a learnable
        # scale. Simpler / smoother than atanh; equivalent to the Bayesian
        # optimum under "each pair is an independent Bernoulli with fixed
        # confidence" — logit ∝ (#agree - #disagree).
        P_E = tok_dom.shape[-1]
        dot = pair @ tok_dom.T                                     # [B*T, V]
        base = dot / math.sqrt(P_E)                                 # std ~1 at init
