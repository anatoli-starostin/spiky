...
from spiky.lutorch.bit_permutation_lut_ex import BitPermutationLUTLin
...
def _make_out(seed_offset):
    return BitPermutationLUTLin(
        ...
        random_seed=cfg['random_seed'] + seed_offset,
        initial_weights_noise=cfg['bit_lut_latent_init_std'],
        soft_backward=cfg.get('bit_lut_soft_backward', False),
        latent_dtype=cfg.get('bit_lut_latent_dtype', 'fp8'),
        device=DEVICE,
    )
...
for layer in model.layers:
    # For Ex, the optimizer hooks on `.voting` (Part 1) — that's what emits
    # the [B, H, V_ex] votes matching output_idx_per_table for the weight-grad projection.
    bit_luts += [layer.q_perm, layer.k_perm, layer.v_perm, layer.out_proj.voting]
...
print(f'Out BitPermLUTLin: in_nap={cfg["out_input_nap"]} in_tph={cfg["out_input_tph"]} '
      f'voting_nap={cfg["out_voting_nap"]} V={cfg["out_V"]} (learnable readout V->P)')
