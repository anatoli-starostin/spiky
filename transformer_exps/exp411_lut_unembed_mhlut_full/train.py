...
class Model(nn.Module):
    def __init__(self):
        ...
        # LUT-based unembedder: final_lut (concat_dim -> FINAL_N_OUT) with
        # partition_sets that restrict anchors to within-layer pairs, followed
        # by DominanceToVector + MultiHeadLut(FINAL_N_OUT -> VOCAB_SIZE).
        fcfg = cfg['final_lut']
        self.final_n_out = int(fcfg['n_outputs'])
        # Partition concat into one set per layer (each set = that layer's e_out dims).
        offset = 0
        partition_sets = []
        ...
        self.final_lut = BitPermutationLUT(
            n_inputs=concat_dim, n_outputs=self.final_n_out, n_heads=1,
        ...
        self.final_p = self.final_n_out * (self.final_n_out - 1) // 2
        # Borda projection of final pair-dominance back to dim=final_n_out, then
        # MultiHeadLut(final_n_out -> V) with hard mode (smooth_mode=False, n_alt=1).
        ucfg = cfg['unembed_lut']
        self.final_to_vec = DominanceToVector(self.final_n_out)
        self.unembed_lut = MultiHeadLut(
            input_dim=self.final_n_out,
            n_heads=1,
            n_outputs=VOCAB_SIZE,
            n_anchor_pairs=int(ucfg['nap']),
            tables_per_head=int(ucfg['tph']),
            smooth_mode=bool(ucfg.get('smooth_mode', False)),
            n_alternatives=int(ucfg.get('n_alternatives', 1)),
            random_seed=cfg['random_seed'] + 901,
            initial_weights_noise=float(ucfg.get('initial_weights_noise', 0.001)),
            device=DEVICE,
        )
    ...
    def forward(self, tokens):
        ...
        B, T, _ = concat.shape
        final_dom = self.final_lut(concat.reshape(B * T, -1))       # [B*T, 1, P]
        final_vec = self.final_to_vec(final_dom).squeeze(1)         # [B*T, final_n_out]
        logits = self.unembed_lut(final_vec).squeeze(1)             # [B*T, V]
...
bit_luts.append(model.final_lut)
