...
Q/K LUTs take cat(x, pos_emb), V LUT takes x, SDPA attention, out_proj LUT, no FFN.
"""
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        super().__init__()
        # Q and K take cat(x, pos_emb) → input_dim = E + P = 48
    ...
    def forward(self, x, pos_emb):
        ...
        # Concatenate position info for Q and K
        xp = torch.cat([x, pos_emb.unsqueeze(0).expand(B, -1, -1)], dim=-1)  # [B, T, E+P]
        xp_flat = xp.reshape(B * T, E + P)
        x_flat = x.reshape(B * T, _E)

        # Q, K, V projections via LUT
        q = self.q_lut(xp_flat).reshape(B, T, H, d_qk).permute(0, 2, 1, 3)  # [B, H, T, d_qk]
        k = self.k_lut(xp_flat).reshape(B, T, H, d_qk).permute(0, 2, 1, 3)
        v = self.v_lut(x_flat).reshape(B, T, H, d_v).permute(0, 2, 1, 3)     # [B, H, T, d_v]

        # Standard scaled dot-product attention
        attn = F.scaled_dot_product_attention(q, k, v, is_causal=True)  # [B, H, T, d_v]
        attn = attn.permute(0, 2, 1, 3).reshape(B * T, H * d_v)        # [B*T, H*d_v]
