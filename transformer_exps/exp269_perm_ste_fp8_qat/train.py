...
# fp8 QAT setup: find all inner LUT weight parameters of every PermutationalLut out_proj.
# Before each forward we quantize their .data to fp8 with per-tensor dynamic scaling
# (round-trip: scale up to fp8 amax, cast, cast back, scale down).
# After loss.backward() we restore the fp32 master.
FP8_QAT = cfg.get('fp8_qat', False)
FP8_DTYPE_MAP = {'e4m3': torch.float8_e4m3fn, 'e5m2': torch.float8_e5m2}
FP8_DTYPE = FP8_DTYPE_MAP[cfg.get('fp8_dtype', 'e4m3')]
FP8_AMAX_MAP = {'e4m3': 448.0, 'e5m2': 57344.0}
FP8_AMAX = FP8_AMAX_MAP[cfg.get('fp8_dtype', 'e4m3')]
perm_weights = []  # list of Parameter tensors to quantize
if FP8_QAT:
    for layer in model.layers:
        if hasattr(layer.out_proj, 'inner'):
            perm_weights.append(layer.out_proj.inner.projection.weights)
    print(f'fp8 QAT: {len(perm_weights)} inner-LUT weight tensors '
          f'(dtype={cfg.get("fp8_dtype", "e4m3")}, max={FP8_AMAX})')


def fp8_quantize_inplace(tensors):
    """Save masters and overwrite .data with fp8 round-trip + per-tensor dynamic scale.

    scale = FP8_AMAX / amax(w)   (so w * scale fills the fp8 range)
    w_q    = round(w * scale, fp8) / scale
    """
    masters = []
    eps = 1e-8
    for w in tensors:
        masters.append(w.data.clone())
        amax = w.data.abs().amax().clamp(min=eps)
        scale = FP8_AMAX / amax
        scaled = w.data * scale
        q = scaled.to(FP8_DTYPE).to(torch.float32)
        w.data.copy_(q / scale)
    return masters


def restore_masters(tensors, masters):
    for w, m in zip(tensors, masters):
        w.data.copy_(m)
...
for step in range(cfg['n_steps']):
    ...
    masters = fp8_quantize_inplace(perm_weights) if FP8_QAT else None
    ...
    # Restore fp32 masters BEFORE optimizer.step so the update is applied to the
    # master weights. Gradients were computed w.r.t. the quantized values, but
    # autograd treats the parameter as the identity — STE behaviour.
    if FP8_QAT:
        restore_masters(perm_weights, masters)
