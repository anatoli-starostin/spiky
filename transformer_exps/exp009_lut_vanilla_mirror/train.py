"""
exp009_lut_vanilla_mirror — LUT transformer mirroring vanilla architecture.
q/k/v/out_proj/ffn/unembedder all MultiHeadLut. SDPA attention. No norms. Residuals.
"""
import sys, os, json
import torch
...
class LUTBlock(nn.Module):
    ...
    def __init__(self, cfg, layer_idx):
        ...
        self.ffn = MultiHeadLut(
            ...
            initial_weights_noise=cfg['initial_weights_noise'],
            uncertainty_mode=UncertaintyMode.INVERSE_L1,
            n_post_processor_inputs=cfg['hidden_dim_ffn'],
            random_seed=cfg['random_seed'] + s + 4,
            device=DEVICE,
        )
        self.n_heads = h
        self.d_head = d_head
    ...
    def forward(self, x):
        ...
        ffn_out = self.ffn(x.reshape(-1, E))[:, 0, :]          # [B*T, E]
        x = x + ffn_out.reshape(B, T, E)
...
class LUTTransformerV2(nn.Module):
    ...
    def __init__(self, cfg, maxlen=CONTEXT_SIZE):
        super().__init__()
        d = cfg['embedding_dim']
        self.token_embedder = nn.Embedding(cfg['vocab_size'], d // 2)
        self.token_embedder.weight.data.uniform_(-0.1, 0.1)
        self.register_buffer('pos_emb', torch.randn(1, maxlen, d // 2) * 0.1)
    ...
    def forward(self, tokens):
        B, T = tokens.shape
        z = self.token_embedder(tokens)                                      # [B, T, d//2]
        z = torch.cat([z, self.pos_emb[:, :T].expand(B, -1, -1)], dim=-1)  # [B, T, d]
