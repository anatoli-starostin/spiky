"""
exp119_multiscale_interleaved_pe — Fork of exp118.

Changes from exp118:
- positional_dim=32 (same as embedding_dim)
- Positional embedding interleaved with token embedding: [e1,p1,e2,p2,...,e32,p32]
  giving lut input_dim=64 instead of concatenated 48
- q/k/v and out_proj both auto->651 tph with MULTISCALE nap=3
"""
import sys, os, json
import torch
...
class LUTBlock(nn.Module):
    def __init__(self, cfg, layer_idx):
        ...
        p       = cfg['positional_dim']             # 32
        h       = cfg['num_heads']                  # 4
        d_qk    = cfg['d_qk']                       # 16
        d_v     = cfg['d_v']                        # 16
        tph_qkv = cfg['qkv_tables_per_head']        # 2048 (auto->651)
        tph_op  = cfg['out_proj_tables_per_head']   # 2048 (auto->651)
        lut_input_dim = (d + p)                     # 64 (interleaved)
        s = layer_idx * 10
    ...
    def forward(self, x, pos):
        B, T, E = x.shape
        # Interleave: [e1,p1,e2,p2,...,e32,p32] -> [B*T, 64]
        x_pos_flat = torch.stack([x, pos], dim=-1).reshape(B * T, E + pos.shape[-1])
