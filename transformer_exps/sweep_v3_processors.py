"""
Sweep: SignedDiff and LinearCombination pairs processors on top-5 configs.
SignedDiff: cat([x_i - x_j, rpe]) — signed, no abs. dim = E + P.
LinearComb: cat([Linear(2E → E)(cat([x_i, x_j])), rpe]) — learned mix. dim = E + P.
"""
...
class SignedDiffProcessor(nn.Module):
    """cat([x_i - x_j, rpe]). dim = E + pos_dim."""
    def forward(self, x_i, x_j, rpe):
        return torch.cat([x_i - x_j, rpe], dim=-1)


class LinearCombProcessor(nn.Module):
    """cat([Linear(2E → E)(cat([x_i, x_j])), rpe]). dim = E + pos_dim."""
    def __init__(self, E):
        super().__init__()
        self.proj = nn.Linear(2 * E, E, bias=False)

    def forward(self, x_i, x_j, rpe):
        combined = self.proj(torch.cat([x_i, x_j], dim=-1))
        return torch.cat([combined, rpe], dim=-1)


def make_model(cfg, processor_cls):
    E = cfg['E']
    P = cfg['P']
    H = cfg['H']
    d_v = cfg['d_v']
    score_input_dim = E + P  # both processors output E + P

    def _make_score(seed_offset):
        if processor_cls == LinearCombProcessor:
            proc = processor_cls(E)
        else:
            proc = processor_cls()
        lut = MultiHeadLut(
...
def run_config(cfg, processor_cls):
    torch.manual_seed(42)
    model = make_model(cfg, processor_cls)
...
BASE_CONFIGS = [
    dict(E=32, P=16, H=4, d_v=16, a_nap=6, a_tph=128, v_nap=4, v_tph=256, o_nap=4, o_tph=1536,
         name="v256_op1536"),
    dict(E=32, P=16, H=4, d_v=16, a_nap=6, a_tph=128, v_nap=4, v_tph=128, o_nap=4, o_tph=1536,
         name="op1536"),
    dict(E=32, P=16, H=4, d_v=16, a_nap=6, a_tph=128, v_nap=4, v_tph=128, o_nap=4, o_tph=2048,
         name="op2048"),
    dict(E=32, P=16, H=4, d_v=16, a_nap=6, a_tph=256, v_nap=4, v_tph=128, o_nap=4, o_tph=1024,
         name="attn256_op1024"),
    dict(E=32, P=16, H=4, d_v=32, a_nap=6, a_tph=128, v_nap=4, v_tph=128, o_nap=4, o_tph=1024,
         name="dv32_heavy_op"),
]

PROCESSORS = [
    ("signed_diff", SignedDiffProcessor),
    ("linear_comb", LinearCombProcessor),
]


if __name__ == '__main__':
    print(f"{'label':<36s} {'params':>8s} {'loss@5k':>8s} {'time':>6s}")
    print("-" * 62)
    results = []
    for proc_name, proc_cls in PROCESSORS:
        for base in BASE_CONFIGS:
            cfg = {k: v for k, v in base.items() if k != 'name'}
            label = f"{base['name']}_{proc_name}"
            n_params, ema, info = run_config(cfg, proc_cls)
            loss_str = f"{ema:.4f}"
            print(f"{label:<36s} {n_params:>8,d} {loss_str:>8s} {info:>6s}")
    ...
    for label, n_params, ema in results:
        print(f"{label:<36s} {n_params:>8,d} {ema:.4f}")
