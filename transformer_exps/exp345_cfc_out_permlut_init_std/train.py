...
# out_proj input is the H*d_v-wide concat of attention heads. With
# `out_partition_by_head=True`, restrict input-anchor pairs to within-head
# pairs so no anchor ever couples features from different heads.
_OUT_PARTITION_SETS = (
    [list(range(h * d_v, (h + 1) * d_v)) for h in range(H)]
    if cfg.get('out_partition_by_head', False)
    else None
)


def _make_out(seed_offset):
    # Hybrid: only out_proj is a PermutationalLut (smooth-rational).
    # q/k/v stay BitPermLUT. See exp345 config for shape.
    # `permlut_init_std` controls PermLut's LProjection weight init (Gaussian).
    # Larger init puts more weights in rational's saturated regime.
    return PermutationalLut(
        n_inputs=H * d_v, n_outputs=E, n_heads=1,
        input_nap=cfg['out_input_nap'], output_nap=cfg['out_output_nap'],
        tph=cfg['out_tph'],
        pair_mode='scrambled', soft_mode='rational', aggregation='matmul',
        random_seed=cfg['random_seed'] + seed_offset,
        initial_weights_noise=cfg.get('permlut_init_std', cfg['bit_lut_latent_init_std']),
        device=DEVICE,
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        # PermLut out_proj emits an E-dim vector directly (no pair dominance /
        # Borda projection). Keep the LN that exp338's DominanceToVector(E)
        # used after out_proj by replacing it with a plain nn.LayerNorm(E).
    ...
    def forward(self, x, pos_emb):
        ...
        # Out: concat heads -> PermLut(E-dim vector, smooth-rational) -> LN.
        out_in = attn.permute(0, 2, 1, 3).reshape(B * T, H * d_v)
        out_vec = self.out_proj(out_in).squeeze(1).reshape(B, T, _E)   # [B, T, E]
        return self.out_ln(out_vec)
...
for layer in model.layers:
    # Hybrid: out_proj is PermutationalLut, uses regular Adam (via
    # model.parameters()) — NOT the BitPermutationLUTOptimizer.
    bit_luts += [layer.q_perm, layer.k_perm, layer.v_perm]

# Separate PermLut weights into their own param group with a higher LR.
# BitPermLUT has STE-gated adaptive updates that keep pushing uncertain bits;
# PermLut's rational Jacobian vanishes as |w| grows, so it needs a higher LR
# to keep up during late training.
permlut_weight_ids = set()
for layer in model.layers:
    if hasattr(layer.out_proj, 'inner'):       # PermutationalLut
        for p in layer.out_proj.parameters():
            permlut_weight_ids.add(id(p))
permlut_params, other_params = [], []
for p in model.parameters():
    if id(p) in permlut_weight_ids:
        permlut_params.append(p)
    else:
        other_params.append(p)
permlut_lr_mult = float(cfg.get('permlut_lr_multiplier', 1.0))
print(f'bit LUTs: {len(bit_luts)} modules')
print(f'Adam params (base): {sum(p.numel() for p in other_params):,}  '
      f'| PermLut params: {sum(p.numel() for p in permlut_params):,}  '
      f'| PermLut LR mult: {permlut_lr_mult}')

adam_opt = torch.optim.Adam([
    {'params': other_params,   'lr': cfg['adam_lr']},
    {'params': permlut_params, 'lr': cfg['adam_lr'] * permlut_lr_mult},
])
...
print(f'Out PermLut:    in_nap={cfg["out_input_nap"]} out_nap={cfg["out_output_nap"]} tph={cfg["out_tph"]}  (smooth-rational, partition-sets)')
