"""
exp216_uniform_lut — All LUTs uniform: input_dim=32, nap=5, tph=128.
Position: per-layer sum. d_qk=8, d_v=8, H=4. Q/K/V/OutProj/FFN all identical LUT specs.
"""
...
def _make_lut(n_heads, n_outputs, seed_offset, init_noise=0.001):
    return MultiHeadLut(
        ...
        initial_weights_noise=init_noise, uncertainty_mode=UncertaintyMode.INVERSE_L1,
        random_seed=cfg['random_seed']+seed_offset, device=DEVICE, recompute_in_backward=True,
    )


QK_INIT = cfg.get('qk_init_noise', 0.001)


class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        super().__init__()
        self.q_lut = _make_lut(H, d_qk, layer_idx, init_noise=QK_INIT)
        self.k_lut = _make_lut(H, d_qk, 100+layer_idx, init_noise=QK_INIT)
...
print(f'Uniform LUTs: input_dim={E}, nap={NAP}, tph={TPH}, d_qk={d_qk}, d_v={d_v}')
