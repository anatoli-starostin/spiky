"""
exp241_progressive_outproj — Progressive out_proj nap/tph across layers.
Early layers: many shallow tables. Late layers: fewer deep tables.
Q/K nap=5 tph=128, V nap=6 tph=128, no FFN.
"""
...
NAP_V = cfg['nap_v']
TPH = cfg['tph']
OUTPROJ_SCHEDULE = cfg['outproj_schedule']
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        op = OUTPROJ_SCHEDULE[layer_idx]
        self.out_proj = _make_lut(1, E, op['nap'], op['tph'], 400+layer_idx)
...
for i, op in enumerate(OUTPROJ_SCHEDULE):
    print(f'  L{i} out_proj: nap={op["nap"]} tph={op["tph"]}')
