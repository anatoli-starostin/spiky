...
from transformer_exps.common import (
    make_sampler, count_params, save_summary,
    BOS_ID, CONTEXT_SIZE,
)


def evaluate_model(model, sampler, batch_size):
    """Eval without text generation (avoids fp16 multinomial issues)."""
    import torch.nn.functional as F
    model.eval()
    losses = []
    with torch.no_grad():
...
NAP = cfg['nap']
TPH = cfg['tph']
USE_FP16 = cfg.get('use_fp16_weights', False)
...
class LUTBlock(nn.Module):
    ...
    def forward(self, x, pos_emb):
        ...
        q = self.q_lut(xp_flat).float().reshape(B, T, H, d_qk).permute(0, 2, 1, 3)
        k = self.k_lut(xp_flat).float().reshape(B, T, H, d_qk).permute(0, 2, 1, 3)
        q = self.q_norm(q)
        k = self.k_norm(k)
        v = self.v_lut(x_flat).float().reshape(B, T, H, d_v).permute(0, 2, 1, 3)

        attn = F.scaled_dot_product_attention(q, k, v, is_causal=True)
        proj = self.out_proj(attn.permute(0, 2, 1, 3).reshape(B*T, H*d_v)).float().squeeze(1).reshape(B, T, _E)
        x = x + self.norm1(proj)

        ffn_out = self.ffn(x.reshape(B*T, _E)).float().squeeze(1).reshape(B, T, _E)
