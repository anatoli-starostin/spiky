"""
exp156_sdpa_ste — fork of exp155.

STE over SDPA in non-smooth mode:
  - forward: sdpa_forward_temperature annealed 1.0 -> 0.01 over first 25k steps
  - backward: sdpa_temperature=1.0 (normal soft attention)
...
# ── sdpa_forward_temperature annealing ────────────────────────────────────────

def get_sdpa_forward_temperature(step, anneal_steps=25000, t_start=1.0, t_end=0.01):
    if step >= anneal_steps:
        return t_end
    return t_start + (t_end - t_start) * step / max(anneal_steps, 1)


def set_sdpa_forward_temperature(model, temp):
    for m in model.modules():
        if isinstance(m, RankAttention):
            m.sdpa_forward_temperature = temp
...
csv_w.writerow(['step', 'train_loss', 'qreg_loss', 'lambda', 'sdpa_fwd_temp', 'val_loss', 'val_loss_snapped'])
...
for step in range(cfg['n_steps']):
    fwd_temp = get_sdpa_forward_temperature(step)
    set_sdpa_forward_temperature(model, fwd_temp)
    ...
    if step % 100 == 0:
        ...
        print(f'step {step:6d} | train_loss={ema:.4f} | qreg={ema_qreg:.4f} | lambda={lam:.5f} | fwd_temp={fwd_temp:.4f} | lr={lr:.2e} ({phase})')
    ...
    if step % cfg['test_every'] == 0:
        ...
        csv_w.writerow([step, f'{ema:.6f}', f'{ema_qreg:.6f}', f'{lam:.6f}', f'{fwd_temp:.6f}', f'{val_loss:.6f}', f'{val_loss_snapped:.6f}'])
        csv_f.flush()
