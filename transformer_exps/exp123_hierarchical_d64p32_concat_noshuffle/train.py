"""
exp123_hierarchical_d64p32_concat_noshuffle — Fork of exp122.

Changes from exp122:
- positional_dim=32 (down from 64)
- Concatenated PE (not interleaved): input_dim=64+32=96 (auto->356 tph q/k/v)
...
class LUTBlock(nn.Module):
    def __init__(self, cfg, layer_idx):
        ...
        d_qk    = cfg['d_qk']                       # 16
        d_v     = cfg['d_v']                        # 16
        tph_qkv = cfg['qkv_tables_per_head']        # 4096 (auto->356)
        tph_op  = cfg['out_proj_tables_per_head']   # 4096 (auto->196)
        lut_input_dim = d + p                       # 96 (concatenated)
        s = layer_idx * 10
    ...
    def forward(self, x, pos):
        B, T, E = x.shape
        P = pos.shape[-1]
        # Concatenate: [B*T, E] || [B*T, P] -> [B*T, E+P]
        x_flat = x.reshape(B * T, E)
        pos_flat = pos.reshape(B * T, P)
        x_pos_flat = torch.cat([x_flat, pos_flat], dim=-1)
