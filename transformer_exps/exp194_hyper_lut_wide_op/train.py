...
def make_score_hyper(seed_offset=0):
    ...
    lut = HyperLUT(
        ...
        n_pairs=cfg['score_n_pairs'], hidden_dim=cfg.get('score_hidden', cfg.get('hidden_dim', 64)),
        temperature=temp, random_seed=cfg['random_seed']+seed_offset, device=DEVICE,
    )
...
def make_value_hyper(seed_offset=200):
    ...
    return HyperLUT(
        ...
        n_pairs=cfg['value_n_pairs'], hidden_dim=cfg.get('value_hidden', cfg.get('hidden_dim', 64)),
        temperature=temp, random_seed=cfg['random_seed']+seed_offset, device=DEVICE,
    )
...
def make_outproj_hyper(seed_offset=400):
    ...
    return HyperLUT(
        ...
        n_pairs=cfg['outproj_n_pairs'], hidden_dim=cfg.get('outproj_hidden', cfg.get('hidden_dim', 64)),
        temperature=temp, random_seed=cfg['random_seed']+seed_offset, device=DEVICE,
    )
...
print(f'Architecture: HyperLUT scores(H={H},pairs={cfg["score_n_pairs"]},hid={cfg.get("score_hidden",64)}) + V(hid={cfg.get("value_hidden",64)}) + outproj(hid={cfg.get("outproj_hidden",64)})')
