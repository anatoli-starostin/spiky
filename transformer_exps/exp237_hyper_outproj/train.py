"""
exp237_hyper_outproj — Q/K/V are LUTs, OutProj is HyperLUT, no FFN.
Q/K nap=5, V nap=6, tph=128. QK LayerNorm.
"""
...
def _make_hyper_outproj(seed_offset):
    h = HyperLUT(
        input_dim=H*d_v, n_heads=1, n_outputs=E,
        n_pairs=cfg['hyper_n_pairs'], hidden_dim=cfg['hyper_hidden_dim'],
        temperature=cfg['hyper_temperature'], soft_mode=cfg['hyper_soft_mode'],
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        self.out_proj = _make_hyper_outproj(400+layer_idx)
...
print(f'Q/K LUT nap={NAP_QK}, V LUT nap={NAP_V}, tph={TPH}')
print(f'OutProj HyperLUT: n_pairs={cfg["hyper_n_pairs"]}, hidden={cfg["hyper_hidden_dim"]}')
