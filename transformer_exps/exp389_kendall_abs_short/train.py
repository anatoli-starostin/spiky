...
class Model(nn.Module):
    ...
    def forward(self, tokens):
        ...
        # Sharpened Kendall-tau metric: k·|k| = sign(k)·k². Compresses near 0
        # (soft at random pair alignment) and preserves sign at tails (sharp
        # at confident agreement / disagreement). Bounded in [-1, +1].
        P_E = tok_dom.shape[-1]
        dot = pair @ tok_dom.T                                     # [B*T, V] in [-P_E, +P_E]
        k = dot / P_E                                              # Kendall-tau in [-1, 1]
        base = k * k.abs()                                         # sign(k) * k²
