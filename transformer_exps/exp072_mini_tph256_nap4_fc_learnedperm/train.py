"""
exp072_mini_tph256_nap4_fc_learnedperm — Same as exp070 but with LearnedSoftPermutations
instead of fixed random permutations. No pos_emb. tph=256, nap=4, FULL_COVERAGE. 50k steps.
"""
import sys, os, json
import torch
...
from spiky.lutorch.ranking_tools import RankAttention, LearnedSoftPermutations
...
class LUTBlock(nn.Module):
    """Single transformer block with LearnedSoftPermutations applied to q and k."""
    ...
    def __init__(self, cfg, layer_idx):
        ...
        self.pos_perm = LearnedSoftPermutations(
            n_perms=CONTEXT_SIZE, n=d_head, temp=cfg['perm_temp'], smooth_mode=False,
        )
        self.n_heads = h
        self.d_head = d_head
        self.d = d
    ...
    def forward(self, x):
        ...
        # Apply per-position learned permutations: (B, H, T, d_head) -> reshape to (B*H, T, d_head)
        BH = B * self.n_heads
        q = self.pos_perm(q.reshape(BH, T, self.d_head)).reshape(B, self.n_heads, T, self.d_head)
        k = self.pos_perm(k.reshape(BH, T, self.d_head)).reshape(B, self.n_heads, T, self.d_head)
