...
bit_luts = []
bit_lut_tags = []  # parallel list of (layer_idx, role) for diagnostic logging
for layer_idx, layer in enumerate(model.layers):
    bit_luts += [layer.q_perm, layer.k_perm, layer.v_perm, layer.out_proj]
    bit_lut_tags += [(layer_idx, 'Q'), (layer_idx, 'K'), (layer_idx, 'V'), (layer_idx, 'Out')]
...
# Grad-norm diagnostics: per-LUT-module L2 norm of grad_out and latent.
diag_path = os.path.join(EXP_DIR, 'grad_diag.csv')
diag_f = open(diag_path, 'w', newline='')
diag_w = csv.writer(diag_f)
diag_w.writerow(['step', 'layer', 'role', 'tph', 'grad_out_norm', 'latent_norm', 'ratio'])
_LOG_GRAD_EVERY = 100
...
for step in range(cfg['n_steps']):
    ...
    # Capture per-module grad_out norm BEFORE bit_opt.step() clears it.
    if step % _LOG_GRAD_EVERY == 0:
        for (lut, state, (L, role)) in zip(bit_luts, bit_opt._states, bit_lut_tags):
            go = state.get("grad_out")
            if go is None:
                continue
            g_n = float(go.float().norm().item())
            # Latent norm (float view — matches what the optimizer sees post-dequant).
            if hasattr(lut, 'latent_bf16'):
                lat = lut.latent_bf16.float()
            elif hasattr(lut, 'latent_fp32'):
                lat = lut.latent_fp32
            else:
                lat = lut.latent_fp8.float() / lut.latent_scale
            l_n = float(lat.norm().item())
            ratio = g_n / max(l_n, 1e-12)
            diag_w.writerow([step, L, role, int(lut.tph), f'{g_n:.6e}', f'{l_n:.6e}', f'{ratio:.6e}'])
        diag_f.flush()
