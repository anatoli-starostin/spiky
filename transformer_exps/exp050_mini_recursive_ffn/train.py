"""
exp050_mini_recursive_ffn — Minimalistic + RecursiveMultiHeadLut for out_proj and ffn.
Each single-head LUT applied twice with residual (same weights), no extra params.
...
class RecursiveMultiHeadLut(nn.Module):
    ...
    def __init__(self, lut: MultiHeadLut, n_iters: int = 2, refine_seed_offset: int = 1000):
        ...
        lut_fits = (lut.n_heads == 1 and lut.n_outputs == lut.input_dim)
        if n_iters > 1 and not lut_fits:
            ...
            self.refine_lut = None  # reuse lut itself or n_iters==1

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: (B, input_dim)
        refine = self.refine_lut if self.refine_lut is not None else self.lut
