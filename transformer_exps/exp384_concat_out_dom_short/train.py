...
class LUTBlock(nn.Module):
    ...
    def forward(self, x, pos_emb):
        ...
        # Also expose out_dom reshaped to [B, T, P_out] for the unembedder.
        out_dom_bt = out_dom.squeeze(1).reshape(B, T, -1)              # [B, T, P_out]
        return out, out_dom_bt
...
class Model(nn.Module):
    def __init__(self):
        ...
        # Unembedder sees concatenated per-layer out_dom (±1 pair-dominance
        # from out_canon) instead of the Borda+LN vectors. Each layer
        # contributes P_out = E*(E-1)/2 = 496 dims, concat across 6 layers
        # gives 2976-dim input. LN + Linear readout to vocab.
        P_out = E * (E - 1) // 2
        self.unembedder = nn.Sequential(
            nn.LayerNorm(N_LAYERS * P_out),
            nn.Linear(N_LAYERS * P_out, VOCAB_SIZE),
        )
    ...
    def forward(self, tokens):
        ...
        for layer, pos_emb in zip(self.layers, self.pos_embs):
            x, out_dom = layer(x, pos_emb)
            outs_dom.append(out_dom)
        concat_dom = torch.cat(outs_dom, dim=-1)                    # [B, T, 6*P_out]
        return self.unembedder(concat_dom)
