"""
exp231_double_depth_wide — pre-LUTs output dim=64, main LUTs take 64-dim input.
No residual. nap=5, tph=128, QK LayerNorm.
"""
...
NAP = cfg['nap']
TPH = cfg['tph']
PRE_DIM = cfg['pre_lut_dim']


def _make_lut(input_dim, n_heads, n_outputs, seed_offset):
    return MultiHeadLut(
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        super().__init__()
        # Pre-LUTs: 32 -> 64
        self.pre_q = _make_lut(E, 1, PRE_DIM, 800+layer_idx)
        self.pre_q_norm = nn.LayerNorm(PRE_DIM)
        self.pre_k = _make_lut(E, 1, PRE_DIM, 900+layer_idx)
        self.pre_k_norm = nn.LayerNorm(PRE_DIM)
        self.pre_v = _make_lut(E, 1, PRE_DIM, 1000+layer_idx)
        self.pre_v_norm = nn.LayerNorm(PRE_DIM)
        self.pre_out = _make_lut(H*d_v, 1, PRE_DIM, 1100+layer_idx)
        self.pre_out_norm = nn.LayerNorm(PRE_DIM)
        self.pre_ffn = _make_lut(E, 1, PRE_DIM, 1200+layer_idx)
        self.pre_ffn_norm = nn.LayerNorm(PRE_DIM)

        # Main LUTs: input_dim=64
        self.q_lut = _make_lut(PRE_DIM, H, d_qk, layer_idx)
        self.k_lut = _make_lut(PRE_DIM, H, d_qk, 100+layer_idx)
        self.q_norm = nn.LayerNorm(d_qk)
        self.k_norm = nn.LayerNorm(d_qk)
        self.v_lut = _make_lut(PRE_DIM, H, d_v, 200+layer_idx)
        self.out_proj = _make_lut(PRE_DIM, 1, E, 400+layer_idx)
        self.norm1 = nn.LayerNorm(E)
        self.ffn = _make_lut(PRE_DIM, 1, E, 600+layer_idx)
    ...
    def forward(self, x, pos_emb):
        ...
        # Pre-LUT(32->64) + LayerNorm -> main LUT(64->...) for Q, K
        ...
        # Pre-LUT(32->64) + LayerNorm -> main LUT(64->...) for FFN
...
print(f'Double-depth wide LUTs: pre_dim={PRE_DIM}, nap={NAP}, tph={TPH}, d_qk={d_qk}, d_v={d_v}')
