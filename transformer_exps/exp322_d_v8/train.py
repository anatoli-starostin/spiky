...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        # Borda-only (no LN) after out_proj — LN applied after the concat
        # so both skip branches share one normalisation before the FFN.
        self.out_to_vec = DominanceToVector(E, normalise=False)
        self.ffn_in_norm = nn.LayerNorm(H * d_v + E)
        # FFN: BitPermLUT on (H*d_v + E)-dim concat -> E, then Borda + LN.
    ...
    def forward(self, x, pos_emb):
        ...
        # Skip-by-concat + LN across the full (H*d_v + E)-dim vector.
        ffn_in = self.ffn_in_norm(torch.cat([out_in, out], dim=-1))
