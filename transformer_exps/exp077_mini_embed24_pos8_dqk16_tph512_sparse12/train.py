"""
exp077_mini_embed24_pos8_dqk16_tph512_sparse12 — Fork of exp075 with tph=512, nap=4,
sparse_output_dim=12. Applied where n_outputs > 12: q/k (16), out_proj/ffn (24),
unembedder (257). v LUT (n_outputs=6) stays full. 50k steps.
"""
import sys, os, json
import torch
...
class LUTBlock(nn.Module):
    def __init__(self, cfg, layer_idx):
        ...
        sod = cfg['sparse_output_dim'] # 12
        s = layer_idx * 10

        # sparse_output_dim=12 applies where n_outputs > 12
        self.q_lut = make_lut(lut_input_dim, h, d_qk, cfg, s + 0, sparse_output_dim=sod)  # 16 > 12 ✓
        self.k_lut = make_lut(lut_input_dim, h, d_qk, cfg, s + 1, sparse_output_dim=sod)
        self.v_lut = make_lut(lut_input_dim, h, d_v,  cfg, s + 2)                         # 6 < 12, full
        self.out_proj = make_lut(d, 1, d, cfg, s + 3, sparse_output_dim=sod)              # 24 > 12 ✓
