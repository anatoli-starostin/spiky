...
def _make_out(seed_offset):
    ...
    return MultiBitPermutationLUT(
        ...
        random_seed=cfg['random_seed'] + seed_offset,
        initial_weights_noise=float(cfg.get('out_init_std', cfg['bit_lut_latent_init_std'])),
...
for layer in model.layers:
    # Hybrid: q/k/v stay BitPermLUT; out_proj is MultiBit.
    bit_luts += [layer.q_perm, layer.k_perm, layer.v_perm]
    multi_bit_luts += [layer.out_proj]

adam_params = list(model.parameters())
print(f'bit LUTs: {len(bit_luts)} modules | multi-bit LUTs: {len(multi_bit_luts)}')
