"""
Sweep: include_diagonal=False with SignedDiff processor on top-5 configs.
Caution: position 0 has no valid keys under strict causal mask (j < i).
We set scores[i=0, j=0] = 0 before softmax so it attends to itself.
"""
...
def make_model(cfg):
    ...
    H = cfg['H']
    d_v = cfg['d_v']
    score_input_dim = E + P  # signed diff
    ...
    def _make_score(seed_offset):
        ...
        return LUTAttentionV3(lut, seq_len=SEQ_LEN, causal=True,
                              include_diagonal=False,
                              pairs_processor=SignedDiffProcessor())
    ...
    class Block(nn.Module):
        ...
        def forward(self, x, rel_pe):
            ...
            # Fix position 0: no valid keys under strict causal (j < i).
            # Set scores[*, *, i=0, j=0] = 0 so softmax gives weight 1.0 to self.
            raw_scores[:, :, 0, 0] = 0.0

            weights = F.softmax(raw_scores, dim=-1)  # [B, H, T, T]
    ...
    class Model(nn.Module):
        def __init__(self):
            ...
            self.rel_pe = nn.Parameter(torch.randn(SEQ_LEN - 1, P) * 0.1)  # T-1 for no diagonal
