"""
exp043_rank_dqk_half_hard_nalt3 — RankAttention d_qk=d//2=16 (120 pairs), d_v=d_head=8,
hard RankAttn (temperature=1.0, smooth_mode=False), LUT non-smooth n_alternatives=3, double FFN.
"""
import sys, os, json
import torch
...
class LUTBlock(nn.Module):
    ...
    def __init__(self, cfg, layer_idx):
        ...
        d_qk = d // 2
        # q/k LUTs: H heads, each outputting d_qk; tph reduced proportionally to keep param count same
        qk_tph = cfg['tables_per_head'] // (d_qk // d_head)
        self.q_lut = make_lut(d, h, d_qk, cfg, s + 0, tables_per_head_override=qk_tph)
        self.k_lut = make_lut(d, h, d_qk, cfg, s + 1, tables_per_head_override=qk_tph)
        ...
        # d_qk=d//2=16 (120 pairs), d_v=d_head=8, hard mode
    ...
    def forward(self, x):
        ...
        # q/k: H heads, d_qk each; v: H heads, d_head each
        q = self.q_lut(x_flat).permute(1, 0, 2)  # [H, B*T, d_qk]
        k = self.k_lut(x_flat).permute(1, 0, 2)  # [H, B*T, d_qk]
        ...
        k = k.reshape(self.n_heads, B, T, self.d_qk).permute(1, 0, 2, 3)      # [B, H, T, d_qk]
