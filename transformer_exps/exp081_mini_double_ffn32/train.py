"""
exp081_mini_double_ffn32 — Fork of exp075 with 2-layer FFN.

ffn1: 24 → 32 (expansion)
ffn2: 32 → 24 (contraction)
tph=256 everywhere. Architecture otherwise identical to exp075.
"""
import sys, os, json
import torch
...
class LUTBlock(nn.Module):
    def __init__(self, cfg, layer_idx):
        ...
        self.ffn1     = make_lut(d, 1, d_mid, cfg, s + 4)  # 24 → 32
        self.ffn2     = make_lut(d_mid, 1, d, cfg, s + 5)  # 32 → 24
    ...
    def forward(self, x, pos):
        ...
        mid = self.ffn1(x.reshape(-1, E))[:, 0, :]          # [B*T, d_mid]
        ffn_out = self.ffn2(mid)[:, 0, :].reshape(B, T, E)  # [B*T, d] → [B, T, d]
        x = x + ffn_out
