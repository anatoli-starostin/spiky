"""
exp126_conv2d_d40p24_concat — Fork of exp124.

Changes from exp124:
- CONV2D anchor sampling policy (nap=8 required)
- embedding_dim=40, positional_dim=24: concat input_dim=64 (8x8 grid, 56 tph auto)
- out_proj input_dim=h*d_v=64: also 8x8 grid, 56 tph auto
- shuffle_per_head=True (head 0 unshuffled, heads 1-3 shuffled)
"""
import sys, os, json
import torch
...
class LUTBlock(nn.Module):
    def __init__(self, cfg, layer_idx):
        super().__init__()
        d       = cfg['embedding_dim']              # 40
        p       = cfg['positional_dim']             # 24
        h       = cfg['num_heads']                  # 4
        d_qk    = cfg['d_qk']                       # 16
        d_v     = cfg['d_v']                        # 16
        tph_qkv = cfg['qkv_tables_per_head']        # 4096 (auto->56)
        tph_op  = cfg['out_proj_tables_per_head']   # 4096 (auto->56)
        lut_input_dim = d + p                       # 64 (8x8 grid)
        s = layer_idx * 10
