"""
exp137_eggroll — Fork of exp130.

Changes from exp130:
- EGGROLL optimizer: gradient-free, antithetic rank-1 perturbations, no backward().
- Perturbs nn.Parameter tensors (LUT table weights, embeddings, unembedder).
  Anchor pairs are buffers — fixed, not perturbed.
- pop_size antithetic pairs evaluated per step; fitness-weighted weight update.
"""
...
# ── EGGROLL Optimizer ──────────────────────────────────────────────────────────
#
# Perturbs nn.Parameter tensors only (LUT table weights, embeddings, unembedder).
# Anchor pairs (anchor_pairs_a/b) are buffers — fixed, never touched here.
#
# For each step:
#   1. Sample n_pairs rank-1 perturbations {δW_i}.
#   2. Evaluate loss at θ + σ·δW_i and θ − σ·δW_i (antithetic pair).
#   3. fitness_i = loss(θ − σ·δW_i) − loss(θ + σ·δW_i)
#      (positive → + direction lowered loss → move that way)
#   4. Standardize fitness across pairs.
#   5. θ += (lr / (n_pairs · σ)) · Σ_i fitness_i · δW_i

class EGGROLLOptimizer:

    def __init__(self, params, lr=0.001, sigma=0.05, pop_size=16, rank=1):
    ...
    def _sample_perturbation(self):
        """Return list of perturbation descriptors, one per param."""
        perts = []
        for p in self.params:
            if p.ndim <= 1:
                # 1-D: simple noise vector normalized to unit RMS
                noise = torch.randn_like(p)
                noise = noise / (noise.norm() / math.sqrt(noise.numel()) + 1e-8)
                perts.append(('vec', noise))
            else:
                # N-D: rank-1 outer product, normalized so ||δW||_F ≈ sqrt(m*n)
                m = p.shape[0]
                n = p.numel() // m
    ...
    def step(self, loss_fn, lr_scale=1.0):
        """
        loss_fn(): callable returning scalar float loss (no grad needed).
        Returns mean loss across all 2*n_pairs evaluations.
        """
        ...
        with torch.no_grad():
            ...
            eff_lr = self.lr * lr_scale / self.n_pairs
...
def run():
    ...
    print(f'EGGROLL: pop_size={cfg["eggroll_pop_size"]}, sigma={cfg["eggroll_sigma"]}, rank={cfg["eggroll_rank"]}')
    ...
    n_steps = cfg['n_steps']
    warmup_frac = cfg.get('lr_warmup_fraction', 0.1)
    ...
    for step in range(n_steps):
        ...
        # Capture batch in closure; reused for all pop_size evaluations this step
        def loss_fn():
            logits = model(inp)
            B, T, V = logits.shape
        ...
        mean_loss, fitness_std, mean_abs_delta = optimizer.step(loss_fn, lr_scale=lr_scale)
        ...
        if step % 100 == 0:
            ...
            print(f'step {step:6d} | train_loss={ema:.4f} | lr={cfg["lr"] * lr_scale:.2e} | fit_std={fitness_std:.4f} | delta={mean_abs_delta:.4f} ({phase})')
