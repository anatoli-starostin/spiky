...
_QK_TPH_PER_LAYER = cfg.get('qk_tph_per_layer')
if _QK_TPH_PER_LAYER is not None:
    assert len(_QK_TPH_PER_LAYER) == N_LAYERS, \
        f"qk_tph_per_layer must have {N_LAYERS} entries, got {len(_QK_TPH_PER_LAYER)}"


def _make_qk(layer_idx, seed_offset):
    tph = (_QK_TPH_PER_LAYER[layer_idx] if _QK_TPH_PER_LAYER is not None
           else cfg['qk_tph'])
...
_qk_tph_str = str(_QK_TPH_PER_LAYER) if _QK_TPH_PER_LAYER is not None else str(cfg['qk_tph'])
print(f'Q/K BitPermLUT: in_nap={cfg["qk_input_nap"]} out_nap={cfg["qk_output_nap"]} tph={_qk_tph_str} d_qk={d_qk} P_qk={D_QK_P}')
