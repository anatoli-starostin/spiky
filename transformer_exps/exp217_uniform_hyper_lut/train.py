"""
exp217_uniform_hyper_lut — All HyperLUTs uniform: input_dim=32, 160 pairs, hidden=320.
Position: per-layer sum. d_qk=8, d_v=8, H=4.
"""
...
SOFT = cfg.get('soft_mode', 'rational')
...
def _make_hyper(n_heads, n_outputs, seed_offset):
    ...
    with torch.no_grad():
        nn.init.normal_(h.fc1.weight, std=0.001)
...
class HyperBlock(nn.Module):
    ...
    def forward(self, x, pos_emb):
        ...
        q = self.q(xp).reshape(B, T, H, d_qk).permute(0, 2, 1, 3)
        k = self.k(xp).reshape(B, T, H, d_qk).permute(0, 2, 1, 3)
        v = self.v(x_flat).reshape(B, T, H, d_v).permute(0, 2, 1, 3)
...
print(f'Uniform HyperLUT: input_dim={E}, pairs={N_PAIRS}, hidden={HID}, d_qk={d_qk}, d_v={d_v}, {SOFT}')
...
for step in range(cfg['n_steps']):
    ...
    if step % cfg['test_every'] == 0:
        ...
        train_losses.append(ema); val_losses.append(val); val_steps.append(step)
        csv_w.writerow([step, f'{ema:.6f}', f'{val:.6f}']); csv_f.flush()
...
plt.tight_layout()
plt.savefig(os.path.join(EXP_DIR, 'loss.png'), dpi=100); plt.close()

summary = {
    'exp_name': cfg['exp_name'], 'best_val_loss': best_val,
