...
from spiky.lutorch.multi_bit_permutation_lut import MultiBitMultiHeadLUT
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        # out_canon intentionally removed: downstream unembedder is a BitPermLUT
        # (exp409+), which reads pair-sign bits and doesn't see magnitudes.
    ...
    def forward(self, x, pos_emb):
        ...
        # Out: concat heads -> bit-LUT pair-dominance -> Borda+LN (no canon).
...
class Model(nn.Module):
    def __init__(self):
        ...
        # Single MultiBitMultiHeadLUT(concat_dim -> VOCAB_SIZE) with K=4
        # signed weights. Scale chosen so post-trained logit std ≈ 1
        # (assumes q distributes ~uniformly): scale = 1/(half_range·√(var_eff·N))
        # where var_eff = var((2q+1)/(2·half_range)) ≈ 0.332 for K=4 and
        # N = tph·output_nap/n_outputs is the per-output vote count.
        fcfg = cfg['final_lut']
        offset = 0
        partition_sets = []
        for pe in _PER_LAYER_E:
            partition_sets.append(list(range(offset, offset + pe)))
            offset += pe
        bit_width = int(fcfg.get('bit_width', 4))
        half_range = 1 << (bit_width - 1)
        votes_per_output = int(fcfg['tph']) * int(fcfg['output_nap']) / float(VOCAB_SIZE)
        # var(midrise effective vote) for K-bit signed: derived from
        # ((2q+1)/(2·half_range))² average over q ∈ [-half_range, half_range-1].
        var_eff = ((2 * half_range) ** 2 - 1) / (3 * (2 * half_range) ** 2)
        unit_scale = 1.0 / (half_range * math.sqrt(var_eff * votes_per_output))
        # final_lut can have a different latent init than transformer layers.
        # For high K (e.g. K=8, threshold=1/half_range=0.008), default 0.001
        # leaves all q at 0 at init — bigger init noise (e.g. 0.1) puts q in
        # the active range from step 0 and avoids the "stuck-at-zero" stall.
        final_lut_init_noise = float(fcfg.get(
            'latent_init_std', cfg['bit_lut_latent_init_std']
        ))
        self.final_lut = MultiBitMultiHeadLUT(
            n_inputs=concat_dim, n_outputs=VOCAB_SIZE, n_heads=1,
            input_nap=int(fcfg['input_nap']), output_nap=int(fcfg['output_nap']),
            tph=int(fcfg['tph']), bit_width=bit_width,
            random_seed=cfg['random_seed'] + 900,
            initial_weights_noise=final_lut_init_noise,
            device=DEVICE,
            partition_sets=partition_sets,
            scale=unit_scale,
        )
        # Scalar learnable scale (linear, not log-space). Init=1.0 so initial
        # logits are unchanged — Adam can adjust to make logits sharper or
        # softer if needed.
        self.logit_scale = nn.Parameter(torch.tensor(
            float(cfg.get('logit_scale_init', 1.0))
        ))
        # Per-token additive bias (zero-init): unigram frequency prior.
    ...
    def forward(self, tokens):
        ...
        B, T, _ = concat.shape
        logits = self.final_lut(concat.reshape(B * T, -1)).squeeze(1)  # [B*T, V] raw + midrise bias
...
multi_bit_luts = [model.final_lut]   # K=4 final LUT — separate optimizer.

adam_params = list(model.parameters())
print(f'bit LUTs: {len(bit_luts)} 1-bit, {len(multi_bit_luts)} {model.final_lut.bit_width}-bit')
...
multi_bit_opt = MultiBitPermutationLUTOptimizer(
    multi_bit_luts,
    lr=cfg.get('multi_bit_lut_lr', cfg['bit_lut_lr']),
    beta1=cfg.get('bit_lut_beta1', 0.9),
    beta2=cfg.get('bit_lut_beta2', 0.999),
    lr_schedule_fn=get_lr_scale,
)
