"""
exp076_mini_embed24_pos8_dqk16_tph128 — Fork of exp075 with mixed tph:
  q/k/v LUTs (input_dim=32, 496 pairs) keep tph=256
  out_proj, ffn, unembedder (input_dim=24, 276 pairs) use tph=128
~3.4M params. 50k steps.
"""
import sys, os, json
import torch
...
class LUTBlock(nn.Module):
    def __init__(self, cfg, layer_idx):
        ...
        tph_qkv = cfg['tables_per_head_qkv']    # 256
        tph_other = cfg['tables_per_head_other'] # 128
        s = layer_idx * 10
        ...
        self.out_proj = make_lut(d, 1, d, tph_other, cfg, s + 3)
        self.ffn      = make_lut(d, 1, d, tph_other, cfg, s + 4)
...
class LUTTransformerV2(nn.Module):
    ...
    def __init__(self, cfg, maxlen=CONTEXT_SIZE):
        ...
        self.unembedder = MultiHeadLut(
            ...
            n_anchor_pairs=cfg['n_anchor_pairs'],
            tables_per_head=cfg['tables_per_head_other'],
            smooth_mode=cfg['smooth_mode'],
            n_alternatives=cfg['n_alternatives'],
            normalize_weights=cfg['normalise_weights'],
