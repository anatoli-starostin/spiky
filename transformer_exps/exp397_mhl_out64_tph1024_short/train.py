...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        # No out_canon in this variant: the concat output is consumed only by
        # LUTs downstream (final_lut via anchor rankings, logit_lut via ±1
        # pair input after final_canon), neither of which relies on
...
class Model(nn.Module):
    def __init__(self):
        ...
        # No final_canon needed: the downstream logit_lut only cares about
        # sign of anchor-pair differences on its input vector, so graded
        # Borda+LN from final_d2v works fine.
