...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        super().__init__()
        self.q_lut = _make_lut(E, H, d_qk, cfg['qk_nap'], cfg['qk_tph'], layer_idx)
        self.k_lut = _make_lut(E, H, d_qk, cfg['qk_nap'], cfg['qk_tph'], 100 + layer_idx)
    ...
    def forward(self, x, pos_emb):
        B, T, _E = x.shape
        xp = x + pos_emb.unsqueeze(0)  # [B, T, E] + [1, T, E]
...
class LUTRankAttnV2(nn.Module):
    def __init__(self):
        ...
        self.pos_emb = nn.Parameter(torch.randn(SEQ_LEN, E) * 0.1)
