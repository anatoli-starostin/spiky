...
from spiky.lutorch.lut_helpers import UncertaintyMode, AnchorSamplingPolicy, SelfExcitementMode
...
def _attn_exclusion_sets():
    """Exclusion sets for attention: x_i, x_j, rel_pe are separate groups."""
...
def make_lut_attention(seed_offset=0):
    ...
    lut = MultiHeadLut(
        ...
        tables_per_head=cfg['attn_tables_per_head'],
    ...
    return LUTAttentionV2(
        ...
        self_excitement_mode=SelfExcitementMode(cfg.get('self_excitement_mode', 'linear')),
    )
...
def make_out_proj(seed_offset=50):
    ...
    return MultiHeadLut(
        ...
        n_outputs=E,
        n_anchor_pairs=cfg['n_anchor_pairs'],
        tables_per_head=cfg['outproj_tables_per_head'],
        smooth_mode=cfg['smooth_mode'],
        n_alternatives=cfg['n_alternatives'],
        normalize_weights=cfg['normalise_weights'],
        calibrate_output=cfg['calibrate_output'],
