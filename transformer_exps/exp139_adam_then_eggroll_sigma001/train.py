"""
exp139_adam_then_eggroll_sigma001 — Fork of exp130.

Two-phase training:
  Phase 1: Adam, 10k steps, warmup+cosine lr schedule, lr=0.001.
  Phase 2: EGGROLL, 40k steps, constant lr=0.1, sigma=1.0, no decay.

Hypothesis: Adam finds a good basin first; EGGROLL then fine-tunes
without gradient-induced bias, potentially escaping local optima that
Adam gets stuck in due to the non-differentiable argmax.
"""
...
class EGGROLLOptimizer:
    ...
    def __init__(self, params, lr=0.1, sigma=1.0, pop_size=16, rank=1):
...
def run():
    ...
    print(f'Phase 2: EGGROLL {n_eggroll_steps} steps, lr={cfg["eggroll_lr"]}, sigma={cfg["eggroll_sigma"]}, pop_size={cfg["eggroll_pop_size"]}')
    ...
    for local_step in range(n_eggroll_steps):
        ...
        mean_loss, fitness_std, mean_abs_delta = eggroll.step(loss_fn)

        ema = mean_loss if ema is None else (1 - alpha) * ema + alpha * mean_loss

        if step % 100 == 0:
            print(f'step {step:6d} | train_loss={ema:.4f} | fit_std={fitness_std:.4f} | delta={mean_abs_delta:.4f}')
