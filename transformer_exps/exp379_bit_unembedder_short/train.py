...
class Model(nn.Module):
    def __init__(self):
        super().__init__()
        # Untied embeddings: separate input (fp32) and output (bit-native).
        # Input side: same as exp378.
        self.token_embedder = nn.Embedding(VOCAB_SIZE, E)
        self.token_embedder.weight.data.uniform_(-0.1, 0.1)
        # Output side: ±1 pair-dominance latent for Kendall-tau logits.
        P_E = E * (E - 1) // 2
        self.token_emb_latent = nn.Parameter(
            torch.randn(VOCAB_SIZE, P_E) * cfg['bit_lut_latent_init_std']
        )
        ...
        # LN across the full 192-dim concat so final_lut sees a
        # consistently-scaled input regardless of per-layer out_to_vec drift.
        self.concat_ln = nn.LayerNorm(concat_dim)
    ...
    def _token_dom(self):
        """STE: forward = sign(latent), backward = d(latent/(T+|latent|))/d latent."""
        T = cfg.get('canon_temperature', 0.1)
        soft = self.token_emb_latent / (T + self.token_emb_latent.abs())
        return soft + (torch.sign(self.token_emb_latent) - soft).detach()

    def forward(self, tokens):
        tok_dom = self._token_dom()                                # [V, P_E] ±1
        ...
        concat = self.concat_ln(concat)                            # normalize across full 192
        B, T, _ = concat.shape
        ...
        logits = self.log_logit_scale.exp() * base
...
# Split Adam params similarly: token_emb_latent has amplified gradient via the
# sum over V=257 tokens, so it gets its own group with lower lr.
token_emb_params = [model.token_emb_latent]
main_adam_params = [
    p for p in model.parameters() if p is not model.token_emb_latent
]
print(f'bit LUTs: {len(bit_luts)} main + {len(unembedder_bit_luts)} unembedder')
print(f'Adam-managed parameters: main={sum(p.numel() for p in main_adam_params):,}, '
      f'token_emb={sum(p.numel() for p in token_emb_params):,}')

adam_opt = torch.optim.Adam([
    {'params': main_adam_params, 'lr': cfg['adam_lr']},
    {'params': token_emb_params, 'lr': cfg.get('unembedder_adam_lr', cfg['adam_lr'])},
])
