...
def _get_pairs_processor():
    mode = cfg.get('pairs_processor', None)
    if mode == 'signed_diff':
        return SignedDiffProcessor()
    return None


def _score_input_dim():
    E = cfg['embedding_dim']
    P = cfg['positional_dim']
    if cfg.get('pairs_processor') == 'signed_diff':
        return E + P
    return 2 * E + P
...
def make_score_attn(seed_offset=0):
    ...
    return LUTAttentionV3(
        ...
        pairs_processor=_get_pairs_processor(),
    )
...
class LUTBlock(nn.Module):
    ...
    def forward(self, x, rel_pe):
        ...
        # Raw attention scores [B, T, T, H, 1] → [B, H, T, T]
        raw_scores = self.score_attn(x, rel_pe)           # [B, T, T, H, 1]
        raw_scores = raw_scores.squeeze(-1)                 # [B, T, T, H]
        raw_scores = raw_scores.permute(0, 3, 1, 2)        # [B, H, T, T]
        attn_weights = F.softmax(raw_scores, dim=-1)        # [B, H, T, T]

        # Values [B, T, H, d_v] → [B, H, T, d_v]
        v = self.value_lut(x.reshape(B * T, E))             # [B*T, H, d_v]
        v = v.reshape(B, T, H, d_v).permute(0, 2, 1, 3)    # [B, H, T, d_v]

        # Attention: softmax(scores) @ V → [B, H, T, d_v]
        attn_out = attn_weights @ v                          # [B, H, T, d_v]
        attn_out = attn_out.permute(0, 2, 1, 3).reshape(B, T, H * d_v)  # [B, T, H*d_v]
        attn_out = self.attn_norm(attn_out)                  # [B, T, H*d_v]

        # Out projection: H*d_v → E
        proj_out = self.out_proj(attn_out.reshape(B * T, H * d_v))  # [B*T, 1, E]
        proj_out = proj_out.squeeze(1).reshape(B, T, E)
