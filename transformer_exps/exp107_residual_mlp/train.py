"""
exp107_residual_mlp — Fork of exp104.

Changes from exp104:
- up_proj replaced by 2-layer MLP: Linear(32,128) -> ReLU -> Linear(128,64), per layer
- res_dim=64 (output of MLP)
"""
import sys, os, json
import torch
...
class LUTTransformerV4(nn.Module):

    def __init__(self, cfg, maxlen=CONTEXT_SIZE):
        super().__init__()
        d       = cfg['embedding_dim']   # 32
        p       = cfg['positional_dim']  # 16
        res_dim = cfg['res_dim']         # 64
        ...
        self.up_projs = nn.ModuleList([
            nn.Sequential(
                nn.Linear(d, 128, bias=False),
                nn.ReLU(),
                nn.Linear(128, res_dim, bias=False),
            )
    ...
    def forward(self, tokens):
        ...
        res = torch.zeros(B, T, cfg['res_dim'], device=tokens.device)  # [B, T, res_dim]
...
model = LUTTransformerV4(cfg).to(DEVICE)
