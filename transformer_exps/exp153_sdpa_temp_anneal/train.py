"""
exp153_sdpa_temp_anneal — fork of exp152.

sdpa_temperature annealed 1.0 -> 0.01 over first 25k steps.
...
# ── SDPA temperature annealing ─────────────────────────────────────────────────

def get_sdpa_temperature(step, anneal_steps=25000, t_start=1.0, t_end=0.01):
...
csv_w.writerow(['step', 'train_loss', 'qreg_loss', 'lambda', 'sdpa_temp', 'val_loss', 'val_loss_snapped'])
...
for step in range(cfg['n_steps']):
    sdpa_temp = get_sdpa_temperature(step)
    set_sdpa_temperature(model, sdpa_temp)
    ...
    if step % 100 == 0:
        ...
        print(f'step {step:6d} | train_loss={ema:.4f} | qreg={ema_qreg:.4f} | lambda={lam:.5f} | sdpa_temp={sdpa_temp:.4f} | lr={lr:.2e} ({phase})')
    ...
    if step % cfg['test_every'] == 0:
        ...
        csv_w.writerow([step, f'{ema:.6f}', f'{ema_qreg:.6f}', f'{lam:.6f}', f'{sdpa_temp:.6f}', f'{val_loss:.6f}', f'{val_loss_snapped:.6f}'])
        csv_f.flush()
