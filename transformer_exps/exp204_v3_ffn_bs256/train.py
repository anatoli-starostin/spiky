...
def make_ffn(seed_offset=600):
    """FFN LUT: input_dim=E → n_outputs=E."""
    E = cfg['embedding_dim']
    return MultiHeadLut(
        input_dim=E,
        n_heads=1,
        n_outputs=E,
        n_anchor_pairs=cfg['ffn_nap'],
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        self.ffn = make_ffn(seed_offset=600 + layer_idx)
    ...
    def forward(self, x, rel_pe):
        ...
        proj_out = self.out_proj(attn_out).squeeze(1).reshape(B, T, E)
        x = x + self.norm1(proj_out)

        ffn_out = self.ffn(x.reshape(B * T, E)).squeeze(1).reshape(B, T, E)
