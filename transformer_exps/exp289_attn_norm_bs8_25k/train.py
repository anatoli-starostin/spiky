"""
exp285_fullperm_rankattn_domv — exp283 fork with all Q/K/V/out_proj as PermutationalLut.
V uses return_dominance=True (replaces manual ranks_to_dominance). Positional embeddings
summed into x for Q/K input. out_tph=1024. ctx=128, 100K bs=32.
"""
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        self.attn_norm = nn.LayerNorm(d_v)
    ...
    def forward(self, x, pos_emb):
        ...
        q = self.q_perm(xp).reshape(B, T, H, d_qk).permute(0, 2, 1, 3)
        k = self.k_perm(xp).reshape(B, T, H, d_qk).permute(0, 2, 1, 3)
        q = self.q_norm(q)
        k = self.k_norm(k)

        # V PermLut returns dominance directly: [B*T, H, P]
...
print(f'RankAttention temperature: {RANK_ATTN_TEMP}, soft_mode={SOFT_MODE}, temp={TEMP}')
