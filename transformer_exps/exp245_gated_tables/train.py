...
TABLE_GATING = cfg.get('table_gating', False)
TABLE_GATING_INIT = cfg.get('table_gating_init', -5.0)
TABLE_GATING_LEAK = cfg.get('table_gating_leak', 0.01)
...
def _make_lut(n_heads, n_outputs, nap, seed_offset):
    return MultiHeadLut(
        ...
        table_gating=TABLE_GATING, table_gating_init=TABLE_GATING_INIT,
        table_gating_leak=TABLE_GATING_LEAK,
    )
...
for step in range(cfg['n_steps']):
    ...
    # Anneal gate temperature: soft early, sharp late
    if TABLE_GATING:
        anneal_steps = cfg.get('table_gating_anneal_steps', 50000)
        t_start = cfg.get('table_gating_temp_start', 1.0)
        t_end = cfg.get('table_gating_temp_end', 10.0)
        ...
        for layer in model.layers:
            for lut in [layer.q_lut, layer.k_lut, layer.v_lut, layer.out_proj, layer.ffn]:
                if hasattr(lut, '_gate_temperature'):
                    lut._gate_temperature = temp
    optimizer.zero_grad()
    loss.backward()
    optimizer.step()
    scheduler.step()
    ...
    if step % 100 == 0:
        ...
        if TABLE_GATING:
            ...
            for layer in model.layers:
                for lut in [layer.q_lut, layer.k_lut, layer.v_lut, layer.out_proj, layer.ffn]:
                    if hasattr(lut, 'gate_logits'):
                        n_open = (torch.sigmoid(lut.gate_logits) > 0.5).sum().item()
                        total_open += n_open
                        total_tables += lut.gate_logits.shape[0]
            print(f'step {step:6d} | loss={ema:.4f} | lr={scheduler.get_last_lr()[0]:.2e} | gates={total_open}/{total_tables} | T={temp:.1f}')
        else:
