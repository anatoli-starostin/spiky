...
class Model(nn.Module):
    ...
    def forward(self, tokens):
        ...
        # LN the output-side token embedding per token (cosine-style): each
        # row normalized to mean=0, std=1 before the dot product.
        out_emb = F.layer_norm(self.token_embedder_out.weight, (E,))  # [V, E]
        logits = self.log_logit_scale.exp() * (pair_vec @ out_emb.T) + self.token_bias
