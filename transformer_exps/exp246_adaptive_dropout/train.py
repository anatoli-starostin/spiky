...
ATD = cfg.get('adaptive_table_dropout', False)
ATD_INIT = cfg.get('adaptive_table_dropout_init', 0.5)
ATD_EMA = cfg.get('adaptive_table_dropout_ema', 0.99)
ATD_MIN = cfg.get('adaptive_table_dropout_min', 0.0)
ATD_MAX = cfg.get('adaptive_table_dropout_max', 0.95)
...
def _make_lut(n_heads, n_outputs, nap, seed_offset):
    return MultiHeadLut(
        ...
        adaptive_table_dropout=ATD, adaptive_table_dropout_init=ATD_INIT,
        adaptive_table_dropout_ema=ATD_EMA, adaptive_table_dropout_min=ATD_MIN,
        adaptive_table_dropout_max=ATD_MAX,
    )
...
for step in range(cfg['n_steps']):
    ...
    optimizer.zero_grad()
    loss.backward()
    # Update adaptive dropout rates from gradients (before optimizer.step clears grads)
    if ATD:
        for layer in model.layers:
            for lut in [layer.q_lut, layer.k_lut, layer.v_lut, layer.out_proj, layer.ffn]:
                lut.update_adaptive_dropout()
    optimizer.step()
    scheduler.step()
    ...
    if step % 100 == 0:
        if ATD:
            # Log dropout stats
            all_rates = torch.cat([lut.table_drop_rates
                for layer in model.layers
                for lut in [layer.q_lut, layer.k_lut, layer.v_lut, layer.out_proj, layer.ffn]
                if hasattr(lut, 'table_drop_rates')])
            n_active = (all_rates < 0.1).sum().item()
            n_pruned = (all_rates > 0.9).sum().item()
            print(f'step {step:6d} | loss={ema:.4f} | lr={scheduler.get_last_lr()[0]:.2e} | drop: active={n_active} pruned={n_pruned} mean={all_rates.mean():.2f}')
        else:
