...
class LUTBlock(nn.Module):
    ...
    def forward(self, x, pos_emb):
        ...
        # Pre-LUT with residual -> main LUT for Q, K
        q_pre = xp_flat + self.pre_q_norm(self.pre_q(xp_flat).squeeze(1))
        k_pre = xp_flat + self.pre_k_norm(self.pre_k(xp_flat).squeeze(1))
        q = self.q_lut(q_pre).reshape(B, T, H, d_qk).permute(0, 2, 1, 3)
        k = self.k_lut(k_pre).reshape(B, T, H, d_qk).permute(0, 2, 1, 3)
        q = self.q_norm(q)
        k = self.k_norm(k)

        # Pre-LUT with residual -> main LUT for V
        v_pre = x_flat + self.pre_v_norm(self.pre_v(x_flat).squeeze(1))
        v = self.v_lut(v_pre).reshape(B, T, H, d_v).permute(0, 2, 1, 3)

        attn = F.scaled_dot_product_attention(q, k, v, is_causal=True)

        # Pre-LUT with residual -> main LUT for OutProj
        attn_flat = attn.permute(0, 2, 1, 3).reshape(B*T, H*d_v)
        out_pre = attn_flat + self.pre_out_norm(self.pre_out(attn_flat).squeeze(1))
        proj = self.out_proj(out_pre).squeeze(1).reshape(B, T, _E)
        x = x + self.norm1(proj)

        # Pre-LUT with residual -> main LUT for FFN
        x_ffn = x.reshape(B*T, _E)
        ffn_pre = x_ffn + self.pre_ffn_norm(self.pre_ffn(x_ffn).squeeze(1))
