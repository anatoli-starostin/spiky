"""
exp250_dual_stream — Dual stream architecture.
Permutational stream (32-dim): pure LUT, no residuals.
Residual stream (256-dim): accumulates MLP projections from each layer.
"""
...
E = cfg['embedding_dim']       # 32 — permutational stream
R = cfg['residual_dim']        # 256 — residual stream
MLP_H = cfg['mlp_hidden']      # 128
H = cfg['n_heads']
d_qk = cfg['d_qk']
d_v = cfg['d_v']
NAP_QK = cfg['nap_qk']
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        # Projection from permutational to residual stream
    ...
    def forward(self, perm, residual, pos_emb):
        """
        perm: [B, T, E] — permutational stream (no residuals)
        residual: [B, T, R] — residual stream (additive)
        pos_emb: [T, E]
        """
        B, T, _E = perm.shape
        xp = (perm + pos_emb.unsqueeze(0)).reshape(B*T, _E)
        x_flat = perm.reshape(B*T, _E)
        ...
        perm_out = self.out_proj(attn.permute(0, 2, 1, 3).reshape(B*T, H*d_v)).squeeze(1).reshape(B, T, _E)
        perm_out = self.out_norm(perm_out)

        # Project to residual stream and add
        residual = residual + self.proj(perm_out)

        return perm_out, residual
...
class DualStreamTransformer(nn.Module):
    def __init__(self):
        ...
        self.unembedder = nn.Linear(R, cfg['vocab_size'])

    def forward(self, tokens):
        B, T = tokens.shape
        perm = self.token_embedder(tokens)          # [B, T, E=32]
        residual = torch.zeros(B, T, R, device=tokens.device)  # [B, T, R=256]
        for layer, pos_emb in zip(self.layers, self.pos_embs):
            perm, residual = layer(perm, residual, pos_emb)
        return self.unembedder(residual)
...
model = DualStreamTransformer().to(DEVICE)

dense_params = []
lut_params = []
for n, p in model.named_parameters():
    if 'unembedder' in n or 'proj.' in n:
...
print(f'Dual stream: perm_dim={E}, residual_dim={R}, mlp_hidden={MLP_H}')
