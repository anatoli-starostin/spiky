"""
exp163_self_excitement — fork of exp161 with self_excitement=True.

Each pair's LUT output is weighted by its own mean |activation| before scatter-sum,
so contrastive/strong patterns dominate the per-token aggregation.
"""
...
DEVICE = 'cuda:0'
SEQ_LEN = CONTEXT_SIZE   # fixed sequence length
...
class LUTBlock(nn.Module):
    ...
    def forward(self, x, rel_pe):
        ...
        out = out.reshape(B, S, E)          # [B, S, E]  (H*O == E)
...
class LUTTransformerV2(nn.Module):
    def __init__(self):
        ...
        # Shared relative positional encoding [SEQ_LEN, pos_dim]
        # SEQ_LEN entries: rel_pe[0] = diagonal (dist 0), rel_pe[k] = distance k
    ...
    def forward(self, tokens):
        ...
        return self.unembedder(x)               # [B, S, vocab_size]
...
model = LUTTransformerV2().to(DEVICE)
