"""
LUT GRU: GRU with all linear layers replaced by MultiHeadLut.
"""
...
def make_gate_lut(input_dim, output_dim, seed_offset):
    return MultiHeadLut(
...
class LUTGRUCell(nn.Module):
    def __init__(self, input_dim, hidden_dim, seed_base):
        super().__init__()
        gate_input = input_dim + hidden_dim
        self.z_lut = make_gate_lut(gate_input, hidden_dim, seed_base)
        self.r_lut = make_gate_lut(gate_input, hidden_dim, seed_base + 10)
        self.h_lut = make_gate_lut(gate_input, hidden_dim, seed_base + 20)
    ...
    def forward(self, x, h):
        ...
        z = torch.sigmoid(self.z_lut(xh).squeeze(1))       # [B, hidden]
        r = torch.sigmoid(self.r_lut(xh).squeeze(1))       # [B, hidden]

        xrh = torch.cat([x, r * h], dim=-1)
        h_tilde = torch.tanh(self.h_lut(xrh).squeeze(1))   # [B, hidden]

        h_new = (1 - z) * h + z * h_tilde
        return h_new


class LUTGRULayer(nn.Module):
    def __init__(self, input_dim, hidden_dim, seed_base):
        super().__init__()
        self.cell = LUTGRUCell(input_dim, hidden_dim, seed_base)
        self.hidden_dim = hidden_dim
...
class LUTCharGRU(nn.Module):
    def __init__(self):
        super().__init__()
        self.embed = nn.Embedding(VOCAB_SIZE, E)
        self.embed.weight.data.uniform_(-0.1, 0.1)
        self.layers = nn.ModuleList([
            LUTGRULayer(E if i == 0 else HIDDEN, HIDDEN, seed_base=i*100)
...
model = LUTCharGRU().to(DEVICE)
total_params = sum(p.numel() for p in model.parameters())
print(f"LUT GRU Parameters: {total_params:,}")
