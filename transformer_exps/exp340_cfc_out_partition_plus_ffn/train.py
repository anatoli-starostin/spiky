...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        # Dominance -> vector after FFN LUT (P_ffn -> E, Borda + LN).
    ...
    def forward(self, x, pos_emb):
        ...
        # FFN: E -> bit-LUT dominance -> Borda+LN to E.
        ffn_dom = self.ffn(out)                                        # [B*T, 1, P_ffn]
        ffn_out = self.ffn_to_vec(ffn_dom).squeeze(1).reshape(B, T, _E)  # [B, T, E]
        return ffn_out
...
print(f'FFN BitPermLUT: in_nap={cfg["ffn_input_nap"]} out_nap={cfg["ffn_output_nap"]} tph={cfg["ffn_tph"]}')
