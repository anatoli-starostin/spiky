...
def _make_ffn(seed_offset):
    # FFN BitPermLUT. Input = concat(out_proj input=H*d_v, out_proj output=E).
    return BitPermutationLUT(
        n_inputs=H * d_v + E, n_outputs=E, n_heads=1,
        input_nap=cfg['ffn_input_nap'], output_nap=cfg['ffn_output_nap'],
        tph=cfg['ffn_tph'],
        random_seed=cfg['random_seed'] + seed_offset,
        initial_weights_noise=cfg['bit_lut_latent_init_std'],
        latent_dtype=cfg.get('bit_lut_latent_dtype', 'fp8'),
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        # Borda-only (no LN) after out_proj — the FFN BitPermLUT consumes
        # the raw concat; anchor lookup is scale-invariant, so an explicit
        # LN before it is redundant.
        self.out_to_vec = DominanceToVector(E, normalise=False)
        # FFN: BitPermLUT on 96-dim concat -> E, then Borda + LN out.
        self.ffn = _make_ffn(500 + layer_idx)
        self.ffn_to_vec = DominanceToVector(E)
    ...
    def forward(self, x, pos_emb):
        ...
        out_in = attn.permute(0, 2, 1, 3).reshape(B * T, H * d_v)      # [B*T, H*d_v=64]
        out_dom = self.out_proj(out_in)                                # [B*T, 1, P_out]
        out = self.out_to_vec(out_dom).squeeze(1)                      # [B*T, E]

        # Skip-by-concat fed straight to FFN (no LN on the concat — anchor
        # lookup is scale-invariant).
        ffn_in = torch.cat([out_in, out], dim=-1)                      # [B*T, 96]
        ffn_dom = self.ffn(ffn_in)                                     # [B*T, 1, P_ffn]
        ffn_out = self.ffn_to_vec(ffn_dom).squeeze(1).reshape(B, T, _E)
        return ffn_out
