...
class Model(nn.Module):
    def __init__(self):
        super().__init__()
        # Untied embeddings: separate input and output nn.Embedding, both
        # stored as [V, E] fp32 vectors. Input is used directly as continuous
        # layer-0 input; output is V2D-projected to ±1 pair-dominance for
        # Kendall-tau logits. Two independent copies; no weight sharing.
        self.token_embedder = nn.Embedding(VOCAB_SIZE, E)
        self.token_embedder.weight.data.uniform_(-0.1, 0.1)
        self.token_embedder_out = nn.Embedding(VOCAB_SIZE, E)
        ...
        # CLIP-style learnable logit scale. Init log_scale=3.3 → α≈27.
        # Base logit std at init ≈ 2*atanh(1/sqrt(P_E)) ≈ 0.09, so
        # logit std ≈ 2.4, giving softmax entropy ~ log(V)/2 (effective vocab
        # ~16 tokens). Previous log_scale=2.0 produced near-uniform softmax
        # at init with weak gradients.
        self.log_logit_scale = nn.Parameter(torch.full((1,), 3.3))
...
# --- init diagnostics: embedding spread + softmax entropy ---
with torch.no_grad():
    _td = model.token_v2d(model.token_embedder_out.weight)          # [V, P_E] ±1
    _pe = _td.shape[-1]
    _ktau = (_td @ _td.T) / _pe                                      # [V, V]
    _iu = torch.triu_indices(_td.shape[0], _td.shape[0], offset=1)
    _offdiag = _ktau[_iu[0], _iu[1]]
    print(f'init Kendall-tau across vocab (off-diag): '
          f'mean={_offdiag.mean():.3f} std={_offdiag.std():.3f} '
          f'min={_offdiag.min():.3f} max={_offdiag.max():.3f}')

    _xb = sampler.sample_training_batch(cfg['batch_size']).long()
    _inp = torch.empty_like(_xb); _inp[:, 0] = BOS_ID; _inp[:, 1:] = _xb[:, :-1]
    _lg = model(_inp)                                                 # [B, T, V]
    _lg_flat = _lg.reshape(-1, _lg.shape[-1])
    _p = torch.softmax(_lg_flat, dim=-1)
    _H = -(_p * (_p.clamp_min(1e-12).log())).sum(dim=-1)
    print(f'init logit std: {_lg_flat.std().item():.3f} '
          f'softmax entropy: mean={_H.mean():.3f} '
          f'(target log(V)/2={math.log(VOCAB_SIZE)/2:.3f}, max log(V)={math.log(VOCAB_SIZE):.3f})')
del _td, _pe, _ktau, _iu, _offdiag, _xb, _inp, _lg, _lg_flat, _p, _H
