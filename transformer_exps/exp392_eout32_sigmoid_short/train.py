...
class Model(nn.Module):
    def __init__(self):
        super().__init__()
        # Untied embeddings with different widths: input stays at E for the
        # full model backbone; output embedding uses a smaller E_OUT to keep
        # the Kendall-tau comparison cheaper (and to test whether 32-dim rank
        # patterns are overkill for V=257 tokens).
        E_OUT = cfg.get('embedding_dim_out', E)
        self.E_OUT = E_OUT
        self.token_embedder = nn.Embedding(VOCAB_SIZE, E)
        self.token_embedder.weight.data.uniform_(-0.1, 0.1)
        self.token_embedder_out = nn.Embedding(VOCAB_SIZE, E_OUT)
        self.token_embedder_out.weight.data.uniform_(-0.1, 0.1)
        self.token_v2d = VectorToDominance(
            E_OUT, smooth_mode=False,
        ...
        self.final_lut = BitPermutationLUT(
            n_inputs=concat_dim, n_outputs=E_OUT, n_heads=1,
            input_nap=cfg.get('final_input_nap', 10),
            output_nap=cfg.get('final_output_nap', E_OUT),
        ...
        self.final_canon = DominanceCanonicalize(
            E_OUT, temperature=cfg.get('canon_temperature', 0.1),
        )
        # CLIP-style learnable logit scale. Init α = exp(2.0) ≈ 7.4, close
        # to the value (2.07, α≈7.9) learned from zero-init over 25k steps.
        # Learnable sigmoid sharpening + outer scale. log_scale init=3 so
        # effective logit range is ±9 from the start — enough for real
        # 257-way discrimination. At init (slope=1, threshold=0) the sigmoid
        # metric is bounded at ±0.46, so low log_scale leaves the loss floor
        # too high (~log(V) - α*0.46 ≈ 5.1 at α=1); α≈20 lets the loss
        # actually descend below that floor while training.
        self.log_logit_scale = nn.Parameter(torch.full((1,), 3.0))
        self.log_sigmoid_slope = nn.Parameter(torch.zeros(1))
    ...
    def forward(self, tokens):
        tok_dom = self.token_v2d(self.token_embedder_out.weight)   # [V, P_E] ±1 via STE, separate embedding
        ...
        k = dot / P_E                                              # in [-1, 1]
        # Sigmoid-sharp Kendall metric with learnable slope and threshold:
        #   metric = 2·sigmoid(slope·(k - threshold)) - 1 ∈ (-1, 1)
        # At slope=1, threshold=0: near-linear (soft). As slope grows and
        # threshold moves toward correct-token k, metric becomes step-like:
        # sharp drop for small k decreases ("2-3 flips → very low metric")
        # while staying bounded (so random init matches can't blow up).
        slope = self.log_sigmoid_slope.exp()
        metric = 2.0 * torch.sigmoid(slope * (k - self.sigmoid_threshold)) - 1.0
        logits = self.log_logit_scale.exp() * metric + self.token_bias
