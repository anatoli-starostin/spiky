"""
exp030_rank_input_noise — RankAttention + input_scale_noise=0.5 on LUTs and RankAttention, nalt=1, smooth.
"""
import sys, os, json
import torch
...
def make_lut(input_dim, n_heads, n_outputs, cfg, seed_offset=0):
    return MultiHeadLut(
        ...
        input_scale_noise=cfg.get('input_scale_noise', 0.0),
...
class LUTBlock(nn.Module):
    """Single transformer block: q/k/v LUTs → RankAttention → out_proj LUT → FFN LUT. Residuals, no norms."""
    ...
    def __init__(self, cfg, layer_idx):
        ...
        self.rank_attn = RankAttention(d_head, smooth_mode=cfg['smooth_mode'], input_scale_noise=cfg.get('input_scale_noise', 0.0))
        self.n_heads = h
        self.d_head = d_head
        self.d = d
