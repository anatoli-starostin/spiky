...
class Model(nn.Module):
    def __init__(self):
        ...
        # Single BitMultiHeadLUT(concat_dim -> VOCAB_SIZE) — 1-bit weights.
        # Match exp412's parameterization: leave LUT.scale=1.0 (raw vote
        # sum) and put the 1/sqrt(vpo) normalization in logit_scale_init,
        # so the bit-LUT optimizer's gradient gate (scale × half_range / 0.5
        # = 2.0) matches exp412 dynamics for an apples-to-apples comparison.
        fcfg = cfg['final_lut']
        offset = 0
        partition_sets = []
        ...
        # Init = 1/sqrt(votes_per_output) so initial logit std ≈ 1.0.
        default_scale_init = 1.0 / math.sqrt(votes_per_output)
        self.logit_scale = nn.Parameter(torch.tensor(
            float(cfg.get('logit_scale_init', default_scale_init))
        ))
