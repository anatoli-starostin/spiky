...
class Model(nn.Module):
    def __init__(self):
        ...
        # Single BitMultiHeadLUT(concat_dim -> VOCAB_SIZE), scale chosen so
        # output sits in [-1, 1] (= 1 / votes_per_output). Output is then
        # atanh-expanded into unbounded log-odds, then re-scaled by the
        # learnable α = exp(log_logit_scale) and shifted by per-token bias.
        fcfg = cfg['final_lut']
        offset = 0
        partition_sets = []
        ...
        unit_scale = 1.0 / votes_per_output
        ...
        # α = exp(log_logit_scale). At init, LUT output std ≈ 1/sqrt(votes_per_output)
        # ≈ 0.051; atanh(0.051) ≈ 0.051, so 2·atanh has std ≈ 0.10. With
        # log_logit_scale init = 3.3 → α ≈ 27 → init logit std ≈ 2.7
        # (matches exp386's target).
        self.log_logit_scale = nn.Parameter(torch.tensor(
            float(cfg.get('log_logit_scale_init', 3.3))
        ))
    ...
    def forward(self, tokens):
        ...
        B, T, _ = concat.shape
        y = self.final_lut(concat.reshape(B * T, -1)).squeeze(1)   # [B*T, V] in ~[-1, 1]
        base = 2.0 * torch.atanh(y.clamp(-0.999, 0.999))           # log-odds expansion
        logits = self.log_logit_scale.exp() * base + self.logit_bias
