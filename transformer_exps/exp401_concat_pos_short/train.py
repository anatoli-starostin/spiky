...
# pos_emb_dim > 0 → positional embedding is concatenated with x (not summed).
_POS_EMB_DIM = cfg.get('pos_emb_dim', 0)


def _make_qk(layer_idx, seed_offset):
    # Q/K see x ∥ pos_emb when concat mode is on.
    n_inputs = _e_in(layer_idx) + (_POS_EMB_DIM if _POS_EMB_DIM > 0 else 0)
...
class LUTBlock(nn.Module):
    ...
    def forward(self, x, pos_emb):
        ...
        if _POS_EMB_DIM > 0:
            # pos_emb: [T, POS_EMB_DIM] → broadcast over batch and concat
            pos = pos_emb.unsqueeze(0).expand(B, -1, -1)               # [B, T, POS_EMB_DIM]
            xp = torch.cat([x, pos], dim=-1).reshape(B * T, _E_in + _POS_EMB_DIM)
        else:
...
class Model(nn.Module):
    def __init__(self):
        ...
        # When POS_EMB_DIM > 0, all layers use the same pos_emb width; when 0
        # (default), pos_emb matches each layer's input dim (for sum mode).
        _pos_dim_fn = (lambda i: _POS_EMB_DIM) if _POS_EMB_DIM > 0 else _e_in
        self.pos_embs = nn.ParameterList([
            nn.Parameter(torch.randn(CONTEXT_SIZE, _pos_dim_fn(i)) * 0.1)
