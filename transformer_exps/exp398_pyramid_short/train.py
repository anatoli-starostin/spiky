...
# Token embedder feeds layer 0 directly — its dim must match layer 0's E_in
# (which equals PER_LAYER_E[0] by convention: block 0 is E_in = E_out = PER_LAYER_E[0]).
...
def _make_qk(layer_idx, seed_offset):
    return BitPermutationLUT(
        n_inputs=_e_in(layer_idx), n_outputs=d_qk, n_heads=H,
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        # Dominance -> vector after out_proj (P_out -> e_out, Borda + LN).
    ...
    def forward(self, x, pos_emb):
        ...
        # Out: concat heads -> bit-LUT dominance -> Borda+LN to e_out.
...
class Model(nn.Module):
    def __init__(self):
        ...
        # Each pos_emb matches its layer's input dim.
        self.pos_embs = nn.ParameterList([
            nn.Parameter(torch.randn(CONTEXT_SIZE, _e_in(i)) * 0.1)
            for i in range(N_LAYERS)
        ])
        self.layers = nn.ModuleList([LUTBlock(i) for i in range(N_LAYERS)])
        concat_dim = sum(_PER_LAYER_E)
        # Per-layer partitions for final_lut input anchors.
        _PART_STARTS = [0]
        for e in _PER_LAYER_E:
            _PART_STARTS.append(_PART_STARTS[-1] + e)
        _FINAL_PART = [list(range(_PART_STARTS[i], _PART_STARTS[i+1]))
                       for i in range(N_LAYERS)] \
            if cfg.get('final_partition_by_layer', False) else None
        _FINAL_N_OUT = cfg.get('final_n_outputs', E)
        self.final_lut = BitPermutationLUT(
            n_inputs=concat_dim, n_outputs=_FINAL_N_OUT, n_heads=1,
            input_nap=cfg.get('final_input_nap', 10),
            output_nap=cfg.get('final_output_nap', _FINAL_N_OUT),
        ...
        self.final_d2v = DominanceToVector(_FINAL_N_OUT)
        self.logit_lut = MultiHeadLut(
            input_dim=_FINAL_N_OUT,
            n_heads=1,
            n_outputs=VOCAB_SIZE,
            n_anchor_pairs=cfg.get('mhl_nap', 6),
            tables_per_head=cfg.get('mhl_tph', 512),
            n_alternatives=1,
            smooth_mode=False,
    ...
    def forward(self, tokens):
        ...
        B, T, _ = concat.shape
        out_dom = self.final_lut(concat.reshape(B * T, -1))        # [B*T, 1, P_E]
        out = self.final_d2v(out_dom).squeeze(1)                   # [B*T, E] Borda+LN
        logits = self.logit_lut(out).squeeze(1) + self.token_bias  # [B*T, V]
