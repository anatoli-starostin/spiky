"""
exp080_mini_sinusoidal — Fork of exp075 with SinusoidalProjection everywhere.

Each LUT entry contributes A*sin(f*j+phi) over output indices (3 params/entry)
instead of an arbitrary vector (n_outputs params/entry). For the unembedder this
is ~85x fewer projection params than LProjection. Architecture otherwise identical
to exp075: embed=24, pos=8 separate, d_qk=16, tph=256, nap=4, FULL_COVERAGE.
"""
import sys, os, json
import torch
...
def make_lut(input_dim, n_heads, n_outputs, cfg, seed_offset=0):
    return MultiHeadLut(
        ...
        sinusoidal_projection=cfg['sinusoidal_projection'],
