...
class LUTTransformer(nn.Module):
    ...
    def forward(self, tokens):
        ...
        z = torch.cat([z, self.pos_emb[:, :S].repeat(B, 1, 1)], dim=-1)
        for layer in self.layers:
            z = layer(z)
        logits = self.unembedder(z.reshape(-1, z.shape[-1]))  # [B*S, 1, vocab_size]
        return logits[:, 0, :].reshape(B, S, -1)
