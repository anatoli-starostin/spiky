"""
exp104_residual_stream — Fork of exp100.

Changes from exp100:
- Separate residual stream of dim res_dim=128, updated after each LUT block via up_proj
- Ranking stream is the raw LUT chain (no skip connections inside LUTBlock)
- res_stream starts at zero; unembedder reads from res_stream
"""
import sys, os, json
import torch
...
class LUTBlock(nn.Module):
    ...
    def forward(self, x, pos):
        # x: [B, T, d] — ranking stream, no skip connection
        B, T, E = x.shape
        ...
        out = self.out_proj(attn)[:, 0, :]
        return out.reshape(B, T, E)  # [B, T, d], no residual
...
class LUTTransformerV3(nn.Module):
    ...
    def __init__(self, cfg, maxlen=CONTEXT_SIZE):
        ...
        self.up_projs = nn.ModuleList([nn.Linear(d, res_dim, bias=False) for _ in range(cfg['num_layers'])])

        self.unembedder = nn.Linear(res_dim, cfg['vocab_size'], bias=False)

    def forward(self, tokens):
        B, T = tokens.shape
        x   = self.token_embedder(tokens)           # [B, T, d]  — ranking stream
        pos = self.pos_emb[:, :T].expand(B, -1, -1)
        res = torch.zeros(B, T, self.up_projs[0].out_features, device=tokens.device)  # [B, T, res_dim]

        for layer, up_proj in zip(self.layers, self.up_projs):
            x   = layer(x, pos)                     # [B, T, d], no skip
            res = res + up_proj(x)                  # [B, T, res_dim]

        return self.unembedder(res)


# ── Run ────────────────────────────────────────────────────────────────────────
sampler = make_sampler(DEVICE, random_seed=1)
model = LUTTransformerV3(cfg).to(DEVICE)
