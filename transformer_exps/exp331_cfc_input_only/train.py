...
# exp331: CANONICAL_FULL_COVERAGE for INPUT anchor sampling only; OUTPUT pair
# sampling stays on CANONICAL_DISTINCT (to isolate which side is responsible
# for the gap seen in exp329 vs exp330).
_INPUT_SAMPLING_POLICY  = AnchorSamplingPolicy.CANONICAL_FULL_COVERAGE
_OUTPUT_SAMPLING_POLICY = AnchorSamplingPolicy.CANONICAL_DISTINCT
...
def _make_qk(seed_offset):
    return BitPermutationLUT(
        ...
        device=DEVICE,
        input_anchor_sampling_policy=_INPUT_SAMPLING_POLICY,
        output_anchor_sampling_policy=_OUTPUT_SAMPLING_POLICY,
    )
