"""
Quick test: top-5 configs with DiffProcessor (|x_i - x_j| + rpe) instead of concat.
Attention input_dim becomes E + pos_dim instead of 2E + pos_dim.
"""
...
class DiffProcessor(nn.Module):
    """Pair features = cat(|x_i - x_j|, rpe). Dim = E + pos_dim."""
...
def make_model(cfg):
    ...
    H = cfg['H']
    d_v = cfg['d_v']
    # With DiffProcessor, score attention input_dim = E + P (not 2E + P)
    score_input_dim = E + P
...
CONFIGS = [
    ...
    dict(E=32, P=16, H=4, d_v=16, a_nap=6, a_tph=256, v_nap=4, v_tph=128, o_nap=4, o_tph=1024, excl=False,
         label="attn256_op1024_diff"),
    # #5 dv32_heavy_op
    dict(E=32, P=16, H=4, d_v=32, a_nap=6, a_tph=128, v_nap=4, v_tph=128, o_nap=4, o_tph=1024, excl=False,
         label="dv32_heavy_op_diff"),
]


if __name__ == '__main__':
    print(f"DiffProcessor: pairs = cat(|x_i - x_j|, rpe), input_dim = E + P = 48")
    ...
    for cfg in CONFIGS:
        ...
        print(f"{label:<28s} {n_params:>8,d} {loss_str:>8s} {info:>6s}")
    ...
    for label, n_params, ema in results:
        loss_str = f"{ema:.4f}" if ema is not None else "N/A"
        print(f"{label:<28s} {n_params:>8,d} {loss_str:>8s}")
