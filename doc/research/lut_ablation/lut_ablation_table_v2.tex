% LUT core-module ablation table, v2 (documentation only; no experiments were run to build it).
% v1 is in git history under lut_ablation_table_v1.{tex,pdf}.
% Build: latexmk -pdf -interaction=nonstopmode -halt-on-error lut_ablation_table_v2.tex
...
\large Comparison table, version 2}
\date{2026-09-11}
\author{}
...
\textbf{$n=8$ anchor pairs per table (nap, $K=2^n=256$ cells), 16{,}000 steps, batch 48 rows $\times$ 512 tokens, corrected eval}
(48 rows $\times$ 100 steps with the leading 12 rows skipped), target hardware nebius-h100.
\textbf{Every validation-bpb cell below is TBD.} Earlier numbers are not carried over, because none was measured on this protocol:
the two prior Gen-2 results (2.1: 1.2281, \code{exp\_g\_0014}; 2.2: 1.2176, \code{exp\_n\_0044}) ran at $n=6$ with the old
batch-coupled eval and tied embeddings, and the Gen-3 runs \code{exp\_g\_0207}/\code{exp\_g\_0206} are 48K-step runs.
...
\code{margin} & $g\big(\sum_j m_j\big)\prod_j\sigma(2m_j)$ (LookupFFN kernel) & 0.56 & 47 (including all four runs cited here\footnotemark) \\
\midrule
...
\stepcounter{footnote}\footnotetext{\code{exp\_g\_0193}, \code{exp\_g\_0194}, \code{exp\_g\_0206}, \code{exp\_g\_0207}.}
...
\item[\note{d}] \textbf{Measured} (\code{continuity\_probe.py}, \code{continuity\_probe\_trained.py}).
  \textbf{3.1, paper config, random init, float64:} at the boundary the score is a median $0.50\times$ the typical table score, never 0.
  The jump is a median 5.4\% of $\|y_h\|$ (p90 14\%), against 4.6e-10 for a same-size step with no flip, i.e. 8 orders of magnitude above the float floor.
  \textbf{3.1, trained} (\code{exp\_g\_0193}, real val tokens): median 3.5\% of the head output and 0.9\% of the FFN output after \code{decompress};
  about 1.2\% of real margins lie within $10^{-2}$ of zero. \textbf{3.2, trained} (\code{exp\_g\_0194}): the argmin-switch jump is a median 0.44\% of the head output
  and 0.11\% of the FFN output, about $8\times$ smaller.
\item[\note{e}] The gain matches \code{margin}'s mean on the same margins; the ratio is stable through training (38.9 at init, 36.3 after 4K steps,
  37.4 on \code{exp\_g\_0193}). \textbf{Confound:} \code{min\_margin} changes continuity \emph{and} selectivity together (within-token CV 1.7--2.0 vs 0.87--0.98 for
...
Per token, one layer, core LUT only. Concrete values at $H=8$, $tph=64$, $d_{in}=d_{out}=48$, $n=8$ ($K=256$), $P=512$, $d=d_{out}$.
Validation bpb on the protocol above.
\footnotesize
...
\item Rows 1.1/1.2 cannot run at all until Gen 1 is wired into \code{CompressionMultiHeadLUT}: \code{MultiHeadLut} takes a shared \code{[B, input\_dim]} input rather than
  the per-head \code{[T,H,d\_in]} layout (for example via \code{anchor\_candidates} restricting each head's anchors).
\end{enumerate}
...
\begin{itemize}\setlength\itemsep{1pt}
\item \textbf{Every validation-bpb cell is TBD} on the protocol ($n=8$, 16K, batch 48, corrected eval).
\item \textbf{Needs implementation first:} 1.1 and 1.2 (wiring); 2.3 and 3.3 (open decisions). Cost cells for 2.3 are n/a until it exists.
\item \textbf{Needs a run:} 2.1, 2.2, 2.4, 3.1, 3.2.
...
\code{bounded} unmatched +0.225 (stopped at 3,200 steps, widening); \code{bounded} $\times$\,12.61 +0.0004; \code{bounded\_norm} +0.0066 (inside the 0.0096 seed sd);
...
\textbf{$-$0.0346} (10.3$\times$ the 0.00335 seed spread). This is why \code{margin} became the standard.
\end{document}
