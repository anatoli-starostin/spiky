...
\code{abl\_NN} names a cell's prepared run folder in \code{experiments/ffn\_replacement/lut\_ablation}.
\footnotesize
...
\textbf{1.165147} (\code{exp\_n\_0135}); seed 2: 1.161798 (\code{exp\_n\_0176}); rerun \code{abl\_10} &
n/a: no LUT tables &
...
\tbd{not yet run} (\code{abl\_11}) &
\tbd{not yet run} (\code{abl\_12}) &
\code{CompressionMultiHeadLUT(lut\_impl='gen1')}: \code{MultiHeadLut(smooth\_mode=False, n\_alternatives=1, uncertainty\_mode=INVERSE\_L1)}, each head's anchors in its own slice. \\
1.2 &
$P(4d+4n+4)=\mathbf{116{,}736}$ &
$P(8d+9)=\mathbf{201{,}216}$ &
$P\cdot2d=\mathbf{49{,}152}$ &
$P\cdot4d=\mathbf{98{,}304}$ &
\tbd{not yet run} (\code{abl\_13}) &
\tbd{not yet run} (\code{abl\_14}) &
...
\textbf{1.180622} (\code{exp\_n\_0121}); rerun \code{abl\_07} &
\tbd{not yet run} (\code{abl\_15}) &
...
\tbd{not yet run} (\code{abl\_01}) &
\tbd{not yet run} (\code{abl\_16}) &
...
\tbd{not yet run} (\code{abl\_02}) &
\tbd{not yet run} (\code{abl\_17}) &
...
$P\,kd=\mathbf{49{,}152}$ &
$P\,2kd=\mathbf{98{,}304}$ &
\tbd{not yet run} (\code{abl\_03}) &
\tbd{not yet run} (\code{abl\_18}) &
...
$P\,kd=\mathbf{24{,}576}$ &
$P\,2kd=\mathbf{49{,}152}$ &
\textbf{1.169328} (\code{exp\_g\_0248}); rerun \code{abl\_08} &
\textbf{1.163698} (\code{exp\_g\_0249}); rerun \code{abl\_09} &
...
$P\,kd=\mathbf{49{,}152}$ &
$P\,2kd=\mathbf{98{,}304}$ &
\tbd{not yet run} (\code{abl\_04}) &
\tbd{not yet run} (\code{abl\_05}) &
...
$0$ &
$P\,d=\mathbf{24{,}576}$ &
\tbd{not yet run} (\code{abl\_19}) &
\tbd{not yet run} (\code{abl\_20}) &
As 3.1 with \code{forward\_mode='hard'}. \\
3.4 &
$P(2n+d)=\mathbf{32{,}768}$ &
$P(5d+16n+29)=\mathbf{203{,}264}$\note{b} &
$0$ &
$P\,2d=\mathbf{49{,}152}$ &
\tbd{not yet run} (\code{abl\_21}) &
\tbd{not yet run} (\code{abl\_22}) &
As 3.2 with \code{forward\_mode='hard'}. \\
\end{longtable}
\normalsize
...
\begin{itemize}\setlength\itemsep{0pt}
\item \textbf{Run the 21:} every cell above (20 LUT cells and V) is prepared, one folder each, in \code{experiments/ffn\_replacement/lut\_ablation}, whose README gives host and trainer; none is run yet.
  2.1, 3.1, 3.1 $+$TV and V are reruns of their published runs, to put the whole table on one set of trainers.
\item \textbf{Seeds:} zero replicates at this geometry; one extra seed of \code{exp\_g\_0248} ($\approx$\,0.95\,h), prepared as \code{abl\_06}, would give a real LUT seed spread.
