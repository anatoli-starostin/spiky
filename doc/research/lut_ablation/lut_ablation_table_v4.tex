...
\item \textbf{Runnable for Gen 3 only:} the trainer's penalty (\code{lut\_tv\_penalty}) collects \code{LightMultiHeadLUT} tables only and is a constant zero on Gen 1 and Gen 2 models,
  so their $+$TV cells need it extended to \code{FastMultiHeadLut} and \code{MultiHeadLut} tables first.
\end{itemize}
...
\item \textbf{Wire TV into Gen 1 and Gen 2:} extend \code{lut\_tv\_penalty} beyond \code{LightMultiHeadLUT}; until then their $+$TV cells cannot be run.
