...
Would be: the softmax of 2.1(b) restricted to the two cells $c_t$ and $c_t'$ (as in 2.2(a)), which is exactly 2.2(a)'s 2-cell blend.
\tbd{not implemented; open decision.} \code{FastMultiHeadLut} refuses this combination when it is built: \code{backward\_topk>0} raises \code{ValueError} unless \code{forward\_mode='hybrid\_smooth'}. Patch the code or drop the row. \\[2pt] \hline
...
so the restricted softmax equals the forward weight $w_t$ and forward and backward coincide; unlike 2.2, the input gradient reaches only pair $j^\star$.
Configured as \code{backward\_topk=1}: it counts neighbours beyond the addressed cell, so 2 alternatives is 1, not 2.} \\[2pt] \hline
