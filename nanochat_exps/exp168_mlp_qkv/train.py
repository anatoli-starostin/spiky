"""nanochat_exps/exp071_canonicalize_out — fork of exp064 with rank-canonicalized output.

Single change vs exp064: each LUTBlock output is rank-canonicalized via
V2D -> D2V before leaving the block. out_proj's real-valued output is
mapped to a 4560-dim ±1 dominance vector then projected back to E=96 via
the Borda matrix and LayerNorm. The previous standalone `out_ln` is
replaced by `DominanceToVector(normalise=True)` (which carries its own
LayerNorm), so param count is unchanged (~192 LN params relocate).

Now the entire path from out_proj to the unembedder is ranking-only —
magnitudes are stripped, only the relative ordering of E=96 components
of out_proj output survives.

How to launch:

    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python \\
        -u nanochat_exps/exp071_canonicalize_out/train.py \\
        > nanochat_exps/exp071_canonicalize_out/stdout.log 2>&1 &
"""
...
class PerHeadMLP(nn.Module):
    """Per-head MLP: x[..., d_in] -> [..., n_heads, d_out].

    Each head has its own (Linear → GELU → Linear → LayerNorm) chain, no bias.
    Implemented via einsum on grouped weight tensors.
    """
    def __init__(self, n_heads, d_in, d_hidden, d_out, seed=None):
        super().__init__()
        ...
        self.W1 = nn.Parameter(torch.empty(n_heads, d_in, d_hidden))
        self.W2 = nn.Parameter(torch.empty(n_heads, d_hidden, d_out))
        with torch.no_grad():
            self.W1.copy_(torch.randn(self.W1.shape, generator=gen) * math.sqrt(2.0 / d_in))
            self.W2.copy_(torch.randn(self.W2.shape, generator=gen) * math.sqrt(1.0 / d_hidden))
        self.ln_weight = nn.Parameter(torch.ones(n_heads, d_out))
        self.ln_bias = nn.Parameter(torch.zeros(n_heads, d_out))

    def forward(self, x):
        h = torch.einsum('...d,hde->...he', x, self.W1)
        h = F.gelu(h)
        h = torch.einsum('...he,hef->...hf', h, self.W2)
        mean = h.mean(dim=-1, keepdim=True)
        var = h.var(dim=-1, keepdim=True, unbiased=False)
        h = (h - mean) / (var + 1e-5).sqrt()
        h = h * self.ln_weight + self.ln_bias
        return h


def _make_qk_mlp(role, layer_idx, seed_offset):
    """Per-head MLP for Q or K. Output dim = d_qk per head."""
    d_in = E + (_pos_emb_dim(layer_idx) if _POS_EMB_ACTIVE else 0)
    return PerHeadMLP(
        n_heads=H,
        d_in=d_in,
        d_hidden=cfg['qk_mlp_hidden'],
        d_out=d_qk,
        seed=cfg['random_seed'] + seed_offset,
    )

def _make_v_mlp(layer_idx, seed_offset):
    """Per-head MLP for V. Output dim = d_v per head. No pos input (V is content-only)."""
    return PerHeadMLP(
        n_heads=H,
        d_in=E,
        d_hidden=cfg['v_mlp_hidden'],
        d_out=d_v,
        seed=cfg['random_seed'] + seed_offset,
    )
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        super().__init__()
        self.mlp_q = _make_qk_mlp('q', layer_idx, 100 + layer_idx)
        self.mlp_k = _make_qk_mlp('k', layer_idx, 150 + layer_idx)
        self.v_mlp = _make_v_mlp(layer_idx, 200 + layer_idx)
        self.out_proj = _make_out(layer_idx, 400 + layer_idx)

        canon_t = cfg.get('canon_temperature', 0.1)
        # No qk_v2d — Q/K stay real-valued for standard SDPA.
        ...
        self._inv_sqrt_dqk = float(1.0 / math.sqrt(d_qk))
    ...
    def forward(self, x, pos_emb):
        ...
        # Per-head MLP Q/K (real-valued), standard SDPA.
        q_flat = self.mlp_q(xp)                                        # [B*T, H, d_qk]
        k_flat = self.mlp_k(xp)
        q = q_flat.reshape(B, T, H, d_qk).permute(0, 2, 1, 3)          # [B, H, T, d_qk]
        k = k_flat.reshape(B, T, H, d_qk).permute(0, 2, 1, 3)

        v_flat = self.v_mlp(x_flat)                                    # [B*T, H, d_v]
        v = v_flat.reshape(B, T, H, d_v).permute(0, 2, 1, 3)            # [B, H, T, d_v]

        # Bake learnable attn_scale into Q so gradient flows; SDPA's `scale=`
        # arg only accepts float. Effective Q@K^T scale = attn_scale / sqrt(d_qk).
        q = q * self.attn_scale
        attn = F.scaled_dot_product_attention(
            q, k, v, is_causal=True, scale=self._inv_sqrt_dqk,
        )
        # attn shape: [B, H, T, d_v]

        out_in = attn.permute(0, 2, 1, 3).reshape(B * T, H * d_v)
        out_real = self.out_proj(out_in).squeeze(1)                    # [B*T, E]
        out_dom  = self.out_v2d(out_real)                              # [B*T, P=E*(E-1)/2] in ±1
...
print(f'Q/K MLPs (per-head): {E}->{cfg["qk_mlp_hidden"]}->ReLU->{d_qk}->LN, H={H}')
print(f'V MLP (per-head): {E}->{cfg["v_mlp_hidden"]}->GELU->{d_v}->LN, H={H}')
