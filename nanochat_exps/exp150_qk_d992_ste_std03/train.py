"""nanochat_exps/exp150_qk_d992_ste_std03 — fork of exp149.

Single change: qk_mhlut_init_std = 0.001 -> 0.03 (qk_lut only).

Targets std(qk_out at init) ≈ τ=0.1 so the STE operates near the
saturation knee at init (soft has meaningful magnitude, signs reflect
weight direction not noise) instead of the deep-linear regime.

How to launch:

    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python \\
        -u nanochat_exps/exp150_qk_d992_ste_std03/train.py \\
        > nanochat_exps/exp150_qk_d992_ste_std03/stdout.log 2>&1 &
"""
...
class STESign(nn.Module):
    """Element-wise sign with straight-through estimator.

    forward returns ±1; gradient flows through soft = x / (tau + |x|).
    """
    ...
    def _soft(self, x: torch.Tensor) -> torch.Tensor:
        return x / (self.temperature + x.abs())
...
def _make_qk_joint(layer_idx, seed_offset):
    ...
    qk_kwargs['initial_weights_noise'] = cfg.get(
        'qk_mhlut_init_std', cfg.get('mhlut_init_std', 0.001)
    )
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        # Element-wise STE sign on qk_lut output (replaces V2D pipeline).
        self.qk_ste = STESign(temperature=cfg.get('ste_temperature',
                                                  cfg.get('canon_temperature', 0.1)))
    ...
    def forward(self, x, pos_emb):
        ...
        qk_bin = self.qk_ste(qk_out)                                   # ±1 STE
        q_vec = qk_bin[..., :d_qk]
        k_vec = qk_bin[..., d_qk:]
...
print(f'STE temperature: {cfg.get("ste_temperature", cfg.get("canon_temperature", 0.1))}')
print(f'qk init_std: {cfg.get("qk_mhlut_init_std", cfg.get("mhlut_init_std", 0.001))} '
      f'(other LUTs: {cfg.get("mhlut_init_std", 0.001)})')
