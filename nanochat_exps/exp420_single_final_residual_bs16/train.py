...
EXP_DIR_FOR_IMPORT = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, EXP_DIR_FOR_IMPORT)
from tiny_multi_nap_multi_head_lut import TinyMultiNapMultiHeadLut
...
_OUT_PROJ_MULTI_NAP = cfg.get('out_proj_multi_nap')   # list of [nap, tph] pairs, or None
def _make_out(layer_idx, seed_offset):
    """out_proj. If `out_proj_multi_nap` is set, builds a multi-NAP sum of
    TinyMHLut(soft) modules; otherwise standard single-NAP TinyMHLut."""
    if _OUT_PROJ_MULTI_NAP is not None:
        kwargs = dict(_TINY_SOFT_KWARGS)
        return TinyMultiNapMultiHeadLut(
            input_dim=H * d_v,
            n_heads=1,
            n_outputs=E,
            nap_tph_pairs=_OUT_PROJ_MULTI_NAP,
            base_random_seed=cfg['random_seed'] + seed_offset,
            device=DEVICE,
            **kwargs,
        )
...
class LUTBlock(nn.Module):
    ...
    def __init__(self, layer_idx):
        ...
        # v_lut removed in exp404 — v comes entirely from qkv_lut's last d_v outputs.
        self.out_proj     = _make_out(layer_idx, 400 + layer_idx)
        # NOTE (exp420): per-layer residual_lut REMOVED — a single final residual_lut
        # of 6× larger tph is applied to the final x_lut after all layers.

        self.q_norm = nn.LayerNorm(d_qk)
        self.k_norm = nn.LayerNorm(d_qk)

        # Pre-norm BEFORE qkv LUT (GPT-2 style); E-stream residual after.
        self.ln_pre  = nn.LayerNorm(E)
        # NOTE (exp420): no per-layer ln_post — moved to model-level ln_post_final.

    def forward(self, x, cos, sin):
        B, T, _ = x.shape
        x_flat = x.reshape(B * T, E)

        # Pre-norm BEFORE qkv LUT (GPT-2 style pre-norm).
        ...
        # v_lut removed: v_vec is just qkv_lut's last d_v outputs.
        v = v_branch.reshape(B, T, H, d_v).permute(0, 2, 1, 3)
...
class Model(nn.Module):
    def __init__(self):
        ...
        # exp420: single final residual_lut applied AFTER all layers, replacing the
        # per-layer accumulation. tph_final = 6 × per-layer tph (matches total params).
        self.ln_post_final  = nn.LayerNorm(E)
        self.residual_lut_final = _make_residual_lut(layer_idx=0, seed_offset=9999)
    ...
    def forward(self, tokens, targets=None, loss_reduction='mean'):
        ...
        # exp420: SINGLE residual_lut applied to the final E-stream.
        x_lut_flat = x_lut.reshape(B * T, E)
        r_in = self.ln_post_final(x_lut_flat)
        x_resid = self.residual_lut_final(r_in).squeeze(1).reshape(B, T, D)
...
print(f'QKV joint TinyMHLut({cfg.get("qkv_backward_mode", "soft")}, noise_eps={_NOISE_EPS}): in_nap={cfg.get("qkv_input_nap", cfg["qk_input_nap"])} tph={cfg.get("qkv_tph", cfg["qk_tph"])} n_out=2*d_qk+d_v={2*d_qk+d_v} (q,k,v all from this LUT; no separate v_lut)')
