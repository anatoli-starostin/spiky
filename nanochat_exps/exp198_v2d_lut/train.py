"""nanochat_exps/exp198_v2d_lut.

Fork of exp197 (soft V2D + per-block attn+FFN, val_bpb=1.6806 @ 8K). The
two MLPs in exp197 (out_proj after SDPA, FFN sub-layer) are replaced with
a single MultiHeadLut + SparseScatter. Block structure becomes single-
residual:

  x = x + scatter(lut(SDPA(V2D(LN(x)))))

LUT consumes raw post-SDPA E-dim activations (no extra LN before LUT).
Defaults: nap=6, tph=384, n_sparse=32, n_alternatives=3, smooth_mode=True,
CANONICAL_FULL_COVERAGE anchor sampling. Param budget per block ≈ 2.36M
(matches the exp197 out_proj+FFN budget).

Launch:
    PYTHONPATH=/home/starost/nanochat \\
      /home/starost/spiky/.venv/bin/python -u \\
      nanochat_exps/exp198_v2d_lut/train.py \\
      > nanochat_exps/exp198_v2d_lut/stdout.log 2>&1 &
"""
...
class V2DSDPABlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # LUT replacing out_proj + FFN: input is raw post-SDPA (no extra LN).
        per_table = self.lut(attn.view(B * T, E))              # [B*T, 1, tph, n_sparse]
