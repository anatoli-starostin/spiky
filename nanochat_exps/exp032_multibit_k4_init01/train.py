...
from spiky.lutorch.multi_bit_permutation_lut import (
    MultiBitPermutationLUT as BitPermutationLUT,
    MultiBitMultiHeadLUT as BitMultiHeadLUT,
)
from spiky.lutorch.multi_bit_permutation_lut_optimizer import (
    MultiBitPermutationLUTOptimizer as BitPermutationLUTOptimizer,
)
...
# --- LUT block helpers --------------------------------------------------------
_BIT_WIDTH = int(cfg.get('bit_width', 4))
_HALF_RANGE = 1 << (_BIT_WIDTH - 1)

def _make_qk_joint(layer_idx, seed_offset):
    """Single MultiBitMultiHeadLUT outputting both Q and K pair-dominance via
    one anchor lookup. K=4: each vote is a signed K-bit int (range [-8, 7]
    for K=4); scale matches MultiBitPermutationLUT's wrapper convention
    `0.5 / (half_range * sqrt(votes_per_pair))`."""
    ...
    qk_scale = 0.5 / (_HALF_RANGE * math.sqrt(votes_per_output))
