...
def _make_v(layer_idx, seed_offset):
    ...
    v_kwargs = dict(v_kwargs)
    v_kwargs['initial_weights_noise'] = cfg.get('v_lut_init_std',
                                                cfg.get('mhlut_init_std', 0.001))
    return TinyMultiHeadLut(
        input_dim=E,
        n_heads=H,
...
def _make_out(layer_idx, seed_offset):
    ...
    kwargs = _TINY_SOFT_KWARGS if mode == 'soft' else _TINY_MULTIALT_KWARGS
    kwargs = dict(kwargs)
    kwargs['initial_weights_noise'] = cfg.get('out_lut_init_std',
                                              cfg.get('mhlut_init_std', 0.001))
    return TinyMultiHeadLut(
        input_dim=H * d_v,
        n_heads=1,
...
def _make_residual_lut(layer_idx, seed_offset):
    ...
    kwargs['initial_weights_noise'] = cfg.get('residual_lut_init_std',
                                              cfg.get('mhlut_init_std', 0.001))
    return TinyMultiHeadLut(
        input_dim=E,
        n_heads=1,
...
print(
    f'optimizer groups: '
    f'lut={sum(p.numel() for p in lut_params):,} (wd=0, lr={_LUT_LR}, betas=({_LUT_BETA1},{_LUT_BETA2})) | '
...
# --- Windowed-gradient smoother on LUT params ---------------------------------
# Maintains a sliding window of the last W per-parameter gradients for the
# LUT weights only. After each backward we call smoother.step_and_smooth(),
# which replaces p.grad with the windowed mean. Other parameters keep their
# regular per-batch gradients.
sys.path.insert(0, EXP_DIR)
from windowed_grad import WindowedGradSmoother
_GRAD_WINDOW = int(cfg.get('grad_window', 1))
if _GRAD_WINDOW > 1:
    smoother = WindowedGradSmoother(lut_params, window_size=_GRAD_WINDOW)
    print(f'WindowedGradSmoother on LUT params: W={_GRAD_WINDOW}, '
          f'buffer={smoother.total_buffer_bytes() / 1e6:.1f} MB')
else:
    smoother = None
    print('WindowedGradSmoother disabled (grad_window <= 1)')
