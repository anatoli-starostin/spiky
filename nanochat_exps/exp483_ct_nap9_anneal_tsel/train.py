"""nanochat_exps/exp483_ct_nap9_anneal_tsel — fork of exp475.

SINGLE change: all 4 TinyMultiHeadLut -> TinyOrderedMultiHeadLut(CONNECTED_TRIPLETS).
NAP=6 with CONNECTED_TRIPLETS -> 2 triplets/table, max_orders=36 (vs 64).
Weight table is 44% smaller; activation cost (score tensor [B,T,n_tbls,64]) unchanged.
Everything else identical to exp475 (LION β=(0.9,0.95), lut_lr=2e-4, bs=16, 8K steps).
"""
...
from spiky.lutorch.tiny_multi_head_lut import TinyOrderedMultiHeadLut
...
# --- LUT factories (all TinyOrderedMultiHeadLut, CONNECTED_TRIPLETS) ----------
_T_SEL_START = cfg['T_sel_start']
_T_SEL_END   = cfg['T_sel_end']

def get_T_sel(step):
    """Log-linear anneal from T_sel_start (step 1) to T_sel_end (step N_STEPS)."""
    progress = (step - 1) / max(N_STEPS - 1, 1)
    return math.exp(math.log(_T_SEL_START) + progress * (math.log(_T_SEL_END) - math.log(_T_SEL_START)))

def set_lut_select_temp(mdl, T_sel):
    log_T = math.log(T_sel)
    for layer in mdl.layers:
        for name in ('qkv_lut', 'v_lut', 'out_proj', 'residual_lut'):
            getattr(layer, name).log_select_temp.data.fill_(log_T)

_ORDERED_KWARGS = dict(
    weight_dtype=torch.float32,
    anchor_sampling_policy=AnchorSamplingPolicy.CONNECTED_TRIPLETS,
    initial_weights_noise=cfg.get('mhlut_init_std', 0.001),
    soft_score_temp=cfg.get('soft_score_temp', 0.5),
    select_temp=_T_SEL_START,
...
# Report max_orders per module type
def _max_orders_info():
    blk = model.layers[0]
    parts = []
    for name in ('qkv_lut', 'v_lut', 'out_proj', 'residual_lut'):
        m = getattr(blk, name)
        parts.append(f'{name}:max_orders={m.max_orders}')
    return ' | '.join(parts)

print(
    f'LUT optimizer = {_LUT_OPT} (lut_lr={_LUT_LR}) | '
    f'lut={sum(p.numel() for p in lut_params):,} | '
    f'decay(unembed)={sum(p.numel() for p in decay_params):,} | '
    f'tok_emb={sum(p.numel() for p in tok_emb_params):,} | '
    f'nodecay_other={sum(p.numel() for p in nodecay_params):,}'
)
print(f'D={D}, E={E}, H={H}, d_qk={d_qk}, d_v={d_v}, L={N_LAYERS}')
print(f'TinyOrderedMHLut CONNECTED_TRIPLETS NAP=6: {_max_orders_info()}')
print(f'qkv_lut  in_nap={cfg["qkv_input_nap"]} tph={cfg["qkv_tph"]} n_out={2*d_qk+d_v}')
print(f'v_lut    in_nap={cfg["v_input_nap"]} tph={cfg["v_tph"]} d_v={d_v}')
print(f'out_proj in_nap={cfg["out_input_nap"]} tph={cfg["out_tph"]} n_out=E={E}')
print(f'residual in_nap={cfg["residual_input_nap"]} tph={cfg["residual_tph"]} n_out=D={D}')
...
print(f'Tracking {len(temp_specs)} learnable temperatures')
...
for step in range(1, N_STEPS + 1):
    ...
    set_lut_select_temp(model, get_T_sel(step))
    ...
    if step % 100 == 0 or step == 1:
        print(f'step {step:6d} | loss={ema:.4f} | lr={lr_scale * cfg["adam_lr"]:.2e} | T_sel={get_T_sel(step):.4f}')
