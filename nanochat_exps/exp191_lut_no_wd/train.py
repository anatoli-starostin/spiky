...
# Identify LUT weight tensors (TinyMultiHeadLut.weights, ndim=3) — exclude
# from weight decay. Hypothesis (exp191): cells receive gradient only when
# their anchor pattern is hit, but constant decay applies every step → rare-
# activation cells get decayed faster than they can learn.
_lut_weight_ids = set()
for m in model.modules():
    if isinstance(m, TinyMultiHeadLut):
        _lut_weight_ids.add(id(m.weights))

decay_params      = [p for p in model.parameters() if p.ndim >= 2 and id(p) not in _lut_weight_ids]
lut_nodecay_params = [p for p in model.parameters() if id(p) in _lut_weight_ids]
...
adam_groups = [
    ...
    dict(params=lut_nodecay_params,  lr=cfg['adam_lr'], betas=(0.9, 0.95), eps=1e-8,
         weight_decay=0.0),
...
print(f'AdamW groups: decay={sum(p.numel() for p in decay_params):,} '
      f'lut_nodecay={sum(p.numel() for p in lut_nodecay_params):,} '
