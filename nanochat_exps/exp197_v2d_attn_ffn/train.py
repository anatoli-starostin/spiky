"""nanochat_exps/exp197_v2d_attn_ffn.

Fork of exp196 (soft V2D + untied head). Restructured block to be more
consistent with a standard transformer: two residual sub-layers per block.

  Attention sub-layer:
    x = x + out_proj(SDPA(V2D(LN1(x))))
  FFN sub-layer:
    x = x + ffn(LN2(x))

out_proj and ffn are both 2-layer MLPs (E -> 4E -> GELU -> E), no bias,
fc2 zero-init so each sub-layer starts as identity residual. Soft V2D and
untied head unchanged.

Launch:
    PYTHONPATH=/home/starost/nanochat \\
      /home/starost/spiky/.venv/bin/python -u \\
      nanochat_exps/exp197_v2d_attn_ffn/train.py \\
      > nanochat_exps/exp197_v2d_attn_ffn/stdout.log 2>&1 &
"""
...
FFN_HIDDEN    = cfg.get('ffn_hidden', 4 * E)
...
class V2DSDPABlock(nn.Module):
    """Two sub-layer block (vanilla-transformer-style):
       attn:  LN -> V2D -> per-head Q/K/V -> SDPA -> MLP out_proj -> residual
       ffn:   LN -> 2-layer MLP                                    -> residual"""
    def __init__(self, layer_idx: int):
        ...
        # out_proj after SDPA: 2-layer MLP, multiplier=4. Zero-init fc2 so the
        # attention sub-layer starts as identity residual.
        out_hidden = cfg.get('out_mlp_hidden', 4 * E)
        ...
        # FFN sub-layer: pre-norm + 2-layer MLP, multiplier=4. Zero-init fc2 so
        # the FFN sub-layer also starts as identity residual.
        self.ln_ffn = nn.LayerNorm(E)
        ffn_hidden = cfg.get('ffn_hidden', 4 * E)
        self.ffn_fc1 = nn.Linear(E, ffn_hidden, bias=False)
        self.ffn_fc2 = nn.Linear(ffn_hidden, E, bias=False)
        nn.init.normal_(self.ffn_fc1.weight, std=0.02)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = self.out_fc2(F.gelu(self.out_fc1(attn)))                  # E->4E->E
        x = x + out                                                     # attn residual
        # === FFN sub-layer ===
        x_ffn = self.ln_ffn(x)
        ffn = self.ffn_fc2(F.gelu(self.ffn_fc1(x_ffn)))                 # E->4E->E
...
print(f'ffn MLP:      hidden={FFN_HIDDEN} (zero-init fc2)')
