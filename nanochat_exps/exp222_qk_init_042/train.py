"""nanochat_exps/exp222_qk_init_042 — fork of exp221.

exp221 (qk_lut_init_std=0.02, no QK-norm) was slow at init: pre-softmax std
~0.034 (vs exp001's 0.154). Diagnosis: matching exp001's *weight* init_std=0.02
doesn't match its *output* std because LUT amplification is sqrt(tph/3)=9.24
while Linear amplification is sqrt(n_embd)=19.6.

Single change vs exp221: bump qk_lut_init_std from 0.02 to 0.0424. This makes
Q/K element std ≈ 0.39 (matching exp001) and pre-softmax std ≈ 0.154
(matching exp001's softmax temperature). Still no QK-norm.

Launch:
    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python -u \\
        nanochat_exps/exp222_qk_init_042/train.py \\
        > nanochat_exps/exp222_qk_init_042/stdout.log 2>&1 &
"""
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        # No QK-norm. Plain SDPA, default 1/sqrt(d_qk) scale, no learnable attn_scale.
...
print(f'Q/K Joint MHLut: in_nap={cfg["qk_input_nap"]} tph={cfg["qk_tph"]} d_qk={d_qk} (n_outputs=2*d_qk={2*d_qk}) init_std={cfg.get("qk_lut_init_std", cfg.get("mhlut_init_std", 0.001))} [no V2D, no QK-norm, raw -> SDPA]')
