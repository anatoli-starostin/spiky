"""nanochat_exps/exp193_v2d_sdpa_mlpout.

Fork of exp192 with the LUT out_proj replaced by a standard 2-layer MLP
(Linear E -> 4E -> GELU -> Linear 4E -> E, no bias). Final linear zero-init,
so each block starts as identity residual.

Architecture (per block, pre-norm):
  1. LN -> VectorToDominance per-head (n_pairs=D, n_heads=H), producing
     dominance vectors [B, T, H, D]. linear_mode=True so V2D = x_a - x_b.
  2. Per-head Q, K, V matrices (D -> E_h), no bias.
  3. F.scaled_dot_product_attention (causal).
  4. Concatenate heads -> [B, T, E].
  5. MLP out_proj: Linear(E, 4E) -> GELU -> Linear(4E, E), no bias.
     fc1 std=0.02; fc2 zero-init.
  6. Add residual.

Final: ln_f -> head (weight-tied to tok_emb).

Launch:
    PYTHONPATH=/home/starost/nanochat \\
      /home/starost/spiky/.venv/bin/python -u \\
      nanochat_exps/exp193_v2d_sdpa_mlpout/train.py \\
      > nanochat_exps/exp193_v2d_sdpa_mlpout/stdout.log 2>&1 &
"""
