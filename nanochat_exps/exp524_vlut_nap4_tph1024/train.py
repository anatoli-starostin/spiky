...
def _make_qk(layer_idx, seed_offset):
    # exp523: HARD argmax TinyMHLut on LION, nap=4/tph=256. n_out = 2*d_qk + d_v:
    # q,k from this PLUS a shallow additive v-branch (last d_v added to v_lut),
    # restoring exp507/exp428's shared-anchor v-branch -- but here from a HARD argmax qk.
    return TinyMultiHeadLut(
        input_dim=E,
        n_heads=H,
        n_outputs=2 * d_qk + d_v,              # q,k + additive v-branch
...
class LUTBlock(nn.Module):
    ...
    def forward(self, x, cos, sin):
        ...
        qk_out = self.qk_lut(x_pre)                     # [B*T, H, 2*d_qk + d_v]
        q_vec = self.q_norm(qk_out[..., :d_qk])
        k_vec = self.k_norm(qk_out[..., d_qk:2 * d_qk])
        v_branch = qk_out[..., 2 * d_qk:]              # [B*T, H, d_v] shared-anchor v-branch
        q = q_vec.reshape(B, T, H, d_qk).permute(0, 2, 1, 3)
        k = k_vec.reshape(B, T, H, d_qk).permute(0, 2, 1, 3)
        q, k = apply_rope(q, k, cos[:T], sin[:T])

        v_vec = self.v_lut(x_pre) + v_branch           # [B*T, H, d_v] + additive v-branch
...
print(f'qk_lut       TinyMHLut(soft argmax, LION) [exp523 HARD q,k + v-branch]: in_nap={cfg["qkv_input_nap"]} tph={cfg["qkv_tph"]} n_out=2*d_qk+d_v={2*d_qk+d_v}  [+additive v-branch]')
