"""exp730 — fork of exp729: FastMultiHeadLUT forward_mode = 'hybrid_smooth' + ball bwd.

Every LUT module uses FastMultiHeadLUT with:
  forward_mode  = 'hybrid_smooth' (top-2 fwd: main row + alt at least-confident
                                   anchor, sigmoid-blended; (1-u)*main + u*alt)
  backward_mode = 'ball'          ((NAP+1)-row Hamming-1 ball softmax surrogate
                                   for input + temperature grads; K-row Z einsum
                                   computed once, gathered to (NAP+1) cols)

Hypothesis: ball-bwd's (NAP+1)-row coverage includes the alt row that top-2 fwd
actually uses, so the gradient signal lines up with forward selection — smaller
hard-soft gap than exp724's K-row dense (which spreads softmax mass across all K).

Bench reference (out_proj, NAP=7, K=128, tph=512, B=12288, after Jun-6
n_outputs>=128 dispatch optimisation):
  FastMHLut hybrid_smooth / ball  = 24.0 ms  (this run's recipe)
  FastMHLut hard           / ball = 21.2 ms  (exp729)
  exp724 Tiny K-row dense bwd     = 34.5 ms
  exp726 Tiny NAP+1 autograd      = 41.8 ms

Quality reference @ 16k bs=24 ga=2:
  exp724 (hybrid_smooth + K-row dense): soft 1.1936, hard 1.2611, gap +0.068
  exp726 (NAP+1 fwd + autograd bwd):    soft 1.1997, hard 1.3399, gap +0.140
  exp729 (hard + ball bwd):                          hard 1.2360, gap 0

Eval: both soft (current hybrid_smooth forward) and hard (FastMHL.forward_mode
flipped to 'hard') bpb are computed at every eval_every step; gap = hard - soft.
"""
...
# --- Eval: both soft (train mode) and hard (forward_mode='hard') --------------
_LUT_MODULES = [m for m in model.modules() if isinstance(m, FastMultiHeadLUT)]

def _evaluate_both(model, val_loader_factory_, eval_steps_, token_bytes_):
    """Return (soft_bpb, hard_bpb). Caller must have already called model.eval()."""
    soft_bpb = evaluate_bpb(model, val_loader_factory_(), eval_steps_, token_bytes_)
    saved_modes = [m.forward_mode for m in _LUT_MODULES]
    for m in _LUT_MODULES:
        m.forward_mode = 'hard'
    try:
        hard_bpb = evaluate_bpb(model, val_loader_factory_(), eval_steps_, token_bytes_)
    finally:
        for m, fm in zip(_LUT_MODULES, saved_modes):
            m.forward_mode = fm
    return soft_bpb, hard_bpb
...
csv_w.writerow(['step', 'train_loss', 'val_bpb_soft', 'val_bpb_hard', 'gap'])

train_losses_logged, val_bpbs_soft, val_bpbs_hard, val_gaps, val_steps = [], [], [], [], []
ema = None
best_soft_bpb = float('inf')
best_hard_bpb = float('inf')
t0 = time.time()
...
for step in range(1, N_STEPS + 1):
    ...
    if step % EVAL_EVERY == 0 or step == N_STEPS:
        model.eval()
        soft_bpb, hard_bpb = _evaluate_both(model, val_loader_factory, EVAL_STEPS, token_bytes)
        gap = hard_bpb - soft_bpb
        if soft_bpb < best_soft_bpb:
            best_soft_bpb = soft_bpb
        if hard_bpb < best_hard_bpb:
            best_hard_bpb = hard_bpb
        print(f'[VAL] step {step}: soft={soft_bpb:.4f}  hard={hard_bpb:.4f}  gap={gap:+.4f}')
        train_losses_logged.append(ema)
        val_bpbs_soft.append(soft_bpb)
        val_bpbs_hard.append(hard_bpb)
        val_gaps.append(gap)
        val_steps.append(step)
        csv_w.writerow([step, f'{ema:.6f}', f'{soft_bpb:.6f}', f'{hard_bpb:.6f}', f'{gap:+.6f}'])
        csv_f.flush()
...
ax2.plot(val_steps, val_bpbs_soft, label='soft (train mode)', color='red')
ax2.plot(val_steps, val_bpbs_hard, label='hard (deployment)', color='darkred', linestyle='--')
ax2.set(xlabel='step', ylabel='bpb', title='Validation BPB (soft vs hard)')
...
summary = dict(
    ...
    best_val_bpb_soft=best_soft_bpb,
    best_val_bpb_hard=best_hard_bpb,
    final_val_bpb_soft=val_bpbs_soft[-1] if val_bpbs_soft else float('nan'),
    final_val_bpb_hard=val_bpbs_hard[-1] if val_bpbs_hard else float('nan'),
    final_gap=val_gaps[-1] if val_gaps else float('nan'),
    n_params=n_params,
