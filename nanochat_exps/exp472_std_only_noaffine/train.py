...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        # Pre/post-norm: PURE standardization (x-mean)/std, NO learnable affine
        # (no gamma, no beta). Tests whether the pre-LUT norm is just magnitude
        # calibration; per-channel gamma was shown inert (exp470~exp471).
        self.ln_pre  = nn.LayerNorm(E, elementwise_affine=False)
        self.ln_post = nn.LayerNorm(E, elementwise_affine=False)
