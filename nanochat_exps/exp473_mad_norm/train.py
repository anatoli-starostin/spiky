...
class CenterScale(nn.Module):
    """exp473: MAD norm = center, then divide by per-token mean-absolute-deviation
    y = (x - mean) / mean(|x - mean|). No learnable params (affine shown inert).
    L1 magnitude calibration: same job as std/RMS (keeps |d| ~ temperature T)
    but cheaper -- only abs/mean/divide, NO square or sqrt. MAD ~ 0.8*std for
    Gaussian; the constant is absorbed by the learnable T. Centering matters here
    for the scale estimate (not for the LUT's mean-invariant d)."""
    ...
    def forward(self, x):
        ...
        mad = xc.abs().mean(dim=-1, keepdim=True)
        return xc / (mad + self.eps)
