...
# exp569: local module for multi-NAP out_proj (sum of TinyMHLut sub-LUTs of
# different NAP/tph; ports exp387's recipe to the E=96 backbone).
...
def _make_out(layer_idx, seed_offset):
    # exp569: when out_proj_multi_nap is set, build a multi-NAP sum of TinyMHLut
    # sub-LUTs (exp387 recipe). Otherwise standard single-NAP TinyMHLut.
...
if _OUT_PROJ_MULTI_NAP is not None:
    print(f'out_proj     TinyMultiNapMultiHeadLut(soft, noise_eps={_NOISE_EPS}): nap_tph_pairs={_OUT_PROJ_MULTI_NAP} n_out=E={E}  [sum]')
else:
