...
def _make_qk_joint(layer_idx, seed_offset):
    """Single BitMultiHeadLUT producing both Q and K pair-dominance via one
    anchor lookup. Output dim = 2*P_qk; first P_qk slots → Q, next P_qk → K.
    output_nap doubled to maintain votes-per-pair vs separate-LUT setup.
    scale matches the CLT convention used by BitPermutationLUT wrapper, so
    output magnitudes (and hence DominanceCanonicalize and the bit-LUT
    optimizer's gradient gate) behave identically to separate Q/K LUTs."""
    n_inputs = _e_in(layer_idx) + (_pos_emb_dim(layer_idx) if _POS_EMB_ACTIVE else 0)
    votes_per_output = cfg['qk_tph'] * 2 * cfg['qk_output_nap'] / (2 * D_QK_P)
    qk_scale = 0.5 / math.sqrt(votes_per_output)
    return BitMultiHeadLUT(
        n_inputs=n_inputs, n_outputs=2 * D_QK_P, n_heads=H,
        input_nap=cfg['qk_input_nap'], output_nap=2 * cfg['qk_output_nap'],
        tph=cfg['qk_tph'],
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        # Joint Q/K LUT: single BitMultiHeadLUT outputs 2*P_qk pair-dominance
        # bits per (head, table). First P_qk slots → Q, next P_qk → K. Both
        # share the same anchor lookup (same table-entry decision per token)
        # so Q and K are produced from one tied lookup pattern.
        self.qk_perm = _make_qk_joint(layer_idx, layer_idx)
    ...
    def forward(self, x, pos_emb):
        ...
        qk = self.qk_perm(xp)                                          # [B*T, H, 2*P_qk]
        q_dom = self.q_canon(qk[..., :D_QK_P])                         # [B*T, H, P_qk]
        k_dom = self.k_canon(qk[..., D_QK_P:])                         # [B*T, H, P_qk]
...
# Two bit-LUT optimizer groups so Q/K can train at a different LR than V/out.
qk_bit_luts = [layer.qk_perm for layer in model.layers]
rest_bit_luts = []
for layer in model.layers:
    rest_bit_luts += [layer.v_perm, layer.out_proj]
bit_luts = qk_bit_luts + rest_bit_luts   # for accounting only

adam_params = list(model.parameters())
adam_param_count = sum(p.numel() for p in adam_params)
print(f'bit LUTs: {len(bit_luts)} ({len(qk_bit_luts)} Q/K, {len(rest_bit_luts)} V+out_proj)')
...
bit_opt_qk = BitPermutationLUTOptimizer(
    qk_bit_luts,
    lr=cfg.get('qk_bit_lut_lr', cfg['bit_lut_lr']),
    beta1=cfg.get('bit_lut_beta1', 0.9),
    beta2=cfg.get('bit_lut_beta2', 0.999),
    lr_schedule_fn=get_lr_scale,
)
bit_opt_rest = BitPermutationLUTOptimizer(
    rest_bit_luts,
    lr=cfg['bit_lut_lr'],
    beta1=cfg.get('bit_lut_beta1', 0.9),
    beta2=cfg.get('bit_lut_beta2', 0.999),
...
print(f'Q/K bit_lut_lr={cfg.get("qk_bit_lut_lr", cfg["bit_lut_lr"])}, V+out_proj bit_lut_lr={cfg["bit_lut_lr"]}')
...
for step in range(1, N_STEPS + 1):
    ...
    adam_opt.zero_grad()
    bit_opt_qk.zero_grad()
    bit_opt_rest.zero_grad()
    accum_loss = 0.0
    ...
    adam_opt.step()
    bit_opt_qk.step()        # Q/K bit-LUT optimizer
    bit_opt_rest.step()      # V + out_proj bit-LUT optimizer
