...
_QK_N_HEADS = cfg.get('qkv_n_heads', H)
def _make_qk(layer_idx, seed_offset):
    # qkv_n_heads can be set to 1 with n_outputs=H*2*d_qk for vanilla-equivalent
    # (single routing decision, heads via output partition).
    return TinyMultiHeadLut(
        input_dim=E,
        n_heads=_QK_N_HEADS,
        n_outputs=(H // _QK_N_HEADS) * 2 * d_qk,
...
class LUTBlock(nn.Module):
    ...
    def forward(self, x, cos, sin):
        ...
        qk_raw = self.qk_lut(x_pre)                                  # [B*T, qkv_n_heads, qkv_n_out]
        qk_out = qk_raw.reshape(B * T, H, 2 * d_qk)                  # canonicalize to per-attn-head
...
print(f'qk_lut       TinyMHLut(soft, LION) [{_QK_N_HEADS}-head]: in_nap={cfg["qkv_input_nap"]} tph={cfg["qkv_tph"]} n_heads={_QK_N_HEADS} n_out={(H // _QK_N_HEADS) * 2 * d_qk} -> reshape to attn H={H} per-head 2*d_qk={2*d_qk}')
