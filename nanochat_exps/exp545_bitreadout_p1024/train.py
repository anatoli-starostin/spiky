...
from spiky.lutorch.bit_readout_head import BitReadoutHead
...
P_BITS = cfg['bit_n_bits']; BIT_SIGN_TEMP = cfg.get('bit_sign_temp', 0.5)
...
class Model(nn.Module):
    def __init__(self):
        ...
        self.head = BitReadoutHead(D, VOCAB_SIZE, P_BITS, sign_temp=BIT_SIGN_TEMP,
...
print(f'Total train params: {n_params:,} | BitReadout P={P_BITS} bits | {model.head}')
print(f'head latent (train) {model.head.latent.numel()/1e6:.1f}M -> deploy codes {VOCAB_SIZE*P_BITS/8/1e6:.1f}MB (popcount) vs Linear 12.6M matmul')
