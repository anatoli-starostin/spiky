"""nanochat_exps/exp085_residual_anchor_inherit — fork of exp084.

Two changes vs exp084:

  (a) Residual_lut input uses INVERSE concat order:
          res_in = torch.cat([*prev_xs, x], dim=-1)
      With this ordering layer i's first i*E slots correspond exactly to
      layer (i-1)'s entire residual_lut input domain.

  (b) After model construction, the first 256*i anchor pairs (a/b) of
      layer i's residual_lut.lookup are overwritten with a copy of layer
      (i-1)'s 256*i anchor pairs (verbatim — same a/b indices). Layer i's
      remaining 256 fresh tables keep their freshly-sampled anchor pairs
      over the full (i+1)*E input.

Param count and table values are identical to exp084. Only anchor
sampling structure changes.

How to launch:

    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python \\
        -u nanochat_exps/exp085_residual_anchor_inherit/train.py \\
        > nanochat_exps/exp085_residual_anchor_inherit/stdout.log 2>&1 &
"""
