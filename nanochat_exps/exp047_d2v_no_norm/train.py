...
def _make_qk_joint(layer_idx, seed_offset):
    """Joint fp32 MultiHeadLut: input=E (sum-mode), n_heads=H, n_outputs=2*d_qk."""
    ...
    return MultiHeadLut(
        ...
        initial_weights_noise=cfg.get('qk_lut_init_std', 0.001),
...
class LUTBlock(nn.Module):
    """Reads the dominance residual stream via D2V (Borda + LN), runs
    Q/K/V/SDPA/out_proj, and adds α·out_proj-dominance back to the stream.
    No canonicalisation or LN per layer — out_proj is BitPermutationLUT which
    is CLT-scaled internally (per-pair std≈0.5).
    """
    def __init__(self, layer_idx):
        ...
        # Learnable per-layer scale on out_proj's CLT-normalised output.
...
class Model(nn.Module):
    def __init__(self):
        ...
        # Initial token-embed → dominance projection (STE).
        self.token_to_dom = VectorToDominance(E, smooth_mode=False, temperature=cfg.get('canon_temperature', 0.1))
        self.layers = nn.ModuleList([LUTBlock(i) for i in range(N_LAYERS)])
        # End: D2V (Borda + LN) → MLP head (E → 6E → V). 6E hidden chosen so
        # head param budget ≈ exp042's LN(384)+Linear(384,V) = 12.6M.
        self.final_d2v = DominanceToVector(E, normalise=False)
...
print(f'Q/K Joint MultiHeadLut: in_nap={cfg["qk_input_nap"]} tph={cfg["qk_tph"]} d_qk={d_qk} P_qk={D_QK_P} (fp32, smooth_mode=True)')
