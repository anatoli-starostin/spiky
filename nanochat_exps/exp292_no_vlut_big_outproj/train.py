...
_USE_V_LUT = cfg.get('use_v_lut', True)

class LUTBlock(nn.Module):
    """exp292 block: v_lut removed. V is just raw x_lut broadcast across H heads
    (no learned per-head V projection). d_v must equal E for the broadcast to work."""
    ...
    def forward(self, x, pos_emb):
        ...
        if _USE_V_LUT:
            ...
            # No v_lut: each head sees the SAME raw x (broadcast). d_v must == E.
            v = x.unsqueeze(1).expand(B, H, T, E).contiguous()         # [B, H, T, E=d_v]
...
if _USE_V_LUT:
    ...
    print(f'V        REMOVED: V = raw x_lut broadcast across H={H} heads (d_v={d_v}=E={E})')
...
def collect_temperature_specs(model):
    ...
    for li, blk in enumerate(model.layers):
        slut_names = ('qk_joint', 'out_proj', 'residual_lut') if not _USE_V_LUT else ('qk_joint', 'v_lut', 'out_proj', 'residual_lut')
        for slut_name in slut_names:
