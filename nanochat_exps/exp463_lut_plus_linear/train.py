...
class LinearParallelLut(nn.Module):
    """exp463 (hacky diagnostic): y = LUT(x) + Linear(x). A plain dense Linear in
    parallel with each LUT, summed. Zero-init so the module starts == exp453 and
    the linear grows in as a clean additive correction (cf. exp457 init lesson).
    Reintroduces dense matmuls (NOT matmul-free) — purely to probe whether dense
    linear capacity helps the LUT body converge. Linear weights (2D) train on
    AdamW; base LUT (3D) on LION."""
    def __init__(self, base, input_dim, n_heads, n_out):
        super().__init__()
        self.base = base
        self.linear = nn.Linear(input_dim, n_heads * n_out)
        nn.init.zeros_(self.linear.weight)
        nn.init.zeros_(self.linear.bias)
        self.n_heads, self.n_out = n_heads, n_out
    def forward(self, x):
        return self.base(x) + self.linear(x).view(-1, self.n_heads, self.n_out)


_LUT_PLUS_LINEAR = cfg.get('lut_plus_linear', True)

def _wrap(base, input_dim, n_heads, n_out):
    if not _LUT_PLUS_LINEAR:
        return base
    return LinearParallelLut(base, input_dim, n_heads, n_out)
...
def _make_qkv_joint(layer_idx, seed_offset):
    ...
    return _wrap(base, E, H, 2 * d_qk + d_v)
