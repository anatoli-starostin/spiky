"""exp745 — exp732 + bf16 unembedder via in-forward autocast.

Fork of exp732 (val=1.3912 @ 276.8M, fp32 LUT storage + bf16 LUT compute via
internal use_bf16=True autocast, fp32 head, standard Lion, no grad clip).
Single change: wrap self.unembedder(x_resid) + F.cross_entropy in
torch.amp.autocast(bf16) so the head matmul runs bf16 too. Everything else
identical to exp732. Tests bf16 head on the "all weights stored fp32, bf16
only for compute" policy baseline (exp737 v2 had bf16 LUT storage + Lion
master, an off-policy probe; this run is the on-policy version).

Compare to exp732 = 1.3912. Expectation: ~0 drift (matches vanilla finding
that bf16 head matmul is free, when the model uses fp32 storage everywhere).
"""
