"""exp757 — exp750 + zero init on out_proj, residual_lut, emb_resid_lut.

Fork of exp750 (4K SOTA 1.3863). Single change: pass
initial_weights_noise=0.0 to the three "residual-write" LUT modules
(out_proj, residual_lut, emb_resid_lut). Mirrors vanilla nano-GPT's
zero-init on residual output projections: at step 0 the model is the
identity — x_lut stays as the embedding through all layers, x_resid
starts at 0, ln_final(0) = 0, logits = 0, loss = ln(V) ≈ 10.40.

Hypothesis: zero-init residual paths give the model a clean starting
point — the residual contributions only "turn on" as gradients flow
through them, which often speeds up early training and improves final
val_bpb. Compare to exp750 = 1.3863.

ORIGINAL exp750 docstring follows:

exp750 — exp737 v2 + clip_grad_norm_(model.parameters(), 1.0).

Fork of exp737 v2 (1.3872 @ 276.8M, bf16 LUT storage + fp32 master Lion +
fp32 head). Single change: add global grad clip(1.0) on all parameters
before optimizer step. exp749 showed clip on top of exp742 (bf16 head)
gave a large early-stage lead before stabilizing. This run isolates the
clip effect on the clean exp737 v2 recipe — does clip alone push below
1.3872? Compare to exp737 v2 = 1.3872.
...
_FAST_KWARGS_ZERO = dict(_FAST_KWARGS); _FAST_KWARGS_ZERO['initial_weights_noise'] = 0.0

def _make_out(seed_offset):
    # Zero init: starts as identity-like residual (out_proj contribution = 0).
    return FastMultiHeadLUT(
...
def _make_residual_lut(seed_offset):
    """Per-layer residual_lut: E -> D, accumulated into the D-stream. Zero init."""
    return FastMultiHeadLUT(
...
def _make_emb_resid_lut(seed_offset):
    ...
    Zero init so x_resid starts at 0 at step 0.
    """
    return FastMultiHeadLUT(
        input_dim=E, n_heads=1, n_outputs=D,
