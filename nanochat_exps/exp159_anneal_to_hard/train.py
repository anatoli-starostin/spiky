...
class LUTBlock(nn.Module):
    ...
    def forward(self, x, pos_emb):
        ...
        # Anneal-to-hard attention. α schedules forward from soft (α=0) to
        # hard (α=1) over training. Backward gradient flows ONLY through
        # A_soft regardless of α (the α*(A_hard - A_soft) term is detached),
        # so attn_scale cannot compensate the schedule — at α=1 the forward
        # is pure argmax, attn_scale only controls surrogate sharpness.
        alpha = 0.0
        if hasattr(self, '_current_step') and hasattr(self, '_total_steps'):
            alpha = float(self._current_step) / max(1, self._total_steps)
        s = torch.matmul(q.float(), k.float().transpose(-2, -1))         # [B,H,T,T]
        s_scaled = s * (self.attn_scale * self._inv_sqrt_p)
        causal_mask = torch.triu(torch.ones(T, T, dtype=torch.bool, device=s.device), 1)
        s_scaled = s_scaled.masked_fill(causal_mask, float('-inf'))
        s_for_argmax = s.masked_fill(causal_mask, float('-inf'))
        idx = s_for_argmax.argmax(dim=-1, keepdim=True)                  # [B,H,T,1]
        A_hard = torch.zeros_like(s_scaled).scatter_(-1, idx, 1.0)
        A_soft = torch.softmax(s_scaled, dim=-1)
        A = A_soft + alpha * (A_hard - A_soft).detach()
        attn = torch.matmul(A, v.float()).to(v.dtype)
...
for step in range(1, N_STEPS + 1):
    ...
    # Inject step for STE-blend α schedule (α = step / N_STEPS).
    for layer in model.layers:
        layer._current_step = step
        layer._total_steps = N_STEPS
