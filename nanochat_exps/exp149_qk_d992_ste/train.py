"""nanochat_exps/exp149_qk_d992_ste — fork of exp144.

Replace V2D on Q/K with simple element-wise STE sign:
    soft = x / (tau + |x|)
    hard = sign(x)
    forward = (hard - soft).detach() + soft

d_qk: 32 -> 992 (= 496*2). qk_lut switches to sparse scatter
(n_sparse_outputs=32, scatter to 2*d_qk=1984), giving ~37 votes/output —
the analogue of V2D's 31 sign comparisons per output dim.

How to launch:

    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python \\
        -u nanochat_exps/exp149_qk_d992_ste/train.py \\
        > nanochat_exps/exp149_qk_d992_ste/stdout.log 2>&1 &
"""
