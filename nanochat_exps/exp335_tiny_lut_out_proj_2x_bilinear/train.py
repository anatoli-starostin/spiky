...
# out_proj's output dim. When > E, this expands the LUT-MLP hidden dim and a
# subsequent ffn_lut narrows back to E. When = E (default), no ffn needed.
_OUT_N_OUT = E * cfg.get('out_proj_widen_factor', 1)
...
# FFN-LUT: narrows wider out_proj output back to E. Takes out_proj's output as
# input_dim. NO residual (input dim ≠ output dim).
_FFN_TPH_PER_LAYER = cfg.get('ffn_tph_per_layer')
_FFN_ACTIVE = (_FFN_TPH_PER_LAYER is not None) or (cfg.get('ffn_tph') is not None)
def _make_ffn(layer_idx, seed_offset):
    if _FFN_TPH_PER_LAYER is not None:
        tph = _FFN_TPH_PER_LAYER[layer_idx]
    else:
        tph = cfg.get('ffn_tph', cfg.get('out_tph'))
    mode = cfg.get('ffn_backward_mode', cfg.get('out_backward_mode', 'soft'))
    kwargs = _TINY_SOFT_KWARGS if mode == 'soft' else _TINY_MULTIALT_KWARGS
    return TinyMultiHeadLut(
        input_dim=_OUT_N_OUT,
        n_heads=1,
        n_outputs=E,
...
class LUTBlock(nn.Module):
    ...
    def __init__(self, layer_idx):
        ...
        if _FFN_ACTIVE:
            self.ffn_lut  = _make_ffn(layer_idx, 800 + layer_idx)

        self.q_norm = nn.LayerNorm(d_qk)
        self.k_norm = nn.LayerNorm(d_qk)

        # Post-norm at block end on E-stream output.
    ...
    def forward(self, x, cos, sin):
        ...
        # out_proj: [B*T, 1, _OUT_N_OUT]; when widened, _OUT_N_OUT = 2*E.
        out_wide = self.out_proj(out_in).squeeze(1)                   # [B*T, _OUT_N_OUT]
        # ffn_lut: narrows back to E. No residual (input dim ≠ output dim).
        if _FFN_ACTIVE:
            out_e = self.ffn_lut(out_wide).squeeze(1)                 # [B*T, E]
        else:
            out_e = out_wide                                          # [B*T, E] (no widening case)
...
print(f'out_proj TinyMHLut({cfg.get("out_backward_mode", "soft")}, noise_eps={_NOISE_EPS}): in_nap={cfg["out_input_nap"]} tph={_out_tph_str} n_out={_OUT_N_OUT} (widen_factor={cfg.get("out_proj_widen_factor", 1)})')
if _FFN_ACTIVE:
    _ffn_tph_str = str(_FFN_TPH_PER_LAYER) if _FFN_TPH_PER_LAYER is not None else str(cfg.get('ffn_tph'))
    print(f'ffn_lut TinyMHLut({cfg.get("ffn_backward_mode", cfg.get("out_backward_mode", "soft"))}, noise_eps={_NOISE_EPS}): in_nap={cfg.get("ffn_input_nap", cfg["out_input_nap"])} tph={_ffn_tph_str} n_in={_OUT_N_OUT} n_out=E={E}  [no residual]')
...
def collect_temperature_specs(model):
    ...
    for li, blk in enumerate(model.layers):
        for slut_name in ('qkv_lut', 'v_lut', 'out_proj', 'ffn_lut', 'residual_lut'):
