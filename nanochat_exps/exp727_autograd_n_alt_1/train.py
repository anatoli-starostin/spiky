...
# Self-consistent input grad: autograd backward with n_alt = NAP (full Hamming-1
# ball gather). Input grad only flows through rows actually used in forward
# (no soft K-row surrogate). save_memory=True streams the (NAP+1)-row gathers
# to keep peak memory manageable. n_alt is set per-module = the module's NAP.
_HSM_AG_KWARGS = dict(
...
_HSM_N_ALT_OVERRIDE = cfg.get('hybrid_smooth_n_alt', None)
def _n_alt_for(mod_nap):
    return _HSM_N_ALT_OVERRIDE if _HSM_N_ALT_OVERRIDE is not None else mod_nap
...
def _make_qk(seed_offset):
    return TinyMultiHeadLut(
        ...
        hybrid_smooth_n_alt=_n_alt_for(cfg['qkv_input_nap']),
        random_seed=cfg['random_seed'] + seed_offset, device=DEVICE,
        **_TINY_SOFT_KWARGS, **_HSM_AG_KWARGS,
    )
...
def _make_v(seed_offset):
    return TinyMultiHeadLut(
        ...
        hybrid_smooth_n_alt=_n_alt_for(cfg['v_input_nap']),
        random_seed=cfg['random_seed'] + seed_offset, device=DEVICE,
        **_TINY_SOFT_KWARGS, **_HSM_AG_KWARGS,
    )
...
def _make_out(seed_offset):
    return TinyMultiHeadLut(
        ...
        hybrid_smooth_n_alt=_n_alt_for(cfg['out_input_nap']),
        random_seed=cfg['random_seed'] + seed_offset, device=DEVICE,
        **_TINY_SOFT_KWARGS, **_HSM_AG_KWARGS,
    )
...
def _make_residual_lut(seed_offset):
    ...
    return TinyMultiHeadLut(
        ...
        hybrid_smooth_n_alt=_n_alt_for(cfg['residual_input_nap']),
        random_seed=cfg['random_seed'] + seed_offset, device=DEVICE,
        **_TINY_SOFT_KWARGS, **_HSM_AG_KWARGS,
    )
...
def _make_emb_resid_lut(seed_offset):
    ...
    return TinyMultiHeadLut(
        ...
        hybrid_smooth_n_alt=_n_alt_for(cfg['emb_resid_input_nap']),
        random_seed=cfg['random_seed'] + seed_offset, device=DEVICE,
        **_TINY_SOFT_KWARGS, **_HSM_AG_KWARGS,
    )
