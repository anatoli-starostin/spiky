...
def _make_out(layer_idx, seed_offset):
    ...
    has per-table n_outputs=out_proj_n_sparse, and scatter-adds into the wider
    E-dim final output. Each table picks `out_proj_n_sparse` random slots out of E.
    """
    tph = _OUT_TPH_PER_LAYER[layer_idx] if _OUT_TPH_PER_LAYER is not None else cfg['out_tph']
    mode = cfg.get('out_backward_mode', 'soft')
    kwargs = dict(_kwargs_for_mode(mode))   # COPY — we mutate below
...
class Model(nn.Module):
    def __init__(self):
        ...
        # exp649: single final residual_lut E -> D applied once after all blocks.
        # tph = cfg['final_residual_tph'] (param-matched to N_LAYERS * residual_tph).
        self.final_residual_norm = MeanAbsNorm(E)
        self.final_residual_lut  = _make_final_residual_lut(seed_offset=900)

        # Final LN on the residual_lut output before the unembedder.
    ...
    def forward(self, tokens, targets=None, loss_reduction='mean'):
        ...
        # Single E -> D projection at the end (no per-layer accumulation).
        x_norm  = self.final_residual_norm(x_lut.reshape(B * T, E))
        x_resid = self.final_residual_lut(x_norm).squeeze(1).reshape(B, T, D)
...
print(f'final_residual_lut TinyMHLut({cfg.get("residual_backward_mode", "soft")}, noise_eps={_NOISE_EPS}): in_nap={cfg.get("residual_input_nap", cfg["out_input_nap"])} tph={cfg["final_residual_tph"]} n_out=D={D}  [single E -> D at end]')
print(f'NO per-layer residual_lut, NO tok_emb_D, NO x_resid accumulation; UNTIED unembedder Linear(D={D}, V={VOCAB_SIZE}); tok_emb_E separate at E={E}; ln_final(D) before unembed')
