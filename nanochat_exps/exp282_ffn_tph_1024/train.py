...
_OUT_PARTITION_BY_HEAD = cfg.get('out_partition_by_head', False)
def _make_out(layer_idx, seed_offset):
    tph = _OUT_TPH_PER_LAYER[layer_idx] if _OUT_TPH_PER_LAYER is not None else cfg['out_tph']
    extra = {}
    if _OUT_PARTITION_BY_HEAD:
        # Each anchor pair (a, b) is intra-head; within a single table, the
        # NAP=6 pairs may still come from different heads (the table reads
        # NAP=6 head-internal pairs sampled independently from each partition).
        extra['partition_sets'] = [list(range(h * d_v, (h + 1) * d_v)) for h in range(H)]
    return TinyMultiHeadLut(
        input_dim=H * d_v,
        n_heads=1,
        n_outputs=E,
...
def collect_temperature_specs(model):
    ...
    for li, blk in enumerate(model.layers):
        for slut_name in ('qk_joint', 'v_lut', 'out_proj', 'ffn_lut', 'residual_lut'):
