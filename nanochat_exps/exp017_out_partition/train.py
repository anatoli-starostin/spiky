...
# Per-head partition for out_proj: input is concat of H attention heads each
# of dim d_v, so partition_sets restricts every anchor pair to dims of the
# SAME head (no cross-head comparisons inside any single pair). The 8 pairs
# in a table can still come from different heads — only each individual
# pair is constrained.
_OUT_PARTITION = [list(range(h * d_v, (h + 1) * d_v)) for h in range(H)]
...
for layer in model.layers:
    bit_luts += [layer.qk_perm, layer.v_perm, layer.out_proj]
