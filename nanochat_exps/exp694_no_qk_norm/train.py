"""exp694 — fork of exp693 (1.4379 @ 97.5M) with q_norm/k_norm removed.

Motivated by attention-pattern analysis: trained exp693 has self_mass=0.45 at
L0 (vs vanilla's 0.013), but at init both models are uniform with self_mass
=0.013. Hypothesis: the q_norm/k_norm (LayerNorm on q,k post-LUT) strip the
magnitude axis that the model could otherwise use to suppress trivial self-
attention. Vanilla GPT has no norm before SDPA -> this exp mirrors that.

Architecture (single change vs exp693):
  - LUTBlock per layer:
      qk_lut       TinyMHLut(soft, NAP=4, tph=256) -> 2*d_qk
      (NO q_norm, NO k_norm)                              # <-- the change
      RoPE on (q,k); SDPA causal.
      v_lut        TinyMHLut(soft, NAP=6, tph=256) -> d_v
      out_proj     TinyMHLut(soft, NAP=6, tph=512) -> E
      residual on E-stream: x_lut_next = x + out_e
      residual_lut TinyMHLut(soft, NAP=5, tph=256) -> D-stream contribution.
"""
...
def _make_qk(seed_offset):
    # Per-LUT init override: target vanilla-equivalent q,k magnitude at init
    # given there is no q_norm/k_norm in exp694 to rescale post-LUT.
    # Vanilla q,k entries std = 0.02 * sqrt(n_embd=384) ~= 0.39.
    # LUT post-embedding_bag std = sqrt(tph) * init_std, so we want
    # init_std = 0.39 / sqrt(tph). For tph=256 -> ~0.024.
    kwargs_qk = dict(_TINY_SOFT_KWARGS)
    kwargs_qk['initial_weights_noise'] = cfg.get('qkv_lut_init_std', cfg.get('mhlut_init_std', 0.001))
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        # exp694: q_norm/k_norm removed (vs exp693).
