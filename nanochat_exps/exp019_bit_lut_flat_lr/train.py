...
bit_opt = BitPermutationLUTOptimizer(
    ...
    lr_schedule_fn=None,        # FLAT — peak LR for entire run, no warmup, no decay.
                                # AdamW on continuous params still gets cosine (above).
)
print('Bit-LUT optimizer: FLAT lr (no schedule); AdamW: cosine warmup_decay to 0.1.')
