"""exp743 — exp742 + device_batch_size 24 -> 48 (grad_accum 2 -> 1).

Fork of exp742 (bf16 head + xent fix, drifted +9 mb at step 2400). Tests
whether grad_accum=2 was interacting with bf16 grad rounding to amplify
the drift. Same effective batch (24576 tokens), same LR schedule, same
4000 steps. Memory estimate: dbs=48 ~35 GB (exp737 v2 dbs=24 = 17.8 GB).

Compare to exp737 v2 = 1.3872 (fp32 head, dbs=24, grad_accum=2). If exp743
closes the drift, grad_accum was the culprit. If it still drifts ~10 mb,
grad_accum is innocent and the bf16-head cost is structural.
"""
