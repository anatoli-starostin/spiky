"""nanochat_exps/exp194_v2d_sdpa_untied.

Fork of exp193. Single change: head (lm_head) is no longer weight-tied to
tok_emb — both get their own [VOCAB, E] tensor, std=0.02 init each.

Hypothesis: with V2D + per-head Q/K/V the embedding tensor lives in a very
different geometry from the unembedder direction. Tying forces the head and
the embedder to share the same vector for each token, which may starve the
softmax of useful directions and bottleneck training.

Launch:
    PYTHONPATH=/home/starost/nanochat \\
      /home/starost/spiky/.venv/bin/python -u \\
      nanochat_exps/exp194_v2d_sdpa_untied/train.py \\
      > nanochat_exps/exp194_v2d_sdpa_untied/stdout.log 2>&1 &
"""
