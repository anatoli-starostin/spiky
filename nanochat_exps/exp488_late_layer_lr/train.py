"""nanochat_exps/exp488_late_layer_lr — fork of exp475 (bs=16) + late-layer LR boost.
...
# Late-layer LR boost: LUT params in the last `_LATE_LR_LAYERS` layers get
# lut_lr * _LATE_LR_MULT (non-zero-sum way to feed the layers where the bs=48
# gain concentrated — L4/L5 carried the entire -0.117 in the logit-lens probe).
_LATE_LR_LAYERS = cfg.get('late_lr_layers', 2)
_LATE_LR_MULT   = cfg.get('late_lr_mult', 1.0)
_LATE_START     = N_LAYERS - _LATE_LR_LAYERS
...
for name, p in model.named_parameters():
    if not p.requires_grad:
        continue
    if p.ndim >= 3:
        if _LATE_LR_MULT != 1.0 and _layer_idx_of(name) >= _LATE_START:
            lut_params_late.append(p)
        else:
            lut_params_early.append(p)
...
lut_params = lut_params_early + lut_params_late
...
# Two groups: early layers at lut_lr, late layers at lut_lr * late_lr_mult.
_lut_groups = [dict(params=lut_params_early, lr=_LUT_LR, weight_decay=0.0)]
if lut_params_late:
    _lut_groups.append(dict(params=lut_params_late, lr=_LUT_LR * _LATE_LR_MULT, weight_decay=0.0))
if _LUT_OPT == 'lion':
    lut_optimizer = Lion(_lut_groups, lr=_LUT_LR, betas=tuple(cfg.get('lut_betas', (0.9, 0.99))))
elif _LUT_OPT == 'signsgd':
    lut_optimizer = SignSGD(_lut_groups, lr=_LUT_LR, momentum=cfg.get('lut_momentum', 0.9))
else:  # 'adamw'
    for _g in _lut_groups:
        _g.update(betas=(0.9, 0.95), eps=1e-8)
    lut_optimizer = torch.optim.AdamW(_lut_groups)
...
print(f'late-layer LR boost: layers [{_LATE_START}..{N_LAYERS-1}] at lut_lr*{_LATE_LR_MULT} '
      f'(={_LUT_LR*_LATE_LR_MULT:.2e}) | late_tensors={len(lut_params_late)} '
      f'late_params={sum(p.numel() for p in lut_params_late):,}')
