"""nanochat_exps/exp152_qk_canon_no_ln — fork of exp151.

Drop the LayerNorm inside DominanceCanonicalize:
    canon = D2V(d_head=32, normalise=False) -> V2D(d_head=32, STE)
            ^ raw Borda, no LN

This restores the natural STE 'linear-at-init -> saturated-at-end'
gradient curriculum (Borda magnitude scales with input dominance).

How to launch:

    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python \\
        -u nanochat_exps/exp152_qk_canon_no_ln/train.py \\
        > nanochat_exps/exp152_qk_canon_no_ln/stdout.log 2>&1 &
"""
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        # Canonicalize each half (Q-dom, K-dom) via raw-Borda -> V2D-STE.
        # No LayerNorm: Borda magnitudes follow input dominance scale, giving
        # STE the natural 'linear-at-init -> saturated-at-end' curriculum.
        canon_d_head = cfg['qk_canon_d_head']
        canon_t = cfg.get('ste_temperature', cfg.get('canon_temperature', 0.1))
        self.qk_d2v = DominanceToVector(canon_d_head, normalise=False)
        self.qk_v2d = VectorToDominance(canon_d_head, smooth_mode=False, temperature=canon_t)
    ...
    def forward(self, x, pos_emb):
        ...
        q_dom = self.qk_v2d(self.qk_d2v(q_raw))                        # raw-Borda -> V2D-STE
        k_dom = self.qk_v2d(self.qk_d2v(k_raw))
...
print(f'qk canonicalize (no LN): D2V(d_head={cfg["qk_canon_d_head"]}, normalise=False) '
      f'-> V2D-STE; P=d_qk={d_qk}; ste_temperature='
      f'{cfg.get("ste_temperature", cfg.get("canon_temperature", 0.1))}')
