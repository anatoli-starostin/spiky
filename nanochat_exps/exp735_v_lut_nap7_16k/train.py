"""exp735 — full-horizon (16K) fork of exp733 (v_lut NAP=7).

Single change vs exp733: n_steps 4000 -> 16000. Otherwise identical to
exp733. Direct apples-to-apples comparison vs exp731 (the prior 16K
hard+dense_K SOTA at 1.2178); the only architectural difference is
v_input_nap 6 -> 7 (+37.7M params -> 314.5M total).

References:
  exp731 (v NAP=6) @ 16K bs=48: hard val = 1.2178 (current SOTA), 4.89 h
  exp733 (v NAP=7) @ 4K bs=48:  val = 1.3857 (-5.5 mb vs exp732), 1.374 h
  exp734 (resid+emb_resid also NAP=7) @ 4K: val = 1.3862 (tied exp733)

Hypothesis: the -5.5 mb lead exp733 had at 4K extends to 16K. If the
gain scales (it often shrinks as training approaches the data ceiling),
exp735 should beat exp731's 1.2178.

Recipe (unchanged from exp733):
  forward_mode  = 'hard'
  backward_mode = 'dense_K' (bmm-wgrad on n_out>=128 modules)
  E=384 NAP-bump arch, bs=24 ga=2 (eff bs=48), Lion lr=2e-4 LUT,
  AdamW 3e-4 non-LUT, soft_score_temp=select_temp=0.5 learnable temps,
  warmup_fraction=0.1 -> 1600 step warmup (same as exp731).

Expected wallclock: ~5.5 h.
"""
