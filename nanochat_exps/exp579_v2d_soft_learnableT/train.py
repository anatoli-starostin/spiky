...
class Model(nn.Module):
    def __init__(self):
        ...
        # exp579: V2D-tied head, SOFT forward, LEARNABLE temperature init=0.5.
        # The fixed T=0.1 in exp578 saturated the rational sign for typical
        # embedding-pair differences (after early training), killing gradient
        # to settled pairs. Larger init T=0.5 keeps the soft sign in its linear
        # regime longer; learnable_temperature lets the optimizer find the right
        # T per pair as the embedding learns its shape.
...
P_v2d = E * (E - 1) // 2
print(f'V2D-TIED head (exp579, SOFT fwd + LEARNABLE T): logits = ln_x_v2d(V2D_soft(x_lut)) @ ln_emb_v2d(V2D_soft(tok_emb_E)).T; V2D dim P=E*(E-1)/2={P_v2d}, smooth_mode=True T_init=0.5 (learnable), ln_emb_v2d.gamma init=1/sqrt(P)={1.0/math.sqrt(P_v2d):.4f}; RoPE base={_ROPE_BASE}')
