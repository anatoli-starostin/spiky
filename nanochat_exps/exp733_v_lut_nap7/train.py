"""exp733 — v_lut NAP=7 fork of exp732 (4K short-horizon).

Single change vs exp732: v_input_nap 6 -> 7. v_lut K doubles 64 -> 128,
adding +37.7M params (-> 314.5M total). Tests whether v_lut benefits
from a deeper input NAP at d_v=64.

v_lut.n_outputs = d_v = 64 < 128, so the bmm-wgrad dispatch correctly
keeps v_lut on index_add. Only the K-wide chain (D/E/F phases) grows.

Reference: exp732 final val_bpb = 1.3912 @ 4000 steps, 1.225 h, bs=24 ga=2.

Recipe (unchanged from exp732):
  forward_mode  = 'hard'      (1-row argmax via F.embedding_bag('sum'))
  backward_mode = 'dense_K'   (full K-row softmax surrogate input grad,
                               1-row weight scatter via bmm for
                               n_outputs>=128 modules; index_add otherwise)
...
Expected wallclock: ~85-90 min (wider v_lut K).
"""
