...
# --- SoftMultiHeadLUT mode: soft forward (hard=False) + temperature anneal ---
# All LUTs in this run are SoftMultiHeadLUT. The training loop overwrites
# their soft_score_temp and select_temp at every step via the schedule below.
# Initial T is set to soft_temp_start so the first step is already at the
# correct (very-soft) temperature.
_T_START = float(cfg.get('soft_temp_start', 5.0))
_T_END   = float(cfg.get('soft_temp_end',   0.05))
_SOFT_MHLUT_KWARGS = dict(
    weight_dtype=torch.float32,
    anchor_sampling_policy=AnchorSamplingPolicy.CANONICAL_FULL_COVERAGE,
    initial_weights_noise=cfg.get('mhlut_init_std', 0.001),
    soft_score_temp=_T_START,
    select_temp=_T_START,
    hard=False,
    gumbel=False,
    learnable_temps=False,
    use_bf16=bool(cfg.get('soft_use_bf16', True)),
    compile_forward=False,
)

def _make_qkv_joint(layer_idx, seed_offset):
    """Joint qkv LUT — n_outputs = 2*d_qk + d_v. Q/K come from this LUT exclusively
    (replaces qk_joint). The last d_v outputs are ADDED to v_lut's output."""
    kwargs = dict(_SOFT_MHLUT_KWARGS)
...
adam_groups = [
    dict(params=lut_params,     lr=cfg['adam_lr'], betas=(0.9, 0.95), eps=1e-8,
         weight_decay=0.0),
...
print(
    f'optimizer groups: '
    f'lut={sum(p.numel() for p in lut_params):,} (wd=0) | '
...
print(f'QKV joint SoftMHLUT(hard=False, anneal): in_nap={cfg.get("qkv_input_nap", cfg["qk_input_nap"])} tph={cfg.get("qkv_tph", cfg["qk_tph"])} n_out=2*d_qk+d_v={2*d_qk+d_v} (q,k from this LUT; last d_v added to v_lut)')
print(f'V_lut    SoftMHLUT(hard=False, anneal): in_nap={cfg["v_input_nap"]} tph={cfg["v_tph"]} d_v={d_v}')
_out_tph_str = str(_OUT_TPH_PER_LAYER) if _OUT_TPH_PER_LAYER is not None else str(cfg['out_tph'])
print(f'out_proj SoftMHLUT(hard=False, anneal): in_nap={cfg["out_input_nap"]} tph={_out_tph_str} n_out=E={E}')
print(f'residual_lut SoftMHLUT(hard=False, anneal): in_nap={cfg.get("residual_input_nap", cfg["out_input_nap"])} tph={cfg.get("residual_tph", cfg["out_tph"])} n_out=D={D}  [E -> D]')
    ...
    for li, blk in enumerate(model.layers):
        for slut_name in ('qkv_lut', 'v_lut', 'out_proj', 'residual_lut'):
            ...
            if getattr(mod, 'learnable_temps', False):
                # TinyMHLut-style learnable Parameter temps.
            ...
            elif hasattr(mod, 'select_temp') and isinstance(mod.select_temp, torch.Tensor):
                # SoftMultiHeadLUT(learnable_temps=False): select_temp is a 0-d
                # buffer mutated by the anneal scheduler; soft_score_temp is a
                # Python float attribute on the module.
                specs.append((f'L{li}.{slut_name}.T_soft',
                              (lambda m=mod: float(m.soft_score_temp))))
                specs.append((f'L{li}.{slut_name}.T_sel',
                              (lambda m=mod: float(m.select_temp.detach()))))
    return specs
...
# --- Soft-temperature annealing scheduler -----------------------------------
# Log-linear interpolation from _T_START (very soft) to _T_END (almost hard).
# Applied to BOTH soft_score_temp and select_temp of every SoftMultiHeadLUT.
_LOG_T_START = math.log(_T_START)
_LOG_T_END   = math.log(_T_END)
_ALL_SOFT_LUTS = []
for _blk in model.layers:
    for _name in ('qkv_lut', 'v_lut', 'out_proj', 'residual_lut'):
        _mod = getattr(_blk, _name, None)
        if isinstance(_mod, SoftMultiHeadLUT):
            _ALL_SOFT_LUTS.append(_mod)
print(f'soft anneal: T_start={_T_START} -> T_end={_T_END}, {len(_ALL_SOFT_LUTS)} SoftMHLUT modules tracked')

def _anneal_temp_for_step(step_):
    progress = (step_ - 1) / max(N_STEPS - 1, 1)            # step 1 -> 0.0, step N -> 1.0
    log_T = _LOG_T_START + (_LOG_T_END - _LOG_T_START) * progress
    T = math.exp(log_T)
    for m in _ALL_SOFT_LUTS:
        m.soft_score_temp = float(T)
        m.set_select_temp(float(T))
    return T
...
for step in range(1, N_STEPS + 1):
    ...
    cur_T = _anneal_temp_for_step(step)
    ...
    if step % 100 == 0 or step == 1:
        print(f'step {step:6d} | loss={ema:.4f} | lr={lr_scale * cfg["adam_lr"]:.2e} | T={cur_T:.4f}')
