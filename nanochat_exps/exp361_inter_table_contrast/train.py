...
# --- Inter-table contrastive loss (LUT-only) ----------------------------------
_CONTRAST_LAMBDA = float(cfg.get('inter_table_contrast_lambda', 0.0))
_CONTRAST_LUT_MODULES = []
if _CONTRAST_LAMBDA > 0.0:
    for _blk in model.layers:
        for _nm in ('qkv_lut', 'v_lut', 'out_proj', 'residual_lut'):
            _mod = getattr(_blk, _nm, None)
            if _mod is not None:
                _CONTRAST_LUT_MODULES.append(_mod)
    print(f'Inter-table contrastive enabled on {len(_CONTRAST_LUT_MODULES)} LUT modules: λ={_CONTRAST_LAMBDA}')

def _inter_table_contrast_loss():
    total = 0.0
    for _mod in _CONTRAST_LUT_MODULES:
        W = _mod.weights                       # [n_tables, K, n_outputs]
        n_t = W.shape[0]
        if n_t < 2:
            continue
        W_flat = W.reshape(n_t, -1)
        W_norm = F.normalize(W_flat, dim=-1)
        sim = W_norm @ W_norm.t()              # [n_t, n_t]
        mask = 1.0 - torch.eye(n_t, device=W.device, dtype=sim.dtype)
        total = total + ((sim * mask) ** 2).sum() / (n_t * (n_t - 1))
    return total
...
for step in range(1, N_STEPS + 1):
    ...
    optimizer.zero_grad()
    accum_loss = 0.0
    contrast_val_step = 0.0
    for _ in range(grad_accum):
        x, y = next(train_loader)
        loss = model(x, targets=y)
        if _CONTRAST_LAMBDA > 0.0:
            contrast = _inter_table_contrast_loss()
            total_loss = loss + _CONTRAST_LAMBDA * contrast
            contrast_val_step += float(contrast.detach()) / grad_accum
        else:
            total_loss = loss
    ...
    if step % 100 == 0 or step == 1:
        if _CONTRAST_LAMBDA > 0.0:
            print(f'step {step:6d} | loss={ema:.4f} | contrast={contrast_val_step:.4f} | lr={lr_scale * cfg["adam_lr"]:.2e}')
        else:
