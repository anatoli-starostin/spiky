"""nanochat_exps/exp246_soft_lut_tsel01 — fork of exp243 with T_sel=0.1.

Single change: select_temp 0.5 -> 0.1. Forward unchanged (hard=True picks
argmax which is T_sel-invariant). Backward changes only through the softmax
surrogate sel_soft = softmax(ts/T_sel):
  - At T_sel=0.5: softmax of ts (range ±sqrt(8) ≈ ±2.8) gives moderate spread
    -> meaningful d_sel/d_ts -> non-trivial gradient back to ts -> p -> x.
  - At T_sel=0.1: ts/0.1 has range ±28 -> nearly one-hot softmax -> d_sel/d_ts
    near zero everywhere -> input-side gradient through soft path is killed.

Tests how much of exp243's win over exp234 came from the soft input-side
gradient (vs. just from hard forward + soft weight gradient).

All else: T_soft=0.5, hard=True, nap=8 V, bf16+compile.

Launch:
    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python -u \\
        nanochat_exps/exp246_soft_lut_tsel01/train.py \\
        > nanochat_exps/exp246_soft_lut_tsel01/stdout.log 2>&1 &
"""
