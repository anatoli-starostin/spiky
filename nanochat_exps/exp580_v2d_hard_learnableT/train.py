...
# exp576: V2D-tied head — pairwise sign-dominance projection of both x_lut and
# tok_emb_E.weight, with LN on each side, then dot product.
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        # exp576: residual_lut REMOVED — no D-stream (head reads from E-stream).
...
class Model(nn.Module):
    def __init__(self):
        ...
        # exp576: NO Linear unembedder; tied dot in V2D space below.
        self.rope = RotaryEmbedding(d_qk, max_seq_len=CONTEXT_SIZE, base=_ROPE_BASE)
        self.layers = nn.ModuleList([LUTBlock(i) for i in range(N_LAYERS)])

        # exp580: V2D-tied head, HARD forward (smooth_mode=False, STE), LEARNABLE T.
        # exp579 (soft fwd + learnable T) plateaued +0.12-0.13 above raw tied dot.
        # Hypothesis for this run: hard forward gives the 1-bit deployment story
        # cleanly, while learnable T governs ONLY the soft-backward STE gradient
        # surrogate — letting the optimizer pick the right gradient scale per pair
        # without changing the forward. Forward is +/-1, backward is rational soft
        # at the learned T. (Logit quantization to 2017 distinct values per (t,v)
        # is back, but that may not be the binding ceiling at this scale anyway.)
        ...
        P = E * (E - 1) // 2
        self.P = P
        self.ln_x_v2d   = nn.LayerNorm(P)
        self.ln_emb_v2d = nn.LayerNorm(P)
        # exp577: gamma_emb = 1/sqrt(P) ~ 0.022 at P=2016. Brings logit scale to
        # ~1.0 at init for the wider V2D-projected vectors. exp576 used 0.1 which
        # was right at E=96 but at P=2016 gave logit std ~ sqrt(P) * 0.1 = 4.5,
        # softmax peaked confidently on random wrong vocab at CE = 19 nats = 7.7 bpb.
        self.ln_emb_v2d.weight.data.fill_(1.0 / math.sqrt(self.P))
    ...
    def forward(self, tokens, targets=None, loss_reduction='mean'):
        ...
        # exp576: V2D-tied head. Both sides hit V2D then LN, then dot product.
        v2d_x   = self.v2d(x_lut)                        # [B, T, P]  ±1 STE
        v2d_emb = self.v2d(self.tok_emb_E.weight)        # [V, P]    cacheable at eval
        ln_x    = self.ln_x_v2d(v2d_x)
        ln_emb  = self.ln_emb_v2d(v2d_emb)
        logits  = ln_x @ ln_emb.t()                       # [B, T, V]
...
print(f'residual_lut REMOVED (exp576: no D-stream; V2D-tied head on E-stream)')
P_v2d = E * (E - 1) // 2
print(f'V2D-TIED head (exp580, HARD fwd STE + LEARNABLE T): logits = ln_x_v2d(V2D_hard(x_lut)) @ ln_emb_v2d(V2D_hard(tok_emb_E)).T; V2D dim P=E*(E-1)/2={P_v2d}, smooth_mode=False (hard ±1 fwd, soft backward STE), T_init=0.5 (learnable, affects backward only), ln_emb_v2d.gamma init=1/sqrt(P)={1.0/math.sqrt(P_v2d):.4f}; RoPE base={_ROPE_BASE}')
...
def collect_temperature_specs(model):
    ...
    for li, blk in enumerate(model.layers):
        # exp576: residual_lut removed; iterate over the 3 surviving LUT modules.
