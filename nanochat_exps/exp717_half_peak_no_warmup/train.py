...
# Load init checkpoint (e.g. exp706 final state) — model weights only.
INIT_CKPT_PATH = cfg.get('init_checkpoint_path', None)
if INIT_CKPT_PATH:
    print(f'Loading initial model state from {INIT_CKPT_PATH}')
    init_ckpt = torch.load(INIT_CKPT_PATH, map_location=DEVICE, weights_only=False)
    src_state = init_ckpt.get('model_state_dict', init_ckpt)
    missing, unexpected = model.load_state_dict(src_state, strict=False)
    print(f'  loaded; missing={len(missing)} unexpected={len(unexpected)}')
    if missing: print(f'  missing keys (first 5): {list(missing)[:5]}')
    if unexpected: print(f'  unexpected keys (first 5): {list(unexpected)[:5]}')
    src_best_bpb = init_ckpt.get('best_val_bpb', float('nan'))
    print(f'  source checkpoint best_val_bpb = {src_best_bpb:.4f}')

_PEAK_LR_SCALE = cfg.get('peak_lr_scale', 1.0)

def get_lr_scale(step):
    n = N_STEPS
    w = int(WARMUP_FRAC * n)
    if step < w:
        return _PEAK_LR_SCALE * step / max(w, 1)
    progress = (step - w) / max(n - w, 1)
    return _PEAK_LR_SCALE * (0.1 + 0.9 * 0.5 * (1 + math.cos(math.pi * progress)))
