...
class CenterScale(nn.Module):
    """exp470: minimal pre-LUT norm = per-token mean-subtract + ONE global
    learnable scalar (init 1.0). No std-division, no per-channel affine.
    NB: for a sign-based LUT this is a near-no-op on SELECTION (mean-sub cancels
    in differences; a single positive scalar scales all differences equally ->
    sign-preserving). It only rescales |d| magnitude (which the learnable temp T
    already controls). So this is effectively an ablation of LayerNorm's
    per-channel affine: if it matches exp453, the affine wasn't load-bearing."""
    def __init__(self, dim):
        super().__init__()
        self.gamma = nn.Parameter(torch.ones(()))   # single scalar

    def forward(self, x):
        return (x - x.mean(dim=-1, keepdim=True)) * self.gamma
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        # Pre-norm BEFORE qkv/v LUTs: minimal CenterScale (mean-sub + scale).
        self.ln_pre  = CenterScale(E)
        # Post-norm on the residual-updated E-stream, feeds residual_lut.
        self.ln_post = CenterScale(E)
