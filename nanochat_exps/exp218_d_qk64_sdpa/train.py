"""nanochat_exps/exp218_d_qk64_sdpa — fork of exp183.

Three changes vs exp183 (E=96, val_bpb=1.6284 @ 302M):
  1) d_qk: 32 -> 64
  2) Remove qk_v2d — Q and K taken directly from joint QK LUT output,
     split as 64|64 of the 128-dim output
  3) Replace BitAttention with F.scaled_dot_product_attention.
     Scale: attn_scale / sqrt(d_qk) = attn_scale / 8

out_v2d / out_d2v rank canonicalization on out_proj output is preserved.

Launch:
    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python -u \\
        nanochat_exps/exp218_d_qk64_sdpa/train.py \\
        > nanochat_exps/exp218_d_qk64_sdpa/stdout.log 2>&1 &
"""
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        # NOTE: qk_v2d removed — Q and K are passed directly to SDPA.
        ...
        # Plain SDPA on raw d_qk-dim Q/K. Effective scale = attn_scale / sqrt(d_qk):
...
print(f'Q/K Joint MHLut: in_nap={cfg["qk_input_nap"]} tph={cfg["qk_tph"]} d_qk={d_qk} (n_outputs=2*d_qk={2*d_qk}) [no V2D, raw -> SDPA]')
