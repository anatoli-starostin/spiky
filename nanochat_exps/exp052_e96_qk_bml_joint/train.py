...
def _make_qk_joint(layer_idx, seed_offset):
    """Joint Q/K BitMultiHeadLUT: input=E (sum-mode), n_heads=H, n_outputs=2*P_qk.

    Output [B*T, H, 2*P_qk] is split downstream into q_raw=[..., :P_qk] and
    k_raw=[..., P_qk:] (each treated as raw pair-dominance for d_qk items),
    then both passed through DominanceCanonicalize(d_qk). CLT scale is applied
    in-kernel so per-pair output sits at unit-variance scale.
    """
    n_inputs = E + (_pos_emb_dim(layer_idx) if _POS_EMB_ACTIVE else 0)
    n_out = 2 * D_QK_P
    n_votes_per_pair = cfg['qk_tph'] * cfg['qk_output_nap'] / float(n_out)
...
for layer in model.layers:
    bit_luts += [layer.qk_joint, layer.v_perm, layer.out_proj]
