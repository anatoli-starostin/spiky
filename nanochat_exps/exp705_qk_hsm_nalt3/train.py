"""exp705 — fork of exp696 (1.4247 @ 100.7M) with qk_lut hybrid_smooth_n_alt=3.

Single change: qk_lut forward widens from top-2 softmax (main + 1 Hamming-1
neighbor) to top-4 softmax (main + 3 neighbors). Backward stays soft K-row +
4-row weight scatter. Same params (100.7M); qk inference bandwidth 4x (vs 2x
default in other LUTs).
"""
...
def _make_qk(seed_offset):
    # qk-only override: hybrid_smooth_n_alt > 1 widens the Hamming-1 blend
    # at the qk forward (top-(n_alt+1) softmax). Other LUTs stay at n_alt=1.
    return TinyMultiHeadLut(
        ...
        hybrid_smooth_n_alt=cfg.get('qkv_hybrid_smooth_n_alt', 1),
        **_TINY_SOFT_KWARGS,
    )
