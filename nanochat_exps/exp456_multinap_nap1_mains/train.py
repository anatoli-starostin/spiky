...
class MultiNapLut(nn.Module):
    """exp456: a NAP=N 'interaction' LUT + a NAP=1 'main-effect' LUT built on the
    SAME anchor pairs as the base (one NAP=1 table for every anchor pair used by
    the base LUT), outputs summed. The NAP=1 tables have only 2 rows, so each row
    sees ~B/2 tokens -> huge visit count -> near-zero gradient noise: a stable,
    well-estimated additive 'per-bit main effect' alongside the noisy 2^N-bucket
    lookup. Goal: faster/cleaner convergence.
    Bandwidth: base does H*tph lookups; main does H*tph*N -> (1+N)x total."""
    def __init__(self, base, input_dim, n_heads, n_out, base_tph, base_nap, seed):
        super().__init__()
        self.base = base
        self.main = TinyMultiHeadLut(
            input_dim=input_dim, n_heads=n_heads, n_outputs=n_out,
            n_anchor_pairs=1, tables_per_head=base_tph * base_nap,
            random_seed=seed, device=DEVICE, **_TINY_SOFT_KWARGS,
        )
        # Make the NAP=1 tables use exactly the base LUT's anchor pairs. base
        # pairs are [H*tph, N] row-major; flatten to [H*tph*N, 1] preserves the
        # per-head ordering (main has H heads, tph*N tables/head).
        with torch.no_grad():
            self.main.soft_anchor_a_long.copy_(base.soft_anchor_a_long.reshape(-1, 1))
            self.main.soft_anchor_b_long.copy_(base.soft_anchor_b_long.reshape(-1, 1))
            self.main.lookup.anchor_pairs_a.copy_(base.lookup.anchor_pairs_a.reshape(-1, 1))
            self.main.lookup.anchor_pairs_b.copy_(base.lookup.anchor_pairs_b.reshape(-1, 1))

    def forward(self, x):
        return self.base(x) + self.main(x)


_MULTINAP = cfg.get('multinap', True)

def _wrap(base, input_dim, n_heads, n_out, base_tph, base_nap, seed):
    if not _MULTINAP:
        return base
    return MultiNapLut(base, input_dim, n_heads, n_out, base_tph, base_nap, seed)
...
def _make_qkv_joint(layer_idx, seed_offset):
    ...
    return _wrap(base, E, H, 2 * d_qk + d_v, cfg['qkv_tph'], cfg['qkv_input_nap'],
                 cfg['random_seed'] + seed_offset + 5000)
...
def _make_v(layer_idx, seed_offset):
    ...
    return _wrap(base, E, H, d_v, cfg['v_tph'], cfg['v_input_nap'],
                 cfg['random_seed'] + seed_offset + 5000)
...
def _make_out(layer_idx, seed_offset):
    ...
    return _wrap(base, H * d_v, 1, E, cfg['out_tph'], cfg['out_input_nap'],
                 cfg['random_seed'] + seed_offset + 5000)
...
def _make_residual_lut(layer_idx, seed_offset):
    ...
    return _wrap(base, E, 1, D, cfg['residual_tph'], cfg['residual_input_nap'],
                 cfg['random_seed'] + seed_offset + 5000)
