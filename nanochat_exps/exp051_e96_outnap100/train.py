...
def _make_qk_joint(layer_idx, seed_offset):
    """Joint fp32 MultiHeadLut: input=E_in (sum-mode), n_heads=H, n_outputs=2*d_qk.

    Each head's output vector is split into [:d_qk] (q) and [d_qk:] (k); both
    halves are passed through VectorToDominance(d_qk) downstream.
    """
