...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        # Fixed (non-learnable) attn_scale. Sets backward softmax sharpness so
        # A_soft ≈ A_hard, minimizing STE gradient mismatch.
        self.register_buffer('attn_scale', torch.tensor(
            float(cfg.get('fixed_attn_scale', 3.0))
        ))
    ...
    def forward(self, x, pos_emb):
        ...
        # Hard-forward / soft-backward attention (STE). q, k are ±1 dominance
        # vectors so q @ k.T == 2*popcount_match - P (same argmax as popcount).
        # Forward A = one-hot(argmax(s)); backward gradient flows through
        # A_soft = softmax(s * attn_scale / sqrt(P)).
