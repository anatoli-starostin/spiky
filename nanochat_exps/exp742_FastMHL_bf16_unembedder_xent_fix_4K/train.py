"""exp742 — exp741 + cross_entropy moved INSIDE the autocast wrap (bug fix).

Fork of exp741 (LUT bs=48 4K bf16 storage + fp32 master Lion + force-bmm
+ bf16 unembedder, final 1.4022 = +15 mb vs exp737 v2's 1.3872). Diagnosed
bug: in exp741, only self.unembedder(x_resid) was inside autocast(bf16);
the subsequent F.cross_entropy call sat OUTSIDE autocast and therefore
ran on bf16 logits at bf16 precision — log_softmax + NLL on 32768-class
softmax over 12288 tokens with no fp32 promotion. exp738 vanilla doesn't
have this bug because its autocast wrap covers the full model.forward
(including cross_entropy).

Fix: extend the autocast block to cover the cross_entropy call too. Now
PyTorch's autocast fp32-promotion rule applies (cross_entropy is in the
fp32 op list) → cross_entropy runs at fp32 internally → stable softmax/NLL.

Compare to exp737 v2 = 1.3872. Expectation: ~0 drift (matches vanilla
finding that bf16 head is free when cross_entropy is fp32-promoted).
"""
