"""nanochat_exps/exp238_soft_lut — fork of exp234 (1.6212 @ 8K, 358M, d_v=32).

Two changes:
  1) Replace all three TinyMultiHeadLut modules (qk_joint, v_lut, out_proj)
     with the new SoftMultiHeadLUT. Soft path is fully matmul-based (no
     gather/scatter), differentiable through both pair signs (rational soft
     sign) and row selection (softmax). Two new temperatures with defaults
     T_soft=0.01, T_sel=1.0; can be annealed later toward 0 to recover the
     discrete-LUT regime. This run uses fixed temps.
  2) v_input_nap 8 -> 6, to keep the soft path's [B, n_tables, 2^nap]
     activation small (table_dim 256 -> 64) so device_batch_size stays at 8.
     V LUT param count drops accordingly (~12.6M -> 3.1M per layer × 6 = ~57M
     total reduction); ~358M -> ~301M total.

Launch:
    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python -u \\
        nanochat_exps/exp238_soft_lut/train.py \\
        > nanochat_exps/exp238_soft_lut/stdout.log 2>&1 &
"""
