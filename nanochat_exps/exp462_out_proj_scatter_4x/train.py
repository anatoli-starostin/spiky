...
def _make_out(layer_idx, seed_offset):
    # 4x output-scatter sparsity on out_proj ONLY (the high-tph module). Each
    # table writes E/sf outputs scattered (balanced) into the full E-wide output,
    # so each out dim is assembled from ~tph/sf tables instead of all tph.
    # out_proj params and per-token output-write bandwidth drop sf x; all other
    # LUTs stay dense. sf=1 -> dense (exp453).
    sf = cfg.get('out_scatter_factor', 4)
    skw = {}
    n_out = E
    if sf > 1:
        assert E % sf == 0
        n_out = E // sf
        skw = dict(sparse_scatter_n_outputs=E,
