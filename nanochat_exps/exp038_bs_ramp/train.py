...
_BS_SCHEDULE = [(0, 1), (500, 2), (1000, 4), (2000, 8), (4000, 16), (6000, 32)]
def batch_size_schedule(step):
    """Doubling ramp at fixed step thresholds: bs=1 → 2 (@500) → 4 (@1000)
    → 8 (@2000) → 16 (@4000) → 32 (@6000)."""
    bs = _BS_SCHEDULE[0][1]
    for thresh, value in _BS_SCHEDULE:
        if step >= thresh:
            bs = value
    return bs
...
for step in range(1, N_STEPS + 1):
    ...
    bs_now = batch_size_schedule(step)
    adam_opt.zero_grad()
    bit_opt.zero_grad()
    accum_loss = 0.0
    for _ in range(grad_accum):
        x_full, y_full = next(train_loader)
        x = x_full[:bs_now]
        y = y_full[:bs_now]
    ...
    if step % 100 == 0 or step == 1:
        print(f'step {step:6d} | loss={ema:.4f} | lr={lr_scale * cfg["adam_lr"]:.2e} | bs={bs_now}')
