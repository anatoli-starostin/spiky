"""exp688 — anneal T_sel on exp684 checkpoint for 1000 steps at 0.1% of peak LR.

Load exp684 final checkpoint (1.4262 @ 140M, hybrid_smooth). Continue training
for 1000 additional steps with:
  - lr_scale = 0.001  (0.1% of peak), constant for the whole anneal phase
  - log-linear anneal of every log_select_temp from its current learned value
    to log(0.001) (so T_sel: ~0.5 -> 0.001 over 1000 steps)
  - log_select_temp is excluded from both optimizers (we drive it externally)
  - log_soft_score_temp stays learnable (AdamW continues to optimise it)
  - Forward stays hybrid_smooth: u = softmax(ts/T_sel)[alt] -> 0 as T_sel -> 0,
    so the 2-row blend smoothly collapses to single-row argmax.

End evals (both with hybrid_smooth weights at the annealed T_sel ~= 0.001):
  - SOFT eval: hybrid_smooth forward at the final low T_sel (~= hard already).
...
ANNEAL_STEPS = 1000
ANNEAL_LR_SCALE = 0.001     # 0.1% of peak
