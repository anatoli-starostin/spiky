...
# FFN-LUT: residual MLP-equivalent operating on x_lut (E -> E). Defaults to
# the same NAP/tph schedule as out_proj. Enabled when cfg has ffn_tph or
# ffn_tph_per_layer (None disables the module).
...
def _make_ffn(layer_idx, seed_offset):
    """E -> E FFN-LUT with residual connection. Shape matches out_proj by default."""
...
class LUTBlock(nn.Module):
    ...
    def __init__(self, layer_idx):
        ...
        # Post-norm on attention output (was the final block output before FFN).
    ...
    def forward(self, x, cos, sin):
        ...
        # FFN with residual: ffn_lut sees raw out_e (no LN between out_proj and
        # FFN). LN is applied AFTER the residual add — post-norm style.
        # When FFN inactive, x_post = out_e (legacy: just LN, no FFN).
        if _FFN_ACTIVE:
            ffn_out = self.ffn_lut(out_e).squeeze(1)                  # [B*T, E]
            x_post = out_e + ffn_out
        else:
            x_post = out_e
        x_post_norm = self.ln_e(x_post)                               # [B*T, E]
        x_lut_next = x_post_norm.reshape(B, T, E)                     # -> next block
        r_out      = self.residual_lut(x_post_norm).squeeze(1).reshape(B, T, D)
...
if _FFN_ACTIVE:
    ...
    print(f'ffn_lut TinyMHLut({cfg.get("ffn_backward_mode", cfg.get("out_backward_mode", "soft"))}, noise_eps={_NOISE_EPS}): in_nap={cfg.get("ffn_input_nap", cfg["out_input_nap"])} tph={_ffn_tph_str} n_out=E={E}  [E -> E, residual]')
