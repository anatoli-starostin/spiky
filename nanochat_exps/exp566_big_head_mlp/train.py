...
class Model(nn.Module):
    def __init__(self):
        ...
        # exp566: big-head MLP (D -> H_HID -> V), GELU. Isolates the head from
        # any backbone change (D=384 stream preserved byte-for-byte vs exp513).
        H_HID = 1024
        self.unembedder = nn.Sequential(
            nn.Linear(D, H_HID, bias=False),
            nn.GELU(),
            nn.Linear(H_HID, VOCAB_SIZE, bias=False),
        )
...
print(f'UNTIED unembedder MLP: Linear(D={D}, 1024) -> GELU -> Linear(1024, V={VOCAB_SIZE}); tok_emb_E at E={E}; ln_final(D) before unembed; RoPE base={_ROPE_BASE}')
