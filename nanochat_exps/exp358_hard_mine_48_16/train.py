...
_HARD_MINE_KEEP = int(cfg.get('hard_mine_keep', 0))   # 0 = disabled
if _HARD_MINE_KEEP > 0:
    if _HARD_MINE_KEEP > DEVICE_BS:
        raise ValueError(f'hard_mine_keep={_HARD_MINE_KEEP} > device_batch_size={DEVICE_BS}')
    print(f'Hard-mine enabled: forward bs={DEVICE_BS}, backward on top-{_HARD_MINE_KEEP} hardest sequences per step')
...
for step in range(1, N_STEPS + 1):
    ...
    for _ in range(grad_accum):
        ...
        if _HARD_MINE_KEEP > 0:
            # Score: no-grad forward, per-sequence mean CE loss.
            with torch.no_grad():
                per_tok = model(x, targets=y, loss_reduction='none')   # [B*T]
                per_seq = per_tok.view(x.size(0), x.size(1)).mean(dim=1)  # [B]
            hard_idx = per_seq.topk(_HARD_MINE_KEEP).indices
            x = x.index_select(0, hard_idx)
            y = y.index_select(0, hard_idx)
