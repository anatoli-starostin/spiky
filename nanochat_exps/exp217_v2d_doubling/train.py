...
E_H = E // H                              # value head dim (= E/H so heads concat -> E)
E_QK = cfg.get('qk_dim', E_H)             # query/key head dim (independent of E_H)
...
class V2DSDPABlock(nn.Module):
    """Two sub-layer block (LUT-based, transformer-style):
       attn:  LN -> V2D -> per-head Q/K/V -> SDPA -> LN -> LUT_out  -> residual
       ffn:                                  LN -> LUT_ffn          -> residual"""
    def __init__(self, layer_idx: int):
        ...
        # ----- Attention sub-layer -----
        # Separate LayerNorms for the Q/K (pos-included) and V (pos-free) paths.
        self.ln_qk = nn.LayerNorm(E)
        self.ln_v  = nn.LayerNorm(E)
        self.v2d = VectorToDominance(
            d_head=E,
            n_pairs=D,
            n_heads=H,
            smooth_mode=cfg.get('v2d_smooth_mode', True),
            ...
            pair_sampling_mode=cfg.get('v2d_pair_sampling_mode', 'uniform'),
        )
        qkv_std = cfg.get('qkv_init_std', 0.005)
        gen = torch.Generator().manual_seed(cfg['random_seed'] + 2000 + layer_idx)
        # Q, K from V2D: per-head linear projections of dominance pairs.
        self.Wq = nn.Parameter(torch.randn(H, D, E_QK, generator=gen) * qkv_std)
        self.Wk = nn.Parameter(torch.randn(H, D, E_QK, generator=gen) * qkv_std)
        # V from a per-head LUT (no V2D for V): consumes ln_v(x) directly,
        # outputs [B*T, H, E_H] (sums across tables, no SparseScatter).
        self.lut_v = MultiHeadLut(
            input_dim=E,
            n_heads=H,
            n_outputs=E_H,
            n_anchor_pairs=cfg.get('v_lut_input_nap', 6),
            tables_per_head=cfg.get('v_lut_tph', 512),
            n_alternatives=cfg.get('v_lut_n_alternatives', 1),
            smooth_mode=cfg.get('v_lut_smooth_mode', cfg.get('lut_smooth_mode', False)),
            anchor_sampling_policy=AnchorSamplingPolicy.CANONICAL_FULL_COVERAGE,
            random_seed=cfg['random_seed'] + 8000 + layer_idx,
            initial_weights_noise=LUT_INIT_STD,
            return_per_table_outputs=False,
            device=torch.device(DEVICE),
        )
        # out_proj LUT (after SDPA): LN -> LUT -> SparseScatter -> residual.
        self.ln_out = nn.LayerNorm(E)
        self.lut_out = MultiHeadLut(
            input_dim=E,
            n_heads=1,
        ...
        self.scatter_out = SparseScatter(
            n_heads=1,
            tables_per_head=LUT_TPH,
        ...
        # ----- FFN sub-layer (separate LUT) -----
        self.ln_ffn = nn.LayerNorm(E)
        self.lut_ffn = MultiHeadLut(
            input_dim=E,
            n_heads=1,
            n_outputs=LUT_N_SPARSE,
            n_anchor_pairs=LUT_NAP,
            ...
            smooth_mode=cfg.get('lut_smooth_mode', False),
            anchor_sampling_policy=AnchorSamplingPolicy.CANONICAL_FULL_COVERAGE,
            random_seed=cfg['random_seed'] + 6000 + layer_idx,
            initial_weights_noise=LUT_INIT_STD,
            return_per_table_outputs=True,
            device=torch.device(DEVICE),
        )
        self.scatter_ffn = SparseScatter(
            n_heads=1,
            tables_per_head=LUT_TPH,
            n_sparse_outputs=LUT_N_SPARSE,
            n_outputs=E,
            seed=cfg['random_seed'] + 7000 + layer_idx,
            device=torch.device(DEVICE),
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        B, T, _ = x.shape
        # === Attention sub-layer ===
        # Per-layer pos_emb is added ONLY for Q/K (consumed at attention time).
        # The residual stream carries raw x — pos info is re-injected fresh per
        # layer, never accumulating through the residual.
        x_norm_qk = self.ln_qk(x + self.pos_emb[:T])
        x_norm_v  = self.ln_v(x)
        x_h_qk = x_norm_qk.unsqueeze(-2).expand(B, T, H, E)
        dom_qk = self.v2d(x_h_qk)
        q = torch.einsum('bthd,hde->bthe', dom_qk, self.Wq)
        k = torch.einsum('bthd,hde->bthe', dom_qk, self.Wk)
        v = self.lut_v(x_norm_v.view(B * T, E)).view(B, T, H, E_H)  # [B, T, H, E_H]
        q = q.transpose(1, 2)
        k = k.transpose(1, 2)
        v = v.transpose(1, 2)
        attn = F.scaled_dot_product_attention(q, k, v, is_causal=True)
        attn = attn.transpose(1, 2).contiguous().view(B, T, E)
        attn_ln = self.ln_out(attn)
        per_table = self.lut_out(attn_ln.view(B * T, E))
        scattered = self.scatter_out(per_table)
        out = scattered.view(B, T, E)
        x = x + out                                            # attn residual
        # === FFN sub-layer (LUT) ===
        x_ffn_ln = self.ln_ffn(x)
        per_table_ffn = self.lut_ffn(x_ffn_ln.view(B * T, E))
        scattered_ffn = self.scatter_ffn(per_table_ffn)
        ffn = scattered_ffn.view(B, T, E)
        x = x + ffn                                            # ffn residual
        return x
...
class V2DSDPATransformer(nn.Module):
    def __init__(self):
        ...
        # Two-layer MLP unembedder: E -> 4*E -> GELU -> VOCAB. No bias.
        unembed_hidden = cfg.get('unembed_hidden', 4 * E)
        self.head_fc1 = nn.Linear(E, unembed_hidden, bias=False)
        self.head_fc2 = nn.Linear(unembed_hidden, VOCAB_SIZE, bias=False)
        nn.init.normal_(self.tok_emb.weight,   std=0.02)
        nn.init.normal_(self.head_fc1.weight,  std=0.02)
        nn.init.normal_(self.head_fc2.weight,  std=0.02)
    ...
    def forward(self, idx, targets=None, loss_reduction='mean'):
        ...
        for block in self.blocks:
            x = block(x)
        logits = self.head_fc2(F.gelu(self.head_fc1(self.ln_f(x))))
...
print(f'V2DSDPATransformer: depth={DEPTH}, E={E}, H={H}, E_H={E_H}, E_QK={E_QK}, D={D}, seq_len={SEQ_LEN}')
