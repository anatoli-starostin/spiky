...
class V2DSDPABlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        B, T, _ = x.shape
        # === Attention sub-layer ===
        # Q, K see x with positional info added; V sees x without pos.
        # Q, K go through V2D + linear projection; V goes through a LUT directly.
        x_pos = x + self.pos_emb[:T]
        x_norm_qk = self.ln_qk(x_pos)
        ...
        # Residual carries the pos-included variant.
        x = x_pos
        q = q.transpose(1, 2)
        k = k.transpose(1, 2)
        v = v.transpose(1, 2)
