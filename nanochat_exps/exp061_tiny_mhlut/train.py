"""nanochat_exps/exp061_tiny_mhlut — bf16 fork of exp060 using TinyMultiHeadLut.

Same architecture as exp060 (E=96, d_qk=32, d_v=16, 6 layers, concat unembedder).
Differences:
  - bf16 confined to TinyMultiHeadLut weight storage + the gather inside the
    block. Each LUT call casts its fp32 input → bf16 just before the kernel,
    and its bf16 output → fp32 immediately after. V2D / D2V / SDPA /
    LayerNorm / embeddings / unembedder all run fp32.
  - TinyMultiHeadLutOptimizer manages the ~107M LUT weights with bf16
    storage, fp32 Adam moments, and stochastic-rounded fp32 → bf16 weight
    update (unbiased; preserves persistent small updates that biased RNE
    would round away).
  - Token embedder + final unembedder + LayerNorms + attn_scale + pos_embs
    all kept fp32 (~22M params), managed by stock AdamW.

The "alt carrier" bug in `_TinyMHLutGatherReduce` was fixed before this run;
without it bf16 measurements were contaminated by a missing-gradient term
(consistent +0.03 bpb gap across all variants). Now that
TinyMultiHeadLut is provably numerically equivalent to MultiHeadLut at fp32
(see `tests/test_tiny_vs_full_equivalence.py`), this run is the real test
of bf16 precision cost vs the exp060 fp32 baseline (final 1.7114 bpb).
...
class LUTBlock(nn.Module):
    """LUT block with bf16 confined to TinyMultiHeadLut storage and the gather
    inside each LUT call. Block input/output are fp32; V2D / D2V / SDPA /
    LayerNorm / attn_scale all run fp32. Output of each TinyMultiHeadLut is
    bf16 (since weights are bf16) → cast back to fp32 immediately after.
    """
    ...
    def forward(self, x, pos_emb):
        # x and pos_emb fp32. TinyMultiHeadLut accepts fp32 input directly
        # (TAPL kernel dispatches over fp32/bf16/fp16) and returns bf16
        # output (= weight dtype). We cast back to fp32 right after.
        B, T, _ = x.shape
        if _POS_EMB_ACTIVE:
        ...
        qk_out = self.qk_joint(xp).float()                             # [B*T, H, 2*d_qk]
        q_vec = qk_out[..., :d_qk]
        k_vec = qk_out[..., d_qk:]
        ...
        v_vec = self.v_lut(x_flat).float()                             # [B*T, H, d_v]
        ...
        out_real = self.out_proj(out_in).squeeze(1).float()            # [B*T, E]
...
# Dual-optimiser split: TinyMultiHeadLut weights (bf16, ~107M params) go to
# TinyMultiHeadLutOptimizer (fp32 moments + stochastic-rounded weight write).
# Everything else (token_embedder, pos_embs, LayerNorms, attn_scale, unembedder
# Linear — all fp32, ~22M params) goes to stock AdamW.
tiny_modules = [m for m in model.modules() if isinstance(m, TinyMultiHeadLut)]
tiny_weight_ids = {id(m.weights) for m in tiny_modules}
adam_params = [p for p in model.parameters() if id(p) not in tiny_weight_ids]
tiny_param_count = sum(m.weights.numel() for m in tiny_modules)
adam_param_count = sum(p.numel() for p in adam_params)
print(f'TinyMultiHeadLut modules: {len(tiny_modules)} (expected {3 * N_LAYERS})')
print(f'Tiny-LUT params (bf16 storage, bf16 moments, bf16 compute + SR via TinyMultiHeadLutOptimizer): {tiny_param_count:,}')
print(f'AdamW params (fp32, non-LUT): {adam_param_count:,}')
...
# AdamW (decay/nodecay) for non-LUT params.
...
# Max-hard mode: bf16 weights, bf16 Adam moments, and bf16 Adam math.
# The only fp32 in the optimizer path is the final stochastic-rounded
# weight update (preserves persistent small updates that biased RNE would
# round away). Tests the hardest case for bf16 viability now that the
# alt-carrier bug is fixed.
tiny_opt = TinyMultiHeadLutOptimizer(
    tiny_modules,
    lr=cfg['adam_lr'],
...
tiny_opt_initial_lr = tiny_opt.lr
...
for step in range(1, N_STEPS + 1):
    # Set LR for both optimisers BEFORE step() so iteration 1 trains at
    # lr_scale = 1/warmup_steps (not 0).
    lr_scale = get_lr_scale(step)
    for g in adam_opt.param_groups:
        g['lr'] = g['initial_lr'] * lr_scale
    tiny_opt.lr = tiny_opt_initial_lr * lr_scale
