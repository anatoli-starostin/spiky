...
# --- LUT factories (all MultiHeadLut, smooth_mode=True, n_alt=3) --------------
# exp610: switch from TinyMHLut(soft) to standard MultiHeadLut(smooth, n_alt=3) per user.
_MHLUT_KWARGS = dict(
    anchor_sampling_policy=AnchorSamplingPolicy.CANONICAL_FULL_COVERAGE,
    initial_weights_noise=cfg.get('mhlut_init_std', 0.001),
    n_alternatives=cfg.get('mhlut_n_alternatives', 3),
    smooth_mode=cfg.get('mhlut_smooth_mode', True),
)
...
print(f'ALL LUTs: MultiHeadLut(smooth_mode={cfg.get("mhlut_smooth_mode", True)}, n_alternatives={cfg.get("mhlut_n_alternatives", 3)})')
print(f'qk_lut       MultiHeadLut: in_nap={cfg["qkv_input_nap"]} tph={cfg["qkv_tph"]} n_out=2*d_qk={2*d_qk}  [no v-branch]')
print(f'v_lut        MultiHeadLut: in_nap={cfg["v_input_nap"]} tph={cfg["v_tph"]} d_v={d_v}')
print(f'out_proj     MultiHeadLut: in_nap={cfg["out_input_nap"]} tph={cfg["out_tph"]} n_out=E={E}')
print(f'residual_lut MultiHeadLut: in_nap={cfg["residual_input_nap"]} tph={cfg["residual_tph"]} n_out=D={D}  [E -> D]')
