"""exp749 — exp742 + clip_grad_norm_(model.parameters(), 1.0) on ALL params.

Fork of exp742 (bf16 head + xent inside autocast, base = exp737 v2). Adds
torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0) BEFORE optimizer
step — matches vanilla exp738's recipe of clipping the global grad norm.
Tests whether bounding the gradient norm dampens chaotic divergence from
bf16 head noise (per diag_drift_mechanism.py showing per-step grad cos
collapses from 0.999995 at step 1 to 0.03 by step 100 under bf16 head).

Note: at step 1 the LUT grad norm is ~14 (Lion sign-step makes magnitude
irrelevant for update direction, but the moment buffer gets scaled). After
warmup, global norm settles to ~1.4 → ~1.12, so clip(1.0) only mildly
scales the steady-state gradient. Effect on Lion-sign-update is indirect
(through momentum integration only).
"""
...
for step in range(1, N_STEPS + 1):
    ...
    # Vanilla-equivalent global grad clip across ALL parameters (including
    # LUT body). Tests whether bounding the gradient norm dampens chaotic
    # divergence introduced by bf16 head noise (per drift-mechanism diag).
