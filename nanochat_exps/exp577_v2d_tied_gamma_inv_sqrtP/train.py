...
class Model(nn.Module):
    def __init__(self):
        ...
        # exp576: V2D-tied head — pairwise sign-dominance projection of x_lut and
        # tok_emb_E.weight (both E-dim) into P = E*(E-1)/2 dim dominance space.
        # Default mode: full triu pair set; deterministic, so both V2D calls share
        # the same pair index set. STE: hard ±1 forward, rational soft backward.
        self.v2d = VectorToDominance(d_head=E, smooth_mode=False, temperature=0.1)
        P = E * (E - 1) // 2
        self.P = P
...
P_v2d = E * (E - 1) // 2
print(f'V2D-TIED head (exp577): logits = ln_x_v2d(V2D(x_lut)) @ ln_emb_v2d(V2D(tok_emb_E)).T; V2D dim P=E*(E-1)/2={P_v2d}, ln_emb_v2d.gamma init=1/sqrt(P)={1.0/math.sqrt(P_v2d):.4f}; RoPE base={_ROPE_BASE}')
