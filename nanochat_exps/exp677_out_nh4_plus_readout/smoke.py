#!/usr/bin/env python3
"""Smoke test: exp677 — exp670 (out_proj nh=4) + residual read_out (nh=4)."""
...
print(f'out_proj: nh={op_nh} tph={op_tph} NAP={op_nap} n_per_head={op_per} params={op.weights.numel():,}')
...
assert ro_nh == 4 and ro_tph == 256 and ro_nap == 6
ro_per = E // ro_nh
ro = TinyMultiHeadLut(input_dim=E, n_heads=ro_nh, n_outputs=ro_per,
                     ...
                     backward_mode='hybrid_smooth', soft_score_temp=0.5, select_temp=0.5,
                     learnable_temps=True, use_bf16=True)
print(f'read_out: nh={ro_nh} tph={ro_tph} NAP={ro_nap} n_per_head={ro_per} params={ro.weights.numel():,}')
assert ro.weights.numel() == 6_291_456

print(f'\nout_proj per-layer 6.29M × 6 layers = 37.75M')
print(f'read_out one-time   6.29M')
print(f'sum (replaces exp666 out_proj+read_out = 37.75M + 6.29M = 44.04M) — same as exp666')
...
r.sum().backward()
assert torch.isfinite(xr.grad).all() and torch.isfinite(ro.weights.grad).all()
