"""exp740 — exp737 v2 + bf16 SDPA + bf16 unembedder (train AND eval).

Fork of exp737 v2 (LUT bs=48 4K bf16 storage + fp32 master Lion + force-bmm,
final val=1.3872 @ 276.8M). Two surgical autocast wraps inside model.forward:
  1. SDPA: wrap F.scaled_dot_product_attention(q,k,v) in autocast(bf16).
     q/k/v come in as fp32 from the explicit .float() casts after qk_lut/v_lut;
     autocast handles the cast, runs bf16 SDPA, and we cast attn back to fp32
     for out_proj input (which expects fp32).
  2. unembedder: wrap self.unembedder(x_resid) in autocast(bf16).

Because model.forward is shared by train and eval (evaluate_bpb calls it),
this naturally covers both paths — no separate eval wrapping needed. SDPA
and the head matmul both run bf16 at train AND eval. Everything else
(LayerNorms, residual adds, out_proj inputs) stays at fp32. LUT modules
continue to use their internal bf16 autocast (use_bf16=True), unchanged.

Result: matches exp738's bf16-everywhere train forward pattern.
Compare to: exp737 v2 = 1.3872. Expectation: Δ ~ 0.
"""
...
class LUTBlock(nn.Module):
    ...
    def forward(self, x, cos, sin):
        ...
        out_in = attn.permute(0, 2, 1, 3).reshape(B * T, H * d_v).float()
