...
def _make_qkv_joint(layer_idx, seed_offset):
    """Joint Q/K/V LUT — single TinyMHL per head producing all three vectors.

    n_outputs = 2*d_qk + d_v. NAP/tph from cfg['qkv_input_nap']/cfg['qkv_tph']
    (default to v's NAP/tph), so all q/k/v outputs share the same lookup tables.
    """
    mode = cfg.get('qkv_backward_mode', cfg.get('v_backward_mode', 'soft'))
    ...
    return TinyMultiHeadLut(
        ...
        n_anchor_pairs=cfg.get('qkv_input_nap', cfg['v_input_nap']),
        tables_per_head=cfg.get('qkv_tph', cfg['v_tph']),
...
class LUTBlock(nn.Module):
    """exp321 block + joint qkv LUT: a single TinyMHL produces q, k, v from x
    (n_outputs = 2*d_qk + d_v). RoPE on q,k before SDPA. Uses v_lut's NAP/tph."""
    def __init__(self, layer_idx):
        super().__init__()
        self.qkv_joint    = _make_qkv_joint(layer_idx, layer_idx)
    ...
    def forward(self, x, cos, sin):
        ...
        # Single joint qkv LUT produces all three vectors from x. q/k get RoPE
        # before SDPA; v is unrotated.
        qkv_out = self.qkv_joint(x_flat)                              # [B*T, H, 2*d_qk + d_v]
        q_vec = self.q_norm(qkv_out[..., :d_qk])
        k_vec = self.k_norm(qkv_out[..., d_qk:2 * d_qk])
        v_vec = qkv_out[..., 2 * d_qk:]
...
print(f'QKV Joint TinyMHLut({cfg.get("qkv_backward_mode", cfg.get("v_backward_mode", "soft"))}, noise_eps={_NOISE_EPS}): in_nap={cfg.get("qkv_input_nap", cfg["v_input_nap"])} tph={cfg.get("qkv_tph", cfg["v_tph"])} n_out=2*d_qk+d_v={2*d_qk+d_v} (d_qk={d_qk}, d_v={d_v})')
...
def collect_temperature_specs(model):
    ...
    for li, blk in enumerate(model.layers):
        for slut_name in ('qkv_joint', 'out_proj', 'residual_lut'):
