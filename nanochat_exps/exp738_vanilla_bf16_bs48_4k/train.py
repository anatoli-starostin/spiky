"""exp738 — untied vanilla MinimalGPT+RoPE bs=48 4K with actual bf16 autocast.

Fork of exp709 (untied vanilla bs=48 16K = 1.1922 @ 35.79M, fp32 storage AND
fp32 compute despite the misleading compute_dtype=bf16 flag, which the parent
defined but never applied). Changes:
  (1) n_steps 16K -> 4K (cheap apples-to-apples 4K-budget run for comparison
      vs exp732/exp737 LUT-LM 4K runs at the same bs=48).
  (2) Wrap forward in torch.amp.autocast('cuda', dtype=bf16) so the flag does
      what it says. Matmuls (qkv proj, out proj, MLP, unembedder) run in bf16
      tensor cores. AdamW keeps fp32 master + fp32 momentum (PyTorch default).

Expected: small bpb regression (~5-15 mb) vs the equivalent fp32 4K run.
Wallclock ~9-10 min. Launched in parallel with exp737 v2 (LUT-LM 4K bf16).
