...
class SparseLut(nn.Module):
    """MultiHeadLut with return_per_table_outputs=True followed by SparseScatter.
    Decouples per-table weight cost from n_outputs: weights ~ n_sparse_outputs
    instead of n_outputs."""
    ...
    def forward(self, x):
        return self.scatter(self.lut(x))  # [B, n_heads, n_outputs]
...
def _make_ffn(layer_idx, seed_offset):
    return SparseLut(
        ...
        n_heads=1,
        n_outputs=E,
        n_anchor_pairs=cfg['ffn_input_nap'],
        tables_per_head=cfg['ffn_tph'],
        n_sparse_outputs=cfg['ffn_n_sparse_outputs'],
        lut_seed=cfg['random_seed'] + seed_offset,
        scatter_seed=cfg['random_seed'] + seed_offset + 10000,
    )
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        self.block_attn_ln = nn.LayerNorm(E)
        self.block_ffn_ln  = nn.LayerNorm(E)
    ...
    def forward(self, x, pos_emb):
        ...
        attn_out = self.block_attn_ln(out_real + x)

        ffn_out = self.ffn_lut(attn_out.reshape(B * T, E)).squeeze(1).reshape(B, T, E)
        return self.block_ffn_ln(ffn_out + attn_out)
...
print(f'ffn SparseLut:       in_nap={cfg["ffn_input_nap"]} tph={cfg["ffn_tph"]} n_sparse={cfg["ffn_n_sparse_outputs"]} (E={E} -> E)')
print(f'Residual:        attention skip + LN, then ffn skip + LN')
