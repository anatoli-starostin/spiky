"""exp736 — unified qkv_lut (n_heads=H=6, NAP=6, tph=256, n_out=2*d_qk+d_v=192).

Fork of exp732 (4K, val=1.3912). Replaces the separate
  qk_lut: H=6, NAP=4, tph=256, n_out=2*d_qk=128
  v_lut : H=6, NAP=6, tph=256, n_out=d_v=64
with one
  qkv_lut: H=6, NAP=6, tph=256, n_out=2*d_qk + d_v = 192

Each head still owns its own anchor router (n_heads=H), but for that head
q, k, v share the same anchor selection and chosen cell — they live in a
single 192-wide table row. Output (B*T, H, 192) is split last-dim into
q, k, v of widths d_qk, d_qk, d_v; q/k get norms + RoPE as before.

Param delta per layer: old qk+v 9.44M -> new qkv 18.87M (+9.43M). Total
model goes from 276.8M to ~333M. Same 4K LR schedule (warmup 0.1).

Phase D GEMM doubles vs exp732's qk+v combined; Phase H bmm-wgrad path
active (n_out=192 >= 128). Expected wallclock ~1.5-1.8 h.
"""
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        super().__init__()
        self.qkv_lut      = _make_qkv(layer_idx)
    ...
    def forward(self, x, cos, sin):
        ...
        q_vec = self.q_norm(qkv_out[..., :d_qk])                      # (B*T, H, d_qk)
        k_vec = self.k_norm(qkv_out[..., d_qk:2 * d_qk])              # (B*T, H, d_qk)
        v_vec = qkv_out[..., 2 * d_qk:]                               # (B*T, H, d_v)
...
print(f'qkv_lut      FastMHLut(unified, LION): in_nap={cfg["qkv_input_nap"]} tph={cfg["qkv_tph"]} n_out=2*d_qk+d_v={2*d_qk+d_v}')
...
def collect_temperature_specs(model):
    ...
    for li, blk in enumerate(model.layers):
        for slut_name in ('qkv_lut', 'out_proj', 'residual_lut'):
