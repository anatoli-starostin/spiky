...
class Model(nn.Module):
    def __init__(self):
        ...
        # 2-layer MLP unembedder (LN + Linear + ReLU + Linear), matching
        # exp002 / exp427 convention. Param budget: ~51M (vs ~12.6M for
        # the simpler LN+Linear used in exp003-013).
        unembed_hidden = concat_dim * 4
        self.unembedder = nn.Sequential(
            nn.LayerNorm(concat_dim),
            nn.Linear(concat_dim, unembed_hidden),
            nn.ReLU(),
            nn.Linear(unembed_hidden, VOCAB_SIZE),
        )
