"""exp687 — fork of exp684 with backward_mode 'hybrid_smooth' -> 'soft'.

Same per-layer residual_lut architecture as exp684. Single change in the
TinyMultiHeadLut kwargs: backward_mode='soft' instead of 'hybrid_smooth'.

  - Forward: single-row argmax with coefficient 1.0
    (kernel: _soft_lut_fwd_body_einsum -> embedding_bag(W[argmax])).
  - Backward: soft K-row attribution to x, soft T_soft/T_sel grads,
    hard index_add at the chosen row for W.

Tests whether training directly for the hard functional form recovers the
34 mb gap measured when the exp684 hybrid_smooth checkpoint is collapsed to
the single-row forward at inference (1.4262 -> 1.4529).
"""
