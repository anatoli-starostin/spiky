"""exp748 — exp742 + dbs 24 -> 48 (grad_accum 2 -> 1), full 4000-step run.

Fork of exp742 (bf16 head + xent inside autocast on exp737 v2 base = bf16
LUT storage + fp32 master Lion). Single change: device_batch_size 24 -> 48
so grad_accum becomes 1. exp743 made this same change but was killed at
step 1400 at +8 mb drift; exp748 runs to step 4000 for the final number.

Hypothesis: with grad_accum=2 the bf16 .grad buffer of the LUT body suffers
precision loss when the second add accumulates bf16-noisy gradient (the
noise originating in the bf16 head matmul). grad_accum=1 eliminates that
second add. If exp748 closes the drift to <2 mb of exp737 v2 (1.3872), the
hypothesis is confirmed.

Note: dbs change shifts data-packing slightly; not bit-exact equivalent of
exp742, but the closest clean test we can run.
"""
