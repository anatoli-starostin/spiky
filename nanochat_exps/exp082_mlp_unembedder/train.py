"""nanochat_exps/exp082_mlp_unembedder — fork of exp081 with an MLP unembedder head.

Single change vs exp081: replace the linear unembedder
    Linear(224, VOCAB)
with a 1-hidden-layer MLP
    LayerNorm(224) -> Linear(224, 4*224=896) -> ReLU -> Linear(896, VOCAB).
The V2D -> D2V canonicalization on the 224-dim concat is unchanged — it still
runs before the MLP head.

Architecture (carried from exp081):
  E=O=32, concat-prepend residual, per-O-chunk QK/V partitions, H head-zone
  partitions on out_proj. Per-layer input dims [32, 64, 96, 128, 160, 192];
  final concat 224. n_steps=8000, out_tph=1024 flat.

How to launch:

    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python \\
        -u nanochat_exps/exp082_mlp_unembedder/train.py \\
        > nanochat_exps/exp082_mlp_unembedder/stdout.log 2>&1 &
"""
