"""exp732 — short-horizon iteration baseline forked from exp731.

Single change vs exp731: n_steps 16000 -> 4000. Reference run that further
4K-step forks will compare against. Comparison to exp731's final 1.2178 is
INVALID (different LR schedule + warmup fraction reaches peak much earlier).

The bmm-based wgrad optimisation introduced today in
src/spiky/lutorch/fast_multi_head_lut.py (sparse-S + bmm for n_outputs>=128
modules) is automatically active and should knock ~10% off the wallclock.

Recipe (unchanged from exp731):
  forward_mode  = 'hard'      (1-row argmax via F.embedding_bag('sum'))
  backward_mode = 'dense_K'   (full K-row softmax surrogate input grad,
                               1-row weight scatter — now via bmm for
                               n_outputs>=128 modules)
  E=384 NAP-bump arch, bs=24 ga=2 (eff bs=48 sequences), Lion lr=2e-4 LUT,
  AdamW 3e-4 non-LUT, soft_score_temp=select_temp=0.5 learnable temps.

Expected wallclock: ~65-70 min (exp731 was 4.89 h for 16k).
"""
