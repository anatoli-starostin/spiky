"""exp751 — exp750 + nn.Linear(D, V, bias=True) on the unembedder.

Fork of exp750 (NEW SOTA 1.3863, exp737 v2 recipe + global clip(1.0)).
Single change: unembedder bias=False -> bias=True. The bias term (shape V)
goes to AdamW's nodecay_params group (wd=0) — 32K extra params, ~negligible.

Hypothesis: per-class log-prior is easier to learn via a direct bias than
via output-weight magnitudes. May speed up convergence at the 4K budget,
especially for very common/rare tokens. Compare to exp750 = 1.3863.
...
class Model(nn.Module):
    def __init__(self):
        ...
        self.unembedder = nn.Linear(D, VOCAB_SIZE, bias=True)
