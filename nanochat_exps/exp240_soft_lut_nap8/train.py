"""nanochat_exps/exp240_soft_lut_nap8 — clean apples-to-apples vs exp234 hard LUT.

Two changes vs exp239:
  1) v_input_nap 6 -> 8 (matches exp234's V LUT exactly). Now SoftMultiHeadLUT
     architecture is identical to exp234 modulo the soft lookup mechanism.
     The bf16+compile speedup makes the larger 2^nap=256 row dim affordable
     (microbench: 7 GB peak/iter, 42 GB across 6 layers — fits on 80 GB).
  2) T_sel 0.1 -> 0.5: middle ground between exp238's broad 1.0 (gradient
     spread evenly across rows but small output magnitude) and exp239's
     too-sharp 0.1 (saturated softmax, dead gradient).

Carried from exp239: bf16 autocast + torch.compile(forward, dynamic=True).
T_soft stays at 0.01.

Launch:
    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python -u \\
        nanochat_exps/exp240_soft_lut_nap8/train.py \\
        > nanochat_exps/exp240_soft_lut_nap8/stdout.log 2>&1 &
"""
