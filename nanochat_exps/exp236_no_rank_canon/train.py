"""nanochat_exps/exp236_no_rank_canon — fork of exp234 (1.6212 @ 8K, 358M, d_v=32).

Single change: drop the rank-canonicalization (V2D + D2V) after out_proj.
Replace with plain LayerNorm(E). Residual stream is no longer "ranking-only";
out_proj output keeps its real-valued magnitude info. Tests whether the rank
canonicalization between blocks is helping or hurting. Magnitude-stripping
behavior (descended from exp071) may have been bottlenecking the trunk.

Launch:
    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python -u \\
        nanochat_exps/exp236_no_rank_canon/train.py \\
        > nanochat_exps/exp236_no_rank_canon/stdout.log 2>&1 &
"""
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        # Rank canonicalization (V2D + D2V) replaced with plain LayerNorm — keeps
        # out_proj magnitude info in the residual stream.
...
print(f'out_proj MHLut:  in_nap={cfg["out_input_nap"]} tph={_tph_str} (n_outputs=E={E}) + LayerNorm (no rank canon)')
