...
if _LUT_OPT == 'lion':
    ...
    lut_optimizer = torch.optim.SGD([dict(params=lut_params, lr=_LUT_LR, weight_decay=0.0)],
                                    lr=_LUT_LR, momentum=cfg.get('lut_momentum', 0.9),
                                    nesterov=cfg.get('lut_nesterov', False))
...
for step in range(1, N_STEPS + 1):
    ...
    if step == 1:
        with torch.no_grad():
            sq = sum(float((p.grad * p.grad).sum()) for p in lut_params if p.grad is not None)
            n  = sum(p.grad.numel() for p in lut_params if p.grad is not None)
            print(f"[GRADPROBE] LUT grad RMS @ step1 = {(sq/n)**0.5:.6e}  "
                  f"(match Adam step ~2.2e-4 -> sgd lr ~{2.2e-4/((sq/n)**0.5):.3e} raw, "
                  f"~{2.2e-5/((sq/n)**0.5):.3e} with mu=0.9 steady-state x10)", flush=True)
