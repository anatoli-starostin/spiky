"""exp697 — fork of exp696 (1.4247 @ 100.7M) with pre_lut added on top.

Composes both modifications:
  + emb_resid_lut (from exp696): a 7th D-stream contribution from
    MeanAbsNorm(tok_emb_E(tokens)) — direct embedding -> D-stream channel
    for the unembedder.
  + pre_lut (from exp695): TinyMHLut(NAP=6, tph=256, n_heads=6, n_out=24)
    applied to MeanAbsNorm(x_lut) right after the embedding; output
    [B*T, 6, 24] reshaped to [B, T, E=144] and added residually to x_lut.
    Transforms the E-stream before the LUTBlock stack.

Diagnostic goal: see whether L0 attention's self_mass changes once pre_lut
handles per-token transformation (it didn't in exp696 — self_mass went
0.45 -> 0.50 because emb_resid_lut only addresses the unembedder-side issue).
"""
...
class Model(nn.Module):
    def __init__(self):
        ...
        self.pre_lut = _make_pre_lut(900)
    ...
    def forward(self, tokens, targets=None, loss_reduction='mean'):
        ...
        # pre_lut: per-token non-linearity on E-stream right after the embedding.
