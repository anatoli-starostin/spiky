...
# exp262: swap TinyMultiHeadLut(backward_mode='soft') for TinyMultiHeadLut(
# backward_mode='ste', n_alternatives=3) + same bernoulli noise on small deltas.
# Apples-to-apples comparator for exp257: TinyMHL-fast forward, n_alt=3
# uncertainty backward (matches MultiHeadLut(smooth=False, n_alt=3)), same
# noise. Isolates the soft-pipeline backward's contribution above n_alt=3
...
def _make_qk_joint(layer_idx, seed_offset):
    """Joint Q/K TinyMHLut(ste, n_alt=3): input=E, n_heads=H, n_outputs=2*d_qk."""
    n_inputs = E + (_pos_emb_dim(layer_idx) if _POS_EMB_ACTIVE else 0)
    qk_kwargs = dict(_TINY_MULTIALT_KWARGS)
...
print(f'Q/K Joint TinyMHLut(ste, n_alt=3, noise_eps={_NOISE_EPS}): in_nap={cfg["qk_input_nap"]} tph={cfg["qk_tph"]} d_qk={d_qk} (n_outputs=2*d_qk={2*d_qk}) init_std={cfg.get("qk_lut_init_std", cfg.get("mhlut_init_std", 0.001))} [no V2D, q_norm+k_norm -> SDPA]')
print(f'V TinyMHLut(ste, n_alt=3, noise_eps={_NOISE_EPS}):       in_nap={cfg["v_input_nap"]} tph={cfg["v_tph"]} d_v={d_v} (n_outputs=d_v)')
_tph_str = str(_OUT_TPH_PER_LAYER) if _OUT_TPH_PER_LAYER is not None else str(cfg['out_tph'])
print(f'out_proj TinyMHLut(ste, n_alt=3, noise_eps={_NOISE_EPS}): in_nap={cfg["out_input_nap"]} tph={_tph_str} (n_outputs=E={E}) + out_v2d/out_d2v')
...
def collect_temperature_specs(model):
    ...
    for li, blk in enumerate(model.layers):
        for slut_name in ('qk_joint', 'v_lut', 'out_proj'):
            ...
            # Multi-alt STE mode: learnable uncertainty T (analog of T_soft).
            if getattr(mod, 'multialt_learnable_temps', False):
                specs.append((f'L{li}.{slut_name}.uncertainty_T',
                              (lambda m=mod: float(m.log_uncertainty_T.detach().exp()))))
