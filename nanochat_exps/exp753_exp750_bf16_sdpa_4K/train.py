"""exp753 — exp750 + bf16 SDPA (consistency with vanilla).

Fork of exp750 (4K SOTA 1.3863, bf16 LUT storage + fp32 master Lion + fp32
head + global clip(1.0), fp32 SDPA). Single change: cast q, k, v to bf16
before F.scaled_dot_product_attention so SDPA runs on bf16 tensor cores
(matches vanilla's pattern where everything is bf16 under autocast). The
attn output is cast back to fp32 for the LUT body downstream.

Test: explicit casts (no autocast wrap, so no Python-side overhead). The
SDPA-only benchmark showed bf16 SDPA is 5.7x faster per call at our shapes
(0.087 vs 0.493 ms) — but SDPA is < 1% of total forward, so wall-clock
impact is < 1%. Validates consistency with vanilla (both run bf16 SDPA).

Compare to exp750 = 1.3863. Expectation: ~0 drift, marginal speedup.
...
class LUTBlock(nn.Module):
    ...
    def forward(self, x, cos, sin):
        ...
        # bf16 SDPA — matches vanilla's pattern. Explicit cast (no autocast
        # overhead). attn cast back to fp32 for the LUT body downstream.
        q = q.to(torch.bfloat16); k = k.to(torch.bfloat16); v = v.to(torch.bfloat16)
        attn = F.scaled_dot_product_attention(q, k, v, is_causal=True).float()
