"""nanochat_exps/exp443_dominance_gate — fork of exp365.

Tests centroid-dominance gating on the soft-mode weight gradient. For each
output coordinate `a` of the chosen LUT row W[k*], the gradient is kept only
if `(W[k*][a] - mean(W[k*])) * (grad_pt[a] - mean(grad_pt)) >= 0`, i.e. when
the gradient is fighting current centroid-dominance. Where dominance is
already in the right direction (product < 0), the gradient is zeroed —
strengthening it further doesn't help downstream LUT/unembedder/LayerNorm
consumers which are all invariant to the mean direction. Also projects out
the mean direction of grad_pt (gauge for all downstream consumers).

Toggled by `dominance_gate: true` in config.json -> calls
`set_dominance_gate(True)` at startup.

Reference: exp365 baseline = 1.6215 @ 8000 steps.
...
from spiky.lutorch.tiny_multi_head_lut import TinyMultiHeadLut, set_dominance_gate  # noqa: F401
...
if cfg.get('dominance_gate', False):
    set_dominance_gate(True)
    print('[dominance_gate] enabled — gating soft-mode LUT weight gradient by centroid-dominance rule')
