...
from spiky.lutorch.tiny_multi_head_lut import TinyMultiHeadLut, enable_soft_weight_grad
...
# Hybrid mode: hard forward (argmax pick, unchanged), but weight gradient is
# attributed across ALL 2^NAP rows via sel_soft instead of index_add at the
# chosen row only. Global toggle on the backward_mode='soft' path.
_SOFT_WGRAD = cfg.get('soft_weight_grad', False)
enable_soft_weight_grad(_SOFT_WGRAD)
print(f"[exp445] soft_weight_grad = {_SOFT_WGRAD}", flush=True)
