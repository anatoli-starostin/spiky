...
class DominanceLinearBranch(nn.Module):
    """exp457: parallel 'main-effect' branch = VectorToDominance(x) -> Linear.
    V2D emits the sign (+/-1, hard forward / soft backward via STE) of every
    pairwise difference x_a-x_b over a pair set (full triu by default); Linear
    maps those P dominance bits to the module output. This is the linear-on-
    dominance-signs function that the exp456 NAP=1 'main-effect' tables compute,
    but as ONE matmul (vs 6x tiny lookups) with DENSE gradients -> near-zero
    sparsity noise. The Linear is a normal 2D weight -> trained on AdamW (not the
    LUT/LION group), so it gets clean dense optimization. Deployable matmul-free
    via popcount on the +/-1 dominance bits."""
    def __init__(self, input_dim, n_heads, n_out, n_pairs, temperature, seed):
        super().__init__()
        self.v2d = VectorToDominance(
            d_head=input_dim, smooth_mode=False, temperature=temperature,
            n_pairs=n_pairs, random_seed=seed)
        P = input_dim * (input_dim - 1) // 2 if n_pairs is None else n_pairs
        self.linear = nn.Linear(P, n_heads * n_out)
        # Zero-init: combined module starts == base LUT (exp453); the dominance
        # 'main-effect' is learned purely as an additive correction that grows in
        # (cf. exp326 additive v-branch). The fixed +/-1 dominance inputs give
        # per-feature gradients on step 1, so zero weight still breaks symmetry.
        nn.init.zeros_(self.linear.weight)
        nn.init.zeros_(self.linear.bias)
        self.n_heads, self.n_out = n_heads, n_out

    def forward(self, x):                       # x: [B, input_dim]
        d = self.v2d(x)                         # [B, P]  in {-1,+1} (STE)
        return self.linear(d).view(-1, self.n_heads, self.n_out)


class ParallelLut(nn.Module):
    """base TinyMHLut + parallel DominanceLinearBranch, outputs summed."""
...
_DOM_BRANCH = cfg.get('dom_branch', True)
_DOM_TEMP   = cfg.get('dom_temperature', 0.5)
_DOM_NPAIRS = cfg.get('dom_n_pairs', None)   # None = full triu (all pairs)

def _wrap(base, input_dim, n_heads, n_out, seed):
    if not _DOM_BRANCH:
        return base
    branch = DominanceLinearBranch(input_dim, n_heads, n_out, _DOM_NPAIRS,
                                   _DOM_TEMP, seed)
    return ParallelLut(base, branch)
...
def _make_qkv_joint(layer_idx, seed_offset):
    ...
    return _wrap(base, E, H, 2 * d_qk + d_v, cfg['random_seed'] + seed_offset + 5000)
...
def _make_v(layer_idx, seed_offset):
    ...
    return _wrap(base, E, H, d_v, cfg['random_seed'] + seed_offset + 5000)
...
def _make_out(layer_idx, seed_offset):
    ...
    return _wrap(base, H * d_v, 1, E, cfg['random_seed'] + seed_offset + 5000)
...
def _make_residual_lut(layer_idx, seed_offset):
    ...
    return _wrap(base, E, 1, D, cfg['random_seed'] + seed_offset + 5000)
