"""nanochat_exps/exp239_soft_lut_fast — fork of exp238 (soft LUT, slow @ T_sel=1.0).

Three changes vs exp238:
  1) SoftMultiHeadLUT now uses bf16 autocast + torch.compile(forward, dynamic=True)
     for the soft pipeline. Microbench on V-LUT shape: 27.6 -> 4.6 ms/iter,
     8.45 -> 2.21 GB peak (~6x faster, ~4x less memory). Softmax stays fp32.
  2) T_sel 1.0 -> 0.1 (sharpens row softmax by 10x).
  3) (carried) v_input_nap=6 keeps activation tensors small.
T_soft stays at 0.01.

Launch:
    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python -u \\
        nanochat_exps/exp239_soft_lut_fast/train.py \\
        > nanochat_exps/exp239_soft_lut_fast/stdout.log 2>&1 &
"""
