"""nanochat_exps/exp243_soft_lut_tsoft05 — fork of exp242 with T_soft=0.5.

Single change: soft_score_temp 0.01 -> 0.5 in the rational soft sign
  p = rd / (T_soft + |rd|).
At T_soft=0.01 the gradient dp/drd = T/(T+|rd|)^2 ≈ 0.01 at |rd|=1 — basically
zero — so soft backward never reaches x. T_soft=0.5 lifts that to ~0.22 at
|rd|=1, a 22x increase in input-side gradient.

Forward is unchanged because hard=True picks argmax(ts) which depends only
on sign(p)=sign(rd), invariant to T_soft. Only backward changes.

Other settings carried from exp242: hard=True, T_sel=0.5, gumbel=False,
nap=8 V LUT, bf16+compile.

Launch:
    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python -u \\
        nanochat_exps/exp243_soft_lut_tsoft05/train.py \\
        > nanochat_exps/exp243_soft_lut_tsoft05/stdout.log 2>&1 &
"""
