...
_OUT_TPH_SCHEDULE = cfg.get('out_tph_schedule', None)
def _make_out(layer_idx, seed_offset):
    if _OUT_TPH_SCHEDULE is not None:
        out_tph = _OUT_TPH_SCHEDULE[layer_idx]
    else:
        out_tph = cfg['out_tph']
    return TinyMultiHeadLut(
        input_dim=H * d_v,
...
print(f'out_proj     TinyMHLut(soft): in_nap={cfg["out_input_nap"]} tph_schedule={_OUT_TPH_SCHEDULE} n_out=E={E}')
