...
def _make_residual_lut(layer_idx, seed_offset):
    ...
    If cfg['residual_sparse_n_outputs'] is set, the per-table output dim is that
    smaller value and TinyMHLut's sparse_scatter_n_outputs=D fans tables out
    over the wider D-dim residual stream. Each output slot of D is the sum of
    ~tph * sparse_n / D tables (balanced coverage default).
    """
    sparse_n = cfg.get('residual_sparse_n_outputs', None)
    extra = {}
    if sparse_n is not None:
        n_outputs = int(sparse_n)
        extra = dict(
            sparse_scatter_n_outputs=D,
            sparse_scatter_seed=cfg['random_seed'] + seed_offset + 7919,
        )
    else:
        n_outputs = D
    return TinyMultiHeadLut(
...
# right before SDPA. d_qk=64 is the head dimension. v is NOT rotated. Drop-in
# adaptation of exp319's RoPE on MinimalGPT — same recipe.
...
class LUTBlock(nn.Module):
    """exp316 block with RoPE on q,k (no additive learned pos_emb)."""
    ...
    def forward(self, x, cos, sin):
        ...
        # qk_joint LUT now sees x directly (no additive pos_emb). Position
        # information enters via RoPE on (q, k) right before SDPA.
...
_RESID_SPARSE_N = cfg.get('residual_sparse_n_outputs', None)
_RESID_NOUT_STR = (
    f'sparse n_out={_RESID_SPARSE_N} -> D={D}'
    if _RESID_SPARSE_N is not None else f'n_out=D={D}'
)
print(f'residual_lut TinyMHLut(soft, noise_eps={_NOISE_EPS}): in_nap={cfg.get("residual_input_nap", cfg["out_input_nap"])} tph={cfg.get("residual_tph", cfg["out_tph"])} {_RESID_NOUT_STR}  [E -> D]')
