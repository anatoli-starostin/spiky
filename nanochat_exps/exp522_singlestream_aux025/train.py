...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        super().__init__()
        # exp522: single E-stream, NO residual_lut / dual stream.
    ...
    def forward(self, x, cos, sin):
        ...
        x_lut_next_flat = x_flat + out_e                # standard E-stream skip
...
class Model(nn.Module):
    ...
    def predict_embedding(self, tokens):
        # exp522: single E-stream; final x_lut IS the predicted embedding.
...
def collect_temperature_specs(model):
    ...
    for li, blk in enumerate(model.layers):
        for slut_name in ('qkv_lut', 'v_lut', 'out_proj'):
...
for step in range(1, N_STEPS + 1):
    ...
    for _ in range(grad_accum):
        ...
        if AUX_CE_W > 0:
            ...
            loss = main_ce                      # exp521: no tied/identity loss at all
            last = dict(aux_ce=0.0, aux_top1=0.0)
    ...
    if step % EVAL_EVERY == 0 or step == N_STEPS:
        ...
        if bpb < best_bpb:
            best_bpb = bpb
        # Monitor whether the embedder<->unembedder tie self-emerges WITHOUT the reg.
        with torch.no_grad():
            _, eval_aux_top1 = model.consistency_loss(n_sample=AUX_BATCH)
        last['aux_top1'] = float(eval_aux_top1)
        print(f'[VAL] step {step}: bpb={bpb:.4f} | aux_top1(self-emerged)={last["aux_top1"]*100:.1f}%')
