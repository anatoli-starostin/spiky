...
class MinimalAttention(nn.Module):
    def __init__(self, n_embd, n_head, layer_idx):
        super().__init__()
        self.n_head = n_head
        self.qk_proj = nn.Linear(n_embd, 2 * n_embd, bias=False)
        ...
        self.out_lut = TinyMultiHeadLut(
            input_dim=n_embd,
            n_heads=1,
            n_outputs=n_embd,
            n_anchor_pairs=cfg['lut_input_nap'],
            tables_per_head=cfg['lut_tph'],
            random_seed=cfg['random_seed'] + 900 + layer_idx,
            device=DEVICE,
            weight_dtype=torch.float32,
            anchor_sampling_policy=AnchorSamplingPolicy.CANONICAL_FULL_COVERAGE,
            initial_weights_noise=cfg.get('mhlut_init_std', 0.001),
    ...
    def forward(self, x, cos, sin):
        B, T, C = x.size()
        q, k = self.qk_proj(x).split(C, dim=2)
        v = self.v_lut(x.reshape(B * T, C)).squeeze(1).reshape(B, T, C)
        ...
        return self.out_lut(y.reshape(B * T, C)).squeeze(1).reshape(B, T, C)
...
class MinimalGPT(nn.Module):
    def __init__(self, vocab_size, n_embd, n_head, n_layer, seq_len):
        ...
        # Attention output projection is a LUT (out_lut) with its own small-init via
        # mhlut_init_std; no zero-init needed.
