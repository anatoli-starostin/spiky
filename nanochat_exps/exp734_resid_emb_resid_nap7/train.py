"""exp734 — residual_lut + emb_resid_lut NAP=7 fork of exp733 (4K).

Two changes vs exp733: residual_input_nap 6 -> 7 AND emb_resid_input_nap 6 -> 7.
Per-layer residual K doubles (+37.7M), emb_resid K doubles (+6.3M),
totalling +44.0M params -> 358.5M.

References:
  exp732 (baseline, v=resid=emb_resid NAP=6): 1.3912 @ 4000, 1.225 h
  exp733 (v NAP=7, resid/emb NAP=6):           1.3857 @ 4000, 1.374 h  (-5.5 mb)

Hypothesis: do residual stream LUTs (n_out=384) benefit from wider K like
v_lut did (-5.5 mb at +37.7M params)?

n_outputs=384 keeps these on the bmm-wgrad path. K-wide chain (D/E/F)
roughly doubles in v + residual + emb_resid simultaneously.
