"""exp690 — anneal T_sel on exp684 checkpoint for 1000 steps at 10% of peak LR.

Third in the LR sweep (exp688=0.1%, exp689=1%, exp690=10%). 10% is the cosine
floor that exp684 itself ended training at, so this is "continue training at
final-cosine LR for 1K more steps while collapsing T_sel".

  - lr_scale = 0.1 (10% of peak, = cosine floor of exp684's schedule)
  ...
  - log_soft_score_temp stays learnable (AdamW updates it at 10% peak LR)
  - Forward = hybrid_smooth (2-row blend); blend -> single-row as T_sel -> 0.

End evals (same as exp688/689):
...
ANNEAL_STEPS = 1000
ANNEAL_LR_SCALE = 0.1       # 10% of peak (= cosine floor of exp684)
...
summary = dict(
    exp_name='exp690_anneal_tsel_lr10',
