...
class Model(nn.Module):
    ...
    def forward(self, tokens, targets=None, loss_reduction='mean'):
        ...
        x_lut = self.ln_final(x_lut)
        # exp571: tied dot head — logits = ln(x_lut) @ tok_emb_E.T.
        logits = x_lut @ self.tok_emb_E.weight.t()
...
print(f'TIED dot head: logits = ln_final(x_lut) @ tok_emb_E.T; tok_emb_E at E={E}; ln_final on E (not D); RoPE base={_ROPE_BASE}')
