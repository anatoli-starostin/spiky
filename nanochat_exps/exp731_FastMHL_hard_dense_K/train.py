"""exp731 — fork of exp729: hard fwd + full K-row dense backward.

Every LUT module uses FastMultiHeadLUT with:
  forward_mode  = 'hard'          (1-row argmax via F.embedding_bag('sum'))
  backward_mode = 'dense_K'       (full K=2^NAP-row softmax surrogate for
                                   input + temperature grads; chosen-row
                                   weight scatter fused inside the same body)

Bench reference (out_proj, NAP=7, K=128, tph=512, B=12288):
  FastMHLut hard / dense_K  = 22.9 ms  (this run's recipe)
  FastMHLut hard / ball     = 21.2 ms  (exp729 SOTA)
  exp724 Tiny K-row dense   = 34.5 ms

Quality references @ 16k bs=24 ga=2:
  exp729 (hard + NAP+1 ball)        hard val = 1.2360 (current SOTA)
  exp730 (hybrid_smooth + ball)      hard val ≈ 1.27-1.29

Hypothesis: at native-hard training (no soft->hard distribution shift),
does the wider K-row gradient surface train the LUT better than ball's
NAP+1 rows? exp724 used K-row dense at soft fwd; exp729 used NAP+1 ball
at hard fwd. This run tests K-row dense at HARD fwd.
"""
