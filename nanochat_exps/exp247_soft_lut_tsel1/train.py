"""nanochat_exps/exp247_soft_lut_tsel1 — fork of exp243 with T_sel=1.0.

Single change: select_temp 0.5 -> 1.0. Forward unchanged (hard=True).
Backward sel_soft = softmax(ts / 1.0) is broader than at T_sel=0.5,
spreading d_sel/d_ts more uniformly across rows. Tests the other side of
the T_sel sweep — exp246 showed T_sel=0.1 hurt by killing sel-side
gradient; this checks if more sel spread helps further.

T_sel sweep so far (with T_soft=0.5, hard=True):
  T_sel=0.1  exp246  trailing badly (sel grad ~ zero)
  T_sel=0.5  exp243  best so far (1.6089)
  T_sel=1.0  exp247  this run
...
Launch:
    PYTHONPATH=/home/starost/nanochat \\
        ...
        nanochat_exps/exp247_soft_lut_tsel1/train.py \\
        > nanochat_exps/exp247_soft_lut_tsel1/stdout.log 2>&1 &
"""
