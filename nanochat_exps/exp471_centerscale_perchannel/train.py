...
class CenterScale(nn.Module):
    """exp471: pre-LUT norm = per-token mean-subtract + PER-CHANNEL learnable
    scale gamma (vector, init 1.0). No std-division, no beta. Isolates the
    per-channel scale from exp470's scalar version (which lost ~+0.05): the
    per-channel gamma is the only part that changes LUT selection (gamma_a*x_a vs
    gamma_b*x_b can flip). If exp471 recovers exp453, per-channel scale is THE
    load-bearing piece and we get a minimal sufficient norm (center + per-chan
    scale, no beta/std)."""
