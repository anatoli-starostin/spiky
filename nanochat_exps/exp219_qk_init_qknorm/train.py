"""nanochat_exps/exp219_qk_init_qknorm — fork of exp218.

exp218 (no V2D, raw Q/K -> SDPA) was ~+0.16 bpb behind exp183 throughout training,
diagnosed as Q/K magnitude too small at init: TinyMultiHeadLut output std with
mhlut_init_std=0.001 and tph=256 is ~0.01, vs ~0.2 for a vanilla q/k Linear
projection on LN'd input.

Two changes vs exp218:
  1) qk_lut_init_std=0.02 (only for QK LUT; V/out_proj stay at 0.001) —
     matches exp001-style q/k weight init.
  2) Add q_norm / k_norm (LayerNorm on d_qk dim) before SDPA — modern QK-norm
     trick (Qwen / Gemma) so the 1/sqrt(d_qk) softmax scale is principled.

attn_scale init stays at 0.4.

Launch:
    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python -u \\
        nanochat_exps/exp219_qk_init_qknorm/train.py \\
        > nanochat_exps/exp219_qk_init_qknorm/stdout.log 2>&1 &
"""
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        self.attn_scale = nn.Parameter(torch.tensor(
            float(cfg.get('learnable_attn_scale_init', 0.4))
        ))
        # Plain SDPA on QK-normed Q/K. Effective scale = attn_scale / sqrt(d_qk):
        # pre-multiply Q by attn_scale (learnable) and let SDPA apply its default 1/sqrt(d_qk).
...
print(f'Attention:       SDPA (causal); Q pre-scaled by learnable attn_scale, SDPA default 1/sqrt(d_qk={d_qk})={1.0/math.sqrt(d_qk):.4f} applied')
