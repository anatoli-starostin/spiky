...
_LUT_BETA1 = cfg.get('lut_beta1', 0.9)
_LUT_BETA2 = cfg.get('lut_beta2', 0.95)

# --- SparseRowAdamW on LUT params; regular AdamW on everything else ----------
# LUT weights have per-(table, row) sparse update structure: a row is only
# updated by the tokens whose argmax picked it. Vanilla Adam advances m_t/v_t
# on every step regardless, which (a) decays state on no-op steps and (b)
# misuses bias correction. SparseRowAdamW keeps per-row visit counters and
# per-row state, advancing only on touched rows with β^{c_r} per-touch
# weighting where c_r = within-batch touch count.
sys.path.insert(0, EXP_DIR)
from visit_tracker import install_visit_trackers
from load_balance import install_load_balance_hooks, collect_load_balance_loss
_LB_LAMBDA = float(cfg.get('load_balance_lambda', 0.01))

# Vanilla AdamW everywhere, single optimizer. We install the visit-tracker only
# for STATS COLLECTION — `_visit_counts` is attached but no optimizer reads it.
adam_groups = [
    dict(params=lut_params,     lr=_LUT_LR,        betas=(_LUT_BETA1, _LUT_BETA2), eps=1e-8,
         weight_decay=0.0),
...
for g in optimizer.param_groups:
    g['initial_lr'] = g['lr']
n_hooked = install_visit_trackers(model)
n_lb_hooked = install_load_balance_hooks(model)
print(
    f'optimizer groups: '
...
print(f'Visit-tracker installed on {n_hooked} TinyMHLut modules (stats only — no optimizer reads c_r)')
print(f'Load-balance hooks installed on {n_lb_hooked} modules, lambda={_LB_LAMBDA}')

# --- c_r distribution logging ------------------------------------------------
# At each eval step, dump per-module c_r statistics computed from the latest
# train-batch forward into c_r_stats.csv. Columns:
#   step, module, touch_frac, c_r_mean, c_r_median, c_r_max,
#   frac_eq1, frac_ge8, frac_ge32, frac_ge64
_cr_csv_path = os.path.join(EXP_DIR, 'c_r_stats.csv')
_cr_csv_f = open(_cr_csv_path, 'w', newline='')
_cr_csv_w = csv.writer(_cr_csv_f)
_cr_csv_w.writerow(['step', 'module', 'touch_frac', 'c_r_mean', 'c_r_median',
                    'c_r_max', 'frac_eq1', 'frac_ge8', 'frac_ge32', 'frac_ge64'])
_cr_modules = []
for li, blk in enumerate(model.layers):
    for nm in ('qkv_lut', 'v_lut', 'out_proj', 'residual_lut'):
        _cr_modules.append((f'L{li}.{nm}', getattr(blk, nm)))

def _log_cr_stats(step_):
    with torch.no_grad():
        for name, mod in _cr_modules:
            c = mod.weights._visit_counts
            flat = c.flatten().to(torch.float32)
            n = flat.numel()
            touched = flat > 0
            touch_frac = touched.float().mean().item()
            if touched.any():
                pos = flat[touched]
                c_mean = pos.mean().item()
                c_med  = pos.median().item()
                c_max  = pos.max().item()
                frac_eq1  = (flat == 1).float().mean().item()
                frac_ge8  = (flat >= 8).float().mean().item()
                frac_ge32 = (flat >= 32).float().mean().item()
                frac_ge64 = (flat >= 64).float().mean().item()
            else:
                c_mean = c_med = c_max = frac_eq1 = frac_ge8 = frac_ge32 = frac_ge64 = 0.0
            _cr_csv_w.writerow([step_, name,
                                f'{touch_frac:.4f}', f'{c_mean:.2f}',
                                f'{c_med:.0f}', f'{c_max:.0f}',
                                f'{frac_eq1:.4f}', f'{frac_ge8:.4f}',
                                f'{frac_ge32:.4f}', f'{frac_ge64:.4f}'])
        _cr_csv_f.flush()
...
for step in range(1, N_STEPS + 1):
    ...
    for _ in range(grad_accum):
        ...
        main_loss = model(x, targets=y)
        lb_loss = collect_load_balance_loss(model)
        total_loss = main_loss + _LB_LAMBDA * lb_loss
        (total_loss / grad_accum).backward()
        accum_loss += main_loss.item() / grad_accum
        accum_lb   += float(lb_loss) / grad_accum
    ...
    if step % 100 == 0 or step == 1:
        print(f'step {step:6d} | loss={ema:.4f} | lb={accum_lb:.4f} | lr={lr_scale * cfg["adam_lr"]:.2e}')
    ...
    if step % EVAL_EVERY == 0 or step == N_STEPS:
        ...
        temp_f.flush()
        # Log c_r distribution stats from the latest train-batch forward.
        _log_cr_stats(step)
        model.train()
