...
def _make_read_out(seed_offset):
    return TinyMultiHeadLut(
        input_dim=E,
        n_heads=cfg['readout_n_heads'],
        n_outputs=cfg['readout_head_dim'],
...
class Model(nn.Module):
    ...
    def forward(self, tokens, targets=None, loss_reduction='mean'):
        ...
        # Multi-head readout: [B*T, H_ro, head_dim] -> concat heads -> [B, T, D]
        x_resid = self.read_out_lut(r_in).reshape(B, T, D)
...
assert cfg['readout_n_heads'] * cfg['readout_head_dim'] == D, \
    f"readout_n_heads*readout_head_dim ({cfg['readout_n_heads']}*{cfg['readout_head_dim']}) must equal D ({D})"
print(f'read_out_lut TinyMHLut(soft) [MULTI-HEAD @ end]: in_nap={cfg["readout_input_nap"]} tph={cfg["readout_tph"]} H_ro={cfg["readout_n_heads"]} head_dim={cfg["readout_head_dim"]} -> concat = D={D}')
