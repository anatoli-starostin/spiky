...
# exp509: qkv is NOT a LUT. Replace it with soft all-pairs dominance -> Linear.
# x:[B*T,E] -> VectorToDominance(soft, P=E*(E-1)/2 features in (-1,1)) -> Linear ->
# [B*T, H, 2*d_qk+d_v]. Produces q,k AND v (v_lut removed; v = the d_v slice). This
# swaps the table lookup for a dense linear readout on the SAME soft-sign dominance
# features the LUT uses as its index — tests whether the lookup table is needed at all.
class QKVDominanceLinear(nn.Module):
    def __init__(self, n_out_per_head):
        super().__init__()
        self.v2d = VectorToDominance(
            E, smooth_mode=True,
            temperature=cfg.get('soft_score_temp', 0.5),
            learnable_temperature=cfg.get('soft_learnable_temps', True),
        )
        self.P = E * (E - 1) // 2
        self.n_out_per_head = n_out_per_head
        self.ln = nn.LayerNorm(self.P)                 # exp510: normalize the 2016 dominance feats
        self.linear = nn.Linear(self.P, H * n_out_per_head)
        # exp510: small-std init so q,k,v start near zero (gentle start, like the LUT's 0.001).
        nn.init.normal_(self.linear.weight, std=cfg.get('qkv_linear_init_std', 0.005))
        nn.init.zeros_(self.linear.bias)

    def forward(self, x):                              # x: [B*T, E]
        dom = self.v2d(x)                              # [B*T, P] soft dominance in (-1,1)
        dom = self.ln(dom)                             # exp510: LayerNorm before Linear
        out = self.linear(dom)                         # [B*T, H*n_out_per_head]
        return out.view(-1, H, self.n_out_per_head)    # [B*T, H, 2*d_qk+d_v]

def _make_qkv_joint(layer_idx, seed_offset):
    return QKVDominanceLinear(2 * d_qk + d_v)
...
class RMSNorm(nn.Module):
    """exp475: cheapest magnitude norm = x / (mean(|x|) + eps). L1, NO centering,
    NO affine, NO square/sqrt. Completes the 2x2 (L1/L2 x center/no-center):
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        super().__init__()
        self.qkv_lut      = _make_qkv_joint(layer_idx, layer_idx)   # exp509: V2D->Linear, emits q,k,v
        ...
        # Pre/post-norm: plain RMSNorm (no centering, no affine).
    ...
    def forward(self, x, cos, sin):
        ...
        qkv_out = self.qkv_lut(x_pre)                   # [B*T, H, 2*d_qk + d_v]  (V2D->Linear)
        ...
        v_vec = qkv_out[..., 2 * d_qk:]                 # [B*T, H, d_v] -- v from V2D->Linear; v_lut removed
...
for name, p in model.named_parameters():
    if not p.requires_grad:
        continue
    if p.ndim >= 3:
        # exp507 hybrid: qkv_lut is the SOFT MatmulMHLut (DENSE gradients) -> AdamW
        # (wd=0.1), exactly like exp493. All other ndim>=3 weights are TinyMHLut
        # (SPARSE argmax gradients) -> LION, like exp475. (LION's edge is sparse-
        # gradient-specific; useless for the dense soft qkv.)
        if 'qkv_lut' in name:
...
print(f'qkv (V2D->LN->Linear) [exp510, AdamW wd=0.1]: VectorToDominance(E={E}, soft) P={E*(E-1)//2} -> LayerNorm -> Linear(init_std={cfg.get("qkv_linear_init_std",0.005)}) -> H*({2*d_qk}+{d_v})  [emits q,k,v; v_lut REMOVED]')
...
def collect_temperature_specs(model):
    ...
    for li, blk in enumerate(model.layers):
        for slut_name in ('out_proj', 'residual_lut'):  # exp509: qkv is V2D->Linear (no learnable_temps); v_lut removed
