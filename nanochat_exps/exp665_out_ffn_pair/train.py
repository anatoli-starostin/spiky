...
def _make_ffn(layer_idx, seed_offset):
    """ffn LUT: same characteristics as out_proj but input_dim=E (consumes
    out_e + x_flat in E-space). Tables-per-head selectable via cfg['ffn_tph']."""
    tph = int(cfg['ffn_tph'])
    mode = cfg.get('ffn_backward_mode', cfg.get('out_backward_mode', 'soft'))
    kwargs = dict(_kwargs_for_mode(mode))
    ffn_nh = int(cfg.get('ffn_n_heads', cfg.get('out_n_heads', 1)))
    assert E % ffn_nh == 0, f"E={E} must be divisible by ffn_n_heads={ffn_nh}"
...
class LUTBlock(nn.Module):
    ...
    def __init__(self, layer_idx):
        ...
        self.ffn          = _make_ffn(layer_idx, 600 + layer_idx)
    ...
    def forward(self, x, cos, sin):
        ...
        # Inner residual around out_proj, then ffn (same LUT shape as out_proj
        # but input_dim=E).  ffn returns [B*T, ffn_n_heads, E//ffn_n_heads].
        ffn_in = out_e + x_flat                                       # [B*T, E]
        ffn_out = self.ffn(ffn_in).reshape(B * T, E)                  # [B*T, E]

        # Outer E-residual skip unchanged — final block output = x + ffn_out.
        return (x_flat + ffn_out).reshape(B, T, E)
...
_ffn_nh = int(cfg.get('ffn_n_heads', cfg.get('out_n_heads', 1)))
_ffn_per_head = E // _ffn_nh
print(f'ffn      TinyMHLut({cfg.get("ffn_backward_mode", cfg.get("out_backward_mode", "soft"))}, noise_eps={_NOISE_EPS}): in_nap={cfg.get("ffn_input_nap", cfg["out_input_nap"])} tph={cfg["ffn_tph"]} n_heads={_ffn_nh} n_out_per_head={_ffn_per_head} (concat -> E={E}, input=out_e+x_flat)')
...
def collect_temperature_specs(model):
    ...
    for li, blk in enumerate(model.layers):
        for slut_name in ('qk_lut', 'v_lut', 'out_proj', 'ffn'):
