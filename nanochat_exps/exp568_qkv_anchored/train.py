...
def _make_qk(layer_idx, seed_offset):
    # exp568: SoftAnchorPairMHLUT with directed-gradient soft pairs + scheduled
    # anchor_tau annealing (handled by `update_anchor_tau` in the training loop)
    # then snap to hard via `.hard = True` at step ANCHOR_ANNEAL_STEPS.
    return SoftAnchorPairMHLUT(
        ...
        anchor_tau_init=cfg.get('anchor_tau_init', 1.0),
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        # Pre/post-norm: L1 mean-abs norm (no centering, no affine).
...
for name, p in model.named_parameters():
    ...
    if name.endswith('.anchor_logits'):
        # exp568: SoftAnchorPair anchor selection logits get continuous softmax
        # gradient -> AdamW (Lion sign-momentum would be wrong on softmax logits).
        anchor_params.append(p)
    elif p.ndim >= 3:
        # exp511: LUT table weights are SPARSE-gradient -> LION (unchanged).
        lut_params.append(p)
...
# exp568: anchor_logits get their own AdamW group at anchor_lr (no weight decay).
adam_groups = [
    ...
    dict(params=anchor_params,  lr=cfg.get('anchor_lr', cfg['adam_lr']),
...
print(
    ...
    f'anchor_logits={sum(p.numel() for p in anchor_params):,} (AdamW lr={cfg.get("anchor_lr", cfg["adam_lr"])}) | '
...
print(f'qk_lut       SoftAnchorPairMHLUT: in_nap={cfg["qkv_input_nap"]} tph={cfg["qkv_tph"]} n_out=2*d_qk={2*d_qk} | anchor_tau {cfg.get("anchor_tau_init", 1.0):.3f}->{cfg.get("anchor_tau_final", 0.001):.3f} cosine over {cfg.get("anchor_anneal_steps", 3000)} steps, then hard=True')
...
def collect_temperature_specs(model):
    ...
    for li, blk in enumerate(model.layers):
        # TinyMHLut modules have log_soft_score_temp and log_select_temp attrs.
        for slut_name in ('v_lut', 'out_proj', 'residual_lut'):
        ...
        # exp568: qk_lut is SoftAnchorPairMHLUT -> log_temps=[sign, sel], plus anchor_tau buffer.
        mod = blk.qk_lut
        if hasattr(mod, 'log_temps') and isinstance(mod.log_temps, nn.Parameter):
            specs.append((f'L{li}.qk_lut.T_sign',
                          (lambda m=mod: float(m.log_temps[0].detach().exp()))))
            specs.append((f'L{li}.qk_lut.T_sel',
                          (lambda m=mod: float(m.log_temps[1].detach().exp()))))
        specs.append((f'L{li}.qk_lut.anchor_tau',
                      (lambda m=mod: float(m.anchor_tau.detach()))))
    return specs
...
# --- exp568: anchor_tau cosine schedule + hard-snap at ANCHOR_ANNEAL_STEPS ----
ANCHOR_ANNEAL_STEPS = cfg.get('anchor_anneal_steps', 3000)
ANCHOR_TAU_INIT     = cfg.get('anchor_tau_init',     1.0)
ANCHOR_TAU_FINAL    = cfg.get('anchor_tau_final',    0.001)

def get_anchor_tau(step):
    if step >= ANCHOR_ANNEAL_STEPS:
        return ANCHOR_TAU_FINAL
    progress = step / ANCHOR_ANNEAL_STEPS
    return ANCHOR_TAU_FINAL + (ANCHOR_TAU_INIT - ANCHOR_TAU_FINAL) * 0.5 * (1 + math.cos(math.pi * progress))

def update_anchor_tau(step):
    if step < ANCHOR_ANNEAL_STEPS:
        tau_val = get_anchor_tau(step)
        with torch.no_grad():
            for blk in model.layers:
                blk.qk_lut.anchor_tau.fill_(tau_val)
    elif step == ANCHOR_ANNEAL_STEPS:
        for blk in model.layers:
            blk.qk_lut.hard = True
        print(f'[ANCHOR] step {step}: snapped qk_lut to hard=True (anchor_tau_final={ANCHOR_TAU_FINAL})')
...
for step in range(1, N_STEPS + 1):
    ...
    # exp568: scheduled anchor_tau anneal + hard-snap at boundary.
    update_anchor_tau(step)
