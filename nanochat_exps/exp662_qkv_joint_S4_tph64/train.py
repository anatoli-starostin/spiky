...
def _make_qkv(layer_idx, seed_offset):
    """exp662: joint qkv_lut producing q, k, v in one LUT.

    LUT shape: n_heads = H * S, n_outputs = (2*d_qk + d_v) // S. The S LUT-heads
    that share a real attention head h are stacked contiguously, so a simple
    .reshape(B*T, H, 2*d_qk + d_v) gives the per-real-head qkv tensor.
    """
    S = int(cfg['qkv_sparsify_s'])
    qkv_dim = 2 * d_qk + d_v
    assert qkv_dim % S == 0, f"2*d_qk + d_v = {qkv_dim} must be divisible by S = {S}"
...
class LUTBlock(nn.Module):
    ...
    def __init__(self, layer_idx):
        super().__init__()
        self.qkv_lut      = _make_qkv(layer_idx, layer_idx)
    ...
    def forward(self, x, cos, sin):
        ...
        # qkv_lut returns [B*T, H*S, (2*d_qk+d_v)//S]; contiguous reshape collapses
        # the S sub-heads of each real head into one (2*d_qk + d_v) vector.
        qkv_out = self.qkv_lut(x_pre)                                 # [B*T, H*S, qkv_per_lut_head]
        qkv_per_head = qkv_out.reshape(B * T, H, 2 * d_qk + d_v)      # [B*T, H, 2*d_qk + d_v]
        q_vec = self.q_norm(qkv_per_head[..., :d_qk])
        k_vec = self.k_norm(qkv_per_head[..., d_qk:2 * d_qk])
        v_vec = qkv_per_head[..., 2 * d_qk:]                          # [B*T, H, d_v]
...
_S = int(cfg['qkv_sparsify_s'])
_qkv_dim = 2 * d_qk + d_v
print(f'QKV joint TinyMHLut({cfg.get("qkv_backward_mode", "soft")}, noise_eps={_NOISE_EPS}): in_nap={cfg.get("qkv_input_nap", cfg["qk_input_nap"])} tph={cfg.get("qkv_tph", cfg["qk_tph"])} n_heads=H*S={H*_S} n_out_per_lut_head=(2*d_qk+d_v)/S={_qkv_dim//_S} (sparsify S={_S}, total qkv_dim={_qkv_dim})')
...
def collect_temperature_specs(model):
    ...
    for li, blk in enumerate(model.layers):
        for slut_name in ('qkv_lut', 'out_proj'):
