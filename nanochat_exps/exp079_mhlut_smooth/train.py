"""nanochat_exps/exp079_mhlut_smooth — fork of exp071 with smooth MultiHeadLut.

Same architecture and config as exp071, but every LUT module
(qk_joint, v_lut, out_proj) is built from MultiHeadLut(n_alternatives=1,
smooth_mode=True, recompute_in_backward=True) instead of TinyMultiHeadLut.
smooth_mode replaces the hard-sign anchor comparisons (with STE backward)
by soft-rational sigmoid scores during forward AND backward — fully
differentiable through the lookup, no STE.
