...
def _make_read_out(seed_offset):
    return TinyMultiHeadLut(
        input_dim=E,
        n_heads=1,
        n_outputs=cfg['readout_n_sparse'],
        sparse_scatter_n_outputs=D,
        sparse_scatter_seed=cfg['readout_sparse_seed'],
...
print(f'read_out_lut TinyMHLut(soft) [ONE @ end, SPARSE-SCATTER]: in_nap={cfg["readout_input_nap"]} tph={cfg["readout_tph"]} n_out(local)={cfg["readout_n_sparse"]} -> sparse_scatter_n_outputs=D={D}')
