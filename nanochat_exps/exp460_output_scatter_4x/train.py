...
_SCATTER_FACTOR = cfg.get('output_scatter_factor', 1)

def _scatter_kw(target, seed_offset):
    """(n_outputs_per_table, extra_kwargs) for output-scatter sparsity.
    factor=1 -> dense (each table writes all `target` outputs, every output dim
    sees all tph tables). factor=f -> each table writes target/f outputs
    scattered into the full target-wide output, so each output dim is assembled
    from ~tph/f tables. f x fewer LUT params and f x less per-token output-write
    bandwidth; output dim unchanged so the rest of the net is identical."""
    if _SCATTER_FACTOR <= 1:
        return target, {}
    assert target % _SCATTER_FACTOR == 0, f"{target} not divisible by {_SCATTER_FACTOR}"
    return target // _SCATTER_FACTOR, dict(
        sparse_scatter_n_outputs=target,
        sparse_scatter_seed=cfg['random_seed'] + seed_offset + 9000,
...
def _make_qkv_joint(layer_idx, seed_offset):
    ...
    n_out, skw = _scatter_kw(2 * d_qk + d_v, seed_offset)
...
def _make_v(layer_idx, seed_offset):
    n_out, skw = _scatter_kw(d_v, seed_offset)
    return TinyMultiHeadLut(
        input_dim=E,
        n_heads=H,
        n_outputs=n_out,
...
def _make_out(layer_idx, seed_offset):
    n_out, skw = _scatter_kw(E, seed_offset)
    return TinyMultiHeadLut(
        input_dim=H * d_v,
        n_heads=1,
        n_outputs=n_out,
...
def _make_residual_lut(layer_idx, seed_offset):
    n_out, skw = _scatter_kw(D, seed_offset)
    return TinyMultiHeadLut(
        input_dim=E,
        n_heads=1,
        n_outputs=n_out,
