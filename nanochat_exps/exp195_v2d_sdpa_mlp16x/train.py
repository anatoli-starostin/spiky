"""nanochat_exps/exp195_v2d_sdpa_mlp16x.

Fork of exp193 (val_bpb=2.0067 @ 8K) with the out_proj MLP widened from
4*E=1536 to 16*E=6144 hidden. The block has no separate FFN sub-layer, so
the out_proj is the only nonlinear path per block; widening it is the
natural lever to increase per-block capacity.

Architecture (per block, pre-norm):
  1. LN -> VectorToDominance per-head (n_pairs=D, n_heads=H), producing
     [B, T, H, D]. linear_mode=True (V2D = x_a - x_b).
  2. Per-head Q, K, V matrices (D -> E_h), no bias.
  3. F.scaled_dot_product_attention (causal).
  4. Concatenate heads -> [B, T, E].
  5. MLP out_proj: Linear(E, 16*E) -> GELU -> Linear(16*E, E), no bias.
     fc1 std=0.02; fc2 zero-init.
  6. Add residual.

Final: ln_f -> head (untied — head has its own [VOCAB, E] tensor).

Launch:
    PYTHONPATH=/home/starost/nanochat \\
      /home/starost/spiky/.venv/bin/python -u \\
      nanochat_exps/exp195_v2d_sdpa_mlp16x/train.py \\
      > nanochat_exps/exp195_v2d_sdpa_mlp16x/stdout.log 2>&1 &
"""
