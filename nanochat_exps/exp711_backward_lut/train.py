...
def _make_backward_lut(seed_offset):
    """Per-block backward_lut: E -> E.
    Predicts the BLOCK INPUT from the BLOCK OUTPUT; trained with detached MSE.
    """
    return TinyMultiHeadLut(
...
_AUX_BWD_WEIGHT = cfg.get('aux_backward_weight', 0.0)
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        self.backward_lut = _make_backward_lut(900 + layer_idx)
        ...
        self.ln_bwd  = MeanAbsNorm(E)    # pre-norm before backward_lut
    ...
    def forward(self, x, cos, sin):
        ...
        # Per-block backward_lut: predict block INPUT from block OUTPUT.
        bwd_in   = self.ln_bwd(x_lut_next_flat)
        bwd_pred = self.backward_lut(bwd_in).squeeze(1)
        # Target is the detached block input.
        bwd_mse  = F.mse_loss(bwd_pred, x_flat.detach())

        return x_lut_next_flat.reshape(B, T, E), r_out, bwd_mse
...
class Model(nn.Module):
    ...
    def forward(self, tokens, targets=None, loss_reduction='mean'):
        ...
        aux_bwd = x_lut.new_zeros(())
        for layer in self.layers:
            x_lut, r, bwd_mse = layer(x_lut, self.rope.cos, self.rope.sin)
            x_resid = x_resid + r
            aux_bwd = aux_bwd + bwd_mse
        ...
        if targets is not None:
            ...
            return ce + _AUX_BWD_WEIGHT * aux_bwd
        return logits
