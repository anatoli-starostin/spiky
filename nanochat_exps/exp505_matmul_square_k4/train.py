"""nanochat_exps/exp497_matmul_relunorm — normalized ReLU gate (sparsemax-style): ReLU(ts+b)/sum.
...
All LUTs use MatmulMultiHeadLut: dense rational-gate matmul, full autograd (no STE).
"""
...
_TEMP_PENALTY = cfg.get('temp_penalty', 0.0)   # lambda for lambda*sum(T_sel) loss
...
# --- LUT factories (all MatmulMultiHeadLut: dense rational-gate matmul, no STE) -
_TINY_SOFT_KWARGS = dict(
    ...
    gate_mode=cfg.get('gate_mode', 'unit'),
    use_bias=cfg.get('use_bias', False),
    hard_sign_ste=cfg.get('hard_sign_ste', False),
    gate_power=cfg.get('gate_power', 2.0),
)
...
for name, p in model.named_parameters():
    if not p.requires_grad:
        continue
    if name.endswith('gate_relu_bias'):
        nodecay_params.append(p)      # per-(table,row) ReLU threshold: a bias, NO weight decay
    elif p.ndim >= 3:
        decay_params.append(p)        # exp489: MatmulMHLut weight tables -> SAME AdamW group as the unembedder (lr=adam_lr, wd, betas all identical)
...
# exp489: MatmulMHLut weight tables share the unembedder's AdamW group (routed into
# decay_params above) — no separate LUT optimizer. lut_params is empty here.
if lut_params:
    if _LUT_OPT == 'lion':
...
print(f'qkv_lut      MatmulMHLut(relu_norm): in_nap={cfg["qkv_input_nap"]} tph={cfg["qkv_tph"]} n_out=2*d_qk+d_v={2*d_qk+d_v}  [q,k from this; last d_v added to v]')
print(f'v_lut        MatmulMHLut(relu_norm): in_nap={cfg["v_input_nap"]} tph={cfg["v_tph"]} d_v={d_v}')
print(f'out_proj     MatmulMHLut(relu_norm): in_nap={cfg["out_input_nap"]} tph={cfg["out_tph"]} n_out=E={E}')
print(f'residual_lut MatmulMHLut(relu_norm): in_nap={cfg["residual_input_nap"]} tph={cfg["residual_tph"]} n_out=D={D}  [E -> D]')
...
for step in range(1, N_STEPS + 1):
    ...
    for _ in range(grad_accum):
        ...
        if _TEMP_PENALTY > 0.0:
            # penalize high T_sel -> push softmax routing toward sharp/argmax
            # (faster convergence + HARDENABLE to single-lookup argmax inference).
            tsel_sum = sum(getattr(layer, n).log_select_temp.exp()
                           for layer in model.layers
                           for n in ('qkv_lut', 'v_lut', 'out_proj', 'residual_lut'))
            loss = loss + _TEMP_PENALTY * tsel_sum
...
torch.save({
    ...
    'lut_optimizer_state_dict': lut_optimizer.state_dict() if lut_optimizer is not None else None,
    'config': cfg,
    'step': N_STEPS,
    'best_val_bpb': best_bpb,
    'final_val_bpb': val_bpbs[-1] if val_bpbs else float('nan'),
