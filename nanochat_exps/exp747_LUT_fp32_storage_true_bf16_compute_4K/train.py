"""exp747 — exp732 (fp32 LUT storage) + TRUE bf16 LUT compute.

Fork of exp732 (val=1.3912 @ 276.8M). Identical config/train.py. The change
lives in src/spiky/lutorch/fast_multi_head_lut.py: when use_bf16=True and
weights are fp32 stored, the body now explicitly casts weights to bf16 before
the embedding_bag gather (which is not autocast-eligible). This ensures the
LUT body runs at bf16 throughout — gather + downstream — matching the bf16
compute path that bf16-storage runs (exp737 v2) had.

In the original exp732, only the downstream ops ran bf16 (via autocast); the
embedding_bag gather output was fp32 because embedding_bag isn't on PyTorch's
autocast list. exp747 fixes that. Storage stays fp32, grad accumulation stays
fp32, standard Lion. Pure compute-precision test.

Compare to:
  exp732 = 1.3912 (fp32 storage + PARTIAL bf16 compute, gather was fp32)
  exp737 v2 = 1.3872 (bf16 storage + full bf16 compute, +fp32 master Lion)

If exp747 ≈ 1.3872, the gap was the bf16 gather. If exp747 ≈ 1.3912, the gap
was from bf16-storage-specific noise (likely grad accumulation in bf16 buffer).
"""
