...
# exp261: swap TinyMultiHeadLut(backward_mode='soft', ...) for MultiHeadLut(
# smooth_mode=False, n_alternatives=3) with bernoulli low-confidence noise.
# Tests whether the noise-as-regularization recipe (exp257 ~1.6060) generalizes
# from soft-pipeline backward to the standard MultiHeadLut STE/WTA path.
# - smooth_mode=False  -> hard WTA forward + STE-style backward
# - n_alternatives=3   -> matches Phase-2 convention, gives the anchor backward
#                         3 candidates to push gradient through
# - argmax_noise_eps>0 -> bernoulli flip at |delta|<eps positions; saved
#                         flip-mask reused in backward for fwd/bwd consistency
# - recompute_in_backward forced True by MultiHeadLut when noise is on
_MHLUT_KWARGS = dict(
    anchor_sampling_policy=AnchorSamplingPolicy.CANONICAL_FULL_COVERAGE,
    initial_weights_noise=cfg.get('mhlut_init_std', 0.001),
    smooth_mode=False,
...
def _make_qk_joint(layer_idx, seed_offset):
    """Joint Q/K MultiHeadLut: input=E, n_heads=H, n_outputs=2*d_qk."""
    n_inputs = E + (_pos_emb_dim(layer_idx) if _POS_EMB_ACTIVE else 0)
    qk_kwargs = dict(_MHLUT_KWARGS)
...
print(f'Q/K Joint MultiHeadLut(smooth=False, n_alt=3, noise_eps={_NOISE_EPS}): in_nap={cfg["qk_input_nap"]} tph={cfg["qk_tph"]} d_qk={d_qk} (n_outputs=2*d_qk={2*d_qk}) init_std={cfg.get("qk_lut_init_std", cfg.get("mhlut_init_std", 0.001))} [no V2D, q_norm+k_norm -> SDPA]')
print(f'V MultiHeadLut(smooth=False, n_alt=3, noise_eps={_NOISE_EPS}):       in_nap={cfg["v_input_nap"]} tph={cfg["v_tph"]} d_v={d_v} (n_outputs=d_v)')
_tph_str = str(_OUT_TPH_PER_LAYER) if _OUT_TPH_PER_LAYER is not None else str(cfg['out_tph'])
print(f'out_proj MultiHeadLut(smooth=False, n_alt=3, noise_eps={_NOISE_EPS}): in_nap={cfg["out_input_nap"]} tph={_tph_str} (n_outputs=E={E}) + out_v2d/out_d2v')
