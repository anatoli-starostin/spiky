...
class LUTBlock(nn.Module):
    """exp643: qk_lut (n_out = 2*d_qk, no v_branch). v_lut alone provides v.
    Inherits exp639/exp642: MeanAbsNorm pre-norm at block start, E-residual
    stream, MeanAbsNorm post-norm before residual_lut.
    """
    ...
    def forward(self, x, cos, sin):
        ...
        # qk_lut produces only q, k (no v_branch).
        ...
        v_vec = v_lut_out                                             # v from v_lut only
        ...
        # E-residual stream: identity skip around attention/LUT block.
        x_lut_next_flat = x_flat + out_e                              # [B*T, E]
        x_lut_next = x_lut_next_flat.reshape(B, T, E)

        # Post-norm fed into residual_lut (MeanAbsNorm).
