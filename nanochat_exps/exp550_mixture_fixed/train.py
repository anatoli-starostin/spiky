...
class Model(nn.Module):
    def __init__(self):
        ...
        self.head = MixtureBitReadout(D, VOCAB_SIZE, MIX_T, bits_per_table=MIX_B,
                                      sign_temp=BIT_SIGN_TEMP,
                                      logit_scale_init=cfg.get('mix_scale_init', None),
...
_HEAD_LR = cfg.get('head_lr', cfg['adam_lr'])   # mixture head on its own (higher) LR to beat the cold-start
adam_groups = [
    ...
    dict(params=head_params, lr=_HEAD_LR, betas=(0.9, 0.95), eps=1e-8, weight_decay=0.0),
]
optimizer = torch.optim.AdamW(adam_groups)
print(f'head_lr={_HEAD_LR} (mixture cold-start fix)')
