...
class Model(nn.Module):
    def __init__(self):
        ...
        # exp571: NO separate unembedder; tied dot head via tok_emb_E.T below.
        self.rope = RotaryEmbedding(d_qk, max_seq_len=CONTEXT_SIZE, base=_ROPE_BASE)
        self.layers = nn.ModuleList([LUTBlock(i) for i in range(N_LAYERS)])
        # exp571: ln_final on E (not D), since head reads from E-stream now.
        self.ln_final = nn.LayerNorm(E)
        # exp573: SEPARATE LN for the tok_emb_E side of the tied dot.
        self.ln_emb = nn.LayerNorm(E)
        # exp581: gamma_emb = 1/sqrt(E) so logit scale at init = sqrt(E) * 1/sqrt(E)
        # = 1.0 regardless of E. Replaces the fixed 0.1 from exp574 with a principled
        # E-scaling rule, matching what worked across the no_resid + tied dot E sweep
        # (exp574 E=96 used 0.1 = ~1/sqrt(E); now we make it explicit). gamma_emb
        # remains LEARNABLE and free to grow during training.
        self.ln_emb.weight.data.fill_(1.0 / math.sqrt(E))
    ...
    def forward(self, tokens, targets=None, loss_reduction='mean'):
        ...
        # exp573: SEPARATE LNs on the two sides of the tied dot. ln_final on x_lut,
        # ln_emb on tok_emb_E.weight. Independent gamma/beta breaks the collapse
        # symmetry from exp572. +2E learnable params (negligible).
        x_normed   = self.ln_final(x_lut)
        emb_normed = self.ln_emb(self.tok_emb_E.weight)
        logits = x_normed @ emb_normed.t()
...
print('residual_lut REMOVED (exp571: no D-stream, no residual LUT)')
print(f'TIED dot head (exp581): logits = ln_final(x_lut) @ ln_emb(tok_emb_E).T; SEPARATE LNs, ln_emb.gamma INIT=1/sqrt(E)={1.0/math.sqrt(E):.4f} (principled E-scaling for logit scale ~1.0 at init); RoPE base={_ROPE_BASE}')
...
def collect_temperature_specs(model):
    ...
    for li, blk in enumerate(model.layers):
        # exp571: residual_lut removed; iterate over the 3 surviving LUT modules.
