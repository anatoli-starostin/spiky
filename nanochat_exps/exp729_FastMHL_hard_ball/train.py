"""exp729 — fork of exp727: swap TinyMultiHeadLut -> FastMultiHeadLUT(hard, ball).

Every LUT module uses the LUTGPT publish-branch FastMultiHeadLUT primitive with:
  ...
  backward_mode = 'ball'          ((NAP+1)-row Hamming-1 ball softmax surrogate
                                   ...
                                   still computed once but immediately gathered)

Bench reference (out_proj, NAP=7, K=128, tph=512, B=12288):
  FastMHLut hard / ball     = 21.2 ms  (this run's recipe)
  FastMHLut hard / dense_K  = 22.9 ms  (lutgpt-publish baseline)
  exp724 K-row dense bwd    = 34.5 ms  (Tiny path)
  exp726 NAP+1 autograd     = 41.8 ms

Quality reference @ 16k bs=24 ga=2:
  exp724 (hybrid_smooth + K-row dense) = 1.1936
  exp726 (hybrid_smooth + NAP+1 ball)  = 1.1997

Dropped from exp727: hybrid_smooth_autograd, hybrid_smooth_save_memory,
hybrid_smooth_n_alt, argmax_noise_eps. FastMultiHeadLUT does not expose these.
"""
