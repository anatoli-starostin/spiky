...
class SparseAmplifiedAdamW(torch.optim.Optimizer):
    def __init__(self, params, lr: float = 1e-3,
                 ...
                 c_threshold: float = 8.0):
        if mode not in ("step_sqrt_cr", "step_linear_cr", "ema_weight_cr",
                        "step_cold_damp_linear"):
            raise ValueError(f"Unknown mode: {mode}")
        defaults = dict(lr=lr, betas=betas, eps=eps, weight_decay=weight_decay,
                        mode=mode, kappa=kappa, c_threshold=c_threshold)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                elif mode == "step_cold_damp_linear":
                    # Hot rows like vanilla Adam (amp=1.0 when c_r >= c_threshold);
                    # cold rows dampened proportionally to c_r.
                    c_th = group['c_threshold']
                    amp = (c_r_f / c_th).clamp(max=1.0).unsqueeze(-1)
                else:
