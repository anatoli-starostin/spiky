...
from sparse_amplified_adamw import SparseAmplifiedAdamW
from visit_tracker import install_visit_trackers
_SPARSE_MODE = cfg.get('sparse_mode', 'step_sqrt_cr')
_SPARSE_KAPPA = float(cfg.get('sparse_kappa', 8.0))
_C_THRESHOLD = float(cfg.get('c_threshold', 8.0))
...
lut_group = [dict(params=lut_params, lr=_LUT_LR, betas=(_LUT_BETA1, _LUT_BETA2),
                  eps=1e-8, weight_decay=0.0)]

optimizer     = torch.optim.AdamW(adam_groups)
lut_optimizer = SparseAmplifiedAdamW(lut_group, lr=_LUT_LR, betas=(_LUT_BETA1, _LUT_BETA2),
                                     eps=1e-8, weight_decay=0.0,
                                     mode=_SPARSE_MODE, kappa=_SPARSE_KAPPA,
                                     c_threshold=_C_THRESHOLD)

# Initial LR snapshot for the cosine-with-warmup schedule. Both optimizers
# share the same scale factor each step.
for g in optimizer.param_groups:
    g['initial_lr'] = g['lr']
for g in lut_optimizer.param_groups:
    g['initial_lr'] = g['lr']

n_hooked = install_visit_trackers(model)
print(
    f'optimizer groups: '
    f'lut={sum(p.numel() for p in lut_params):,} -> SparseAmplifiedAdamW(lr={_LUT_LR}, betas=({_LUT_BETA1},{_LUT_BETA2}), mode={_SPARSE_MODE}, kappa={_SPARSE_KAPPA}, c_threshold={_C_THRESHOLD}) [{n_hooked} TinyMHLut modules hooked] | '
