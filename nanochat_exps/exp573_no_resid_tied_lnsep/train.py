...
class Model(nn.Module):
    def __init__(self):
        ...
        # exp573: SEPARATE LN for the tok_emb_E side of the tied dot. Decouples
        # gamma-collapse dynamics (which appeared to cause exp572's step-200 uniform
        # softmax at 4.19 bpb). gamma_x and gamma_emb can find independent temperatures.
...
print(f'TIED dot head (exp573): logits = ln_final(x_lut) @ ln_emb(tok_emb_E).T; SEPARATE LNs on each side; tok_emb_E at E={E}; independent gammas avoid the exp572 collapse symmetry; RoPE base={_ROPE_BASE}')
