...
class PerHeadMLP(nn.Module):
    ...
    Each head has its own (Linear → ReLU → Linear → LayerNorm) chain.
    Implemented via einsum on grouped weight tensors of shape [H, d_in, d_hidden]
    and [H, d_hidden, d_out].
    """
    def __init__(self, n_heads, d_in, d_hidden, d_out, seed=None):
        ...
        self.b1 = nn.Parameter(torch.zeros(n_heads, d_hidden))
        self.W2 = nn.Parameter(torch.empty(n_heads, d_hidden, d_out))
        self.b2 = nn.Parameter(torch.zeros(n_heads, d_out))
        with torch.no_grad():
    ...
    def forward(self, x):
        h = torch.einsum('...d,hde->...he', x, self.W1) + self.b1
        h = F.relu(h)
        h = torch.einsum('...he,hef->...hf', h, self.W2) + self.b2
