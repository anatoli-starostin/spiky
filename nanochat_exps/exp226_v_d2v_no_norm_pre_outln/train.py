"""nanochat_exps/exp226_v_d2v_no_norm_pre_outln — fork of exp225.

exp225 (V D2V normalise=False) trailed exp224 by ~0.013 — the per-head LN was
load-bearing. This run keeps v_d2v_normalise=False but moves the normalization
downstream: a single LayerNorm(H*d_v) is applied AFTER head concatenation and
BEFORE out_proj. Tests whether normalizing once across all heads jointly
(rather than per-head inside D2V) is enough — and possibly a cleaner place to
put it.

Launch:
    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python -u \\
        nanochat_exps/exp226_v_d2v_no_norm_pre_outln/train.py \\
        > nanochat_exps/exp226_v_d2v_no_norm_pre_outln/stdout.log 2>&1 &
"""
...
d_qk        = cfg['d_qk']
d_v         = cfg['d_v']
V_DOM_DIM   = d_v * (d_v - 1) // 2  # PermutationalLut n_outputs for V (dominance space)
...
def _make_v(layer_idx, seed_offset):
    """V PermutationalLut: x -> 276-dim dominance vector per head (P = d_v*(d_v-1)/2)."""
    return PermutationalLut(
        ...
        n_heads=H,
        tph=cfg['v_tph'],
        pair_mode=cfg.get('v_pair_mode', 'scrambled'),
        soft_mode=cfg.get('v_soft_mode', 'rational'),
        borda_scale_mode=cfg.get('v_borda_scale_mode', 'borda'),
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        # V LUT is now PermutationalLut producing 276-dim dominance vectors; SDPA mixes
        # them in dominance space, then v_d2v projects back to d_v=24 per head.
        self.v_d2v = DominanceToVector(d_v, normalise=cfg.get('v_d2v_normalise', True))
        # LayerNorm across concatenated heads, applied right before out_proj.
        self.pre_out_ln = nn.LayerNorm(H * d_v) if cfg.get('pre_out_ln', False) else None
    ...
    def forward(self, x, pos_emb):
        ...
        v_dom = self.v_lut(x_flat)                                     # [B*T, H, V_DOM_DIM=276]
        v = v_dom.reshape(B, T, H, V_DOM_DIM).permute(0, 2, 1, 3)       # [B, H, T, 276]

        attn = F.scaled_dot_product_attention(q, k, v, is_causal=True)
        # attn: [B, H, T, V_DOM_DIM=276] (dominance-space weighted average)

        # Project dominance vectors back to d_v via Borda + LayerNorm.
        attn_v = self.v_d2v(attn)                                       # [B, H, T, d_v]

        out_in = attn_v.permute(0, 2, 1, 3).reshape(B * T, H * d_v)
        if self.pre_out_ln is not None:
            out_in = self.pre_out_ln(out_in)
...
print(f'V PermLut:       in_nap={cfg["v_input_nap"]} out_nap={cfg["v_output_nap"]} tph={cfg["v_tph"]} d_v={d_v} (n_outputs=P={V_DOM_DIM}) pair_mode={cfg.get("v_pair_mode","scrambled")} soft_mode={cfg.get("v_soft_mode","rational")} borda={cfg.get("v_borda_scale_mode","borda")}')
