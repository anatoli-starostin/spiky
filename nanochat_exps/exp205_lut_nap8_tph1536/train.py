...
class V2DSDPABlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: [B, T, E]
        B, T, _ = x.shape
        # Q, K see x with positional info added; V sees x without positional
        # info, both passed through the same LN and the same V2D.
        x_pos = x + self.pos_emb[:T]                           # for Q, K
        x_norm_qk = self.ln_qk(x_pos)
        x_norm_v  = self.ln_v(x)
        x_h_qk = x_norm_qk.unsqueeze(-2).expand(B, T, H, E)    # [B, T, H, E]
        x_h_v  = x_norm_v.unsqueeze(-2).expand(B, T, H, E)
        dom_qk = self.v2d(x_h_qk)                              # [B, T, H, D]
        ...
        # x for the residual is the pos-included variant (so position info is
        # carried through the residual stream consistently).
        x = x_pos
        q = q.transpose(1, 2)
        k = k.transpose(1, 2)
        v = v.transpose(1, 2)
