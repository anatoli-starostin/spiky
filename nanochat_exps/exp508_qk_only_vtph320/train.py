...
def _make_qk(layer_idx, seed_offset):
    return MatmulMultiHeadLut(
        input_dim=E,
        n_heads=H,
        n_outputs=2 * d_qk,                    # exp508: q,k only -- v-branch removed
...
print(f'qk_lut       MatmulMHLut(SOFT gate={cfg.get("qkv_gate_mode","softmax")}) [exp508 q,k only, AdamW wd=0.1]: in_nap={cfg["qkv_input_nap"]} tph={cfg["qkv_tph"]} n_out=2*d_qk={2*d_qk}  [no v-branch]')
