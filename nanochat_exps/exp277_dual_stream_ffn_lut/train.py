...
class LUTBlock(nn.Module):
    """exp277 block: exp272-style dual-stream with an extra ffn_lut (E->E) inserted
    between out_proj and the post-norm. Single ln_block_end at end of block, after
    ffn_lut; feeds both the next-block x_lut and residual_lut."""
    ...
    def forward(self, x, pos_emb):
        ...
        # NEW: ffn_lut pointwise E->E
        ffn_out = self.ffn_lut(out_e).squeeze(1)                       # [B*T, E]

        # Post-norm at block end (after ffn_lut); feeds both next-block and residual_lut.
        ffn_out_norm = self.ln_block_end(ffn_out)                      # [B*T, E]
