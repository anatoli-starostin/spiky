...
def _make_qk(layer_idx, seed_offset):
    """exp663: split qk_lut with S-sparsify. n_heads=H*S, n_out=(2*d_qk)//S.
    Reshape [B*T, H*S, 2*d_qk//S] -> [B*T, H, 2*d_qk] (sub-heads of real head h
    are contiguous LUT heads h*S..h*S+S-1)."""
    S = int(cfg['qk_sparsify_s'])
    qk_dim = 2 * d_qk
    assert qk_dim % S == 0, f"2*d_qk = {qk_dim} must be divisible by S = {S}"
...
def _make_v(layer_idx, seed_offset):
    """exp663: split v_lut with S-sparsify. n_heads=H*S, n_out=d_v//S.
    Reshape [B*T, H*S, d_v//S] -> [B*T, H, d_v]."""
    S = int(cfg['v_sparsify_s'])
    assert d_v % S == 0, f"d_v = {d_v} must be divisible by S = {S}"
...
def _make_out(layer_idx, seed_offset):
    ...
    cfg['out_n_heads'] selects how many heads out_proj has (default 1). When >1,
    the LUT produces [B*T, n_heads, E//n_heads] and the caller concatenates to E.
    """
    tph = _OUT_TPH_PER_LAYER[layer_idx] if _OUT_TPH_PER_LAYER is not None else cfg['out_tph']
    mode = cfg.get('out_backward_mode', 'soft')
    kwargs = dict(_kwargs_for_mode(mode))
    out_nh = int(cfg.get('out_n_heads', 1))
    assert E % out_nh == 0, f"E={E} must be divisible by out_n_heads={out_nh}"
...
def _make_readout(seed_offset):
    """read_out LUT applied once after all blocks, before unembedder.
    Same structure as out_proj (NAP=6, n_heads=6, n_out_per_head=E/n_heads)
    but input_dim=E (consumes the residual stream directly)."""
    mode = cfg.get('readout_backward_mode', cfg.get('out_backward_mode', 'soft'))
    kwargs = dict(_kwargs_for_mode(mode))
    nh = int(cfg.get('readout_n_heads', cfg.get('out_n_heads', 1)))
    assert E % nh == 0, f"E={E} must be divisible by readout_n_heads={nh}"
    n_per_head = E // nh
    return TinyMultiHeadLut(
        input_dim=E,
        n_heads=nh,
        n_outputs=n_per_head,
        n_anchor_pairs=cfg.get('readout_input_nap', cfg['out_input_nap']),
        tables_per_head=int(cfg.get('readout_tph', cfg['out_tph'])),
        random_seed=cfg['random_seed'] + seed_offset,
        device=DEVICE,
        **kwargs,
    )
...
class LUTBlock(nn.Module):
    ...
    def forward(self, x, cos, sin):
        ...
        # qk_lut: split-S → reshape sub-heads contiguously into real heads.
        qk_out = self.qk_lut(x_pre)                                   # [B*T, H*S_qk, 2*d_qk // S_qk]
        qk_per_head = qk_out.reshape(B * T, H, 2 * d_qk)              # [B*T, H, 2*d_qk]
        q_vec = self.q_norm(qk_per_head[..., :d_qk])
        k_vec = self.k_norm(qk_per_head[..., d_qk:])
        q = q_vec.reshape(B, T, H, d_qk).permute(0, 2, 1, 3)
        k = k_vec.reshape(B, T, H, d_qk).permute(0, 2, 1, 3)
        q, k = apply_rope(q, k, cos[:T], sin[:T])

        # v_lut: split-S.
        v_out = self.v_lut(x_pre)                                     # [B*T, H*S_v, d_v // S_v]
        v_vec = v_out.reshape(B * T, H, d_v)                          # [B*T, H, d_v]
        v = v_vec.reshape(B, T, H, d_v).permute(0, 2, 1, 3)

        attn = F.scaled_dot_product_attention(q, k, v, is_causal=True)
        out_in = attn.permute(0, 2, 1, 3).reshape(B * T, H * d_v)
        # out_proj returns [B*T, out_n_heads, E//out_n_heads]; concat heads -> E.
        out_e = self.out_proj(out_in).reshape(B * T, E)               # [B*T, E]
...
class Model(nn.Module):
    def __init__(self):
        ...
        # read_out LUT applied once after all blocks, residual-wired.
        self.read_out = _make_readout(800)
    ...
    def forward(self, tokens, targets=None, loss_reduction='mean'):
        ...
        # exp666: one residual read_out LUT before ln_final + unembedder.
        ro = self.read_out(x_lut.reshape(B * T, E)).reshape(B, T, E)
        x_lut  = x_lut + ro
...
_Sqk = int(cfg['qk_sparsify_s'])
_Sv  = int(cfg['v_sparsify_s'])
print(f'qk_lut split-S TinyMHLut({cfg.get("qkv_backward_mode", "soft")}, noise_eps={_NOISE_EPS}): in_nap={cfg["qk_input_nap"]} tph={cfg["qk_tph"]} n_heads=H*S={H*_Sqk} n_out_per_lut_head=2*d_qk/S={2*d_qk//_Sqk} (sparsify S={_Sqk}, total qk_dim={2*d_qk})')
print(f'v_lut  split-S TinyMHLut({cfg.get("v_backward_mode", "ste")}, noise_eps={_NOISE_EPS}): in_nap={cfg["v_input_nap"]} tph={cfg["v_tph"]} n_heads=H*S={H*_Sv} n_out_per_lut_head=d_v/S={d_v//_Sv} (sparsify S={_Sv}, total d_v={d_v})')
_out_tph_str = str(_OUT_TPH_PER_LAYER) if _OUT_TPH_PER_LAYER is not None else str(cfg['out_tph'])
_out_nh = int(cfg.get('out_n_heads', 1))
_out_per_head = E // _out_nh
print(f'out_proj TinyMHLut({cfg.get("out_backward_mode", "soft")}, noise_eps={_NOISE_EPS}): in_nap={cfg["out_input_nap"]} tph={_out_tph_str} n_heads={_out_nh} n_out_per_head={_out_per_head} (concat -> E={E})')
_ro_nh = int(cfg.get('readout_n_heads', cfg.get('out_n_heads', 1)))
_ro_per_head = E // _ro_nh
print(f'read_out TinyMHLut({cfg.get("readout_backward_mode", cfg.get("out_backward_mode", "soft"))}, noise_eps={_NOISE_EPS}): in_nap={cfg.get("readout_input_nap", cfg["out_input_nap"])} tph={cfg.get("readout_tph", cfg["out_tph"])} n_heads={_ro_nh} n_out_per_head={_ro_per_head} (residual on E={E} before unembedder)')
...
def collect_temperature_specs(model):
    ...
    ro_mod = getattr(model, 'read_out', None)
    if ro_mod is not None and getattr(ro_mod, 'learnable_temps', False):
        specs.append(('read_out.T_soft',
                      (lambda m=ro_mod: float(m.log_soft_score_temp.detach().exp()))))
        specs.append(('read_out.T_sel',
                      (lambda m=ro_mod: float(m.log_select_temp.detach().exp()))))
    return specs
