#!/usr/bin/env python3
"""Smoke test: exp678 — exp670 (out_proj nh=4) + monolithic read_out (nh=1)."""
...
# read_out (monolithic nh=1)
ro_nh=int(cfg['readout_n_heads']); ro_tph=int(cfg['readout_tph']); ro_nap=int(cfg['readout_input_nap'])
assert ro_nh == 1, f"expected readout_n_heads=1, got {ro_nh}"
ro_per = E // ro_nh  # = E = 384
...
print(f'\nTotals expected: out_proj 6.29M × 6 layers = 37.75M; read_out one-time = 6.29M; sum 44.04M (= exp666 same)')
