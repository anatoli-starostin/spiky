"""exp692 — fork of exp684 (1.4262 @ 140M) with out_tph 1024 -> 512.

Halves out_proj's tables and bandwidth. -28.3M params -> ~112M total. Tests
out_proj sensitivity at fixed everything else.

Architecture (dual-stream per-layer residual_lut):
  - LUTBlock per layer:
      qk_lut       TinyMHLut(soft, NAP=4, tph=256) -> 2*d_qk
      q_norm, k_norm LayerNorm(d_qk); RoPE on (q,k); SDPA causal.
      v_lut        TinyMHLut(soft, NAP=6, tph=512) -> d_v
      out_proj     TinyMHLut(soft, NAP=6, tph=512) -> E   # <-- the change
      residual on E-stream: x_lut_next = x + out_e
      residual_lut TinyMHLut(soft, NAP=5, tph=256) -> D-stream contribution.
  - Model:
      tok_emb_E -> 6 LUTBlocks (E-stream) accumulating r_out into x_resid (D-stream).
