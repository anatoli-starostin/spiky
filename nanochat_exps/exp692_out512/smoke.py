#!/usr/bin/env python3
"""Smoke test: exp692 — exp684 with out_tph 1024 -> 512."""
...
assert cfg['out_tph'] == 512, f'exp692 requires out_tph=512, got {cfg["out_tph"]}'
...
assert vl.weights.numel() == 4_718_592, vl.weights.shape   # 6 * 512 * 64 * 24
...
assert op.weights.numel() == 4_718_592, op.weights.shape   # 1 * 512 * 64 * 144 (HALF of exp684's 9.44M)
...
assert rl.weights.numel() == 3_145_728, rl.weights.shape   # 1 * 256 * 32 * 384
...
print(f'  delta vs exp684 (op was 56.62M): -{(9_437_184 - op.weights.numel())*N_L/1e6:.2f}M')
