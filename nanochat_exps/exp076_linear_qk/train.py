"""nanochat_exps/exp076_linear_qk — fork of exp072 replacing LUT Q/K with plain Linears.

Replaces exp072's qk_joint TinyMultiHeadLut + qk_v2d ranking with two
standard nn.Linear(E=96, H*d_qk=384) projections (Q and K), feeding
directly into SDPA at dim d_qk=64 (vs exp072's 2016 ranking dominance dim).
V LUT and out_proj rank-canonicalize unchanged.

Param delta: -75M (167.05M -> ~92M). Compute: SDPA Q/K dim is now 64
instead of 2016 — much faster wall-clock. Tests whether the LUT Q/K +
V2D pathway pulls weight or whether standard linear attention works
just as well for the Q/K side.

How to launch:

    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python \\
        -u nanochat_exps/exp076_linear_qk/train.py \\
        > nanochat_exps/exp076_linear_qk/stdout.log 2>&1 &
"""
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        super().__init__()
        # Standard linear Q and K projections (replaces qk_joint LUT + qk_v2d).
        n_qk_in = E + (_pos_emb_dim(layer_idx) if _POS_EMB_ACTIVE else 0)
        self.q_proj = nn.Linear(n_qk_in, H * d_qk, bias=True)
        self.k_proj = nn.Linear(n_qk_in, H * d_qk, bias=True)
    ...
    def forward(self, x, pos_emb):
        ...
        k = self.k_proj(xp).reshape(B, T, H, d_qk).permute(0, 2, 1, 3)  # [B, H, T, d_qk]
