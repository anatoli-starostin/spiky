...
def _make_qk_joint(layer_idx, seed_offset):
    """Joint Q/K TinyMultiHeadLut with optional sparse_scatter."""
    n_inputs = E + (_pos_emb_dim(layer_idx) if _POS_EMB_ACTIVE else 0)
    n_sparse = cfg.get('qk_n_sparse_outputs', None)
    out_dim = 2 * d_qk
    return TinyMultiHeadLut(
        input_dim=n_inputs,
        n_heads=H,
        n_outputs=n_sparse if n_sparse is not None else out_dim,
        n_anchor_pairs=cfg['qk_input_nap'],
        tables_per_head=cfg['qk_tph'],
        random_seed=cfg['random_seed'] + seed_offset,
        device=DEVICE,
        sparse_scatter_n_outputs=out_dim if n_sparse is not None else None,
        sparse_scatter_seed=cfg['random_seed'] + seed_offset + 10000,
        **_TINY_KWARGS,
    )

def _make_v(layer_idx, seed_offset):
    """V TinyMultiHeadLut with optional sparse_scatter."""
    n_sparse = cfg.get('v_n_sparse_outputs', None)
    out_dim = d_v
    return TinyMultiHeadLut(
        input_dim=E,
        n_heads=H,
...
def _make_out(layer_idx, seed_offset):
    """out_proj TinyMultiHeadLut with optional sparse_scatter."""
    tph = _OUT_TPH_PER_LAYER[layer_idx] if _OUT_TPH_PER_LAYER is not None else cfg['out_tph']
    n_sparse = cfg.get('out_n_sparse_outputs', None)
    out_dim = E
    return TinyMultiHeadLut(
        input_dim=H * d_v,
        n_heads=1,
