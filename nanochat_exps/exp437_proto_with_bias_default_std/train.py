...
class Model(nn.Module):
    def __init__(self):
        ...
        self.unembedder = nn.Linear(D, VOCAB_SIZE, bias=cfg.get('unembedder_bias', False))
        if 'unembedder_init_std' in cfg:
            nn.init.normal_(self.unembedder.weight, std=cfg['unembedder_init_std'])
        if self.unembedder.bias is not None:
            nn.init.zeros_(self.unembedder.bias)
