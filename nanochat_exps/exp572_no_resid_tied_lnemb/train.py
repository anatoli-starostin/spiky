...
class Model(nn.Module):
    ...
    def forward(self, tokens, targets=None, loss_reduction='mean'):
        ...
        # exp572: BOTH sides of the dot product go through the same ln_final.
        # tok_emb_E stays small (preserves residual mixing because LUT input goes
        # through MeanAbsNorm anyway); LN renormalizes vocab rows to unit-variance,
        # giving logit scale ~sqrt(E) at init instead of ~sqrt(E)*emb_std (~10 vs ~0.6).
        # LN gamma acts as the effective softmax temperature; beta is a constant under
        # softmax. Zero extra parameters vs exp571.
        x_normed   = self.ln_final(x_lut)
        emb_normed = self.ln_final(self.tok_emb_E.weight)
...
print(f'TIED dot head (exp572): logits = ln_final(x_lut) @ ln_final(tok_emb_E).T; SAME ln_final on both sides; tok_emb_E at E={E}; LN gamma is effective temperature; RoPE base={_ROPE_BASE}')
