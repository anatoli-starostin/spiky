...
TAU_END     = cfg.get('anchor_tau_end', 0.02)
ANNEAL_FRAC = cfg.get('anchor_anneal_frac', 0.8)
...
def anchor_tau_at(step):
    """Log-linear anneal TAU_START -> TAU_END over the first ANNEAL_FRAC of
    training, held at TAU_END afterwards."""
    denom = max(1, int(ANNEAL_FRAC * N_STEPS))
    p = min(step / denom, 1.0)
    return math.exp((1.0 - p) * math.log(TAU_START) + p * math.log(TAU_END))
...
print(f'SoftAnchorPairMHLUT: sign(softmax(a/tau).x - softmax(b/tau).x); tau {TAU_START} -> {TAU_END} (anneal_frac={ANNEAL_FRAC})')
