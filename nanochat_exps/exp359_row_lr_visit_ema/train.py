...
# --- Per-row LR scaling via visit EMA (sparse-aware booster) ------------------
_USE_ROW_LR_SCALE  = bool(cfg.get('use_row_lr_scale', False))
_VISIT_EMA_BETA    = float(cfg.get('visit_ema_beta', 0.99))
_VISIT_POWER       = float(cfg.get('visit_scale_power', 0.5))
_VISIT_CLIP        = (float(cfg.get('visit_scale_min', 0.5)),
                      float(cfg.get('visit_scale_max', 2.0)))
_ROW_LR_MODULES = []   # list of TinyMHLut modules with .weights param
if _USE_ROW_LR_SCALE:
    for _blk in model.layers:
        for _nm in ('qkv_lut', 'v_lut', 'out_proj', 'residual_lut'):
            _mod = getattr(_blk, _nm, None)
            if _mod is None: continue
            # buffer shape matches first two dims of weights [n_tables, K]
            _ema = torch.full(_mod.weights.shape[:-1], 1e-3,
                              dtype=torch.float32, device=_mod.weights.device)
            _mod.register_buffer('_visit_ema', _ema, persistent=False)
            _ROW_LR_MODULES.append(_mod)
    print(f'Row-LR scaling enabled on {len(_ROW_LR_MODULES)} LUT modules: '
          f'β={_VISIT_EMA_BETA}, power={_VISIT_POWER}, clip={_VISIT_CLIP}')
...
for step in range(1, N_STEPS + 1):
    ...
    # Per-row LR scaling: cold rows (low visit EMA) get gradient boost.
    if _USE_ROW_LR_SCALE and _ROW_LR_MODULES:
        with torch.no_grad():
            for _mod in _ROW_LR_MODULES:
                _g = _mod.weights.grad
                if _g is None:
                    continue
                # Per-row activity this step (binary mask, shape [n_tables, K]).
                _active = (_g.abs().sum(dim=-1) > 0).to(torch.float32)
                _mod._visit_ema.mul_(_VISIT_EMA_BETA).add_(_active, alpha=1.0 - _VISIT_EMA_BETA)
                _mean = _mod._visit_ema.mean().clamp(min=1e-6)
                _ratio = _mean / _mod._visit_ema.clamp(min=1e-6)
                _scale = _ratio.pow(_VISIT_POWER).clamp(*_VISIT_CLIP)  # [n_tables, K]
                _g.mul_(_scale.unsqueeze(-1).to(_g.dtype))
