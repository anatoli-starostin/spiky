"""nanochat_exps/exp249_v2d_learnable_scale — fork of exp248.

Single change: out_v2d gains a learnable scalar scale Parameter, init at 0.5.
The V2D STE output (which forwards as ±1 in {-1, +1}) is now multiplied by
`scale` after the STE block, so initial dominance vectors are ±0.5 entering
D2V's Borda projection. Adam adjusts each layer's scale to whatever magnitude
the residual stream prefers.

Carried from exp248: SoftMultiHeadLUT with learnable temps (init 0.5/0.5),
hard STE, nap=8 V, bf16+compile.

Launch:
    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python -u \\
        nanochat_exps/exp249_v2d_learnable_scale/train.py \\
        > nanochat_exps/exp249_v2d_learnable_scale/stdout.log 2>&1 &
"""
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        self.out_v2d = VectorToDominance(
            E, smooth_mode=False, temperature=canon_t,
            learnable_scale_init=cfg.get('out_v2d_scale_init', None),
        )
