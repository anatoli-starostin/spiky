"""nanochat_exps/exp220_no_attn_scale — fork of exp219.

exp219 (qk_lut_init_std=0.02 + QK-norm, attn_scale=0.4) at step 1400 was 1.9694,
still ~+0.10 behind exp001 (1.9234) which uses no learnable scale at all and runs
at pre-softmax std ~0.15 vs exp219's 0.4.

Single change vs exp219: drop the learnable attn_scale entirely; use plain SDPA
with its default 1/sqrt(d_qk)=0.125 scale. With QK-norm, pre-softmax std is now
sqrt(d_qk)/sqrt(d_qk) = 1.0 (same as a vanilla GPT block on LN'd input would
have if it didn't dampen via small init — comparable in regime to exp001's 0.15).

Launch:
    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python -u \\
        nanochat_exps/exp220_no_attn_scale/train.py \\
        > nanochat_exps/exp220_no_attn_scale/stdout.log 2>&1 &
"""
