...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        # Block-output LayerNorm: residual stream stays real-valued.
        # V2D->D2V is moved to Model level, applied only on the unembedder path.
    ...
    def forward(self, x, pos_emb):
        ...
        out_normed = self.out_ln(out_real).reshape(B, T, E)            # [B, T, E] real, LN-only
        return out_normed
...
class Model(nn.Module):
    def __init__(self):
        ...
        # Shared V2D->D2V applied once per layer-output before concat
        # (only for the unembedder path; residual stream stays real-valued).
        # D2V uses normalise=False because the unembedder starts with LN(concat_dim).
        canon_t = cfg.get('canon_temperature', 0.1)
        self.unembed_v2d = VectorToDominance(E, smooth_mode=False, temperature=canon_t)
        self.unembed_d2v = DominanceToVector(E, normalise=False)
    ...
    def forward(self, tokens, targets=None, loss_reduction='mean'):
        ...
        for layer, pos_emb in zip(self.layers, self.pos_embs):
            x = layer(x, pos_emb)                             # [B, T, E] real, LN-only
            outs.append(x)
        # Batch-process V2D->D2V across all layer outputs in one call.
        stacked = torch.stack(outs, dim=2)                    # [B, T, N_LAYERS, E]
        B, T, L, _ = stacked.shape
        flat = stacked.reshape(B * T * L, E)
        dom = self.unembed_v2d(flat)                          # [B*T*L, P]
        canon = self.unembed_d2v(dom).reshape(B, T, L * E)    # [B, T, N_LAYERS*E]
        logits = self.unembedder(canon)
