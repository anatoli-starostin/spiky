...
_AUX_LOSS_WEIGHT = cfg.get('aux_loss_weight', 1.0)
# Vanilla teacher arch (matches exp416 by default; override per-cfg if needed).
_TEACHER_N_EMBD  = cfg.get('teacher_n_embd',  D)
_TEACHER_N_HEAD  = cfg.get('teacher_n_head',  6)
_TEACHER_N_LAYER = cfg.get('teacher_n_layer', N_LAYERS)
...
# --- Frozen vanilla teacher (MinimalGPT + RoPE, copy of exp416 arch) ---------
...
class _VanillaBlock(nn.Module):
    def __init__(self, n_embd, n_head):
        ...
        self.attn = _VanillaAttention(n_embd, n_head)
...
class VanillaTeacher(nn.Module):
    """MinimalGPT + RoPE matching exp416. Used frozen as a hidden-state oracle."""
    ...
    def __init__(self, vocab_size, n_embd, n_head, n_layer, seq_len):
        ...
        self.blocks = nn.ModuleList([_VanillaBlock(n_embd, n_head) for _ in range(n_layer)])
        self.ln_f = nn.LayerNorm(n_embd)
        self.head = nn.Linear(n_embd, vocab_size, bias=False)
        # Weight tying as in exp416.
        self.head.weight = self.tok_emb.weight

    def forward_with_states(self, idx):
        """Return list of L+1 hidden states: [h_0=tok_emb, h_1, ..., h_L].
        h_i is the residual stream AFTER block i (pre-ln_f). h_0 is the
        token-embedding output (no positional add, exp416 doesn't add one)."""
        x = self.tok_emb(idx)
        states = [x]
...
class Model(nn.Module):
    def __init__(self):
        ...
        # Frozen vanilla teacher for layer-alignment distillation.
        self.teacher = None  # filled in after construction (see below)

    def get_device(self):
        return self.tok_emb_E.weight.device

    def forward(self, tokens, targets=None, loss_reduction='mean'):
        # exp432 requires a teacher: x_resid is bootstrapped from vanilla.tok_emb
        # in both train and eval (the vanilla token table is effectively a frozen
        # part of the LUT model at inference too). Aux MSE is added only in train.
        assert self.teacher is not None, "exp432 requires a teacher; check config.teacher_checkpoint"
        with torch.no_grad():
            teacher_states = self.teacher.forward_with_states(tokens)
        x_resid = teacher_states[0].to(dtype=self.tok_emb_E.weight.dtype)
        x_lut   = self.tok_emb_E(tokens)
        accumulate_aux = self.training
        aux_mse_sum = x_resid.new_zeros(()) if accumulate_aux else None
        for i, layer in enumerate(self.layers):
            x_lut, r = layer(x_lut, self.rope.cos, self.rope.sin)
            x_resid = x_resid + r
            if accumulate_aux:
                aux_mse_sum = aux_mse_sum + F.mse_loss(
                    x_resid, teacher_states[i + 1].to(dtype=x_resid.dtype)
                )
        ...
        if targets is not None:
            ...
            ce_scalar = ce.detach().mean().item() if ce.numel() > 1 else ce.detach().item()
            if accumulate_aux:
                self._last_ce_item  = ce_scalar
                self._last_aux_item = aux_mse_sum.detach().item()
                return ce + _AUX_LOSS_WEIGHT * aux_mse_sum
            self._last_ce_item  = ce_scalar
            self._last_aux_item = 0.0
            return ce
        return logits
...
# Load + freeze full vanilla teacher (head, hidden states, the whole thing).
# Steals the tied head matrix for the student's unembedder, and runs the
# teacher in parallel each forward to supply per-layer hidden state targets.
_TEACHER_CKPT = cfg.get('teacher_checkpoint')
if _TEACHER_CKPT:
    teacher_state = torch.load(_TEACHER_CKPT, map_location=DEVICE, weights_only=False)
    if isinstance(teacher_state, dict) and 'head.weight' not in teacher_state:
        teacher_state = teacher_state.get('model_state_dict', teacher_state)
    # (a) transplant unembedder = vanilla.head.weight (tied with tok_emb in exp416)
    teacher_head_w = teacher_state['head.weight']
    assert teacher_head_w.shape == model.unembedder.weight.shape, \
        f"teacher head shape {tuple(teacher_head_w.shape)} != student unembedder {tuple(model.unembedder.weight.shape)}"
    model.unembedder.weight.data.copy_(teacher_head_w)
    model.unembedder.weight.requires_grad = False

    # (b) build + load + freeze a full VanillaTeacher for hidden-state distillation.
    teacher = VanillaTeacher(
        vocab_size=VOCAB_SIZE,
        n_embd=_TEACHER_N_EMBD,
        n_head=_TEACHER_N_HEAD,
        n_layer=_TEACHER_N_LAYER,
    ...
    model.teacher = teacher  # attached AFTER param iteration so iteration sees it but it's all frozen
    print(f'TEACHER: loaded full vanilla MinimalGPT from {_TEACHER_CKPT}; '
          f'n_embd={_TEACHER_N_EMBD}, n_head={_TEACHER_N_HEAD}, n_layer={_TEACHER_N_LAYER}; frozen')
    print(f'TEACHER: layer-alignment aux loss enabled (aux_loss_weight={_AUX_LOSS_WEIGHT}); '
          f'x_resid bootstrapped from teacher.tok_emb output (not zeros)')

n_params    = sum(p.numel() for p in model.parameters())
n_trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
print(f'Total params (all fp32): {n_params:,}; trainable: {n_trainable:,}')

_CONSTANT_LR = cfg.get('constant_lr', False)
...
for step in range(1, N_STEPS + 1):
    ...
    for _ in range(grad_accum):
        ...
        accum_ce   += getattr(model, '_last_ce_item',  0.0) / grad_accum
        accum_aux  += getattr(model, '_last_aux_item', 0.0) / grad_accum
    ...
    if step % 100 == 0 or step == 1:
        if accum_aux > 0:
            print(f'step {step:6d} | loss={ema:.4f} | ce={accum_ce:.4f} | aux_mse={accum_aux:.4f} | lr={lr_scale * cfg["adam_lr"]:.2e}')
        else:
