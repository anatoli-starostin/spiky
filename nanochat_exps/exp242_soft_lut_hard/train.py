"""nanochat_exps/exp242_soft_lut_hard — fork of exp240 with hard=True (STE).

Single change: hard=True for SoftMultiHeadLUT. Forward is exactly the
one-hot row gather at argmax(softmax(ts/T_sel)) — identical magnitude to
the discrete TinyMultiHeadLut. Backward uses the soft sel as gradient
surrogate (rational soft signs + temperature softmax provide smooth
gradient back to ts and weights).

This should match exp234's hard-LUT *forward* exactly while keeping the
soft path's smoother gradients. T_sel=0.5 (carried from exp240) only
controls backward smoothness now, since forward argmax is invariant to T.

Launch:
    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python -u \\
        nanochat_exps/exp242_soft_lut_hard/train.py \\
        > nanochat_exps/exp242_soft_lut_hard/stdout.log 2>&1 &
"""
