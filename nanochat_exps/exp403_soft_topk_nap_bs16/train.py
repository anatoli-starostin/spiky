...
_TINY_SOFT_TOPK_KWARGS = dict(
    weight_dtype=torch.float32,
    anchor_sampling_policy=AnchorSamplingPolicy.CANONICAL_FULL_COVERAGE,
    initial_weights_noise=cfg.get('mhlut_init_std', 0.001),
    backward_mode='soft_topk',
    n_alternatives=3,  # placeholder; overridden in _select_kwargs when cfg says 'nap'
    soft_score_temp=cfg.get('soft_score_temp', 0.5),
    select_temp=cfg.get('select_temp', 0.5),
    learnable_temps=cfg.get('soft_learnable_temps', True),
    use_bf16=cfg.get('soft_use_bf16', True),
...
def _select_kwargs(mode, nap=None):
    """Returns kwargs dict for TinyMultiHeadLut. When mode='soft_topk' and
    cfg['soft_topk_n_alt'] is the string 'nap', resolve n_alternatives to the
    module's own NAP (full 1-bit-flip Hamming ball around chosen)."""
    if mode == 'soft':
        return _TINY_SOFT_KWARGS
    if mode == 'soft_topk':
        kwargs = dict(_TINY_SOFT_TOPK_KWARGS)
        cfg_n_alt = cfg.get('soft_topk_n_alt', 3)
        if cfg_n_alt == 'nap':
            if nap is None:
                raise ValueError("soft_topk_n_alt='nap' requires a module NAP value")
            kwargs['n_alternatives'] = int(nap)
        else:
            kwargs['n_alternatives'] = int(cfg_n_alt)
        return kwargs
...
def _make_qkv_joint(layer_idx, seed_offset):
    ...
    qkv_nap = cfg.get('qkv_input_nap', cfg['qk_input_nap'])
    kwargs = _select_kwargs(mode, nap=qkv_nap)
    kwargs = dict(kwargs)
...
def _make_v(layer_idx, seed_offset):
    ...
    v_kwargs = _select_kwargs(v_mode, nap=cfg['v_input_nap'])
    return TinyMultiHeadLut(
        input_dim=E,
        n_heads=H,
        n_outputs=d_v,
...
def _make_out(layer_idx, seed_offset):
    ...
    kwargs = _select_kwargs(mode, nap=cfg['out_input_nap'])
    return TinyMultiHeadLut(
        input_dim=H * d_v,
        n_heads=1,
        n_outputs=E,
...
def _make_residual_lut(layer_idx, seed_offset):
    """E -> D residual projection. Backward mode selectable via cfg['residual_backward_mode'] (default 'soft')."""
    mode = cfg.get('residual_backward_mode', 'soft')
    res_nap = cfg.get('residual_input_nap', cfg['out_input_nap'])
    kwargs = _select_kwargs(mode, nap=res_nap)
    return TinyMultiHeadLut(
        input_dim=E,
        n_heads=1,
        n_outputs=D,
