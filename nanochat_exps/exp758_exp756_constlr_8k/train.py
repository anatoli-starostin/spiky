"""exp758 — resume exp756 + 8K more steps, half-peak cosine, no warmup.

Loads exp756's step-16000 checkpoint (E=192 hard, 1.2301) and continues
training for 8K more steps at eff bs=48. LR schedule:
  scale(step) = 0.1 + 0.4 * 0.5 * (1 + cos(pi * step / 8000))
i.e. start at 0.5 (half of exp756's peak) and cosine anneal to 0.1.
No warmup.

Tests whether the ~2h wallclock budget exp755 spent on hybrid_smooth
(5.42 h vs exp756's 3.40 h) is better invested in extending exp756's
hard-mode training instead. Expected wallclock: ~1.7 h (half of exp756's
3.4 h). Compare:
  exp756 (E=192 hard, 16K)             = 1.2301
  exp755 hard-eval (E=192 soft-train)  = 1.2778
  exp754 hard-eval (E=384 soft-train)  = 1.2498
  exp752 (E=384 hard, 16K)             = 1.2162

ORIGINAL exp756 docstring follows:

exp756 — exp755 + forward_mode hybrid_smooth -> hard.

Fork of exp755 (E=192, dv=32 + exp754 recipe). Single change:
forward_mode hybrid_smooth -> hard. Same as exp752 recipe at narrow
E=192/dv=32 (~176 M params). Deployment-quality counterpart to exp755.

ORIGINAL exp755 docstring follows:
...
def get_lr_scale(step):
    # exp758: start at 0.5 * peak, cosine anneal to 0.1 * peak over N_STEPS.
    # No warmup (resuming from a fully-trained checkpoint).
    progress = step / max(N_STEPS, 1)
    return 0.1 + 0.4 * 0.5 * (1 + math.cos(math.pi * progress))
...
# Load exp756 checkpoint (model, AdamW state, Lion master+momentum state).
_RESUME = "/home/starost/spiky/nanochat_exps/exp756_exp755_hard_16k/checkpoint.pt"
print(f'Loading checkpoint from {_RESUME}')
_ck = torch.load(_RESUME, weights_only=False, map_location=DEVICE)
model.load_state_dict(_ck['model_state_dict'])
optimizer.load_state_dict(_ck['optimizer_state_dict'])
lut_optimizer.load_state_dict(_ck['lut_optimizer_state_dict'])
print(f'Resumed from step {_ck["step"]}, best_val_bpb={_ck["best_val_bpb"]:.4f}')
# Restore initial_lr after load_state_dict (the load overwrites group dicts).
for o in all_optimizers:
    for g in o.param_groups:
        g['initial_lr'] = g['lr']
