"""nanochat_exps/exp492_matmul_hamming — learnable per-hamming-shell weight on soft scores.
...
print(f'qkv_lut      MatmulMHLut(hamming): in_nap={cfg["qkv_input_nap"]} tph={cfg["qkv_tph"]} n_out=2*d_qk+d_v={2*d_qk+d_v}  [q,k from this; last d_v added to v]')
print(f'v_lut        MatmulMHLut(hamming): in_nap={cfg["v_input_nap"]} tph={cfg["v_tph"]} d_v={d_v}')
print(f'out_proj     MatmulMHLut(hamming): in_nap={cfg["out_input_nap"]} tph={cfg["out_tph"]} n_out=E={E}')
print(f'residual_lut MatmulMHLut(hamming): in_nap={cfg["residual_input_nap"]} tph={cfg["residual_tph"]} n_out=D={D}  [E -> D]')
