#!/usr/bin/env python3
"""Smoke test: exp707 — exp696 with E=96, d_v=24 (out_proj asymmetric: in=144, out=96)."""
...
N_L = cfg['num_layers']
assert E == 96 and D == 384 and d_v == 24 and H * d_v == 144 and H * d_v != E
...
assert vl.weights.numel() == 2_359_296   # 6 * 256 * 64 * 24 (unchanged from exp696)
...
assert op.weights.numel() == 3_145_728   # 1 * 512 * 64 * 96 (out 144->96)
...
print(f'  delta vs exp696 LUT: -9.43M (out_proj -1.57M/layer)')
...
o = op(xa); r = rl(x)
assert o.shape == (B*T, 1, E), o.shape   # out_proj produces E=96
