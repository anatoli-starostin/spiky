...
# exp507 HYBRID: qkv_lut is the exp493 SOFT MatmulMultiHeadLut(gate_mode='softmax')
# (dense gated-matmul, full autograd). All other modules stay TinyMHLut(soft argmax).
...
def _make_qkv_joint(layer_idx, seed_offset):
    return MatmulMultiHeadLut(
        ...
        device=DEVICE,
        initial_weights_noise=cfg.get('qkv_lut_init_std', cfg.get('mhlut_init_std', 0.001)),
        **_MATMUL_SOFT_KWARGS,
    )
...
print(f'qkv_lut      MatmulMHLut(SOFT gate={cfg.get("qkv_gate_mode","softmax")}) [exp507 hybrid, AdamW wd=0.1]: in_nap={cfg["qkv_input_nap"]} tph={cfg["qkv_tph"]} n_out=2*d_qk+d_v={2*d_qk+d_v}  [q,k from this; last d_v added to v]')
