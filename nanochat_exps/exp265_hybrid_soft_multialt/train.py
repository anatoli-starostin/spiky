...
# exp265: hybrid soft / multi-alt config.
#   Small LUTs (NAP=6 — qk_joint, out_proj):  TinyMHL(soft) +noise.
#     Reason: at NAP=6, soft has ~0.009 bpb edge over multi-alt
#     (see exp257 1.6060 vs exp262 1.6148 at 8K).
#   Big LUT (NAP=8 — v_lut):                  TinyMHL(ste, n_alt=3) +noise.
#     Reason: at NAP=8, soft's [B, T, K=256] intermediate blows memory
#     to ~17 GB and slows to 22 ms; multi-alt is 1.9 GB / 12 ms.
# Same argmax_noise_eps regularisation applied in both modes.
...
def _make_qk_joint(layer_idx, seed_offset):
    """Q/K TinyMHLut(soft): NAP=6 ⇒ soft wins."""
...
def _make_v(layer_idx, seed_offset):
    """V TinyMHLut(ste, n_alt=3): NAP=8 ⇒ multi-alt wins (memory + speed)."""
    return TinyMultiHeadLut(
        input_dim=E,
        n_heads=H,
        n_outputs=d_v,
...
def _make_out(layer_idx, seed_offset):
    """out_proj TinyMHLut(soft): NAP=6 ⇒ soft wins."""
