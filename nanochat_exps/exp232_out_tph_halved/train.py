"""nanochat_exps/exp232_out_tph_halved — fork of exp223 (1.6249 @ 8K, 339M).

Single change: out_proj tph halved across the board (keeps the early/late asymmetry).
  - out_tph_per_layer: [2048, 2048, 1024, 1024, 1024, 1024]
                    -> [1024, 1024,  512,  512,  512,  512]
nap=6 unchanged. Halves the out_proj LUT params (~25M reduction). exp231 just
showed adding capacity to late layers hurts — this tests the other direction:
do we have unnecessary capacity that's only diluting gradient signal?

Launch:
    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python -u \\
        nanochat_exps/exp232_out_tph_halved/train.py \\
        > nanochat_exps/exp232_out_tph_halved/stdout.log 2>&1 &
"""
