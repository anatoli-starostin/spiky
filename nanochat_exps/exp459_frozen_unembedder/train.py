...
# ---- CHEAT: load a converged unembedder and FREEZE it -----------------------
# exp453 has no checkpoint; exp454 is the same recipe (beta2=0.93 vs 0.95,
# 1.4969 ~ 1.4967) -> its unembedder is an exp453-class converged decoder.
# Freezing it gives the LUT body a perfect, fixed output decoder for free, to
# see how fast the body alone converges. Frozen params are skipped by the param
# grouping below (requires_grad=False), so they leave the optimizer entirely.
_FROZEN_UNEMB = cfg.get('frozen_unembedder_ckpt', None)
if _FROZEN_UNEMB:
    _csd = torch.load(_FROZEN_UNEMB, map_location=DEVICE)['model_state_dict']
    with torch.no_grad():
        model.unembedder.weight.copy_(_csd['unembedder.weight'])
    model.unembedder.weight.requires_grad_(False)
    print(f'FROZEN unembedder.weight {tuple(model.unembedder.weight.shape)} '
          f'loaded from {_FROZEN_UNEMB} (ln_final stays trainable)')
...
# Drop empty groups (frozen unembedder empties decay_params).
