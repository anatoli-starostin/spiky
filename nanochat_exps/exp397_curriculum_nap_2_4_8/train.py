...
TARGET_NAP = cfg['target_nap']         # 8
TARGET_TPH = cfg['target_tph']         # {'qkv': 16, 'v_lut': 32, 'out_proj': 128, 'residual_lut': 64}
import math
...
class LUTBlock(nn.Module):
    """Same as exp365 LUTBlock, but LUT module shapes (NAP, tph) are
    determined by the current stage."""
    def __init__(self, layer_idx, nap, tph_for_stage):
        super().__init__()
        self.qkv_lut = _make_lut(
            input_dim=E, n_heads=H, n_outputs=2 * d_qk + d_v,
            n_anchor_pairs=nap, n_tables_total=H * tph_for_stage['qkv'],
            random_seed=cfg['random_seed'] + 100 * layer_idx + 1,
        )
        self.v_lut = _make_lut(
            input_dim=E, n_heads=H, n_outputs=d_v,
            n_anchor_pairs=nap, n_tables_total=H * tph_for_stage['v_lut'],
            random_seed=cfg['random_seed'] + 100 * layer_idx + 2,
        )
        self.out_proj = _make_lut(
            input_dim=H * d_v, n_heads=1, n_outputs=E,
            n_anchor_pairs=nap, n_tables_total=tph_for_stage['out_proj'],
            random_seed=cfg['random_seed'] + 100 * layer_idx + 3,
        )
        self.residual_lut = _make_lut(
            input_dim=E, n_heads=1, n_outputs=D,
            n_anchor_pairs=nap, n_tables_total=tph_for_stage['residual_lut'],
            random_seed=cfg['random_seed'] + 100 * layer_idx + 4,
        )
...
class Model(nn.Module):
    def __init__(self, nap, tph_for_stage):
        super().__init__()
        ...
        self.layers = nn.ModuleList([LUTBlock(i, nap, tph_for_stage) for i in range(N_LAYERS)])
...
def stage_tph(stage_idx):
    """tph per module at stage_idx. tph_target × 2^(N_STAGES - 1 - stage_idx)."""
    mult = 2 ** (N_STAGES - 1 - stage_idx)
    return {k: v * mult for k, v in TARGET_TPH.items()}


def apply_anchor_trees_to_model(model, trees, nap_for_this_stage):
    """Override anchor buffers in the model's LUT modules from the trees,
    using the tree level corresponding to this stage's NAP.
    tree_level = log2(TARGET_NAP / nap_for_this_stage).
    """
    tree_level = int(math.log2(TARGET_NAP // nap_for_this_stage))
    for layer in model.layers:
        for name in ('qkv_lut', 'v_lut', 'out_proj', 'residual_lut'):
...
def merge_weights_from_prev_stage(model, prev_state_dict, prev_nap):
    """For each LUT module's weights tensor, apply merge_weight_tensor on the
    prev-stage's weights to produce this stage's init.
    Non-LUT params (LN, embeddings, unembedder) are copied directly."""
    new_state = {}
    for name, p in model.named_parameters():
        if name not in prev_state_dict:
            # Some buffers may not be in prev (e.g. log_temps reinitialized)
            continue
        prev_p = prev_state_dict[name]
        if p.ndim == 3 and name.endswith('.weights'):
            # LUT weights: merge
    ...
    # Load with strict=False because keys might mismatch slightly
...
def get_global_lr_scale(global_step):
    """Continuous cosine schedule over ALL stages combined.
    Warmup at start (lr_warmup_fraction of TOTAL_N_STEPS), then cosine
    decay from peak to 0.1× peak over the rest.
    """
    n = TOTAL_N_STEPS
    w = int(WARMUP_FRAC * n)
    if global_step < w:
        return global_step / max(w, 1)
    progress = (global_step - w) / max(n - w, 1)
...
def train_stage(stage_idx, model, bs, n_steps, csv_writer, csv_file, val_csv_writer, val_csv_file, global_step_offset):
    ...
    for step in range(1, n_steps + 1):
        ...
        lr_scale = get_global_lr_scale(global_step)
        for g in optimizer.param_groups:
...
for i, st in enumerate(STAGES):
    print(f'  Stage {i}: NAP={st["nap"]}, bs={st["bs"]}, steps={st["n_steps"]}')
...
for stage_idx, st in enumerate(STAGES):
    ...
    bs = st['bs']
    n_steps = st['n_steps']
    tph = stage_tph(stage_idx)
    print(f'\n=== Stage {stage_idx}: NAP={nap}, bs={bs}, n_steps={n_steps}, tph={tph} ===')

    model = Model(nap=nap, tph_for_stage=tph).to(DEVICE)
    apply_anchor_trees_to_model(model, trees, nap_for_this_stage=nap)
    ...
    if prev_ckpt_path is not None:
        ...
        prev_state = torch.load(prev_ckpt_path, map_location=DEVICE, weights_only=True)
        missing, unexpected = merge_weights_from_prev_stage(model, prev_state, prev_nap)
        print(f'  merge_state load: missing={len(missing)}, unexpected={len(unexpected)}')

    train_stage(stage_idx, model, bs=bs, n_steps=n_steps,
    ...
    prev_ckpt_path = os.path.join(EXP_DIR, f'stage{stage_idx}.pt')
    torch.save(model.state_dict(), prev_ckpt_path)
    print(f'  Saved {prev_ckpt_path}')
    prev_nap = nap
