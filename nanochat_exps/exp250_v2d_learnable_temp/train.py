"""nanochat_exps/exp250_v2d_learnable_temp — fork of exp248 with learnable V2D temperature.

Single change vs exp248: out_v2d's `temperature` becomes a per-layer learnable
Parameter (log-parametrized as exp(log_T) so it stays positive). Initialized at
0.5 instead of the historical 0.1, since past sweeps suggest the canon_temperature
of 0.1 starves gradients in the rational soft sign d/(T+|d|) used as the STE
backward; 0.5 should give a stronger backward signal initially.

Also dumps a `temperatures.csv` with all learnable temperatures across the model
(SoftMultiHeadLUT T_soft/T_sel for qk_joint, v_lut, out_proj per layer + V2D T
per layer) at every eval step, for later dynamics analysis.

Baselines:
  exp243 (fixed T_soft=0.5, T_sel=0.5)                 -> 1.6089 @ 8K
  exp248 (learnable T_soft/T_sel, V2D T fixed @ 0.1)   -> 1.6071 @ 8K
  exp236 SOTA                                           -> 1.6042 @ 8K

Launch:
    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python -u \\
        nanochat_exps/exp250_v2d_learnable_temp/train.py \\
        > nanochat_exps/exp250_v2d_learnable_temp/stdout.log 2>&1 &
"""
...
_SOFT_KWARGS = dict(
    ...
    anchor_sampling_policy=AnchorSamplingPolicy.CANONICAL_FULL_COVERAGE,
    initial_weights_noise=cfg.get('mhlut_init_std', 0.001),
    soft_score_temp=cfg.get('soft_score_temp', 0.01),
    select_temp=cfg.get('select_temp', 1.0),
    gumbel=cfg.get('soft_gumbel', False),
    hard=cfg.get('soft_hard', False),
    learnable_temps=cfg.get('soft_learnable_temps', False),
    use_bf16=cfg.get('soft_use_bf16', False),
    compile_forward=cfg.get('soft_compile_forward', False),
)

def _make_qk_joint(layer_idx, seed_offset):
    """Joint Q/K SoftMultiHeadLUT: input=E, n_heads=H, n_outputs=2*d_qk."""
    n_inputs = E + (_pos_emb_dim(layer_idx) if _POS_EMB_ACTIVE else 0)
    qk_kwargs = dict(_SOFT_KWARGS)
...
print(f'Q/K Joint SoftLUT: in_nap={cfg["qk_input_nap"]} tph={cfg["qk_tph"]} d_qk={d_qk} (n_outputs=2*d_qk={2*d_qk}) init_std={cfg.get("qk_lut_init_std", cfg.get("mhlut_init_std", 0.001))} T_soft={cfg.get("soft_score_temp",0.01)} T_sel={cfg.get("select_temp",1.0)} [no V2D, q_norm+k_norm -> SDPA]')
print(f'V SoftLUT:       in_nap={cfg["v_input_nap"]} tph={cfg["v_tph"]} d_v={d_v} (n_outputs=d_v)')
_tph_str = str(_OUT_TPH_PER_LAYER) if _OUT_TPH_PER_LAYER is not None else str(cfg['out_tph'])
print(f'out_proj SoftLUT: in_nap={cfg["out_input_nap"]} tph={_tph_str} (n_outputs=E={E}) + out_v2d/out_d2v')
