...
from spiky.lutorch.multi_kendall_readout import MultiKendallReadout
...
MK_T = cfg.get('mk_n_tables', 8); MK_NAP = cfg.get('mk_nap', 48)
MK_HIDDEN = cfg.get('mk_mlp_hidden', 16)
...
class Model(nn.Module):
    def __init__(self):
        ...
        self.head = MultiKendallReadout(E, MK_T, MK_NAP, mlp_hidden=MK_HIDDEN,
                                        sign_temp=KEND_SIGN_TEMP, device=DEVICE, seed=cfg['random_seed'])
...
print(f'Total params: {n_params:,} (residual->E={E}, MultiKendall T={MK_T} x NAP={MK_NAP}, MLP h={MK_HIDDEN}) | {model.head}')
