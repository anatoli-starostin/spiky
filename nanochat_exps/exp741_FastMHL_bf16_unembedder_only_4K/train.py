"""exp741 — exp737 v2 + bf16 unembedder ONLY (no bf16 SDPA).

Fork of exp740 (LUT bs=48 4K bf16 storage + fp32 master Lion + force-bmm
+ bf16 SDPA + bf16 unembedder). exp740 drifted +10 mb behind exp737 v2 by
step 3200 — bf16 SDPA suspected as the culprit. exp741 removes the SDPA
autocast wrap; only self.unembedder(x_resid) is bf16. Isolates the cost
of the bf16 unembedder alone vs exp737 v2 = 1.3872.

If exp741 tracks exp737 v2 cleanly, bf16 SDPA was the source of drift;
if it also drifts, the bf16 unembedder is the culprit.
"""
