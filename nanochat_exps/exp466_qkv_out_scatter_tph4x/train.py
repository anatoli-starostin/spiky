...
def _make_qkv_joint(layer_idx, seed_offset):
    ...
    # 4x output-scatter sparsity on qkv_lut ONLY: each table writes target/sf
    # outputs scattered into the full target-wide output; all other LUTs dense.
    sf = cfg.get('qkv_scatter_factor', 4)
    target = 2 * d_qk + d_v
    n_out = target
    skw = {}
    if sf > 1:
        assert target % sf == 0
        n_out = target // sf
        skw = dict(sparse_scatter_n_outputs=target,
...
def _make_out(layer_idx, seed_offset):
    # 4x output-scatter sparsity on out_proj (combined with 4x tph via out_tph).
