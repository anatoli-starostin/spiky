...
class LUTBlock(nn.Module):
    """Reads the canonical-dominance residual stream, runs Q/K/V/SDPA/out_proj,
    and writes the canonicalised stream back. Block emits raw (non-canonical)
    dominance from out_proj; canonicalisation happens after the residual add.
    """
    def __init__(self, layer_idx):
        ...
        # Per-layer Borda projection: stream_dom (canonical, ±1) → Euclidean E.
        # normalise=True applies LayerNorm with elementwise_affine (separate per layer).
        self.stream_to_x = DominanceToVector(E, normalise=True)
        # Residual canonicalise: applied to (stream + block_out_dom) sum.
        self.residual_canon = DominanceCanonicalize(E, temperature=canon_t)
    ...
    def forward(self, stream_dom, pos_emb):
        # stream_dom: [B, T, P_STREAM]
        B, T, _ = stream_dom.shape
        x = self.stream_to_x(stream_dom)                                # [B, T, E]
        ...
        return self.residual_canon(stream_dom + block_out_dom)
...
class Model(nn.Module):
    def __init__(self):
        ...
        # Simple linear unembedder: P_STREAM → V (no LN, no concat).
        self.unembedder = nn.Linear(P_STREAM, VOCAB_SIZE)
    ...
    def forward(self, tokens, targets=None, loss_reduction='mean'):
        ...
        stream_dom = self.token_to_dom(x)                     # [B, T, P_STREAM]
        for layer, pos_emb in zip(self.layers, self.pos_embs):
            stream_dom = layer(stream_dom, pos_emb)           # canonical dominance after residual
        logits = self.unembedder(stream_dom)                  # [B, T, V]
