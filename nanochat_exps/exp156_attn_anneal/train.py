...
class LUTBlock(nn.Module):
    ...
    def forward(self, x, pos_emb):
        ...
        # Annealing ramp on effective attn scale: linear from 1.0x at step 0
        # to attn_scale_target_ramp at step n_steps. Read current_step from
        # the model attribute (set by the train loop each iter).
        ramp = 1.0
        if hasattr(self, '_current_step') and hasattr(self, '_total_steps'):
            progress = float(self._current_step) / max(1, self._total_steps)
            target = float(cfg.get('attn_scale_target_ramp', 1.0))
            ramp = 1.0 + progress * (target - 1.0)
        effective_scale = (self.attn_scale * ramp) * self._inv_sqrt_p
...
for step in range(1, N_STEPS + 1):
    ...
    # Inject current step into each LUTBlock so the attn-scale ramp factor
