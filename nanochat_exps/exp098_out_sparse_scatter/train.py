...
class SparseOutProj(nn.Module):
    """MultiHeadLut producing per-table outputs of dim n_sparse_outputs,
    followed by SparseScatter that scatters them into a dense [B, 1, E]
    via fixed random per-table index subsets."""
    def __init__(self, layer_idx, seed_offset):
        super().__init__()
        n_sparse = cfg['out_n_sparse_outputs']
        tph = cfg['out_tph']
        ...
        self.scatter = SparseScatter(
            ...
            n_outputs=E,
            seed=cfg['random_seed'] + seed_offset + 10000,
            device=torch.device(DEVICE),
        )

    def forward(self, x):
        per_table = self.lut(x)  # [B, 1, tph, n_sparse]
        return self.scatter(per_table)  # [B, 1, E]

def _make_out(layer_idx, seed_offset):
    return SparseOutProj(layer_idx, seed_offset)
...
print(f'out_proj MHLut + SparseScatter: in_nap={cfg["out_input_nap"]} tph={cfg["out_tph"]} n_sparse_outputs={cfg["out_n_sparse_outputs"]} -> E={E}')
