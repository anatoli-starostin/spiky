"""nanochat_exps/exp089_residual_per_chunk — fork of exp088 with strict per-chunk anchors.

Each layer i's residual_lut is built with partition_sets covering the
i+1 E-slot chunks of the inverse-concat input, and partition_pair_weights
that put all weight on chunk_i (last chunk = current x). At construction
every table samples anchor pairs strictly within chunk_i. Inheritance
then copies the first RES_TPH_F*i tables from layer (i-1), preserving
each previous layer's per-chunk anchors.

Final structure: rows [j*RES_TPH_F, (j+1)*RES_TPH_F) of layer i's
residual_lut have anchors strictly within chunk j. Each E-dim chunk
gets a dedicated 512-table sub-LUT learned at the layer where it was
first introduced.
...
How to launch:
    ...
    PYTHONPATH=/home/starost/nanochat \\
        ...
        -u nanochat_exps/exp089_residual_per_chunk/train.py \\
        > nanochat_exps/exp089_residual_per_chunk/stdout.log 2>&1 &
"""
...
def _make_residual(layer_idx, seed_offset):
    ...
    All tables are constrained at construction time to sample anchor pairs
    only within the LAST E-slot chunk (chunk i, slots [i*E, (i+1)*E)) via
    partition_sets + partition_pair_weights. Inheritance later overwrites
    the first RES_TPH_F * i rows with layer (i-1)'s pattern, yielding a
    strict per-chunk decomposition (rows [j*RES_TPH_F, (j+1)*RES_TPH_F) of
    layer i sample within chunk j).
    """
    in_dim = (layer_idx + 1) * E
    tph = RES_TPH_F * (layer_idx + 1)
    partition_sets = [set(range(j * E, (j + 1) * E)) for j in range(layer_idx + 1)]
    partition_pair_weights = [0.0] * layer_idx + [1.0]
    return TinyMultiHeadLut(
        input_dim=in_dim,
        n_heads=1,
        n_outputs=E,
...
class Model(nn.Module):
    ...
    def _inherit_residual_anchors(self):
        """Copy first RES_TPH_F*i anchor pairs of layer i's residual_lut.lookup from
        ...
        for i in range(1, N_LAYERS):
            ...
            n_inherit = prev_lk.anchor_pairs_a.shape[0]   # RES_TPH_F * i
