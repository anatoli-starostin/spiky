...
class Model(nn.Module):
    def __init__(self):
        ...
        # exp584: ADD parallel UNTIED Linear(E, V) head from the final E embedding,
        # with its own LayerNorm. Separate weights (not tied to tok_emb_E) so the
        # two heads can learn independent prediction directions — unlike exp582/583
        # which used a TIED dot on the E-side and didn't stack.
        self.ln_final_e = nn.LayerNorm(E)
        self.unembedder_e = nn.Linear(E, VOCAB_SIZE, bias=False)
    ...
    def forward(self, tokens, targets=None, loss_reduction='mean'):
        ...
        logits_d = self.unembedder(x_resid)
        # exp584: parallel UNTIED Linear from E-stream final state.
        x_lut_normed = self.ln_final_e(x_lut)
        logits_e = self.unembedder_e(x_lut_normed)
        logits = logits_d + logits_e
...
print(f'DUAL UNTIED head (exp584): logits = Linear(D={D}, V={VOCAB_SIZE})(ln_final_d(x_resid)) + Linear(E={E}, V={VOCAB_SIZE})(ln_final_e(x_lut)); tok_emb_E at E={E}; RoPE base={_ROPE_BASE}')
