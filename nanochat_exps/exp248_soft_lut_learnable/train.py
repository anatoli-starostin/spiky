"""nanochat_exps/exp248_soft_lut_learnable — fork of exp243 with learnable T_soft, T_sel.

Single change: learnable_temps=True. T_soft and T_sel are now per-LUT-instance
trainable Parameters (parametrized as exp(log_T) so they stay positive).
Initialized at 0.5 / 0.5 (the winning fixed config from exp243). The Adam
optimizer can adjust them per LUT — qk, v, and out_proj per layer can each
find their own preferred operating point.

Background sweep (fixed):
  T_soft=0.5, T_sel=0.5  -> 1.6089 (exp243, best)
  T_soft=0.1, T_sel=0.5  -> trailing (exp245)
  T_soft=1.0, T_sel=0.5  -> trailing (exp244)
  T_soft=0.5, T_sel=0.1  -> trailing (exp246)
  T_soft=0.5, T_sel=1.0  -> trailing (exp247)

All else: hard=True, nap=8 V, bf16+compile.

Launch:
    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python -u \\
        nanochat_exps/exp248_soft_lut_learnable/train.py \\
        > nanochat_exps/exp248_soft_lut_learnable/stdout.log 2>&1 &
"""
