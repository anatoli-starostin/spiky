"""nanochat_exps/exp151_qk_canon_d496 — fork of exp149.

Replace element-wise STESign with DominanceCanonicalize(d_head=32).

qk_lut emits 2*d_qk = 992 reals/head via sparse scatter, split into two
496-dim "raw dominance" vectors (Q-dom, K-dom). Each is canonicalized via
D2V(Borda+LN) -> V2D-STE, projecting onto the realizable-dominance
manifold and yielding 496-dim ±1 vectors. SDPA on 496-dim ±1 (matches
exp144's effective attention dim).

How to launch:

    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python \\
        -u nanochat_exps/exp151_qk_canon_d496/train.py \\
        > nanochat_exps/exp151_qk_canon_d496/stdout.log 2>&1 &
"""
...
from spiky.lutorch.ranking_tools import (
    VectorToDominance, DominanceToVector, DominanceCanonicalize,
)
...
def _make_qk_joint(layer_idx, seed_offset):
    """Joint Q/K TinyMultiHeadLut with sparse scatter.

    Per-table contributes `qk_n_sparse_outputs` values into a 2*d_qk-wide
    output via fixed random scatter. Final per-head output: 2*d_qk = 1984.
    """
    n_inputs = E + (_pos_emb_dim(layer_idx) if _POS_EMB_ACTIVE else 0)
    n_outputs = 2 * d_qk
    n_sparse  = cfg['qk_n_sparse_outputs']
    return TinyMultiHeadLut(
        ...
        sparse_scatter_seed=cfg['random_seed'] + seed_offset + 10000,
        **_TINY_KWARGS,
    )
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        # Canonicalize each half of qk_lut output (Q-dom, K-dom) by projecting
        # onto the realizable-dominance manifold via D2V(Borda+LN) -> V2D-STE.
        # Input: 496-dim raw dominance; Output: 496-dim ±1 on-manifold.
        self.qk_canon = DominanceCanonicalize(
            d_head=cfg['qk_canon_d_head'],
            smooth_mode=False,
            temperature=cfg.get('ste_temperature', cfg.get('canon_temperature', 0.1)),
        )
        ...
        self.attn_scale = nn.Parameter(torch.tensor(
            float(cfg.get('learnable_attn_scale_init', 0.0317))
        ))
    ...
    def forward(self, x, pos_emb):
        ...
        q_raw = qk_out[..., :d_qk]                                     # raw Q-dominance
        k_raw = qk_out[..., d_qk:]                                     # raw K-dominance
        q_dom = self.qk_canon(q_raw)                                   # 496-dim ±1 on-manifold
        k_dom = self.qk_canon(k_raw)
        q = q_dom.reshape(B, T, H, d_qk).permute(0, 2, 1, 3)
        k = k_dom.reshape(B, T, H, d_qk).permute(0, 2, 1, 3)
...
print(f'Q/K Joint MHLut (sparse): in_nap={cfg["qk_input_nap"]} tph={cfg["qk_tph"]} '
      f'd_qk={d_qk} n_sparse={cfg["qk_n_sparse_outputs"]} scatter_to={2*d_qk}')
print(f'V MHLut:         in_nap={cfg["v_input_nap"]} tph={cfg["v_tph"]} d_v={d_v}')
_tph_str = str(_OUT_TPH_PER_LAYER) if _OUT_TPH_PER_LAYER is not None else str(cfg['out_tph'])
print(f'out_proj MHLut:  in_nap={cfg["out_input_nap"]} tph={_tph_str} (n_outputs=E={E})')
print(f'qk DominanceCanonicalize: d_head={cfg["qk_canon_d_head"]} '
      f'(P=d_qk={d_qk}); ste_temperature={cfg.get("ste_temperature", cfg.get("canon_temperature", 0.1))}')
print(f'attn_scale_init: {cfg.get("learnable_attn_scale_init", 0.0317)} '
      f'(principled 1/sqrt(d_qk)={1.0/math.sqrt(d_qk):.4f})')
