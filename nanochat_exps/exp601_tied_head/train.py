...
class Model(nn.Module):
    def __init__(self):
        ...
        # Tied unembedder: ln_emb normalizes rows of tok_emb_E, then logits = x_resid @ emb_norm.T.
        # gamma_emb init small (~1/sqrt(D)) to keep init logit std ~1 — prevents random-peak
        # softmax failure (yesterday's exp572-573 lesson; exp574 used 0.1 at E=96).
        self.ln_emb = nn.LayerNorm(E)
        self.ln_emb.weight.data.fill_(cfg.get('ln_emb_gamma_init', 0.05))
    ...
    def forward(self, tokens, targets=None, loss_reduction='mean'):
        ...
        # Tied head: dot-product against ln_emb'd token embeddings.
        emb_norm = self.ln_emb(self.tok_emb_E.weight)              # [V, E=D]
        logits = x_resid @ emb_norm.t()                            # [B, T, V]
...
print(f'TIED unembedder: logits = x_resid @ ln_emb(tok_emb_E.weight).T (D={D}=E={E}); ln_emb gamma_init={cfg.get("ln_emb_gamma_init", 0.05)}; ln_final(D); RoPE base={_ROPE_BASE}')
