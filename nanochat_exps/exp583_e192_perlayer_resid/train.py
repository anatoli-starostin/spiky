...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        # exp583: per-layer residual_lut RESTORED (E -> D each layer, accumulated into x_resid).
    ...
    def forward(self, x, cos, sin):
        ...
        # Per-layer residual_lut (D-stream contribution).
...
class Model(nn.Module):
    def __init__(self):
        ...
        # exp583: D-stream restored to exp567-style — per-layer residual_lut accumulates
        # into x_resid, ln_final_d normalizes it, Linear(D, V) produces parallel logits.
        # Final logits = (tied dot on E-stream) + (Linear on accumulated D-stream).
    ...
    def forward(self, tokens, targets=None, loss_reduction='mean'):
        ...
        # exp583: parallel Linear head from accumulated D-stream after per-layer residual_lut.
        x_resid    = self.ln_final_d(x_resid)
        logits_lin = self.unembedder(x_resid)                              # [B, T, V]
...
print(f'residual_lut (exp583, PER-LAYER, x6): TinyMHLut(soft, in_nap={cfg["residual_input_nap"]} tph={cfg["residual_tph"]}, E={E}->D={D})')
print(f'DUAL head (exp583): logits = tied_dot(ln_final(x_lut), ln_emb(tok_emb_E)) + Linear(D={D}, V={VOCAB_SIZE})(ln_final_d(sum_per_layer(residual_lut))); ln_emb.gamma INIT=0.1; RoPE base={_ROPE_BASE}')
