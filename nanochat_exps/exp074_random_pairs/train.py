"""nanochat_exps/exp074_random_pairs — fork of exp072 with random-pair Q/K V2D.

Same architecture as exp072 (d_qk=64, rank-canonicalized output) except
qk_v2d uses a per-head random subset of `qk_random_pairs` pair indices
instead of the full C(64, 2) = 2016 triu pairs. Each of H=6 heads gets its
own independent random pair subset, fixed at init. SDPA Q/K dim becomes
qk_random_pairs (vs 2016). Param-neutral.

How to launch:

    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python \\
        -u nanochat_exps/exp074_random_pairs/train.py \\
        > nanochat_exps/exp074_random_pairs/stdout.log 2>&1 &
"""
...
# When qk_random_pairs is set, qk_v2d uses a per-head random subset of pairs;
# Q/K SDPA dim becomes qk_random_pairs instead of the full D_QK_P.
QK_RANDOM_PAIRS = cfg.get('qk_random_pairs', None)
QK_PAIRS = QK_RANDOM_PAIRS if QK_RANDOM_PAIRS is not None else D_QK_P
D_V_P       = d_v * (d_v - 1) // 2
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        if QK_RANDOM_PAIRS is not None:
            self.qk_v2d = VectorToDominance(
                d_qk, smooth_mode=False, temperature=canon_t,
                n_pairs=QK_RANDOM_PAIRS, n_heads=H,
                random_seed=cfg['random_seed'] + 800 + layer_idx,
            )
        else:
    ...
    def forward(self, x, pos_emb):
        ...
        q = q_dom.reshape(B, T, H, QK_PAIRS).permute(0, 2, 1, 3)
        k = k_dom.reshape(B, T, H, QK_PAIRS).permute(0, 2, 1, 3)
