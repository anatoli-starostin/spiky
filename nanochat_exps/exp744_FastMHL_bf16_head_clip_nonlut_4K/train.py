"""exp744 — exp742 + vanilla-equivalent clip_grad_norm on non-LUT params.

Fork of exp742 (bf16 head + xent fix, drifted +9 mb at step 2400). Adds
torch.nn.utils.clip_grad_norm_(decay_params + tok_emb_params + nodecay_params,
1.0) — mirrors vanilla's clip(1.0) on its dense params, but skips the LUT
body (whose grad norm at 251 M params naturally sits around ~36; clipping
to 1.0 would over-constrain it). Tests whether vanilla's grad clip was the
silent protector against bf16-head drift.

Compare to exp737 v2 = 1.3872. If exp744 closes the gap, clipping the
non-LUT params is the missing protector. If it still drifts ~10 mb, the
no-clip hypothesis is wrong and the bf16-head cost is structural elsewhere.
"""
...
for step in range(1, N_STEPS + 1):
    ...
    # Vanilla-equivalent grad clip on non-LUT params only (unembedder + tok_emb
    # + LayerNorm). LUT body left unclipped — its grad norm scales with 251 M
    # params (typical ~36) so clip(1.0) would over-constrain.
    torch.nn.utils.clip_grad_norm_(
        decay_params + tok_emb_params + nodecay_params, 1.0)
