...
class Model(nn.Module):
    def __init__(self):
        ...
        # exp574: initialize gamma_emb SMALL (0.1 instead of 1.0). Logit scale at init
        # = sqrt(E) * gamma_x * gamma_emb ~ 9.8 * 1.0 * 0.1 ~ 1.0 (vs ~10 in exp573).
        # Brings softmax over V=32768 into the soft regime at init — avoids the random
        # high-variance peaks that gave exp572/573 CE = log(V) at step 200. gamma_emb
        # remains LEARNABLE and free to grow if the optimizer wants higher temperature.
...
print(f'TIED dot head (exp574): logits = ln_final(x_lut) @ ln_emb(tok_emb_E).T; SEPARATE LNs, ln_emb.gamma INIT=0.1 to give logit scale ~1.0 at init (soft softmax); RoPE base={_ROPE_BASE}')
