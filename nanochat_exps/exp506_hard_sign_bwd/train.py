...
from spiky.lutorch.tiny_multi_head_lut import TinyMultiHeadLut, set_hard_sign_bwd
...
# exp506: hard signs in the soft backward (softmax over INTEGER hamming scores).
# The argmax forward is sign-only -> identical to exp475; this isolates the
# backward-gradient effect of hard vs soft signs (TinyMHLut analog of exp500).
if cfg.get('hard_sign_bwd', False):
    set_hard_sign_bwd(True)
    print('HARD-SIGN BACKWARD enabled (set_hard_sign_bwd(True))')
