"""nanochat_exps/exp092_residual_chunk_partitions — fork of exp091 with within-chunk anchor pairs.

Single change vs exp091: each layer i's residual_lut is built with
partition_sets = [chunk_0, ..., chunk_i] (E-slot contiguous chunks of
the inverse-concat input). No partition_pair_weights (uniform sampling
across chunks). Effect: every sampled pair is within a single E-chunk,
so cross-chunk pairs are prohibited; a table can still contain pairs
from multiple chunks (each pair within one chunk). Anchor inheritance
and split LR/WD unchanged.

How to launch:

    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python \\
        -u nanochat_exps/exp092_residual_chunk_partitions/train.py \\
        > nanochat_exps/exp092_residual_chunk_partitions/stdout.log 2>&1 &
"""
...
def _make_residual(layer_idx, seed_offset):
    ...
    Partition input into i+1 contiguous E-slot chunks; pass partition_sets
    so the sampled pool restricts to within-chunk pairs (no cross-chunk
    pairs). No partition_pair_weights => uniform sampling across chunks,
    so a single table can contain pairs from multiple chunks (each pair
    within one chunk).
    """
