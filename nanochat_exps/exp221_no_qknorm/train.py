"""nanochat_exps/exp221_no_qknorm — fork of exp220 (final 1.6403 @ 8K, 339M).

Single change: drop q_norm and k_norm. Q and K come straight from the QK LUT
(qk_lut_init_std=0.02), pass through plain SDPA with default 1/sqrt(d_qk).
At init, LUT Q/K element std ≈ 0.18 (vs 0.39 for exp001's Linear projection),
giving pre-softmax std ≈ 0.034 — gentler softmax than exp001 (0.154) and exp220
(1.0). Tests whether QK-norm was load-bearing or just a scale convenience.

Launch:
    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python -u \\
        nanochat_exps/exp221_no_qknorm/train.py \\
        > nanochat_exps/exp221_no_qknorm/stdout.log 2>&1 &
"""
