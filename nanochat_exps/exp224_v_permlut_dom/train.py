"""nanochat_exps/exp224_v_permlut_dom — fork of exp223 (1.6249 @ 8K, 339M).

Single change: replace V LUT (TinyMultiHeadLut, n_outputs=d_v=24) with a
PermutationalLut whose n_outputs is the dominance-pair dim P = d_v*(d_v-1)/2 = 276.
Settings: tph=256, output_nap=24, pair_mode='scrambled', soft_mode='rational',
borda_scale_mode='borda'. SDPA mixes V vectors in the 276-dim dominance space.
After SDPA we apply DominanceToVector(d_v=24, normalise=True) to project back
to 24-dim per head before out_proj. V vector merging happens in dominance space.

Launch:
    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python -u \\
        nanochat_exps/exp224_v_permlut_dom/train.py \\
        > nanochat_exps/exp224_v_permlut_dom/stdout.log 2>&1 &
"""
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        self.v_d2v = DominanceToVector(d_v, normalise=True)
