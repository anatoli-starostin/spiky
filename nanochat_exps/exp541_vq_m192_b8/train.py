"""exp539+: exp513 recipe (matmul-free LUT backbone, hard everywhere) with the
Linear unembedder replaced by a VQ VocabLUT (product-quantized, transposed-LUT
head). Trained end-to-end from scratch so the backbone co-adapts to a
PQ-friendly D-stream. Config sets vq_m / vq_b (compression ratio).

Head: logit(v) = x . concat_j C_j[code_v[j]] ; codes learned VQ-style (STE +
commitment + codebook loss). Deploy keeps codes [V,m] + codebooks [m,2^b,D/m];
V-side is matmul-free. eval bpb is the QUANTIZED (deployable) head.

vs exp513 Linear head: 1.4825 (10-step) / 1.4656 (100-step). Offline PQ of the
frozen head at the same ratios: m=192/8x +0.034, m=96/14x +0.13, m=48/26x +0.37.
This run tests whether LEARNED codes + co-adapting backbone beat that.
"""
...
from spiky.lutorch.vq_vocab_lut import VQVocabLUT
...
VQ_M = cfg['vq_m']; VQ_B = cfg['vq_b']; VQ_COMMIT_BETA = cfg.get('vq_commit_beta', 0.25)
...
class Model(nn.Module):
    def __init__(self):
        ...
        self.unembedder = VQVocabLUT(D, VOCAB_SIZE, VQ_M, VQ_B,
                                     commit_beta=VQ_COMMIT_BETA, device=DEVICE,
...
# deploy params = backbone + tok_emb + VQ deploy (codes + codebooks), not the latent
deploy_head = VQ_M * (1 << VQ_B) * (D // VQ_M) + (VOCAB_SIZE * VQ_M * VQ_B) / 32.0
backbone_params = n_params - model.unembedder.latent.numel() - model.unembedder.codebooks.numel()
print(f'Total train params: {n_params:,} | VQ head: latent={model.unembedder.latent.numel():,} '
      f'codebooks={model.unembedder.codebooks.numel():,}')
print(f'DEPLOY head ~{deploy_head/1e6:.3f}M float-equiv (codes+codebooks) vs Linear {VOCAB_SIZE*D/1e6:.2f}M '
      f'({VOCAB_SIZE*D/deploy_head:.0f}x less) | {model.unembedder}')
...
for name, p in model.named_parameters():
    ...
    if name.startswith('unembedder.'): head_params.append(p)       # VQ latent + codebooks -> AdamW (no wd)
    elif p.ndim >= 3: lut_params.append(p)                          # backbone LUT tables -> LION
...
print(f'lut(LION)={sum(p.numel() for p in lut_params):,} | head(AdamW)={sum(p.numel() for p in head_params):,} | '
      f'VQ m={VQ_M} b={VQ_B} commit_beta={VQ_COMMIT_BETA}')
...
csv_w = csv.writer(csv_f); csv_w.writerow(['step', 'train_loss', 'val_bpb', 'vq_loss', 'code_usage'])
...
for step in range(1, N_STEPS + 1):
    ...
    for _ in range(grad_accum):
        ...
        ce = model(x, targets=y)
        loss = ce + model.unembedder._vq_loss
        (loss / grad_accum).backward()
        accum += ce.item() / grad_accum
    for o in all_optimizers: o.step()
    ema = accum if ema is None else 0.99 * ema + 0.01 * accum
    if step % 100 == 0 or step == 1:
        print(f'step {step:6d} | ce_ema={ema:.4f} | vq={float(model.unembedder._vq_loss):.4f} | '
              f'code_use={model.unembedder._code_usage:.3f} | lr={lr_scale*cfg["adam_lr"]:.2e}')
    ...
    if step % EVAL_EVERY == 0 or step == N_STEPS:
        ...
        print(f'[VAL] step {step}: bpb={bpb:.4f} | code_use={model.unembedder._code_usage:.3f}')
        val_bpbs.append(bpb); val_steps.append(step); train_losses.append(ema)
        csv_w.writerow([step, f'{ema:.6f}', f'{bpb:.6f}', f'{float(model.unembedder._vq_loss):.6f}', f'{model.unembedder._code_usage:.4f}'])
        csv_f.flush(); model.train()
...
summary = dict(exp_name=cfg['exp_name'], best_val_bpb=best_bpb,
               ...
               n_params=n_params, deploy_head_M=deploy_head/1e6,
               final_code_usage=model.unembedder._code_usage,
               training_time_hours=round(elapsed / 3600, 3))
with open(os.path.join(EXP_DIR, 'summary.json'), 'w') as f: json.dump(summary, f, indent=2)
torch.save({'model_state_dict': model.state_dict(), 'config': cfg, 'step': N_STEPS, 'best_val_bpb': best_bpb},
