"""nanochat_exps/exp075_perm_sparse_canon — PermutationalLut Q/K + sparse-dom canonicalize.

Major architecture change vs exp072:
  - qk_joint TinyMultiHeadLut → PermutationalLut producing [B*T, H, 2048].
    Split into Q (first 1024 dims) and K (last 1024 dims).
  - Each 1024-dim half is treated as sparse dominance scores in a virtual
    d_qk_virtual=128 ranking space (full triu = 8128 dims).
  - SparseDominanceCanonicalize (one shared between Q and K) projects
    each sparse-1024-dim → Borda-128 → back to sparse-1024-dim via per-head
    random pair indices (tied D2V/V2D).
  - SDPA Q/K dim 1024 (vs exp072's 2016).
  - V LUT and out_proj rank canonicalize unchanged from exp072.

PermutationalLut config: input_nap=6, output_nap=64, tph=512, n_outputs=2048,
pair_mode='aligned', soft_mode='ste', aggregation='matmul'. Inner MHLut
weights match exp072 magnitude (≈12.58M / layer × 6 = 75.5M total).

How to launch:

    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python \\
        -u nanochat_exps/exp075_perm_sparse_canon/train.py \\
        > nanochat_exps/exp075_perm_sparse_canon/stdout.log 2>&1 &
"""
...
from spiky.lutorch.ranking_tools import (
    VectorToDominance, DominanceToVector, SparseDominanceCanonicalize,
)
...
# PermutationalLut Q/K config
QK_OUTPUT_NAP   = cfg['qk_output_nap']        # 64
QK_N_OUT_ROLE   = cfg['qk_n_outputs_per_role'] # 1024 (per Q or K)
QK_VIRTUAL_D    = cfg['qk_virtual_d']         # 128 — virtual ranking-space dim
...
def _make_qk_joint(layer_idx, seed_offset):
    """Joint Q/K PermutationalLut.

    Outputs [B*T, H, 2*qk_n_outputs_per_role] real-valued sparse dominance
    scores; downstream we split into Q and K halves and feed each through
    a SparseDominanceCanonicalize.
    """
    n_inputs = E + (_pos_emb_dim(layer_idx) if _POS_EMB_ACTIVE else 0)
    return PermutationalLut(
        n_inputs=n_inputs,
        n_outputs=2 * QK_N_OUT_ROLE,                 # 2048 = 2 * 1024
        input_nap=cfg['qk_input_nap'],               # 6
        output_nap=QK_OUTPUT_NAP,                    # 64
        n_heads=H,
        tph=cfg['qk_tph'],                            # 512
        pair_mode='scrambled',
        soft_mode='ste',
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        # One shared sparse-dominance canonicalizer for both Q and K. Tied
        # per-head pair indices between D2V and V2D (round-trip onto the
        # realizable-dominance manifold of a virtual qk_virtual_d=128 space).
        self.qk_canon = SparseDominanceCanonicalize(
            d_head=QK_VIRTUAL_D,
            n_pairs=QK_N_OUT_ROLE,
            n_heads=H,
            random_seed=cfg['random_seed'] + 700 + layer_idx,
            smooth_mode=False,
            temperature=canon_t,
        )
        # Rank-canonicalize each block's output: real -> dominance -> Borda+LN.
        # Replaces the standalone out_ln; D2V(normalise=True) carries its own LayerNorm.
    ...
    def forward(self, x, pos_emb):
        ...
        qk_out = self.qk_joint(xp)                                     # [B*T, H, 2*QK_N_OUT_ROLE]
        q_raw = qk_out[..., :QK_N_OUT_ROLE]                            # [B*T, H, 1024]
        k_raw = qk_out[..., QK_N_OUT_ROLE:]                            # [B*T, H, 1024]
        q_dom = self.qk_canon(q_raw)                                   # [B*T, H, 1024] sparse ±1
        k_dom = self.qk_canon(k_raw)                                   # [B*T, H, 1024] sparse ±1
        q = q_dom.reshape(B, T, H, QK_N_OUT_ROLE).permute(0, 2, 1, 3)  # [B, H, T, 1024]
        k = k_dom.reshape(B, T, H, QK_N_OUT_ROLE).permute(0, 2, 1, 3)
...
print(f'Q/K Joint PermLut: in_nap={cfg["qk_input_nap"]} tph={cfg["qk_tph"]} '
      f'output_nap={QK_OUTPUT_NAP} n_outputs={2 * QK_N_OUT_ROLE} '
      f'(virtual_d={QK_VIRTUAL_D}, sparse n_pairs={QK_N_OUT_ROLE})')
