...
from spiky.lutorch.tiny_multi_head_lut import (
    TinyMultiHeadLut, enable_visit_count_stash, get_visit_count)
...
class GaussNewton(torch.optim.Optimizer):
    """Damped Gauss-Newton on LUT weights, exploiting that the LUT is LINEAR in
    its weights for a fixed selection, with diagonal Hessian = per-row visit
    count. Step per (table,row):  ΔW[t,r] = -lr * m[t,r] / (count_ema[t,r] + lam)
    where m = EMA(summed grad) and count_ema = EMA(visit count). lam is
    Levenberg-Marquardt damping (lam=0 == undamped Newton == exp441, which blew
    up on count=1 rows). With `normalize`, the per-tensor update is scaled to RMS
    `lr` so lr is directly comparable to LION/signSGD (which step at ±lr); this
    isolates the per-row count weighting from the global step scale. Visit counts
    come from tiny_multi_head_lut's backward via get_visit_count(p).
    State: m (1x weights) + count_ema (1/n_out x weights) -> LION-tier memory."""
    def __init__(self, params, lr=2e-4, beta1=0.9, count_beta=0.9, lam=32.0,
                 normalize=True, eps=1e-12):
        super().__init__(params, dict(lr=lr, beta1=beta1, count_beta=count_beta,
                                      lam=lam, normalize=normalize, eps=eps))
        self._probe_done = False
    @torch.no_grad()
    def step(self):
        probe = []
        for grp in self.param_groups:
            lr, b1, cb = grp['lr'], grp['beta1'], grp['count_beta']
            lam, normalize, eps = grp['lam'], grp['normalize'], grp['eps']
            for p in grp['params']:
                if p.grad is None:
                    continue
                g = p.grad                                  # [n_tables, K, n_out]
                nt, K, n_out = p.shape
                st = self.state[p]
                cnt = get_visit_count(p)                    # [n_tables*K] or None
                if cnt is None:
                    cnt = torch.ones(nt * K, device=p.device, dtype=p.dtype)
                cnt = cnt.to(p.dtype).view(nt, K)
                if 'exp_avg' not in st:
                    st['exp_avg'] = torch.zeros_like(p)
                    st['count_ema'] = cnt.clone()           # unbiased start
                m, count_ema = st['exp_avg'], st['count_ema']
                m.mul_(b1).add_(g, alpha=1.0 - b1)
                count_ema.mul_(cb).add_(cnt, alpha=1.0 - cb)
                u = m / (count_ema.unsqueeze(-1) + lam)      # damped Newton dir
                if normalize:
                    rms = u.pow(2).mean().sqrt()
                    u = u / (rms + eps)
                p.add_(u, alpha=-lr)
                if not self._probe_done:
                    probe.append((tuple(p.shape), cnt, m, count_ema, lam, lr,
                                  rms.item() if normalize else float('nan')))
        if not self._probe_done and probe:
            print("\n=== GN STEP-1 PROBE (lr=%.2e, lam=%.1f) ===" % (
                self.param_groups[0]['lr'], self.param_groups[0]['lam']))
            allc = torch.cat([c.flatten() for _, c, *_ in probe])
            print("visit count over %d rows: mean=%.1f median=%.1f max=%.0f frac_zero=%.3f"
                  % (allc.numel(), allc.mean(), allc.median(), allc.max(),
                     (allc == 0).float().mean()))
            for shp, c, m, ce, lam, lr, rms in probe[:1]:
                raw = (m / (ce.unsqueeze(-1) + lam))
                print("  example %s: raw m/(c+lam) RMS=%.3e -> normalized step RMS=%.3e"
                      % (shp, raw.pow(2).mean().sqrt().item(), lr if rms == rms else float('nan')))
            self._probe_done = True

_LUT_LR  = cfg.get('lut_lr', cfg['adam_lr'])
_LUT_OPT = cfg.get('lut_optimizer', 'adamw')   # 'adamw' | 'lion' | 'signsgd' | 'gn'
...
if _LUT_OPT == 'lion':
    ...
    enable_visit_count_stash(True)   # backward stashes per-row visit counts
    lut_optimizer = GaussNewton(
        [dict(params=lut_params, lr=_LUT_LR)],
        lr=_LUT_LR, beta1=cfg.get('gn_beta1', 0.9),
        count_beta=cfg.get('gn_count_beta', 0.9), lam=cfg.get('gn_lambda', 32.0),
        normalize=cfg.get('gn_normalize', True))
