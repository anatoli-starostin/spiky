...
class RMSNorm(nn.Module):
    """exp474: plain RMSNorm, NO centering, NO affine: y = x / RMS(x),
    RMS(x)=sqrt(mean(x^2)). vs LayerNorm-no-affine it drops the mean-subtract and
    divides by RMS=sqrt(sigma^2+mu^2) >= sigma (slightly more aggressive shrink by
    sqrt(1+(mu/sigma)^2)). For the LUT, mu cancels in differences -> only the
    per-token scale differs, absorbed by temperature T; expect ~= exp453."""
