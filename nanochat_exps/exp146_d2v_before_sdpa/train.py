...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        self.qk_d2v = DominanceToVector(d_qk, normalise=True)
    ...
    def forward(self, x, pos_emb):
        ...
        q_vec = qk_out[..., :d_qk]
        k_vec = qk_out[..., d_qk:]
        q_dom = self.qk_v2d(q_vec)                                     # [B*T, H, P]
        k_dom = self.qk_v2d(k_vec)
        q_rank = self.qk_d2v(q_dom)                                    # [B*T, H, d_qk] (Borda+LN)
        k_rank = self.qk_d2v(k_dom)
        q = q_rank.reshape(B, T, H, d_qk).permute(0, 2, 1, 3)
        k = k_rank.reshape(B, T, H, d_qk).permute(0, 2, 1, 3)
