...
_QKV_MULTI_NAP = cfg.get('qkv_multi_nap')          # list of [nap, tph] pairs
_V_MULTI_NAP = cfg.get('v_multi_nap')              # list of [nap, tph] pairs
def _make_qkv_joint(layer_idx, seed_offset):
    """Joint qkv LUT — n_outputs = 2*d_qk + d_v."""
    if _QKV_MULTI_NAP is not None:
...
def _make_v(layer_idx, seed_offset):
    if _V_MULTI_NAP is not None:
        kwargs = dict(_TINY_SOFT_KWARGS)
        return TinyMultiNapMultiHeadLut(
            input_dim=E, n_heads=H, n_outputs=d_v,
            nap_tph_pairs=_V_MULTI_NAP,
            base_random_seed=cfg['random_seed'] + seed_offset,
            device=DEVICE, **kwargs,
        )
    ...
    return TinyMultiHeadLut(
        ...
        random_seed=cfg['random_seed'] + seed_offset, device=DEVICE, **v_kwargs,
    )
...
_RESIDUAL_MULTI_NAP = cfg.get('residual_multi_nap')  # list of [nap, tph] pairs
def _make_residual_lut(layer_idx, seed_offset):
    """E -> D residual projection."""
    if _RESIDUAL_MULTI_NAP is not None:
        kwargs = dict(_TINY_SOFT_KWARGS)
        return TinyMultiNapMultiHeadLut(
            input_dim=E, n_heads=1, n_outputs=D,
            nap_tph_pairs=_RESIDUAL_MULTI_NAP,
            base_random_seed=cfg['random_seed'] + seed_offset,
            device=DEVICE, **kwargs,
        )
    return TinyMultiHeadLut(
