#!/usr/bin/env python3
"""Smoke test: exp683 — exp611 architecture with E=144, d_v=24, D=384."""
...
N_L = cfg['num_layers']
assert E == 144 and D == 384 and d_v == 24, f"expected E=144 D=384 d_v=24, got E={E} D={D} d_v={d_v}"
...
assert vl.weights.numel() == 4_718_592
...
assert op.weights.numel() == 9_437_184
...
print(f'read_out: input={E} n_heads=1 n_out={D} NAP={cfg["readout_input_nap"]} tph={cfg["readout_tph"]} params={ro.weights.numel():,}')
assert ro.weights.numel() == 18_874_368
...
print(f'  qkv: {qkv.weights.numel()*N_L/1e6:.2f}M, v: {vl.weights.numel()*N_L/1e6:.2f}M, op: {op.weights.numel()*N_L/1e6:.2f}M, ro: {ro.weights.numel()/1e6:.2f}M')
