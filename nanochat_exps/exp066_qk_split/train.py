"""nanochat_exps/exp066_qk_split — fork of exp064 with separate Q and K LUTs.

Single change vs exp064: split the joint Q/K TinyMultiHeadLut into two
independent LUTs. Joint had n_outputs=2*d_qk=64 with shared anchor sampling;
now we have q_lut (n_outputs=d_qk=32) and k_lut (n_outputs=d_qk=32) with
independent anchor sampling. Param-invariant (each LUT half-size, two of
them). Same fp32 numerics, same hyperparams, same 8K steps.

How to launch:

    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python \\
        -u nanochat_exps/exp066_qk_split/train.py \\
        > nanochat_exps/exp066_qk_split/stdout.log 2>&1 &
"""
...
def _make_q_or_k(layer_idx, seed_offset):
    """Per-role TinyMultiHeadLut for Q or K: input=E, n_heads=H, n_outputs=d_qk."""
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        super().__init__()
        self.q_lut    = _make_q_or_k(layer_idx, layer_idx)
        self.k_lut    = _make_q_or_k(layer_idx, 600 + layer_idx)
    ...
    def forward(self, x, pos_emb):
        ...
        q_vec = self.q_lut(xp)                                         # [B*T, H, d_qk]
        k_vec = self.k_lut(xp)                                         # [B*T, H, d_qk]
