...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        self.q_ln = nn.LayerNorm(d_qk)
        self.k_ln = nn.LayerNorm(d_qk)
    ...
    def forward(self, x, pos_emb):
        ...
        q_vec = self.q_ln(qk_out[..., :d_qk])                           # LayerNorm-standardise
        k_vec = self.k_ln(qk_out[..., d_qk:])
        q = q_vec.reshape(B, T, H, d_qk).permute(0, 2, 1, 3)            # [B, H, T, d_qk] real
        k = k_vec.reshape(B, T, H, d_qk).permute(0, 2, 1, 3)            # [B, H, T, d_qk] real
