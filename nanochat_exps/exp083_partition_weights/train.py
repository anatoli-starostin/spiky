"""nanochat_exps/exp083_partition_weights — fork of exp082 with B1 weighted partition pair sampling.

Single change vs exp082: each LUTBlock's QK and V TinyMultiHeadLut now receive
`partition_pair_weights` so that per-pair sampling probability is proportional
to the partition's weight. Schedule = 'linear':
  L_k>=1 with (k+2) zones: weights = [k+2, k+1, ..., 1]
  (most-recent block output first, token_emb at the tail).
out_proj keeps unweighted H head-partition sampling.

Architecture (carried from exp082):
  E=O=32, concat-prepend residual, per-O-chunk QK/V partitions, H head-zone
  partitions on out_proj, MLP unembedder. Per-layer input dims
  [32, 64, 96, 128, 160, 192]; final concat 224. n_steps=8000, out_tph=1024 flat.

How to launch:

    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python \\
        -u nanochat_exps/exp083_partition_weights/train.py \\
        > nanochat_exps/exp083_partition_weights/stdout.log 2>&1 &
"""
...
_PARTITION_WEIGHT_SCHEDULE = cfg.get('partition_weight_schedule', 'flat')

def _qkv_pair_weights(input_dim):
    """Per-zone sampling weights (most-recent chunk first → token_emb tail).
    Returns None when there is no partition (L0)."""
    if input_dim <= O:
        return None
    n_zones = input_dim // O
    sched = _PARTITION_WEIGHT_SCHEDULE
    if sched == 'flat':
        return None  # no weighting → use default uniform full-coverage path
    if sched == 'linear':
        return [float(n_zones - i) for i in range(n_zones)]
    if sched == 'geometric_05':
        return [0.5 ** i for i in range(n_zones)]
    if sched == 'geometric_07':
        return [0.7 ** i for i in range(n_zones)]
    raise ValueError(f"unknown partition_weight_schedule: {sched!r}")
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx, input_dim):
        ...
        qkv_weights = _qkv_pair_weights(input_dim)
        ...
        self.qk_joint = TinyMultiHeadLut(
            ...
            partition_pair_weights=qkv_weights,
...
class Model(nn.Module):
    def __init__(self):
        ...
        unemb_mult = cfg.get('unembedder_mlp_mult', 4)
        unemb_hidden = unemb_mult * UNEMBEDDER_DIM
        self.unembedder = nn.Sequential(
            nn.LayerNorm(UNEMBEDDER_DIM),
            nn.Linear(UNEMBEDDER_DIM, unemb_hidden),
            nn.ReLU(),
            nn.Linear(unemb_hidden, VOCAB_SIZE),
        )
...
print(f'Partition weight schedule: {_PARTITION_WEIGHT_SCHEDULE}')
for li, dim in enumerate(LAYER_INPUT_DIMS):
    w = _qkv_pair_weights(dim)
    print(f'  L{li} (in={dim}): {"none" if w is None else w}')
