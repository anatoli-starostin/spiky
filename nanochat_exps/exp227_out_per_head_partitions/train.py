"""nanochat_exps/exp227_out_per_head_partitions — fork of exp223 (1.6249 @ 8K, 339M).

Single change: out_proj LUT now uses partition_sets=[head0_dims, head1_dims, ...]
to forbid CROSS-HEAD pairs. Each anchor pair (a, b) in the out_proj LUT must
have both endpoints in the same head's d_v=24 slice of the H*d_v=144 input;
pairs spanning two different heads are excluded. A single table can still
contain pairs drawn from multiple heads — only the individual pair endpoints
are constrained. Tests whether the model benefits from never directly
comparing dimensions across heads at the out_proj boundary.

Launch:
    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python -u \\
        nanochat_exps/exp227_out_per_head_partitions/train.py \\
        > nanochat_exps/exp227_out_per_head_partitions/stdout.log 2>&1 &
"""
...
_OUT_PARTITION_PER_HEAD = cfg.get('out_partition_per_head', False)
def _make_out(layer_idx, seed_offset):
    tph = _OUT_TPH_PER_LAYER[layer_idx] if _OUT_TPH_PER_LAYER is not None else cfg['out_tph']
    out_kwargs = dict(_TINY_KWARGS)
    if _OUT_PARTITION_PER_HEAD:
        # Forbid cross-head pairs: each pair (a, b) must have both endpoints
        # within the same head's d_v slice. A table can still mix pairs drawn
        # from different heads.
        out_kwargs['partition_sets'] = [
...
print(f'out_proj MHLut:  in_nap={cfg["out_input_nap"]} tph={_tph_str} (n_outputs=E={E}) per_head_partitions={_OUT_PARTITION_PER_HEAD} + out_v2d/out_d2v')
