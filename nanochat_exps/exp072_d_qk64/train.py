"""nanochat_exps/exp072_d_qk64 — fork of exp071 with d_qk doubled.

Single config change vs exp071: d_qk 32 -> 64. Q/K joint LUT n_outputs
doubles (2*d_qk: 64 -> 128); +37.75M params total (129.30M -> 167.05M).
D_QK_P = d_qk*(d_qk-1)/2 scales 496 -> 2016, so the dominance vectors
entering SDPA are 4x bigger (Q/K SDPA flop is 4x). Same rank-only
canonicalization on output, same other hyperparams, same 8K steps.

How to launch:

    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python \\
        -u nanochat_exps/exp072_d_qk64/train.py \\
        > nanochat_exps/exp072_d_qk64/stdout.log 2>&1 &
"""
