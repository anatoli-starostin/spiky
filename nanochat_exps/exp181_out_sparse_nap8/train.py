...
_OUT_N_SPARSE = cfg.get('out_n_sparse', 0)  # 0 = dense; >0 = sparse_scatter to E
def _make_out(layer_idx, seed_offset):
    """out_proj TinyMultiHeadLut: input=H*d_v, n_heads=1, n_outputs=E.

    With out_n_sparse > 0, each table writes only out_n_sparse of E slots,
    scatter-aggregated to E. Per-table weights = table_dim * out_n_sparse,
    so per-layer cost shrinks from tph * 2^nap * E to tph * 2^nap * out_n_sparse.
    """
    ...
    if _OUT_N_SPARSE > 0:
        extra = dict(
            ...
            sparse_scatter_seed=cfg['random_seed'] + seed_offset + 17,
        )
        n_out = _OUT_N_SPARSE
    else:
        n_out = E
