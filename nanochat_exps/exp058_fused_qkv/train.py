...
def _make_qk_joint(layer_idx, seed_offset):
    """Joint Q/K BitMultiHeadLUT on (x + pos_emb): input=E, n_outputs=2*P_qk."""
    n_inputs = E + (_pos_emb_dim(layer_idx) if _POS_EMB_ACTIVE else 0)
    n_out = 2 * D_QK_P
    n_votes_per_output = cfg['qk_tph'] * cfg['qk_output_nap'] / float(n_out)
    qk_scale = 0.5 / math.sqrt(max(n_votes_per_output, 1.0))
...
def _make_v(layer_idx, seed_offset):
    """V BitMultiHeadLUT on x (no pos): input=E, n_outputs = 2*P_qk + P_v.

    Output split downstream:
      delta_q = v_out[..., :P_qk]         # added to qk_lut's q_raw
      delta_k = v_out[..., P_qk:2*P_qk]   # added to qk_lut's k_raw
      v_dom   = v_out[..., 2*P_qk:]       # used raw
    """
    n_out = 2 * D_QK_P + D_V_P
    n_votes_per_output = cfg['v_tph'] * cfg['v_output_nap'] / float(n_out)
    v_scale = 0.5 / math.sqrt(max(n_votes_per_output, 1.0))
    return BitMultiHeadLUT(
        n_inputs=E,
        n_outputs=n_out,
        n_heads=H,
...
class LUTBlock(nn.Module):
    """Two-LUT block. qk_lut on (x + pos_emb) outputs 2*P_qk; v_lut on x
    outputs 2*P_qk + P_v (extra Q/K residual + V dominance). Q, K formed by
    summing qk_lut's q/k_raw with v_lut's delta_q/delta_k before canonicalize.
    V used raw.
    """
    def __init__(self, layer_idx):
        ...
        self.qk_lut = _make_qk_joint(layer_idx, layer_idx)
    ...
    def forward(self, x, pos_emb):
        ...
        qk_out = self.qk_lut(xp)                                       # [B*T, H, 2*P_qk]
        v_out  = self.v_lut(x_flat)                                    # [B*T, H, 2*P_qk + P_v]

        delta_q = v_out[..., :D_QK_P]
        delta_k = v_out[..., D_QK_P:2 * D_QK_P]
        v_dom_flat = v_out[..., 2 * D_QK_P:]                           # [B*T, H, P_v] raw

        q_raw = qk_out[..., :D_QK_P] + delta_q                         # qk_lut(xp) + v_lut(x) residual
        k_raw = qk_out[..., D_QK_P:] + delta_k

        q_dom = self.q_canon(q_raw)
        k_dom = self.k_canon(k_raw)
        q = q_dom.reshape(B, T, H, D_QK_P).permute(0, 2, 1, 3)
        k = k_dom.reshape(B, T, H, D_QK_P).permute(0, 2, 1, 3)

        v_dom = v_dom_flat.reshape(B, T, H, D_V_P).permute(0, 2, 1, 3)
...
for layer in model.layers:
    bit_luts += [layer.qk_lut, layer.v_lut, layer.out_proj]
...
print(f'qk_lut: in_nap={cfg["qk_input_nap"]} out_nap={cfg["qk_output_nap"]} tph={cfg["qk_tph"]} (n_outputs=2*P_qk={2*D_QK_P})')
print(f'v_lut:  in_nap={cfg["v_input_nap"]} out_nap={cfg["v_output_nap"]} tph={cfg["v_tph"]} (n_outputs=2*P_qk+P_v={2*D_QK_P+D_V_P}, residual to qk_raw)')
