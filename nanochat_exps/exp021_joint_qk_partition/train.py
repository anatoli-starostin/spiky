...
def _make_qk_joint(layer_idx, seed_offset):
    """Single BitMultiHeadLUT outputting both Q and K pair-dominance via one
    anchor lookup. Output dim 2*P_qk, first P_qk → Q half, next P_qk → K half.
    output_nap is 2× per-side count (so each half gets the same votes-per-pair
    a separate-LUT setup would give). Scale matches the CLT convention."""
    n_inputs = _e_in(layer_idx) + (_pos_emb_dim(layer_idx) if _POS_EMB_ACTIVE else 0)
    out_nap_total = 2 * cfg['qk_output_nap']
    n_out = 2 * D_QK_P
    votes_per_output = cfg['qk_tph'] * out_nap_total / n_out
    qk_scale = 0.5 / math.sqrt(votes_per_output)
    return BitMultiHeadLUT(
        n_inputs=n_inputs, n_outputs=n_out, n_heads=H,
        input_nap=cfg['qk_input_nap'], output_nap=out_nap_total,
        tph=cfg['qk_tph'],
...
# SAME head (within-head comparisons only); a single table can still mix
# heads across its 8 anchor pairs.
...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        # Joint Q/K via single BitMultiHeadLUT — one anchor lookup feeds both
        # Q and K halves of the output (slots [:P_qk] → Q, [P_qk:] → K).
