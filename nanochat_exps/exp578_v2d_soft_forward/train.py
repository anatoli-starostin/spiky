...
class Model(nn.Module):
    def __init__(self):
        ...
        # exp578: V2D-tied head with SOFT forward (smooth_mode=True). Hard +/-1 fwd
        # would constrain v2d_x . v2d_emb to only 2017 distinct integer values
        # (sum of P=2016 +/-1 terms) — massive logit quantization, the head can't
        # express fine distinctions between vocab targets. Soft forward gives
        # continuous d_p = (x_a-x_b)/(T+|x_a-x_b|) in (-1,1) — full-precision logits.
        # (Deployment can still switch back to hard via 1-bit storage; that is a
        # post-training choice. For training quality we want soft.)
        self.v2d = VectorToDominance(d_head=E, smooth_mode=True, temperature=0.1)
        P = E * (E - 1) // 2
        self.P = P
...
P_v2d = E * (E - 1) // 2
print(f'V2D-TIED head (exp578, SOFT fwd): logits = ln_x_v2d(V2D_soft(x_lut)) @ ln_emb_v2d(V2D_soft(tok_emb_E)).T; V2D dim P=E*(E-1)/2={P_v2d}, smooth_mode=True T=0.1, ln_emb_v2d.gamma init=1/sqrt(P)={1.0/math.sqrt(P_v2d):.4f}; RoPE base={_ROPE_BASE}')
