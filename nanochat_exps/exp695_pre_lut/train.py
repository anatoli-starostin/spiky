"""exp695 — fork of exp693 (1.4379 @ 97.5M) with a pre_lut after the embedding.

Single addition: a pre_lut between tok_emb_E and the LUTBlock stack, with
v_lut's settings (NAP=6, tph=256, n_heads=6, n_out=d_v=24). Output [B*T, 6, 24]
reshaped to [B, T, E=144] (6*24 = E) and added residually to x_lut.

Motivated by attention analysis: trained exp693 has L0 self_mass=0.45 (vs vanilla
0.013, 35x higher); the model appears to use L0's attention as a per-token non-
linear transform path because there is no FFN in LUTBlocks. pre_lut gives that
role a dedicated module right after the embedding, freeing L0 attention for
cross-token routing.

Architecture (vs exp693):
  + pre_lut: TinyMHLut(NAP=6, tph=256, n_heads=6, n_out=24); out reshaped to E
            and added residually to x_lut after tok_emb_E.
  - LUTBlocks: unchanged (qk_lut, v_lut, out_proj, residual_lut, q/k_norm all stay).
"""
...
def _make_pre_lut(seed_offset):
    """pre_lut after tok_emb_E: E -> H*d_v = E (residual to x_lut).

    Mirrors v_lut's NAP/tph/heads — output [B*T, H, d_v] is reshaped to [B, T, E]
    since H*d_v = E. Single module (not per-layer).
    """
    return TinyMultiHeadLut(
        input_dim=E, n_heads=H, n_outputs=d_v,
...
class Model(nn.Module):
    def __init__(self):
        ...
        self.pre_lut = _make_pre_lut(800)
        self.ln_pre0 = MeanAbsNorm(E)
    ...
    def forward(self, tokens, targets=None, loss_reduction='mean'):
        ...
        # pre_lut: per-token non-linearity right after the embedding.
        x_pre0 = self.ln_pre0(x_lut.reshape(B * T, E))
        pre_out = self.pre_lut(x_pre0).reshape(B, T, E)
        x_lut = x_lut + pre_out
