"""nanochat_exps/exp225_v_d2v_no_norm — fork of exp224 (1.6279 @ 8K, 339M).

Single change: V-side DominanceToVector now has normalise=False — removes the
trailing LayerNorm on the 24-dim per-head output, so we get raw Borda
projection out of dominance space. Tests whether the LN was constraining the
attention output's magnitude beneficially or unnecessarily.

Launch:
    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python -u \\
        nanochat_exps/exp225_v_d2v_no_norm/train.py \\
        > nanochat_exps/exp225_v_d2v_no_norm/stdout.log 2>&1 &
"""
