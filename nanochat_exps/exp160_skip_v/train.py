...
def _make_out(layer_idx, seed_offset):
    """out_proj TinyMultiHeadLut: input=H*E (skip-v), n_heads=1, n_outputs=E (+ LayerNorm)."""
...
class LUTBlock(nn.Module):
    ...
    def forward(self, x, pos_emb):
        ...
        # Skip V projection: pass x directly as the value tensor, broadcast
        # across heads. All heads share the same E-dim value vector; per-head
        # differentiation comes from the q/k attention pattern only.
        v = x.unsqueeze(1).expand(B, H, T, E).contiguous()                  # [B, H, T, E]
        ...
        out_in = attn.permute(0, 2, 1, 3).reshape(B * T, H * E)
...
print(f'V MHLut:         SKIPPED (v = x broadcast across heads)')
_tph_str = str(_OUT_TPH_PER_LAYER) if _OUT_TPH_PER_LAYER is not None else str(cfg['out_tph'])
print(f'out_proj MHLut:  in_nap={cfg["out_input_nap"]} tph={_tph_str} (input=H*E={H*E}, n_outputs=E={E}) + LayerNorm')
