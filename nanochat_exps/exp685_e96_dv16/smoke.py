#!/usr/bin/env python3
"""Smoke test: exp685 — exp684 dual-stream at E=96, d_v=16."""
...
N_L = cfg['num_layers']
assert E == 96 and D == 384 and d_v == 16
...
assert vl.weights.numel() == 3_145_728
...
# Bandwidth
bw_qkv = 2 * H * cfg['qkv_tph'] * 2*d_qk * 2
bw_v   = 2 * H * cfg['v_tph']   * d_v    * 2
bw_op  = 2 * 1 * cfg['out_tph'] * E      * 2
bw_rl  = 2 * 1 * cfg['residual_tph'] * D * 2
bw_per_layer = bw_qkv + bw_v + bw_op + bw_rl
print(f'\nBandwidth/layer: qkv={bw_qkv/1024:.0f} KB, v={bw_v/1024:.0f} KB, op={bw_op/1024:.0f} KB, residual={bw_rl/1024:.0f} KB; total per layer = {bw_per_layer/1024:.0f} KB')
print(f'Bandwidth × {N_L} layers = {bw_per_layer*N_L/1024/1024:.2f} MB/token')
