...
def _make_v(layer_idx, seed_offset):
    """V MultiHeadLut: input=E, n_heads=H, n_outputs=d_v (real-valued, V2D'd downstream)."""
    return MultiHeadLut(
        input_dim=E,
        n_heads=H,
        n_outputs=d_v,
...
def _make_out(layer_idx, seed_offset):
    """out_proj MultiHeadLut: input=H*d_v, n_heads=1, n_outputs=E (real-valued + LayerNorm)."""
    return MultiHeadLut(
        input_dim=H * d_v,
        n_heads=1,
        n_outputs=E,
...
class LUTBlock(nn.Module):
    """All-MultiHeadLut block. Q/K and V outputs are real-valued, V2D'd before
    SDPA. out_proj outputs real-valued E-dim, normalized with LayerNorm.
    """
    def __init__(self, layer_idx):
        ...
        # Q/K real-valued vector → ±1 dominance via STE.
        self.qk_v2d = VectorToDominance(d_qk, smooth_mode=False, temperature=canon_t)
        # V real-valued vector → ±1 dominance via STE.
        ...
        # LayerNorm on out_proj's E-dim real-valued output.
    ...
    def forward(self, x, pos_emb):
        ...
        v_dom = self.v_v2d(v_vec)                                      # [B*T, H, P_v]
...
class Model(nn.Module):
    def __init__(self):
        ...
        # Concat-of-layer-outputs unembedder: LN + Linear.
...
print(f'MultiHeadLut modules: {3 * N_LAYERS}')
...
# AdamW with two groups (decay/nodecay). All MultiHeadLut weights are
# nn.Parameter (managed by Adam directly).
...
print(f'out_proj MHLut:  in_nap={cfg["out_input_nap"]} tph={cfg["out_tph"]} (n_outputs=E={E}) + LayerNorm')
