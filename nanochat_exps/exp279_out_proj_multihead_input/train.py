...
def _make_v(layer_idx, seed_offset):
    """V LUT. Backward mode selectable via cfg['v_backward_mode']:
    'ste' (default, multi-alt n_alt=3 — historical default at NAP=8)
    or 'soft' (out_proj-style; only valid when soft@NAP fits memory, i.e. NAP<=6)."""
    mode = cfg.get('v_backward_mode', 'ste')
...
def _make_out_heads(layer_idx, seed_offset):
    """Multi-head INPUT out_proj: H separate per-head LUTs, each input_dim=d_v,
    outputs summed at forward. Each per-head LUT samples anchor pairs only
    within its own d_v slice (mirrors transformer's per-head out_proj structure)."""
    tph_total = _OUT_TPH_PER_LAYER[layer_idx] if _OUT_TPH_PER_LAYER is not None else cfg['out_tph']
    tph_per_head = tph_total // H
    return nn.ModuleList([
        TinyMultiHeadLut(
            ...
            tables_per_head=tph_per_head,
            random_seed=cfg['random_seed'] + seed_offset + h,
            device=DEVICE,
            **_TINY_SOFT_KWARGS,
        )
        for h in range(H)
...
def _make_ffn_lut(layer_idx, seed_offset):
    """FFN LUT: E -> E pointwise transformation between out_proj and the residual bridge.
    Backward mode selectable via cfg['ffn_backward_mode'] (default 'soft')."""
    mode = cfg.get('ffn_backward_mode', 'soft')
    kwargs = _TINY_SOFT_KWARGS if mode == 'soft' else _TINY_MULTIALT_KWARGS
    return TinyMultiHeadLut(
        input_dim=E,
        n_heads=1,
        n_outputs=E,
        n_anchor_pairs=cfg.get('ffn_input_nap', cfg['out_input_nap']),
        tables_per_head=cfg.get('ffn_tph', 256),
        random_seed=cfg['random_seed'] + seed_offset,
        device=DEVICE,
        **kwargs,
    )
...
def _make_residual_lut(layer_idx, seed_offset):
    """E -> D residual projection. Backward mode selectable via cfg['residual_backward_mode']:
    'soft' (default, out_proj-style; soft@NAP=8 blows memory) or 'ste' (multi-alt, v_lut-style)."""
    mode = cfg.get('residual_backward_mode', 'soft')
    kwargs = _TINY_MULTIALT_KWARGS if mode == 'ste' else _TINY_SOFT_KWARGS
    return TinyMultiHeadLut(
        input_dim=E,
        n_heads=1,
        n_outputs=D,
...
class LUTBlock(nn.Module):
    """exp278 block: exp277-style with a SKIP connection around ffn_lut.
    ffn_residual = out_e + ffn_lut(out_e); ln_block_end applied to ffn_residual.
    The LN'd value feeds both the next-block x_lut and residual_lut."""
    def __init__(self, layer_idx):
        super().__init__()
        self.qk_joint     = _make_qk_joint(layer_idx, layer_idx)
        self.v_lut        = _make_v(layer_idx, 200 + layer_idx)
        self.out_proj     = _make_out_heads(layer_idx, 400 + layer_idx * 10)
        self.ffn_lut      = _make_ffn_lut(layer_idx, 500 + layer_idx)
        ...
        # Single LN at block end (post-norm), after ffn_lut. Same LN'd value
        # feeds both the next-block x_lut and residual_lut.
        self.ln_block_end = nn.LayerNorm(E)
    ...
    def forward(self, x, pos_emb):
        ...
        # Multi-head input out_proj: each per-head LUT consumes its own d_v slice.
        # attn is [B, H, T, d_v]; permute to [B, T, H, d_v] then index per head.
        attn_per_head = attn.permute(0, 2, 1, 3).contiguous()         # [B, T, H, d_v]
        out_e = None
        for h, head_lut in enumerate(self.out_proj):
            slice_h = attn_per_head[:, :, h, :].reshape(B * T, d_v)
            head_out = head_lut(slice_h).squeeze(1)                   # [B*T, E]
            out_e = head_out if out_e is None else out_e + head_out   # sum across heads

        # NEW: ffn_lut pointwise E->E with SKIP connection around it
        ffn_residual = out_e + self.ffn_lut(out_e).squeeze(1)          # [B*T, E]

        # Post-norm at block end (after the skip-add); feeds both next-block and residual_lut.
        ffn_out_norm = self.ln_block_end(ffn_residual)                 # [B*T, E]
        x_lut_next   = ffn_out_norm.reshape(B, T, E)                   # -> next block
        r_out        = self.residual_lut(ffn_out_norm).squeeze(1).reshape(B, T, D)
...
class Model(nn.Module):
    def __init__(self):
        ...
        # D-dim token embedding for the residual stream; tied with unembedder.
        self.tok_emb_D = nn.Embedding(VOCAB_SIZE, D)
        self.tok_emb_D.weight.data.uniform_(-0.1, 0.1)
        ...
        # Final LN on the residual stream before the tied unembedder.
    ...
    def forward(self, tokens, targets=None, loss_reduction='mean'):
        x_resid = self.tok_emb_D(tokens)              # (B, T, D)
        x_lut   = self.tok_emb_E(tokens)              # (B, T, E)
        for layer, pos_emb in zip(self.layers, self.pos_embs):
            x_lut, r = layer(x_lut, pos_emb)
            x_resid = x_resid + r                     # accumulate (r is already LN'd via ln_e -> residual_lut)
        x_resid = self.ln_final(x_resid)              # LN before unembed
        # Tied unembedder: single linear x_resid @ tok_emb_D.weight.T
        logits = F.linear(x_resid, self.tok_emb_D.weight)
...
print(f'V        TinyMHLut({cfg.get("v_backward_mode", "ste")}, noise_eps={_NOISE_EPS}): in_nap={cfg["v_input_nap"]} tph={cfg["v_tph"]} d_v={d_v}')
...
print(f'ffn_lut  TinyMHLut({cfg.get("ffn_backward_mode", "soft")}, noise_eps={_NOISE_EPS}): in_nap={cfg.get("ffn_input_nap", cfg["out_input_nap"])} tph={cfg.get("ffn_tph", 256)} n_out=E={E}  [E -> E]')
print(f'residual_lut TinyMHLut({cfg.get("residual_backward_mode", "soft")}, noise_eps={_NOISE_EPS}): in_nap={cfg.get("residual_input_nap", cfg["out_input_nap"])} tph={cfg.get("residual_tph", cfg["out_tph"])} n_out=D={D}  [E -> D]')
print(f'Tied embed/unembed at D={D}; tok_emb_E separate at E={E}; ln_final(D) before unembed')
...
def collect_temperature_specs(model):
    ...
    for li, blk in enumerate(model.layers):
        for slut_name in ('qk_joint', 'v_lut', 'ffn_lut', 'residual_lut'):
        ...
        # out_proj is now a ModuleList of per-head LUTs; tracking head 0 as representative.
        out_proj = getattr(blk, 'out_proj', None)
        if out_proj is not None and len(out_proj) > 0:
            h0 = out_proj[0]
            if getattr(h0, 'learnable_temps', False):
                specs.append((f'L{li}.out_proj_h0.T_soft',
                              (lambda m=h0: float(m.log_soft_score_temp.detach().exp()))))
                specs.append((f'L{li}.out_proj_h0.T_sel',
                              (lambda m=h0: float(m.log_select_temp.detach().exp()))))
    return specs
