"""exp542: ScoreLUT head — logits produced DIRECTLY by the residual LUTs, no
intermediate D-readout matmul.

exp513 backbone, but the residual LUTs emit a SCORE SPACE (n_outputs = m*k,
accumulated across layers) instead of D=384, and the unembedder Linear is replaced
by the ScoreLUT head: each vocab token reads m scores from the score table
S [N, m, k] (one slot per subspace, learned code + weight) and sums them ->
logit. logit(v) = Σ_j w_v[j]·S[j, code_v[j]]. Codes learned via per-token query
latents matched to slot keys (soft-assign fwd -> directed; argmax gather at
deploy). V-side is matmul-free (gather). Dual eval: soft (trained) + hard
(deployable argmax-gather).

vs exp513 Linear head 1.4825 (10-step)/1.4656 (100-step). residual_tph reduced
16 (from 128) so it's ~param-neutral despite the m*k=4096 output; this trades
residual-table count for a big direct score space.
"""
...
from spiky.lutorch.score_lut_head import ScoreLUTHead
...
SC_M = cfg['sc_m']; SC_K = cfg['sc_k']; SC_DZ = cfg.get('sc_d_z', 8)
SCORE_SPACE = SC_M * SC_K            # the residual/score dim (replaces D=384)
RES_TPH = cfg['residual_tph']
...
class LUTBlock(nn.Module):
    def __init__(self, li):
        ...
        # residual LUT now emits the SCORE SPACE (m*k) instead of D=384.
        self.residual_lut = _mk(E, 1, SCORE_SPACE, cfg['residual_input_nap'], RES_TPH, 600 + li)
    ...
    def forward(self, x, cos, sin):
        ...
        xn = xf + oe
        r = self.residual_lut(self.ln_post(xn)).squeeze(1).reshape(B, T, SCORE_SPACE)
...
class Model(nn.Module):
    def __init__(self):
        ...
        self.head = ScoreLUTHead(VOCAB_SIZE, SC_M, SC_K, d_z=SC_DZ,
                                 assign_temp=cfg.get('sc_assign_temp', 1.0),
                                 assign_init_std=cfg.get('sc_assign_init_std', 1.0),
                                 weight_init_noise=cfg.get('sc_weight_init_noise', 0.5),
                                 device=DEVICE, seed=cfg['random_seed'])
        self.rope = RotaryEmbedding(d_qk, CONTEXT_SIZE, base=_ROPE_BASE)
        self.layers = nn.ModuleList([LUTBlock(i) for i in range(N_LAYERS)])
        self.ln_final = nn.LayerNorm(SCORE_SPACE)
    def get_device(self): return self.tok_emb_E.weight.device
    def forward(self, tokens, targets=None, loss_reduction='mean'):
        B, T = tokens.shape
        x_resid = torch.zeros(B, T, SCORE_SPACE, device=tokens.device, dtype=self.tok_emb_E.weight.dtype)
        x_lut = self.tok_emb_E(tokens)
        for layer in self.layers:
            x_lut, r = layer(x_lut, self.rope.cos, self.rope.sin); x_resid = x_resid + r
        S = self.ln_final(x_resid).reshape(B * T, SC_M, SC_K)     # score table
        logits = self.head(S)                                     # [B*T, V]
...
head_params_n = sum(p.numel() for p in model.head.parameters())
print(f'Total params: {n_params:,} | score_space={SCORE_SPACE} (m={SC_M} k={SC_K}) residual_tph={RES_TPH}')
print(f'HEAD ScoreLUT params={head_params_n/1e6:.3f}M (deploy=codes+weights, matmul-free gather) vs Linear {VOCAB_SIZE*384/1e6:.1f}M | {model.head}')
...
adam_groups = [
    ...
    dict(params=head_params, lr=cfg['adam_lr'], betas=(0.9, 0.95), eps=1e-8, weight_decay=0.0),
]
...
def eval_bpb(hard):
    model.head.hard = hard; model.eval()
    bpb = evaluate_bpb(model, val_loader_factory(), EVAL_STEPS, token_bytes)
    model.train(); model.head.hard = False
    return bpb
...
csv_w = csv.writer(csv_f); csv_w.writerow(['step', 'train_loss', 'val_bpb', 'val_bpb_hard', 'code_usage'])
val_bpbs, val_bpbs_hard, val_steps, train_losses = [], [], [], []
ema = None; best_bpb = float('inf'); best_bpb_hard = float('inf'); t0 = time.time()
...
for step in range(1, N_STEPS + 1):
    ...
    if step % 100 == 0 or step == 1:
        print(f'step {step:6d} | ce_ema={ema:.4f} | code_use={model.head._code_usage:.3f} | lr={lr_scale*cfg["adam_lr"]:.2e}')
    ...
    if step % EVAL_EVERY == 0 or step == N_STEPS:
        bpb = eval_bpb(False); bpb_hard = eval_bpb(True)
        best_bpb = min(best_bpb, bpb); best_bpb_hard = min(best_bpb_hard, bpb_hard)
        print(f'[VAL] step {step}: bpb={bpb:.4f} | bpb_hard={bpb_hard:.4f} | code_use={model.head._code_usage:.3f}')
        val_bpbs.append(bpb); val_bpbs_hard.append(bpb_hard); val_steps.append(step); train_losses.append(ema)
        csv_w.writerow([step, f'{ema:.6f}', f'{bpb:.6f}', f'{bpb_hard:.6f}', f'{model.head._code_usage:.4f}']); csv_f.flush()
...
ax2.plot(val_steps, val_bpbs, color='red', label='soft'); ax2.plot(val_steps, val_bpbs_hard, color='purple', ls='--', label='hard')
ax2.axhline(1.4825, ls=':', color='gray', label='exp513 Linear'); ax2.set(xlabel='step', ylabel='bpb', title='val bpb'); ax2.legend(); ax2.grid(True)
plt.tight_layout(); plt.savefig(os.path.join(EXP_DIR, 'loss.png'), dpi=120); plt.close(fig)
summary = dict(exp_name=cfg['exp_name'], best_val_bpb=best_bpb, best_val_bpb_hard=best_bpb_hard,
