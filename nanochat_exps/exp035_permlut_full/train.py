...
# --- LUT block helpers --------------------------------------------------------
# PermutationalLut (fp32, smooth_mode=True, n_alternatives=1) variant. Drop-in
# replacement for the BitPermutationLUT factories: same I/O contract (returns
# pair-dominance scores [B*T, H, P]) when return_dominance=True. Anchor sampling
# uses CANONICAL_FULL_COVERAGE for parity with bit-LUT defaults.
from spiky.lutorch.lut_helpers import AnchorSamplingPolicy

_PERMLUT_KWARGS = dict(
    pair_mode='scrambled',
    soft_mode='rational',
    aggregation='matmul',
    ...
    initial_weights_noise=cfg.get('lut_init_std', 0.001),
)
...
n_lut_modules = sum(1 for layer in model.layers for _ in (layer.q_perm, layer.k_perm, layer.v_perm, layer.out_proj))
n_lut_params = sum(p.numel() for layer in model.layers
                   for m in (layer.q_perm, layer.k_perm, layer.v_perm, layer.out_proj)
                   for p in m.parameters())
print(f'PermutationalLut modules: {n_lut_modules}')
print(f'PermutationalLut params:  {n_lut_params:,}')
print(f'Adam-managed (all) params: {adam_param_count:,}')
...
print(f'Q/K PermLut: in_nap={cfg["qk_input_nap"]} out_nap={cfg["qk_output_nap"]} tph={cfg["qk_tph"]} d_qk={d_qk} P_qk={D_QK_P}')
_v_tph_str = str(_V_TPH_PER_LAYER) if _V_TPH_PER_LAYER is not None else str(cfg['v_tph'])
print(f'V PermLut:   in_nap={cfg["v_input_nap"]} out_nap={cfg["v_output_nap"]} tph={_v_tph_str} d_v={d_v} P_v={D_V_P}')
_tph_str = str(_OUT_TPH_PER_LAYER) if _OUT_TPH_PER_LAYER is not None else str(cfg['out_tph'])
print(f'Out PermLut: in_nap={cfg["out_input_nap"]} out_nap={cfg["out_output_nap"]} tph={_tph_str}')
...
summary = {
    ...
    'n_lut_modules': n_lut_modules,
    'n_lut_params': n_lut_params,
