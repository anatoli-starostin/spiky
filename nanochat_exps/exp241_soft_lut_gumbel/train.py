"""nanochat_exps/exp241_soft_lut_gumbel — fork of exp240, switch to Gumbel softmax.

Two changes vs exp240:
  1) gumbel=True (use F.gumbel_softmax for the row selector). Adds Gumbel(0,1)
     noise to the row scores before the temperature-scaled softmax, giving a
     stochastic-but-differentiable row sample.
  2) T_sel 0.5 -> 0.1. With Gumbel noise in the logits, T_sel=0.1 produces
     near-one-hot but stochastic samples — exploration through noise rather
     than through softmax breadth. The reparameterised sample lets gradient
     still flow back to the underlying logits.

Architecture (matches exp234 / exp240): nap=8 V, all SoftMultiHeadLUT.
Carried: bf16 autocast + torch.compile.

Launch:
    PYTHONPATH=/home/starost/nanochat \\
        /home/starost/spiky/.venv/bin/python -u \\
        nanochat_exps/exp241_soft_lut_gumbel/train.py \\
        > nanochat_exps/exp241_soft_lut_gumbel/stdout.log 2>&1 &
"""
