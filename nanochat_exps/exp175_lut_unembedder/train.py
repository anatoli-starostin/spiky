...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        # Block output: drop V2D/D2V rank-canonicalization, keep LayerNorm.
        # Magnitudes flow through; LN normalizes scale/center per token.
    ...
    def forward(self, x, pos_emb):
        ...
        out = self.out_ln(out_real).reshape(B, T, E)                   # [B, T, E]
        return out


class LutUnembedder(nn.Module):
    """LayerNorm → TinyMultiHeadLut (built-in sparse scatter) → +bias → vocab.

    Hard-lookup LUT (n_alternatives=1, no smooth_mode). Each table reads `nap`
    anchor pairs from the LN'd concat, looks up `n_sparse` outputs per table,
    then scatter-adds into the vocab-size output. Trainable per-vocab bias.
    """
    def __init__(self, in_dim, vocab_size, tph, nap, n_sparse, seed=42):
        super().__init__()
        self.in_dim = in_dim
        ...
        self.lut = TinyMultiHeadLut(
            ...
            anchor_sampling_policy=AnchorSamplingPolicy.CANONICAL_FULL_COVERAGE,
            random_seed=seed,
            initial_weights_noise=cfg.get('unembed_init_std', 0.1),
    ...
    def forward(self, x):
        # x: [..., in_dim]
        orig_shape = x.shape
        x_flat = x.reshape(-1, self.in_dim)        # [B*T, in_dim]
        x_flat = self.ln(x_flat)
        out = self.lut(x_flat)                     # [B*T, 1, vocab]
        out = out.squeeze(1) + self.bias           # [B*T, vocab]
        return out.reshape(*orig_shape[:-1], self.vocab_size)
...
class Model(nn.Module):
    def __init__(self):
        ...
        self.unembedder = LutUnembedder(
            in_dim=concat_dim,
            vocab_size=VOCAB_SIZE,
            tph=cfg.get('unembed_tph', 4096),
            nap=cfg.get('unembed_nap', 8),
            n_sparse=cfg.get('unembed_n_sparse', 128),
            seed=cfg['random_seed'] + 999,
        )
