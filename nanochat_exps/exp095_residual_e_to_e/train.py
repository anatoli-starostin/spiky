...
def _make_residual(layer_idx, seed_offset):
    """Residual LUT: E -> E, nap=RES_NAP, tph=RES_TPH (fixed across layers)."""
    return TinyMultiHeadLut(
        input_dim=E,
        n_heads=1,
        n_outputs=E,
        n_anchor_pairs=RES_NAP,
        tables_per_head=RES_TPH,
        random_seed=cfg['random_seed'] + seed_offset,
        device=DEVICE,
        **_TINY_KWARGS,
    )
...
class LUTBlock(nn.Module):
    ...
    def forward(self, x, pos_emb):
        ...
        res_out = self.residual_lut(x.reshape(B * T, E)).squeeze(1).reshape(B, T, E)
...
print(f'residual MHLut:  in_nap={RES_NAP} tph={RES_TPH} (E={E} -> E, fixed across layers, no concat, no inheritance)')
