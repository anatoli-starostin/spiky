"""exp759 — resume exp756 + 8K more steps, STANDARD warmup+cosine schedule.

Loads exp756's step-16000 checkpoint (E=192 hard, 1.2301) and continues
training for 8K more steps at eff bs=48 with the standard nanochat
schedule: warmup_fraction=0.1 (800 steps), then cosine from 1.0 to 0.1.
...
def get_lr_scale(step):
    # exp759: standard nanochat schedule (warmup + cosine) for 8K steps.
    n = N_STEPS
    w = int(WARMUP_FRAC * n)
    if step < w:
        return step / max(w, 1)
