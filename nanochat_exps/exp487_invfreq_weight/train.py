"""nanochat_exps/exp487_invfreq_weight — fork of exp475 (bs=16) + inv-freq loss weighting.
...
# --- Inverse-frequency token loss weighting -----------------------------------
# Upweight rare-token loss to feed rare-token LUT paths more gradient per step
# (tests the bs=48 'rare tokens improve most' finding at bs=16, 1x tokens).
# w(t) = (smoothed_freq(t))^(-alpha), capped, normalized so the FREQUENCY-WEIGHTED
# mean weight = 1 (preserves loss scale; eval bpb stays standard/unweighted).
_FREQ_ALPHA = cfg.get('freq_weight_alpha', 0.5)
_FREQ_CAP   = cfg.get('freq_weight_cap', 5.0)
TOKEN_WEIGHT = None
if _FREQ_ALPHA > 0.0:
    _freq_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'token_freq.pt')
    _counts = torch.load(_freq_path).float().to(DEVICE)          # [V]
    _p = (_counts + 1.0) / (_counts + 1.0).sum()                 # add-1 smoothed freq
    _w = _p.pow(-_FREQ_ALPHA)
    _w = _w / (_p * _w).sum()                                    # E_freq[w] = 1
    _w = _w.clamp(max=_FREQ_CAP)
    _w = _w / (_p * _w).sum()                                    # renormalize after cap
    TOKEN_WEIGHT = _w
    print(f'inverse-freq weighting: alpha={_FREQ_ALPHA} cap={_FREQ_CAP} | '
          f'w range [{_w.min().item():.3f}, {_w.max().item():.3f}] | '
          f'freq-weighted mean={(_p*_w).sum().item():.4f}')
...
class Model(nn.Module):
    ...
    def forward(self, tokens, targets=None, loss_reduction='mean', token_weight=None):
        B, T = tokens.shape
        ...
        if targets is not None:
            ...
            if token_weight is not None:
                # inverse-frequency weighted CE; per-batch normalized so the
                # weighted mean preserves loss scale. valid = (tv != -1).
                ce = F.cross_entropy(logits.view(-1, logits.size(-1)), tv,
                                     reduction='none', ignore_index=-1)
                valid = (tv != -1)
                w = token_weight[tv.clamp(min=0)] * valid.float()
                return (ce * w).sum() / w.sum().clamp(min=1e-9)
            return F.cross_entropy(
                logits.view(-1, logits.size(-1)), tv,
...
for step in range(1, N_STEPS + 1):
    ...
    for _ in range(grad_accum):
        ...
        loss = model(x, targets=y, token_weight=TOKEN_WEIGHT)
