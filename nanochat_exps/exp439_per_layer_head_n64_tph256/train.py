...
RES_N_OUT   = cfg.get('residual_n_out', D)  # per-layer residual_lut output dim
_HEAD_BIAS  = cfg.get('per_layer_head_bias', False)
...
class LUTBlock(nn.Module):
    ...
    def forward(self, x, cos, sin):
        ...
        # Post-norm fed into residual_lut. Output dim is RES_N_OUT (per-layer
        # private channels into vocab logits via a dedicated head).
        r_in   = self.ln_post(x_lut_next_flat)          # [B*T, E]
        r_out  = self.residual_lut(r_in).squeeze(1).reshape(B, T, RES_N_OUT)
...
class Model(nn.Module):
    def __init__(self):
        ...
        # Per-layer: LayerNorm on the RES_N_OUT residual_lut output, then its own
        # head to vocab. Final logits = sum_i head_i(LN_i(r_i)).
        self.head_lns = nn.ModuleList([nn.LayerNorm(RES_N_OUT) for _ in range(N_LAYERS)])
        self.head_layers = nn.ModuleList([
            nn.Linear(RES_N_OUT, VOCAB_SIZE, bias=_HEAD_BIAS) for _ in range(N_LAYERS)
        ])
        if _HEAD_BIAS:
    ...
    def forward(self, tokens, targets=None, loss_reduction='mean'):
        ...
        for i, layer in enumerate(self.layers):
            ...
            r_norm = self.head_lns[i](r)
            layer_logits = self.head_layers[i](r_norm)
...
print(f'residual_lut TinyMHLut(soft, noise_eps={_NOISE_EPS}): in_nap={cfg["residual_input_nap"]} tph={cfg["residual_tph"]} n_out=RES_N_OUT={RES_N_OUT}  [E -> {RES_N_OUT}]')
print(f'PER-LAYER HEADS: {N_LAYERS} x Linear({RES_N_OUT}, V={VOCAB_SIZE}, bias={_HEAD_BIAS}); per-layer LayerNorm({RES_N_OUT}) before each head; logits = sum_i head_i(LN_i(r_i)); tok_emb_E at E={E}; RoPE base={_ROPE_BASE}')
