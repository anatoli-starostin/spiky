...
(function () {
  ...
  // Pages deployment ships only the clean + evolved datasets, so clean is the DEFAULT
  // (bare inspector.html works); ?v=evolved selects the neuroevolution result.
  const VARIANT = _v.startsWith('evo') ? 'evolved' : _v.startsWith('grad') ? 'graded'
    : _v.startsWith('lat') ? 'latency' : _v.startsWith('coinc') ? 'coincidence' : 'clean';
