...
def _test_andn_layer_backward_hebb(
    ...
    conf_name = f'device_{device}_sum_dt_{summation_dtype}_antihebb_cf_{anti_hebb_coeff}'
    img.save(f'andn_layer_backward_hebb_test_{conf_name}_color_rescale_{color_rescaler:.2}.png')
...
def main():
    print("=" * 60)
    print("ANDNLayer BACKWARD HEBB TEST")
    print("=" * 60)
    ...
    for device in devices:
        for summation_dtype in [torch.float32, torch.int32]:
            ...
            success = test_andn_layer_backward_hebb(device, summation_dtype)
