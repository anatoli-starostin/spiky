...
class LUTTransformerNoResidualsExp(nn.Module):
    def _create_single_attention(
            self, input_dim, output_dim, _synapse_meta, summation_dtype, _int_rescaler, seed,
        ...
        if isinstance(input_dim, int):
            ...
            return Conv2DLUTLayer(
                ...
                output_kernel_shape=output_dim,
                sequence_length=self.context_size,
                concatenation_product=self.concatenation_product,
                sliced_product_mode=self.sliced_product_mode,
                positional_dim=p_dim,
                use_sinusoidal_pe=p_dim > 0 and self.use_sinusoidal_pe,
                weights_gradient_policy=self.weights_gradient_policy,
                receptive_field_shape=input_dim,
                receptive_field_stride_shape=input_dim,
                synapse_meta=_synapse_meta,
                shared_context=self.lut_shared_context,
                summation_dtype=summation_dtype,
                random_seed=seed,
    ...
    def __init__(
        ...
        if isinstance(embedding_dim, int):
            initial_n_embeddings = embedding_dim
        else:
            assert len(embedding_dim) == 2
            initial_n_embeddings = embedding_dim[0] * embedding_dim[1]

        self.token_embedder = nn.Embedding(vocab_size, initial_n_embeddings, device=device)
        ...
        for layer_idx in range(num_layers):
            ...
            if use_batch_norm:
                layer['attention_bn'] = nn.BatchNorm1d(output_dim, device=device)
            else:
            ...
            if layer_norm_d is not None:
                ...
                layer['attention_ln'] = nn.LayerNorm(
                    (context_size, output_dim) if layer_norm_d == 2 else output_dim, device=device
                )
            else:
    ...
    def get_profile_statistics(self) -> str:
        ...
        for layer in self.layers:
            ...
            # FFN is always a single LUTLayer (if it exists)
            if not self.no_ffn:
