...
def _lut_tables(m):
    """The LUT weight tensors, across both modes (joint has one FastMHL, independent has one per head)."""
    return [m.lut.weights] if hasattr(m, "lut") else [lut.weights for lut in m.luts]
