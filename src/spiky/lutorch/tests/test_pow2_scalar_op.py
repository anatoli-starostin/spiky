...
Gates:
  drift     the op's integers == the fused eval kernel's integers (both call p2::table_scalars, csrc/pow2_scalars.cuh), on
  ...
  train=int the quant_mode training forward equals the integer read (LightMultiHeadLUT.forward_int) BIT FOR BIT on CUDA fp32,
            and the artefact's off / cells / fused paths agree bit for bit
...
def test_training_forward_equals_integer_read_bit_for_bit():
    ...
    outs = {}
    for mode in ("off", "cells", "fused"):
        art.kernel, art.kernel_min_tokens = mode, 0
        outs[mode] = art(x)
    assert torch.equal(outs["off"], outs["cells"]) and torch.equal(outs["cells"], outs["fused"])
