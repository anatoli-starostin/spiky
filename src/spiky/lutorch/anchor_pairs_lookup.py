...
class AnchorPairsLookup(nn.Module):
    ...
    Args:
        ...
        anchor_candidates: Optional tensor of shape [n_tables, max_anchors_per_table] with input indices
                          (all values must be >= 0, no padding). When provided, balanced anchor pairs
                          are drawn per table from these candidates via `get_balanced_anchor_pairs`.
                          When None, balanced coverage is computed over the full input dimension.
        ...
        if use_native_eval_cuda:
            ...
            if return_alternatives:
                if self.n_alternatives == self.n_anchor_pairs:
                    # Prefer generic CUDA path when using all positions as alternatives.
                    lookup_indices, lookup_alt_indices, lookup_alt_deltas, _, _ = native.anchor_pairs_lookup_forward_all(
                        x,
                        anchor_pairs_a,
                        anchor_pairs_b,
                        float(self.cmp_eps),
