...
//   * p2_scalar_kernel  -- the forward of the spiky_lutorch::p2_scalars custom op (training forward and the "cells" eval
//                          regime), and
//   * p2_int8_kernel    -- the "fused" eval regime, inline in the same launch as the int8 accumulation.
// Because both call this one function on the same margins d = z[a] - z[b], training, cells and fused take the same c1, c2,
...
//   s = S exp(g + gamma ls)            (the score itself; not an output of either kernel -- the op's backward recomputes it)
#pragma once
...
struct TableScalars {
  ...
  bool skip, drop;
  float s, ls;              // score s and sum logsigmoid(beta m)
};
