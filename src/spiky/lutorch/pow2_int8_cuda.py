"""Fused CUDA kernel for the int8 power-of-two LUT read (QuantisedLightFFN eval) -- opt-in, hardware-gated, never raises.

csrc/pow2_int8_read.cu computes the note's Section 6 integer read for every (token, head) in one launch: each cell's int8
row is loaded 16 bytes at a time (int4 vector loads), every byte sign-extended to int32 in registers, shifted by k' + 6 or
k' + 6 - q and added into int32 accumulators that stay in registers across the table loop; skipped tables and dropped
second cells are excluded; the accumulators are converted to float once. Two regimes:
...
build, SPIKY_P2_CUDA_DISABLE=1 -- makes `load()` return None and callers keep the PR 1 compiled path.
"""
import os
...
def pack_cells(idx: torch.Tensor, q: torch.Tensor, k: torch.Tensor, skip: torch.Tensor, drop: torch.Tensor) -> torch.Tensor:
    """Cells for the "cells" regime: uint8 [N, H, T, 3] = (c1, c2, sh1 | sh2 << 4), sh = discard code 15 for a cell that is
    not read. idx [N, H, T, 2] are cell numbers inside their table (0..2^NAP - 1, no table offset). Compile-friendly."""
    group = pow2_read.shift_groups(q, k, skip, drop)                       # [.., 2], discard = N_SHIFTS (11)
    sh = torch.where(group == pow2_read.N_SHIFTS, torch.full_like(group, 15), group)
...
def read_cells(tables_stride: torch.Tensor, cells: torch.Tensor, n_anchor_pairs: int, D: int, lo: int, hi: int, Q: int,
    ...
    """The int32 accumulators (as float32, units of 2^-6) [N, H, D] from torch-computed cells."""
