...
SPIKING = os.path.dirname(HERE)
RESULTS = os.path.join(SPIKING, "results")
...
# name -> (history json, checkpoint npz or None, pool size K, d_max)
EXPERIMENTS = {
    "k32_baseline": ("steady_state_stdp_k32_b64_lr0.01_partial136.json", None, 32, 20),
    "k128":         ("steady_state_stdp_k128.json", "ck_k128.npz", 128, 20),
    "small8020":    ("steady_state_small8020.json", "ck_small8020.npz", 32, 20),
    "delay148":     ("steady_state_delay148.json", "ck_delay148.npz", 32, 48),
}
...
def available():
    ...
    out = {}
    for name, (hist, ck, k, dmax) in EXPERIMENTS.items():
        hp = os.path.join(RESULTS, hist)
        if os.path.exists(hp):
            cp = os.path.join(RESULTS, ck) if ck else None
