...
This does not test our training code — it tests exactly what nucstar will ship:

  1. builds a throwaway package that mirrors the server layout
     (actors/base.py + actors/fastlut_lse.py + models/<npz>),
...
Usage:  python verify_deploy.py [--episodes 20]
"""
import argparse
import os
import shutil
...
NPZ = "walker2d_fastlut_lse_exp19.npz"
...
def build_fake_server(tmp):
    ...
    shutil.copy(os.path.join(DEPLOY, "fastlut_lse.py"), os.path.join(pkg, "fastlut_lse.py"))
...
def main():
    ...
    from actors.fastlut_lse import FastLutLSEActor            # noqa: E402
    ...
    actor = FastLutLSEActor(env.action_space)
    print(f"actor name: {actor.name!r}   tau {actor.tau:.6f}   tables {actor.T}   "
          f"nap {actor.nap}")
