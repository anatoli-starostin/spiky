#!/usr/bin/env bash
# exp22: STACKED LIFLayer actor (17->32->64->6, rescale/rescale/log) + exp19-style MLP-exp critic.
# 3 seeds in PARALLEL. Results -> exp22_liflayer-32-64-mlpexpcrit/.  (arch takes no --tables-per-head.)
set -uo pipefail
...
D="$BASE/exp22_liflayer-32-64-mlpexpcrit"
mkdir -p "$D"
