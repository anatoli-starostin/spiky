"""Live Slack progress monitor for exp20 (single group, 3 seeds in parallel):
LIF-detector LUT actor (tph32, n_buckets64) + exp19-style MLP-exp critic. Read-only over the
run logs. Bound to BODY_TASK 3e5c58fc's thread."""
...
D = f"{BASE}/exp20_lif-actor-mlpexpcrit"
TASK = "3e5c58fc"
SEEDS = [0, 1, 2]
TOTAL = 768
WALL_MIN = 40.0             # LIF actor is compute-heavier; generous anchor for the ETA
...
def main():
    h = progress.progress_start("PPO exp20 · LIF-detector actor (tph32,M64) + exp19 MLP-exp critic (3 seeds)",
