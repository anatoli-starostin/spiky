#!/usr/bin/env bash
# 3 PPO seeds IN PARALLEL, 768 updates (2x bench3). Writes bench5/.
...
( while true; do nvidia-smi --query-gpu=utilization.gpu,memory.used --format=csv,noheader,nounits; sleep 1; done > bench5/agg.gpu 2>/dev/null ) &
SAMP=$!

start=$SECONDS
pids=()
for s in 0 1 2; do
  $PY -u train.py --algo ppo --arch mlp --envs 8192 --graph --updates 768 --lr-schedule cosine --seed "$s" \
      --out "bench5/ppo_s${s}.json" > "bench5/ppo_s${s}.log" 2>&1 &
  pids+=("$!")
done
