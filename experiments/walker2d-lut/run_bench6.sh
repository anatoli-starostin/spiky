#!/usr/bin/env bash
# 3 PPO seeds IN PARALLEL, 768 updates (2x bench3). Writes bench6/.
...
( while true; do nvidia-smi --query-gpu=utilization.gpu,memory.used --format=csv,noheader,nounits; sleep 1; done > bench6/agg.gpu 2>/dev/null ) &
SAMP=$!

start=$SECONDS
pids=()
for s in 0 1 2; do
  $PY -u train.py --algo ppo --arch mlp --envs 8192 --graph --updates 768 --lr-schedule cosine --lr-min 1e-5 --logstd-min -1.897 --ent 0.005 --seed "$s" \
      --out "bench6/ppo_s${s}.json" > "bench6/ppo_s${s}.log" 2>&1 &
  pids+=("$!")
done
