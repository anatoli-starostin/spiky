...
> **Two caveats before comparing this table to the paper.**
>
> 1. **These are `final_val_bpb`; the paper's Table 4 quotes `best_val_bpb`.** The two
>    differ for several runs, so the numbers here are slightly *higher* than the paper's
>    for `0118` (1.17460 vs 1.17431), `0121` (1.19146 vs 1.19142), `0129` (1.18148 vs
>    1.18074), `0130` (1.19405 vs 1.19363), `0133` (1.17961 vs 1.17910) and `0137`
>    (1.19448 vs 1.19432). Neither is wrong; they are different conventions. The paper
>    is authoritative for anything quoted from it.
> 2. **Rows for `0120`, `0125` and `exp073` are not cited by the paper** and their
>    experiment dirs are therefore not part of this folder. They are kept here as
>    historical context from the full research grid.
