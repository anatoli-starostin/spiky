...
# HERE = <repo>/experiments/ffn_replacement/benchmark, so the repo root is three up
...
def family_of(m):
    """'fastmhl' (CompressionMHL/anchor-pair) or 'dense'.

    Dispatching on this matters: gather.patch() matches only FastMultiHeadLut and
    would silently patch NOTHING on a model of another family, reporting an
    unoptimized run as if it were optimized.
    """
    if M.lut_modules(m):
        return 'fastmhl'
    return 'dense'
...
def main():
    ...
    ap.add_argument('--root',
                    default=os.path.join(REPO, 'experiments', 'ffn_replacement', 'runs'))
    ...
    fam = family_of(ref)
    n_slots = len(luts) if fam == 'fastmhl' else 0
