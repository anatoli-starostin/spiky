#!/usr/bin/env python3
"""v2 (after the four library kernel steps of task 611335da): config B is measured BEFORE (the pre-optimisation kernel,
fig_0127_vs_int8/kernel_before_step0/, casts as torch launches) and AFTER (the library, bf16 code in / bf16 accumulators out),
with every stage timed, in the same interleave as every other row.
...
def main():
    ...
    KB = os.path.join(HERE, 'fig_0127_vs_int8', 'kernel_before_step0')
    ext_before = _load(name='p2snap_fig_step0', sources=[os.path.join(KB, 'pow2_int8_read.cu')],
                   extra_cuda_cflags=['-O3', '-std=c++20', '--fmad=false'], extra_cflags=['-O3', '-std=c++20'], verbose=False)
    e8 = torch.empty(0, device='cuda', dtype=torch.uint8)
    bcnt = {'before ext.read': 0}

    def read_before(z):
        bcnt['before ext.read'] += 1
        return ext_before.read(z, kc['anchor_a'], kc['anchor_b'], e8, kc['tables'], z.shape[0], H, T, NAP, 1 << NAP, Din, D, lo, hi, Q,
                           *kc['scalars'], 64, True, True)

    def b0_slot():                                      # BEFORE: pre-optimisation kernel, casts as torch launches
        z = F.linear(xq16, cw16, cb16).view(N, H, Din).float()
        return F.linear(read_before(z).reshape(N, H * D).to(BF), dw16, db16).reshape(B_, SEQ, C)

    def b4_slot():                                      # AFTER: library read on the bf16 code, bf16 accumulators out
        z = F.linear(xq16, cw16, cb16).view(N, H, Din)
        return F.linear(read(z).reshape(N, H * D), dw16, db16).reshape(B_, SEQ, C)
    ...
    with torch.no_grad():
        ...
        accb32 = read_before(zbf).reshape(N, H * D).contiguous()
        accb16 = accb32.to(BF).contiguous()
        acc4_16 = read(zb16).reshape(N, H * D).contiguous()
        assert acc4_16.dtype == BF and torch.equal(acc4_16, accb16)
    ...
    stages = {
        ...
        'B0': {'compress 384->192': lambda: F.linear(xq16, cw16, cb16), 'dtype casts': lambda: zb16.float(),
               'int8 read (kernel)': lambda: read_before(zbf), 'dtype casts 2': lambda: accb32.to(BF),
               'decompress 192->384': lambda: F.linear(accb16, dw16, db16)},
        'B4': {'compress 384->192': lambda: F.linear(xq16, cw16, cb16),
               'int8 read (kernel)': lambda: read(zb16),
               'decompress 192->384': lambda: F.linear(acc4_16, dw16, db16)},
    ...
    per = {}
    with torch.no_grad():
        for k in ('P127_32', 'P127_16', 'Q32', 'B0', 'B4', 'T16'):
            before = dict(cnt)
            for _ in range(10):
                rows[k]()
    ...
    with torch.no_grad():
        chk['B4_slot_eq_B0_slot'] = bool(torch.equal(rows['B4'](), rows['B0']()))
        n0 = bcnt['before ext.read']
        for _ in range(10):
            rows['B0']()
        chk['B0_before_kernel_reads_per_10'] = bcnt['before ext.read'] - n0
    gate = gate and chk['B4_slot_eq_B0_slot']
    chk['gate_pass'] = gate
    res['checks'] = chk
    V.log(f'[{ARGS.precision}/{ARGS.tag}] checks {json.dumps(chk)}')
    if not gate:
        V.log('correctness gate FAILED: refusing to time')
        json.dump(res, open(os.path.join(V.OUT_DIR, f'fig0127v2_{ARGS.precision}_{ARGS.tag}.json'), 'w'), indent=2, default=str)
        return 2
