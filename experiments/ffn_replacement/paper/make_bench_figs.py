...
# committed results.json files under ../benchmark/paper_timings{,_h100}/ -- the same files the
# timing harness writes, read directly so the figure tracks them. (Upstream this read a
# duplicated bench_data/ directory; those JSONs were byte-identical to these, so the copies were
# dropped in favour of the single source of truth.) The two JSONs use DIFFERENT schemas; both are
# normalised here to the SAME horizontal stacked-bar layout. Every stage value is read from the
# committed data -- nothing is invented (the only derived quantity is "other" = slot total minus
# the summed named stages, which is ~0 for the H100 routed bars).
# NOTE: reads only the committed JSON -- no checkpoints and no GPU are needed to redraw this.
BASE = os.path.dirname(os.path.abspath(__file__))
BENCH = os.path.join(os.path.dirname(BASE), "benchmark")
J5090 = os.path.join(BENCH, "paper_timings", "results.json")
JH100 = os.path.join(BENCH, "paper_timings_h100", "results.json")
