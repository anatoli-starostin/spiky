...
# Per-step eval bpb for the super-long 144k-step run and its matched vanilla baseline, read
# straight from each run's committed metrics.csv (columns: step, train_loss, val_bpb) under
# ../runs/, so the figure tracks the run data rather than a private copy of it. (Upstream this
# read a duplicated superlong_data/ directory; those CSVs were byte-identical to these, so the
# copies were dropped in favour of the single source of truth.)
#   runs/exp_n_0158_long144k_from_0127/metrics.csv  (LUT, 0127 architecture)
#   runs/exp_n_0157_long144k_vanilla/metrics.csv    (untied dense baseline)
BASE = os.path.dirname(os.path.abspath(__file__))
ROOT = os.path.join(os.path.dirname(BASE), "runs")
RUNS = [
    ("exp_n_0157_long144k_vanilla",   "vanilla (exp_n_0157)",       "#111111", "--"),
    ("exp_n_0158_long144k_from_0127", "LUT exp_n_0158 (0127 arch)", "#1D4ED8", "-"),
]
...
lutf = curve("exp_n_0158_long144k_from_0127")[1][-1]
vanf = curve("exp_n_0157_long144k_vanilla")[1][-1]
