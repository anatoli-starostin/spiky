...
## Table-COUNT capacity at H8 (0212) — doubling tph HELPS (unlike doubling cells at H4)

**exp_g_0212** = 0207 with **tph 64→128 (n_tables 512→1024, table params doubled)**, constant cell, everything else 0207. Final **1.126865** (best 1.126549, 4.88h; run on the worker VM). σ=0.00335.
- **−1.32σ vs its constant twin 0207 (1.131286)** — doubling the table COUNT clearly HELPED, and 0212 is the **overall best LUT arm** (1.126865, **+3.42σ vs dense GELU** — the smallest gap in the survey, edging out 0213 gated_affine's 1.128510 by −0.49σ and 0206 by −1.05σ).
- **This refines the earlier "capacity is not a lever" reading.** That came from 0204 (nap9, double CELLS at H4) which *hurt* (+2.21σ). But at the better H8 full-rank geometry, doubling table COUNT (more independent tables/experts) *helps*. So capacity IS a lever — as more tables, at H8 — even though more cells-per-table at H4 was not. The effect is geometry- and axis-dependent: widen the ensemble (tph, at full-rank H8), don't deepen each table (nap).
- Best LUT arms now, all n=1 pair at H8 full-rank: **0212 (2× tables) 1.126865 < 0213 (gated cell) 1.128510 < 0206/0207 ~1.130**. All still +3.4–3.9σ over dense. Detail: `exp_g_0212_.../` (worker VM).
