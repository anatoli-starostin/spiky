...
# --- spiky-source override (exp_n_0192 repro of exp_g_0191; host-portability only) --------
# This host's editable `spiky` install points at ~/projects/spiky/src (branch
# hyperplane_ffn_next), which predates lut_z_norm / the current compression_mhl. This config
# needs lut_impl="light" + lut_z_norm=true, which only exist on research/ffn_replacement_fix.
# So we import spiky from THIS worktree's src instead of the editable install, by prepending
# it to sys.path and dropping the editable finder BEFORE any spiky import. This is the ONLY
# change vs gpustar's exp_g_0191 train.py; it does not touch model/optimizer/eval logic.
_WT_SRC = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..', '..', 'src'))
if os.path.exists(os.path.join(_WT_SRC, 'spiky', 'lutorch', 'light_multi_head_lut.py')):
...
def setup_optimizer(model, lr, weight_decay, tables_no_decay=False):
    ...
    exempt = ((FastMultiHeadLut, LightMultiHeadLUT) if tables_no_decay
