...
# (`git show f1dd5e9d:experiments/ffn_replacement/runs_corrected/exp_n_0157_long144k_vanilla/train.py`).
# =============================================================================
...
if os.environ.get('SMOKE'):
    print("tied? lm_head.weight is tok_emb.weight:", model.head.weight is model.tok_emb.weight, "| same data_ptr:", model.head.weight.data_ptr()==model.tok_emb.weight.data_ptr()); print("SMOKE OK"); sys.exit(0)
...
for step in range(1, N_STEPS + 1):
    ...
    if step % EVAL_EVERY == 0 or step == N_STEPS:
        ...
        model.train()
    if step % 1000 == 0:
        # periodic checkpoint: atomic replace -> keep ONLY the single latest file
        _ckpt = os.path.join(EXP_DIR, 'checkpoint.pt')
        _tmp = _ckpt + '.tmp'
        torch.save(model.state_dict(), _tmp)
        os.replace(_tmp, _ckpt)
        print(f'[CKPT] step {step}: wrote {_ckpt} (keep-last-only)')
