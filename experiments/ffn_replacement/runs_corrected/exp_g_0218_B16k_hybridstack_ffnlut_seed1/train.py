...
# --- residual-STACK hybrid instrumentation: per-layer LayerScale scalars + realized
# output-norm ratios, logged every eval (the whole point of this run). Strictly additive
# columns; empty for non-stack runs.
_STACK = model.hybrid_stack_blocks()
_NS = len(_STACK)
STACK_COLS = ([f'sffn_L{i}' for i in range(_NS)] + [f'slut_L{i}' for i in range(_NS)]
              + [f'rffn_L{i}' for i in range(_NS)] + [f'rlut_L{i}' for i in range(_NS)])
...
def stack_stats():
    if not _NS:
        return []
    s = model.hybrid_stack_stats()
    return [f'{v:.6f}' for v in s['s_ffn']] + [f'{v:.6f}' for v in s['s_lut']] \
        + [f'{v:.6f}' for v in s['ratio_ffn']] + [f'{v:.6f}' for v in s['ratio_lut']]


csv_f = open(os.path.join(EXP_DIR, 'metrics.csv'), 'w', newline='')
csv_w = csv.writer(csv_f)
csv_w.writerow(['step', 'train_loss', 'val_bpb'] + LN_COLS + TAU_COLS + STACK_COLS)
...
for step in range(1, N_STEPS + 1):
    ...
    if step % EVAL_EVERY == 0 or step == N_STEPS:
        ...
        csv_w.writerow([step, f'{ema:.6f}', f'{bpb:.6f}'] + ln_stats() + tau_stats() + stack_stats())
        csv_f.flush()
        if _NS:
            _s = model.hybrid_stack_stats()
            print('[STACK] step %d: ' % step + ' '.join(
                f'L{i}(sf={_s["s_ffn"][i]:.3f} sl={_s["s_lut"][i]:.3f} '
                f'rf={_s["ratio_ffn"][i]:.3f} rl={_s["ratio_lut"][i]:.3f})' for i in range(_NS)))
