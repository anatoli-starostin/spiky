...
# --- spiky-source override (exp_n_0187 local run) ---------------------------------
# This host's editable `spiky` install points at ~/projects/spiky/src (branch
# hyperplane_ffn_next), whose CompressionMHL predates lut_impl/Light. This run uses
# lut_impl="light", which only exists on research/ffn_replacement_fix. So we import
# spiky from THIS worktree's src (ffn_replacement_fix) instead of the editable install,
# by prepending it to sys.path and dropping the editable finder BEFORE any spiky import.
# Non-invasive: does not modify ~/projects/spiky or the venv; scoped to this process.
