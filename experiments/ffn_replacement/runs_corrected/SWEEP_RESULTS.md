...
# The historical grid, re-scored (in progress)
...
what the 4k proxy sweeps concluded. Regenerated by `summarise_grid.py`; inventory and
classification by `inventory_old_grid.py`; scoring by `score_old_grid.py`, which refuses to
publish a number unless the checkpoint loads with 0 missing / 0 unexpected keys **and** the
rebuilt parameter count matches the run's own `summary.json`.

**Status: 11 of 25 done.** 18 runs in `runs/` are in scope; 4 had checkpoints on this machine
and are now scored (0126, 0127, 0128, 0151); **14 are waiting on a single approval to reach
nebius-h100**, where their checkpoints live. Nothing is unscoreable — every directory under
`runs/` has a `config.json`, a `summary.json` with a `final_val_bpb`, and a recorded param
count.

| run | shape | params | steps | dbs | orig bpb | corrected | correction | vs vanilla |
|---|---|---|---|---|---|---|---|---|
| 0151 | dense 4× MLP | 35,792,640 | 48000 | 48 | 1.151444 | **1.115420** | −0.036025 | −0.049727 |
...
† 0160 and 0161 were trained by nebius *on* the corrected protocol, so their "original" number
already is the corrected one.

## The correction is a function of the original eval batch, not of the model

| original `device_batch_size` | old window | n | mean correction | range |
|---|---|---|---|---|
| 12 | 61,440 tok | 7 | **−0.010232** | [−0.012744, −0.007700] |
| 24 | (fixed protocol) | 2 | 0 | — |
| 48 | 245,760 tok | 2 | **−0.036159** | [−0.036293, −0.036025] |

The bs48 cohort corrects **3.5× more** than the bs12 cohort, and within each cohort the
correction is tight (bs48 spans 0.00027 across two runs, bs12 spans 0.0050 across seven).
This is exactly the confound `FIXED_EVAL.md` documents, now measured across eleven runs: the
old number depended on which slice of the val stream the *training* batch happened to select.

## Which claims survive

**Within the bs12 cohort the ordering is perfectly preserved** — 0118 < 0133 < 0129 < 0127 <
0128 < 0136 < 0126, identical before and after. All seven shift by nearly the same amount, so
every comparison made *within* that cohort still stands.

**Across cohorts it reshuffles, and the vanilla anchor is the run that moves.** Original order
put 0135 seventh of ten; corrected it is second. Five runs flip sign against the anchor —
0133, 0129, 0127, 0160, 0161 — and 0118 falls from −0.02684 to −0.00021, a dead heat. **No
compression run in the grid beats the corrected vanilla baseline.** The grid's "LUT beats
dense" reading was an artefact of comparing bs12 numbers against a bs48 anchor.

Note 0151 (48k steps) at 1.115420 is far below everything else — it is a longer run, not a
better architecture, and belongs only with the other long runs.

## Cross-check against the proxy sweeps

Two clean iso-parameter pairs exist in the corrected data so far, and both agree with the
proxy sweeps' finding that **pushing cells up is the wrong axis**:

| budget | fewer cells / more tables | more cells / fewer tables | Δ |
|---|---|---|---|
| 180,597,900 (C = 131072) | 0118 tph256/c512 **1.164939** | 0133 tph128/c1024 1.169059 | +0.004119 |
| 48,477,324 | 0127 tph128/c128 **1.187011** | 0128 tph64/c256 1.192465 | +0.005454 |

Same direction, same order of magnitude, at 16k and at full batch. That is the proxy's
iso-param result (S1/S7 beat S6) reproduced on real runs.

The other two proxy findings **cannot yet be checked**: the historical grid is almost entirely
`d_in = d_out = 48`, so it contains no `d_out` ladder, and the four runs that vary the head
split at constant `H·d_c = 192` (0129 H4×48 scored; 0131 H2×96, 0132 H8×24, 0137 H1×192 all
pending) are exactly the ones still waiting on checkpoints. **The `H·d_in ≥ 128` floor and the
`d_out` optimum at 48 are therefore neither confirmed nor contradicted yet** — 0131/0132/0137
are the decisive runs and they are in the blocked set.

**The shortlist (S5 / R6 / S2) is unchanged so far**, since nothing re-scored contradicts the
findings it rests on. That conclusion is provisional until the remaining 14 land.
