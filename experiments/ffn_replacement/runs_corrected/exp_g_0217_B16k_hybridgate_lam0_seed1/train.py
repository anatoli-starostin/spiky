...
HYBRID_LAMBDA = float(cfg.get('hybrid_lambda', 0.0))   # L1 FFN-sparsity penalty weight (0=off)
...
# --- per-layer HYBRID GATE logging (this run's whole point) ---------------------------
# g_l = sigmoid(theta_l) for each hybrid block, logged at every eval so the full gate
# trajectory (which layers migrate toward dense-FFN vs LUT) is recoverable + plottable.
# Strictly additive columns; empty for non-hybrid runs.
_N_GATES = len(model.hybrid_gates())
GATE_COLS = [f'gate_L{i}' for i in range(_N_GATES)]


@torch.no_grad()
def gate_stats():
    return [f'{g:.6f}' for g in model.hybrid_gates()]


csv_f = open(os.path.join(EXP_DIR, 'metrics.csv'), 'w', newline='')
csv_w = csv.writer(csv_f)
csv_w.writerow(['step', 'train_loss', 'val_bpb'] + LN_COLS + TAU_COLS + GATE_COLS)
...
for step in range(1, N_STEPS + 1):
    ...
    # L1 FFN-sparsity penalty on the gates (lambda*Σ_l g_l), once per optimizer step. The
    # gates already learn from the CE loss regardless; this only adds scarcity pressure and is
    # a no-op at HYBRID_LAMBDA=0 (the primary run). Extra backward accumulates grad into theta.
    if HYBRID_LAMBDA > 0.0:
        (HYBRID_LAMBDA * model.hybrid_gate_penalty()).backward()
    ...
    if step % EVAL_EVERY == 0 or step == N_STEPS:
        ...
        csv_w.writerow([step, f'{ema:.6f}', f'{bpb:.6f}'] + ln_stats() + tau_stats() + gate_stats())
        csv_f.flush()
        if _N_GATES:
            print(f'[GATE] step {step}: ' + ' '.join(f'L{i}={g:.3f}' for i, g in enumerate(model.hybrid_gates())))
