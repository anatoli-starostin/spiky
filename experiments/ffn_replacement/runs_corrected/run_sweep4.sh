#!/bin/bash
# Run the 6 second-sweep runs sequentially, in sweep4_manifest.json order.
...
RUNS=$("$PY" -c "
import json;print(' '.join(r['run'] for r in json.load(open('$RC/sweep4_manifest.json'))['runs']))")
...
for r in $RUNS; do
  ...
  want=$("$PY" -c "
import json;m=json.load(open('$RC/sweep4_manifest.json'))
