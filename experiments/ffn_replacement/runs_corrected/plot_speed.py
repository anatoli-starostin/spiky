...
MODEL = {  # name: (step ms, eval ms, peak GiB)
    ...
    'light\n+bag fusion': (68.4, 21.3, 5.51),
    'light\n+SCORED KERNEL': (68.4, 11.1, 5.51),
}
...
bars = a.bar(x, step, .58, color=[C_GREY, C_GREEN, C_BLUE, '#e8d3bb', '#d8b48a', C_ORANGE])
...
a.set_xticks(x), a.set_xticklabels(names, fontsize=8, rotation=20, ha='right')
a.set_ylabel('training step, ms (6,144 tokens)')
a.set_title('TRAINING: Light is 4.1x faster than the gated arms,\n'
            '3.2x faster than gate-off Fast, 3.9x less memory', fontsize=11)
a.grid(axis='y', alpha=.25)

# ---- 2. eval forward: the opposite story ---------------------------------------------
b = ax[1]
ev = [MODEL[n][1] for n in names]
b.bar(x, ev, .58, color=[C_GREY, C_GREEN, C_BLUE, '#e8d3bb', '#d8b48a', C_ORANGE])
for i, v in enumerate(ev):
    b.annotate(f'{v:.1f} ms', (i, v), xytext=(0, 4), textcoords='offset points',
               ha='center', fontsize=9)
b.annotate('gate disables Fast\'s\nnative kernel (+13%)', (2, ev[2]),
           xytext=(-4, 40), textcoords='offset points', fontsize=7.5, ha='center',
           arrowprops=dict(arrowstyle='->', lw=.9, color='#555'))
b.annotate('score fused into the\nbit-pack pass', (5, ev[5]),
           xytext=(0, 34), textcoords='offset points', fontsize=7.5, ha='center',
           arrowprops=dict(arrowstyle='->', lw=.9, color='#555'))
b.set_xticks(x), b.set_xticklabels(names, fontsize=8, rotation=20, ha='right')
b.set_ylabel('eval forward, ms (6,144 tokens)')
b.set_title('INFERENCE: 5.5x slower -> 2.1x -> 1.08x.\n'
            'Parity with Fast, and better than gated Fast', fontsize=11)
...
c.set_title('Light matches DENSE quality (+0.003, inside noise) at\n'
            '3.2x faster training and now parity inference', fontsize=11)
c.grid(alpha=.25), c.legend(fontsize=8, loc='upper right')

fig.suptitle('LightMultiHeadLUT after the embedding_bag fusion + a score-fused CUDA kernel — RTX 5090, fp32, anchor sizing', fontsize=13, y=1.0)
plt.tight_layout()
