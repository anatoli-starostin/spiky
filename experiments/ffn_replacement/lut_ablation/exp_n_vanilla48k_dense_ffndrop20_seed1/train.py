...
# --- W&B tracking (tools/wandb_tracking.py) -- OFF unless WANDB_BASE_URL is set; never fatal ----
import wandb_tracking
wandb_tracking.GROUP = 'lut_ablation'
from wandb_tracking import Tracker
tracker = Tracker.start(cfg, EXP_DIR, grad_accum=grad_accum, total_params=total_params,
                        extra_tags=[f'ffndrop:{float(cfg.get("dense_ffn_dropout", 0.0)):g}'])
...
for step in range(1, N_STEPS + 1):
    ...
    if step % EVAL_EVERY == 0 or step == N_STEPS:
        ...
        tracker.eval_step(step, bpb, ema, {}, model)
