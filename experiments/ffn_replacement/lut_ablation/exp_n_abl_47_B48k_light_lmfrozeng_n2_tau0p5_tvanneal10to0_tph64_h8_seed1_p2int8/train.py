...
for step in range(1, N_STEPS + 1):
    ...
    # TV ANNEAL (this run only): linearly decay the TV coefficient from LUT_TV_LAMBDA (10.0) at step 1
    # to 0.0 at the final step. 1 - step/N_STEPS is 0 exactly at step == N_STEPS. Backward-only change to
    # the penalty weight; the model, data, RNG and eval are untouched.
    tv_coef = LUT_TV_LAMBDA * (1.0 - step / N_STEPS)
    if tv_coef > 0.0:
        (tv_coef * model.lut_tv_penalty()).backward()
    ...
    if step % EVAL_EVERY == 0 or step == N_STEPS:
        ...
        tracker.eval_step(step, bpb, ema, {**dict(zip(LN_COLS, map(float, _ln))),
                                           **dict(zip(TV_COLS, map(float, _tv))), 'tv_coef': tv_coef}, model)
