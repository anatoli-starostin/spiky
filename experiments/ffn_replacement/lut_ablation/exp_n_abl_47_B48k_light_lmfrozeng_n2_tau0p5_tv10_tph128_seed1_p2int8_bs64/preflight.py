...
PIN_SPIKY = "686e5c4468f64fcf97c383a7a63bb20a2774b59a"  # bs64 variant created at HEAD 686e5c44 (= pinned library 4ad02234 + only the abl_47 folder addition; no src drift)
...
check("config quantised arm (lut_quant_mode p2_int8)", cfg.get("lut_quant_mode") == "p2_int8")
expect = dict(device_batch_size=64, total_batch_size=32768, seq_len=512, lr=0.0003, weight_decay=0.1, lr_warmup_fraction=0.1,
              eval_every=500, random_seed=1, lut_base_seed=1000, lut_impl="light", lut_read_top_n=2, lut_read_tau=0.5,
              lut_cell_smoothness=10.0, lut_tables_per_head=128, lut_n_heads=4, lut_n_anchor_pairs=8, lut_inner_in_dim=48,
...
if spiky and nanochat and base:
    ...
    check("params 67,351,698 (same as the 16K runs)", "params=67,351,698" in out)
    q = [ln.split()[1:] for ln in out.splitlines() if ln.startswith("P2_QUANT_LAYERS")]
    check("quantised path ACTIVE on all 6 layers", bool(q) and q[0] == ["6", "6"], str(q))
    impl = next((ln.split()[1] for ln in out.splitlines() if ln.startswith("P2_IMPL")), "UNKNOWN")
    reason = next((ln[len("P2_REASON "):] for ln in out.splitlines() if ln.startswith("P2_REASON")), "")
    disabled = os.environ.get("SPIKY_P2_CUDA_DISABLE") == "1"
    gate_p = os.path.join(HERE, "kernel_gate.json")
    gate = json.load(open(gate_p))["verdict"] if os.path.exists(gate_p) else "NOT RUN"
    line = (f"IMPLEMENTATION SERVING THE QUANTISED PATH: {impl.replace('_', ' ')} | kernel gate: {gate} | "
            f"SPIKY_P2_CUDA_DISABLE={'1' if disabled else 'unset'} | {reason}")
    print("\n" + "=" * 110 + "\n" + line + "\n" + "=" * 110, flush=True)
    open(os.path.join(HERE, "preflight_implementation.txt"), "w").write(line + "\n")
    check("kernel gate has been run (validate_kernel.py)", gate in ("PASS", "FAIL"), gate)
    if impl == "CUDA_KERNEL":
        check("kernel serves only after a PASS gate", gate == "PASS", "" if gate == "PASS" else "gate is not PASS: set SPIKY_P2_CUDA_DISABLE=1 in run.env")
    else:
        check("fallback is deliberate", disabled or gate != "PASS", "" if (disabled or gate != "PASS") else
              "the gate PASSED but the kernel is not serving and SPIKY_P2_CUDA_DISABLE is unset -- investigate: " + reason)
