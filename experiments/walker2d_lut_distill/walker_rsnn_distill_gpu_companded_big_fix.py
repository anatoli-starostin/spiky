...
def main():
    ...
    log("=== RSNN distill Walker2d int4 LUT (BIG H256/64, 4096 steps, PINNED std=0.1 + SURR anneal 5->30) ===")
    ...
    open(f"{EVO}/walker_rsnn_report_gpu_companded_big_fix.txt", "w").write("\n".join(rep))
    json.dump(dict(device=DEV, peak_gpu_mem_gb=gpu_mem, chosen_std=chosen, sweep=sweep,
              ...
              open(f"{EVO}/walker_rsnn_result_gpu_companded_big_fix.json", "w"), indent=2)
    log("wrote walker_rsnn_report_gpu_companded_big_fix.txt + walker_rsnn_result_gpu_companded_big_fix.json")
