...
# ---- discrete K=7-levels-per-joint target (per-dim quantile boundaries -> ~equiprobable levels) ----
K_LEVELS = 7
_YPD = oracle_actions(sample_obs(16384, seed=7))          # (16384, 6) teacher torques for binning stats
QBOUND = np.stack([np.quantile(_YPD[:, d], [i / K_LEVELS for i in range(1, K_LEVELS)]) for d in range(NO)])  # (6,6)
LEVELVAL = np.zeros((NO, K_LEVELS))                        # per-dim bin-representative torque (bin mean)
for _d in range(NO):
...
def to_levels(A):                                         # (M,6) torques -> (M,6) int levels 0..6 via boundaries
    return np.stack([np.digitize(A[:, d], QBOUND[d]) for d in range(NO)], axis=1)

# ---- ARM C band/deadzone: per-dim finite bin edges (data min, 6 boundaries, data max) -> 7 bins ----
A_MIN = _YPD.min(0); A_MAX = _YPD.max(0)                  # (6,) per-dim torque range for the outer bin edges
EDGES_np = np.concatenate([A_MIN[:, None], QBOUND, A_MAX[:, None]], axis=1)   # (6, 8): edges of the 7 bins
EDGES = torch.tensor(EDGES_np, dtype=torch.float32, device=DEV)              # (6, 8)
HW_FRAC = float(os.environ.get("RSNN_HWFRAC", "0.4"))    # half-width frac: 0.4=inner-80% band, 0.5=full bin (margin 0)
...
def loss_fn(model, Ispk, labels):                         # ARM C band/deadzone: labels (B,6) int levels
    act, a = model.forward(Ispk, ret_act=True)            # act = companded-decoded action (B,6)
    B = labels.shape[0]
    E = EDGES.unsqueeze(0).expand(B, -1, -1)              # (B,6,8)
    e_lo = torch.gather(E, 2, labels.unsqueeze(-1)).squeeze(-1)        # bin low edge  E[d, y]
    e_hi = torch.gather(E, 2, (labels + 1).unsqueeze(-1)).squeeze(-1)  # bin high edge E[d, y+1]
    center = 0.5 * (e_lo + e_hi); half = HW_FRAC * (e_hi - e_lo)
    lo_eff = center - half; hi_eff = center + half        # correct bin shrunk toward centre by the margin
    dz = F.relu(lo_eff - act).pow(2) + F.relu(act - hi_eff).pow(2)     # 0 inside band, quadratic outside
    task = dz.sum(1).mean()
    l1 = L1_W * a["wsum"]
    dpen = DELAY_W * a["dmean"]
    rate = RATE_W * ((a["h_rate_t"] - RATE_TGT) ** 2 + (a["o_rate_t"] - RATE_TGT) ** 2)
    return task + l1 + dpen + rate, task.item(), a
...
def main():
    ...
    log(f"=== RSNN DISCRETE ARM C (band/deadzone loss, 6 outputs, HW_FRAC={HW_FRAC:g}={'inner-80%' if HW_FRAC==0.4 else 'full-bin' if HW_FRAC==0.5 else 'custom'}); SPARSE H256/64 ===")
    ...
    # ARM A ("snap"): discretize the companded-decoded continuous action into the 7 per-joint level bins
    dec = ev["dec"]                                        # (512,6) decoded torques
    true_lvl = to_levels(Yval); pred_lvl = to_levels(dec)
    pj_acc = (pred_lvl == true_lvl).mean(0)               # per-joint accuracy (6,)
    mean_acc = float(pj_acc.mean()); exact = float((pred_lvl == true_lvl).all(1).mean())
    pred_torque = LEVELVAL[np.arange(NO)[None, :], pred_lvl]
    lvl_err = float(np.abs(pred_torque - Yval).mean() / ev["arange"])
    log(f"\n=== ARM C (band/deadzone, HW_FRAC={HW_FRAC:g}) — discrete 7-level-per-joint classification ===")
    log("per-dim quantile boundaries + level values (K=7):")
    for d in range(NO):
        log(f"  joint{d}: bounds {np.round(QBOUND[d],3).tolist()}  levelvals {np.round(LEVELVAL[d],3).tolist()}")
    log(f"per-joint acc: {[round(float(x)*100,1) for x in pj_acc]} %")
    log(f"MEAN per-joint acc: {100*mean_acc:.2f}%  |  exact-match (all 6): {100*exact:.2f}%  "
        f"(chance {100/K_LEVELS:.1f}%/joint, {100/K_LEVELS**NO:.5f}% exact)")
    log(f"context: top-1 argmax {100*top1:.1f}%  |  snapped-level readback mean|err| {100*lvl_err:.2f}% of range")
    ...
    open(f"{EVO}/walker_rsnn_report_gpu_disc_C_hw{HW_FRAC:g}.txt", "w").write("\n".join(rep))
    json.dump(dict(device=DEV, peak_gpu_mem_gb=gpu_mem, chosen_std=chosen, sweep=sweep,
                   arm="C", hw_frac=HW_FRAC, per_joint_acc=[float(x) for x in pj_acc], mean_acc=mean_acc, exact_match=exact,
                   level_readback_err_pct=100*lvl_err, qbound=QBOUND.tolist(), levelval=LEVELVAL.tolist(),
              ...
              open(f"{EVO}/walker_rsnn_result_gpu_disc_C_hw{HW_FRAC:g}.json", "w"), indent=2)
    log(f"wrote walker_rsnn_report_gpu_disc_C_hw{HW_FRAC:g}.txt + walker_rsnn_result_gpu_disc_C_hw{HW_FRAC:g}.json")
