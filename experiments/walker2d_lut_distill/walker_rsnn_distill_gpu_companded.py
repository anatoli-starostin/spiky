...
def main():
    ...
    log("=== RSNN distillation of the Walker2d int4 LUT actor (GPU + random batches + PHASED + COMPANDED OUTPUT) ===")
    ...
    open(f"{EVO}/walker_rsnn_report_gpu_companded.txt", "w").write("\n".join(rep))
    json.dump(dict(device=DEV, peak_gpu_mem_gb=gpu_mem, chosen_std=chosen, sweep=sweep,
              ...
              open(f"{EVO}/walker_rsnn_result_gpu_companded.json", "w"), indent=2)
    log("wrote walker_rsnn_report_gpu_companded.txt + walker_rsnn_result_gpu_companded.json")
