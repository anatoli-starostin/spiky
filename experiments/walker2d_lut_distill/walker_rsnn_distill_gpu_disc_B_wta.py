...
def evaluate(model, Xn, Yn):                              # ARM B classification eval (hard first-spike argmin per joint)
    ...
    group_nofire = float((of >= float(T)).all(-1).mean())     # frac of (sample,joint) groups with NO neuron firing
    return dict(pred=pred, true=true, pj_acc=pj, mean_acc=float(pj.mean()),
                exact=float((pred == true).all(1).mean()), lvl_err=lvl_err, group_nofire=group_nofire, a=a)
...
def main():
    ...
    log(f"=== RSNN ARM B WTA read-out-window sweep (L={READOUT}, T={T}); 7 neurons/joint, first-spike CE; SPARSE H256/64 ===")
    ...
    log(f"WTA read-out window L={READOUT} (T={T}) | joint-groups with NO neuron firing in window: {100*ev['group_nofire']:.2f}%")
    log(f"\n(elapsed {time.time()-t0:.0f}s, device={DEV}, peak_gpu_mem={gpu_mem:.2f}GB)")

    tag = f"disc_B_wta_ro{READOUT}"
    open(f"{EVO}/walker_rsnn_report_gpu_{tag}.txt", "w").write("\n".join(rep))
    json.dump(dict(device=DEV, arm="B_wta", readout=READOUT, T=T, peak_gpu_mem_gb=gpu_mem, chosen_std=chosen,
                   ...
                   group_nofire=ev["group_nofire"], o_nonfire=a["o_nonfire"],
