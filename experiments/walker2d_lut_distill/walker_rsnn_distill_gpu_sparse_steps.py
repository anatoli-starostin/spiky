...
def main():
    ...
    full_steps = 8 if SMOKE else int(os.environ.get("RSNN_STEPS", "4096"))   # parameterized for step-count calib
    tag = f"steps{full_steps}"                            # SURR anneal & DCLS anneal are s/(steps-1) -> auto-rescaled
    ...
    open(f"{EVO}/walker_rsnn_report_gpu_sparse_{tag}.txt", "w").write("\n".join(rep))
    json.dump(dict(device=DEV, peak_gpu_mem_gb=gpu_mem, chosen_std=chosen, sweep=sweep,
              ...
              open(f"{EVO}/walker_rsnn_result_gpu_sparse_{tag}.json", "w"), indent=2)
    log(f"wrote walker_rsnn_report_gpu_sparse_{tag}.txt + walker_rsnn_result_gpu_sparse_{tag}.json")
