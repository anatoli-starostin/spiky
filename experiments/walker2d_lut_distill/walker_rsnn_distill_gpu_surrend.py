...
SURR_START = 5.0
SURR_END = float(os.environ.get("RSNN_SURR_END", "30"))   # anneal endpoint (swept)
ST_TAIL = os.environ.get("RSNN_ST_TAIL", "0") == "1"      # straight-through tail: hard-forward/soft-backward first-spike
ST_TAIL_FRAC = 0.90                                       # anneal 5->SURR_END over first 90%, ST for last 10%
...
class RSNN(torch.nn.Module):
    def __init__(self, init_std):
        ...
        self.sigma = 2.0
        self.hard = False
        self.st_tail = False                               # straight-through: forward first-spike = hard, grad = soft
    ...
    def forward(self, Ispk, ret_act=False, soft=True):
        ...
        if not soft:
            tval = o_first                                 # eval: true hard first-spike
        elif self.st_tail:
            tval = o_first.detach() + (soft_t - soft_t.detach())   # straight-through: hard forward, soft-E[t] gradient
        else:
            tval = soft_t                                  # soft E[t]
        act = self.decode(tval)
        if ret_act:
...
def train_run(init_std, steps, Xtr, Ytr, log, resample=False):
    ...
    for s in range(steps):
        ...
        if resample:
            ...
            if ST_TAIL:                                    # anneal 5->SURR_END over first ST_TAIL_FRAC, then ST tail
                set_surr(SURR_START + (SURR_END - SURR_START) * min(1.0, frac / ST_TAIL_FRAC))
                model.st_tail = (frac >= ST_TAIL_FRAC)
            else:
                set_surr(SURR_START + (SURR_END - SURR_START) * frac)   # anneal surrogate/gate slope to endpoint
...
def main():
    ...
    log("=== RSNN distill Walker2d int4 LUT (SPARSE H256/64, SURROGATE-endpoint sweep) ===")
    log(f"surrogate anneal: SURR {SURR_START:g} -> {SURR_END:g}"
        + (f" over first {ST_TAIL_FRAC:.0%}, then STRAIGHT-THROUGH tail (hard first-spike fwd / soft grad)" if ST_TAIL
           else " (fraction of training)"))
    ...
    full_steps = 8 if SMOKE else int(os.environ.get("RSNN_STEPS", "2048"))   # fast ranker budget
    tag = f"surr{SURR_END:g}{'_sttail' if ST_TAIL else ''}_steps{full_steps}"
