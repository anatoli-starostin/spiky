...
def main():
    ...
    log("=== RSNN distillation of the Walker2d int4 LUT actor (GPU + random batches + PHASED schedule) ===")
    ...
    open(f"{EVO}/walker_rsnn_report_gpu_phased.txt", "w").write("\n".join(rep))
    json.dump(dict(device=DEV, peak_gpu_mem_gb=gpu_mem, chosen_std=chosen, sweep=sweep,
                   val=ev['mean'], val_pct=100*ev['mean']/ev['arange'], w1=ev['w1'], w2=ev['w2'], spars=spars,
              ...
              open(f"{EVO}/walker_rsnn_result_gpu_phased.json", "w"), indent=2)
    log("wrote walker_rsnn_report_gpu_phased.txt + walker_rsnn_result_gpu_phased.json")
