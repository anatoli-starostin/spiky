"""Companded INPUT+OUTPUT variant. Builds on walker_rsnn_distill_gpu_companded.py: keeps the per-dim probit
COMPANDED OUTPUT code AND adds a per-dim probit COMPANDED INPUT latency code. Only new change vs the
companded-output run is linear->companded INPUT (isolates the input-companding contribution). Originals
untouched; nothing committed.

INPUT companding: the linear latency t = C_IN - ALPHA_IN*x (spread across read-in [0,64)) is replaced by a
per-dim Gaussian-quantile latency. Per input dim: standardize z=(x-mu_in)/sig_in (mu_in/sig_in = mean/std of
the normalized obs over a 16384 sample, seed=7; they are ~0/1 but centered/scaled for safety), u=Phi(z) in
(0,1), tick = round((READIN-1)*(1-u)) clamped to [0,READIN-1]. Orientation matches the linear code
(larger x -> earlier spike): u->1 (large x) -> tick 0; u->0 (small x) -> tick READIN-1. So tick-resolution is
FINE near x=0 (dense input mass, dt/dx max at z=0) and COARSE toward the tails. Same 64-tick read-in budget.
(Input raster is a hard scatter -> not differentiated, so the probit here is just a fixed tick remap; no
erfinv-gradient concern on the input side.)

--- companded-OUTPUT header ---
...
# ---- per-dim INPUT companding (probit latency) ----  (cpu tensors: input_raster builds on cpu then .to(DEV))
_XPOOL = sample_obs(16384, seed=7)
MU_IN_np = _XPOOL.mean(0); SIG_IN_np = _XPOOL.std(0)
MU_IN = torch.tensor(MU_IN_np, dtype=torch.float32)
SIG_IN = torch.tensor(SIG_IN_np, dtype=torch.float32)


def input_raster(Xn):
    """COMPANDED latency code into the READ-IN window: per-dim z=(x-mu_in)/sig_in, u=Phi(z), spike tick
    = round((READIN-1)*(1-u)) clamped to [0,READIN-1]. Larger x -> earlier spike (matches the old linear
    orientation); fine resolution near x=0, coarse at the tails. Raster length is full T."""
    x = torch.as_tensor(Xn, dtype=torch.float32)
    z = (x - MU_IN) / SIG_IN
    u = 0.5 * (1.0 + torch.erf(z / (2.0 ** 0.5)))                        # Phi(z) in (0,1)
    t = torch.round((READIN - 1) * (1.0 - u)).clamp(0, READIN - 1).long()  # (M,17) -> read-in window
...
def main():
    ...
    log("=== RSNN distillation of the Walker2d int4 LUT actor (GPU + random batches + PHASED + COMPANDED INPUT+OUTPUT) ===")
    ...
    log(f"input companding: probit per-dim latency, MU_IN={np.round(MU_IN_np,3).tolist()} "
        f"SIG_IN={np.round(SIG_IN_np,3).tolist()}")
    ...
    open(f"{EVO}/walker_rsnn_report_gpu_companded_io.txt", "w").write("\n".join(rep))
    json.dump(dict(device=DEV, peak_gpu_mem_gb=gpu_mem, chosen_std=chosen, sweep=sweep,
              ...
              open(f"{EVO}/walker_rsnn_result_gpu_companded_io.json", "w"), indent=2)
    log("wrote walker_rsnn_report_gpu_companded_io.txt + walker_rsnn_result_gpu_companded_io.json")
