...
# ------------------------------------------------------------------ sizes / hyperparams
N_JOINTS, K_LEVELS = 6, 7                                 # ARM B: 7 latency output neurons per joint
NI, NEX, NINH, NO = 17, 256, 64, N_JOINTS * K_LEVELS      # NO=42 output neurons (grouped by joint), H256/64
if SMOKE:
    NEX, NINH = 32, 8
NH = NEX + NINH
...
MU = torch.tensor(MU_np, dtype=torch.float32, device=DEV)   # (unused by Arm B; kept harmless)
SIGMA = torch.tensor(SIGMA_np, dtype=torch.float32, device=DEV)

# ---- discrete K=7-levels-per-JOINT target (per-joint quantile boundaries -> ~equiprobable levels) ----
_YPD = oracle_actions(sample_obs(16384, seed=7))          # (16384, 6) teacher torques
QBOUND = np.stack([np.quantile(_YPD[:, d], [i / K_LEVELS for i in range(1, K_LEVELS)]) for d in range(N_JOINTS)])
LEVELVAL = np.zeros((N_JOINTS, K_LEVELS))                  # per-joint bin-representative torque (bin mean)
for _d in range(N_JOINTS):
    _lab = np.digitize(_YPD[:, _d], QBOUND[_d])
    for _k in range(K_LEVELS):
        _m = _lab == _k
        LEVELVAL[_d, _k] = _YPD[_m, _d].mean() if _m.any() else 0.0

def to_levels(A):                                         # (M,6) torques -> (M,6) int levels 0..6 via boundaries
    return np.stack([np.digitize(A[:, d], QBOUND[d]) for d in range(N_JOINTS)], axis=1)

# ---- Arm B init-symmetry-break levers (break the within-group tie via the H->output synapses) ----
OUT_STD_MULT = float(os.environ.get("RSNN_OUT_STD_MULT", "3.0"))   # (1) higher init std on H->output weights
O_BIAS_SCALE = float(os.environ.get("RSNN_O_BIAS", "0.3"))         # (2) per-level bias ramp on output drive
CE_TEMP = float(os.environ.get("RSNN_CE_TEMP", "10.0"))            # (3) logit temperature (the operative fix)
...
class RSNN(torch.nn.Module):
    def __init__(self, init_std):
        ...
        def wpar(shape, sm=1.0):                            # sm = init-std multiplier (>1 for H->output symmetry-break)
            w0 = (torch.randn(shape, generator=g) * init_std * sm).abs()
        ...
        self.tXO, self.dXO = wpar((NEX, NO), OUT_STD_MULT), dpar((NEX, NO))   # lever 1: higher-variance output init
        self.tNH = wpar((NINH, NH))
        self.tNO = wpar((NINH, NO), OUT_STD_MULT)                             # lever 1: higher-variance output init
        ...
        # lever 2: per-level bias ramp on the output drive (monotone within each 7-group) -> spread init spike-times
        _lvl = torch.arange(NO).float() % K_LEVELS - (K_LEVELS - 1) / 2.0     # -3..+3 within each joint-group
        self.register_buffer("o_bias", O_BIAS_SCALE * _lvl)                  # added to output membrane current each tick
    ...
    def forward(self, Ispk, ret_act=False, soft=True):
        ...
        for t in range(T):
            ...
            cur_o = (torch.einsum('bdp,dpo->bo', Hbuf, EW_XO)
                     + prev_h[:, NEX:] @ wNO + self.o_bias)   # lever 2: per-level tie-breaking bias current
            vh = BETA * vh + cur_h
            ...
            prev_h = s_h
        soft_t = soft_t + float(T) * notfired              # never-fired mass -> max latency T
        act = soft_t if soft else o_first                  # ARM B: RAW first-spike times (B,NO=42); classify downstream
        if ret_act:
...
def loss_fn(model, Ispk, labels):                         # labels: (B, N_JOINTS) int levels 0..K-1
    act, a = model.forward(Ispk, ret_act=True)            # act: soft E[t] first-spike times (B, NO=42)
    logits = (-act / CE_TEMP).reshape(-1, N_JOINTS, K_LEVELS)   # lever 3: tempered logits (avoid confidently-wrong init)
    ce = F.cross_entropy(logits.reshape(-1, K_LEVELS), labels.reshape(-1))   # first-spike cross-entropy per joint
    l1 = L1_W * a["wsum"]
    dpen = DELAY_W * a["dmean"]
    rate = RATE_W * ((a["h_rate_t"] - RATE_TGT) ** 2 + (a["o_rate_t"] - RATE_TGT) ** 2)
    return ce + l1 + dpen + rate, ce.item(), a


# ------------------------------------------------------------------ train / eval
def evaluate(model, Xn, Yn):                              # ARM B classification eval (hard first-spike argmin per joint)
    was = model.hard; model.hard = True
    Ispk = input_raster(Xn)
    with torch.no_grad():
        act, a = model.forward(Ispk, ret_act=True, soft=False)   # act = hard o_first (B, NO=42)
    model.hard = was
    of = act.reshape(-1, N_JOINTS, K_LEVELS).cpu().numpy()
    pred = of.argmin(-1)                                  # predicted level = earliest hard first-spike in the 7-group
    true = to_levels(Yn)
    pj = (pred == true).mean(0)                           # per-joint accuracy (6,)
    arange = float(Yn.max() - Yn.min())
    pred_tq = LEVELVAL[np.arange(N_JOINTS)[None, :], pred]     # read predicted levels back to torque
    lvl_err = float(np.abs(pred_tq - Yn).mean() / arange)
    return dict(pred=pred, true=true, pj_acc=pj, mean_acc=float(pj.mean()),
                exact=float((pred == true).all(1).mean()), lvl_err=lvl_err, a=a)
...
def train_run(init_std, steps, Xtr, Ytr, log, resample=False):
    ...
    Ispk = input_raster(Xtr); target = torch.as_tensor(to_levels(oracle_actions(Xtr)), dtype=torch.long).to(DEV)
    l0 = None; hist = []
    for s in range(steps):
        ...
        if resample:
            ...
            Ispk = input_raster(Xb); target = torch.as_tensor(to_levels(oracle_actions(Xb)), dtype=torch.long).to(DEV)
        opt.zero_grad()
        loss, mse, a = loss_fn(model, Ispk, target)        # 'mse' is the CE value here
        loss.backward()
...
def main():
    ...
    log("=== RSNN ARM B INITBREAK (WTA + init-symmetry-break); SPARSE H256/64 ===")
    log(f"init-break levers: OUT_STD_MULT={OUT_STD_MULT:g} (H->out init std x), O_BIAS_SCALE={O_BIAS_SCALE:g} "
        f"(per-level bias ramp +-{O_BIAS_SCALE*(K_LEVELS-1)/2:g}), CE_TEMP={CE_TEMP:g}")
    ...
    full_steps = 8 if SMOKE else int(os.environ.get("RSNN_STEPS", "2048"))   # fast budget
    log(f"\n-- ARM B latency-classifier training: std={chosen:g}, {full_steps} steps, first-spike CE, "
        f"{NO} outputs ({N_JOINTS}x{K_LEVELS}), RANDOM batches (size {BATCH}) --")
    model, l0, hist = train_run(chosen, full_steps, Xtr, Ytr, log, resample=True)

    ev_tr = evaluate(model, Xtr, Ytr)
    ev = evaluate(model, Xval, Yval)
    a = ev["a"]
    gpu_mem = (torch.cuda.max_memory_allocated() / 1e9) if DEV == "cuda" else 0.0
    log("\n=== ARM B (latency classifier) — discrete 7-level-per-joint ===")
    log(f"device={DEV}  peak_gpu_mem={gpu_mem:.2f}GB  CE loss: start {l0:.4f} -> end {hist[-1][1]:.4f}")
    log("per-joint quantile boundaries + level values (K=7):")
    for d in range(N_JOINTS):
        log(f"  joint{d}: bounds {np.round(QBOUND[d],3).tolist()}  levelvals {np.round(LEVELVAL[d],3).tolist()}")
    log(f"per-joint acc (held-out): {[round(float(x)*100,1) for x in ev['pj_acc']]} %")
    log(f"per-joint acc (train):    {[round(float(x)*100,1) for x in ev_tr['pj_acc']]} %")
    log(f"MEAN per-joint acc: {100*ev['mean_acc']:.2f}% (train {100*ev_tr['mean_acc']:.2f}%)  "
        f"|  exact-match (all 6): {100*ev['exact']:.2f}%  (chance {100/K_LEVELS:.1f}%/joint, {100/K_LEVELS**N_JOINTS:.5f}% exact)")
    log(f"level-readback continuous mean|err|: {100*ev['lvl_err']:.2f}% of range")
    log(f"firing rates: H_ex={a['hex_rate']:.3f} H_inh={a['hinh_rate']:.3f} O={a['o_rate']:.3f}  "
        f"O non-firing (of {512*NO}): {a['o_nonfire']}")
    log(f"\n(elapsed {time.time()-t0:.0f}s, device={DEV}, peak_gpu_mem={gpu_mem:.2f}GB)")

    np.savez(f"{EVO}/walker_rsnn_disc_B_initbreak_policy_outputs.npz",
             pred_levels=ev["pred"], true_levels=ev["true"], levelval=LEVELVAL, qbound=QBOUND, val_obs=Xval)
    open(f"{EVO}/walker_rsnn_report_gpu_disc_B_initbreak.txt", "w").write("\n".join(rep))
    json.dump(dict(device=DEV, arm="B", peak_gpu_mem_gb=gpu_mem, chosen_std=chosen,
                   per_joint_acc=[float(x) for x in ev["pj_acc"]], mean_acc=ev["mean_acc"], exact_match=ev["exact"],
                   train_mean_acc=ev_tr["mean_acc"], level_readback_err_pct=100*ev["lvl_err"],
                   qbound=QBOUND.tolist(), levelval=LEVELVAL.tolist(), o_nonfire=a["o_nonfire"],
                   rates=dict(hex=a["hex_rate"], hinh=a["hinh_rate"], o=a["o_rate"]),
                   l0=l0, lend=hist[-1][1], steps=full_steps, elapsed=time.time()-t0),
              open(f"{EVO}/walker_rsnn_result_gpu_disc_B_initbreak.json", "w"), indent=2)
    log("wrote walker_rsnn_report_gpu_disc_B_initbreak.txt + walker_rsnn_result_gpu_disc_B_initbreak.json + policy_outputs.npz")
