...
def main():
    ...
    log("=== RSNN DISCRETE ARM A (snap decoded->7 levels/joint); SPARSE H256/64, companded-MSE training ===")
    ...
    log("\n=== ARM A (snap) — discrete 7-level-per-joint classification ===")
    ...
    open(f"{EVO}/walker_rsnn_report_gpu_disc_A.txt", "w").write("\n".join(rep))
    json.dump(dict(device=DEV, peak_gpu_mem_gb=gpu_mem, chosen_std=chosen, sweep=sweep,
                   arm="A", per_joint_acc=[float(x) for x in pj_acc], mean_acc=mean_acc, exact_match=exact,
              ...
              open(f"{EVO}/walker_rsnn_result_gpu_disc_A.json", "w"), indent=2)
    log("wrote walker_rsnn_report_gpu_disc_A.txt + walker_rsnn_result_gpu_disc_A.json")
