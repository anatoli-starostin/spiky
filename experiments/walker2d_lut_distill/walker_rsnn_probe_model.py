...
class RSNN(torch.nn.Module):
    ...
    def forward(self, Ispk, ret_act=False, soft=True):
        ...
        h_first = torch.full((B, NH), float(T), device=dev); h_done = torch.zeros(B, NH, dtype=torch.bool, device=dev)
        for t in range(T):
            ...
            _newh = (s_h > 0.5) & (~h_done)                # hidden first-spike latency (over full rollout)
            h_first = torch.where(_newh, torch.full_like(h_first, float(t)), h_first); h_done = h_done | _newh
            vo = BETA * vo + cur_o
        ...
        if ret_act:
            return act, dict(h_rate=(h_spk_sum.mean() / T).item(),
                             ...
                             h_first=h_first.detach(), h_count=h_spk_sum.detach())   # PROBE: hidden features
        return act
...
def main():
    ...
    open(f"{EVO}/walker_rsnn_report_gpu_sparse_fanout.txt", "w").write("\n".join(rep))
    json.dump(dict(device=DEV, peak_gpu_mem_gb=gpu_mem, chosen_std=chosen, sweep=sweep,
              ...
              open(f"{EVO}/walker_rsnn_result_gpu_sparse_fanout.json", "w"), indent=2)
    log("wrote walker_rsnn_report_gpu_sparse_fanout.txt + walker_rsnn_result_gpu_sparse_fanout.json")
