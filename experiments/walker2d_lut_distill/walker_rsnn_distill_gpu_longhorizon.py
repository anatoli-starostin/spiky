...
def train_run(init_std, steps, Xtr, Ytr, log, resample=False, Xval=None, Yval=None, ckpt_every=0, curve=None):
    ...
    for s in range(steps):
        ...
        if s % max(1, steps // 20) == 0 or s == steps - 1:
            ...
            log(f"    step {s:5d}  mse={mse:.4f}  surr={SURR:.1f}  hid_rate={a['h_rate']:.3f} out_rate={a['o_rate']:.3f} "
                f"o_nonfire={a['o_nonfire']}")
        if ckpt_every and s > 0 and s % ckpt_every == 0:          # periodic held-out (hard-eval) checkpoint
            ev = evaluate(model, Xval, Yval, hard=True)
            mp = 100 * ev["mean"] / ev["arange"]
            if curve is not None: curve.append((s, round(mp, 3)))
            log(f"  [CKPT] step {s:5d}: held-out mean |err| {mp:.2f}% of range")
...
def main():
    ...
    log("=== RSNN distill Walker2d int4 LUT (LONG-HORIZON, SPARSE 32+6 H256/64, pinned std=0.1, SURR anneal) ===")
    ...
    full_steps = 8 if SMOKE else int(os.environ.get("RSNN_STEPS", "16384"))   # long-horizon
    ckpt_every = 2 if SMOKE else 2048                     # held-out checkpoint cadence
    curve = []
    log(f"\n-- LONG-HORIZON training: std={chosen:g}, {full_steps} steps, held-out ckpt every {ckpt_every}, "
        f"SURR anneal 5->30 over full run, RANDOM per-step batches (size {BATCH}) --")
    model, l0, hist = train_run(chosen, full_steps, Xtr, Ytr, log, resample=True,
                                Xval=Xval, Yval=Yval, ckpt_every=ckpt_every, curve=curve)
    ...
    curve.append((full_steps, round(100 * ev["mean"] / ev["arange"], 3)))
    gpu_mem = (torch.cuda.max_memory_allocated() / 1e9) if DEV == "cuda" else 0.0
    log("\n=== held-out mean |err| (% of range) checkpoint curve ===")
    log("  " + "  ".join(f"{s}:{m:.2f}%" for s, m in curve))
    ...
    open(f"{EVO}/walker_rsnn_report_gpu_longhorizon.txt", "w").write("\n".join(rep))
    json.dump(dict(device=DEV, peak_gpu_mem_gb=gpu_mem, chosen_std=chosen, sweep=sweep, curve=curve,
              ...
              open(f"{EVO}/walker_rsnn_result_gpu_longhorizon.json", "w"), indent=2)
    log("wrote walker_rsnn_report_gpu_longhorizon.txt + walker_rsnn_result_gpu_longhorizon.json")
