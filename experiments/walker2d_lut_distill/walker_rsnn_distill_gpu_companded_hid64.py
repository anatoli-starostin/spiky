...
if SMOKE:
    ...
    HIDDEN  = int(os.environ.get("RSNN_HIDDEN", "64"))     # recurrent settling (32->64: T=160, readout [128,160))
...
def main():
    ...
    log("=== RSNN distill Walker2d int4 LUT (GPU + random batches + PHASED HIDDEN=64 T=160 + COMPANDED OUT, H128/32) ===")
    ...
    open(f"{EVO}/walker_rsnn_report_gpu_companded_hid64.txt", "w").write("\n".join(rep))
    json.dump(dict(device=DEV, peak_gpu_mem_gb=gpu_mem, chosen_std=chosen, sweep=sweep,
              ...
              open(f"{EVO}/walker_rsnn_result_gpu_companded_hid64.json", "w"), indent=2)
    log("wrote walker_rsnn_report_gpu_companded_hid64.txt + walker_rsnn_result_gpu_companded_hid64.json")
