...
# ---- LR schedule: linear warmup -> cosine decay to a small floor (peak 2e-3) ----
WARMUP_STEPS = 200
PEAK_LR, FLOOR_LR = 2e-3, 1e-5
def lr_at(s, steps):
    if s < WARMUP_STEPS:
        return PEAK_LR * (s + 1) / WARMUP_STEPS            # linear 0 -> peak over WARMUP_STEPS
    prog = (s - WARMUP_STEPS) / max(1, steps - WARMUP_STEPS)   # 0..1 across the rest
    return FLOOR_LR + 0.5 * (PEAK_LR - FLOOR_LR) * (1.0 + float(np.cos(np.pi * min(1.0, prog))))
...
def train_run(init_std, steps, Xtr, Ytr, log, resample=False):
    ...
    for s in range(steps):
        ...
        if resample:
            ...
            for pg in opt.param_groups:                    # warmup -> cosine LR schedule
                pg["lr"] = lr_at(s, steps)
        ...
        if s % max(1, steps // 10) == 0 or s == steps - 1:
            ...
            log(f"    step {s:4d}  mse={mse:.4f}  lr={opt.param_groups[0]['lr']:.2e}  surr={SURR:.1f}  "
                f"hid_rate={a['h_rate']:.3f} out_rate={a['o_rate']:.3f} o_nonfire={a['o_nonfire']}")
...
def main():
    ...
    log("=== RSNN distill Walker2d int4 LUT (SPARSE fan-out + WARMUP+COSINE LR, H256/64, pinned std=0.1, SURR anneal) ===")
    log(f"LR schedule: linear warmup {WARMUP_STEPS} steps 0->{PEAK_LR:g}, then cosine decay to floor {FLOOR_LR:g}")
    ...
    full_steps = 8 if SMOKE else 4096                     # up from 1000
    ...
    open(f"{EVO}/walker_rsnn_report_gpu_sparse_cosine.txt", "w").write("\n".join(rep))
    json.dump(dict(device=DEV, peak_gpu_mem_gb=gpu_mem, chosen_std=chosen, sweep=sweep,
              ...
              open(f"{EVO}/walker_rsnn_result_gpu_sparse_cosine.json", "w"), indent=2)
    log("wrote walker_rsnn_report_gpu_sparse_cosine.txt + walker_rsnn_result_gpu_sparse_cosine.json")
