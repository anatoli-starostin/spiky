...
def loss_fn(model, Ispk, labels):                         # labels: (B, N_JOINTS) int levels 0..K-1
    ...
    logits = (-act).reshape(-1, N_JOINTS, K_LEVELS)       # earlier E[t] -> higher logit (softmin over the 7-group)
...
def main():
    ...
    log("=== RSNN DISCRETE ARM B (latency classifier, 7 neurons/joint, first-spike CE); SPARSE H256/64 ===")
    ...
    np.savez(f"{EVO}/walker_rsnn_disc_B_policy_outputs.npz",
             pred_levels=ev["pred"], true_levels=ev["true"], levelval=LEVELVAL, qbound=QBOUND, val_obs=Xval)
    open(f"{EVO}/walker_rsnn_report_gpu_disc_B.txt", "w").write("\n".join(rep))
    json.dump(dict(device=DEV, arm="B", peak_gpu_mem_gb=gpu_mem, chosen_std=chosen,
              ...
              open(f"{EVO}/walker_rsnn_result_gpu_disc_B.json", "w"), indent=2)
    log("wrote walker_rsnn_report_gpu_disc_B.txt + walker_rsnn_result_gpu_disc_B.json + policy_outputs.npz")
