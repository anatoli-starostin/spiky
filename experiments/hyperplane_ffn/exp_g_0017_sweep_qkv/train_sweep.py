...
# --- exp_g_0017 ATTENTION-side sweep: env-driven per-run parameterization ------
# COMPLEMENT of nebius's exp101_sweep_outproj, which swept out_proj and held q/k/v
# fixed. Here it is the mirror image: the q/k/v CompressionMHL projections are
# SWEPT (inner_in=QKV_IN, tph=QKV_TPH) and out_proj is PINNED at the exp101
# baseline geometry (inner_in 48 / tph 64 / n_heads 8 / inner_out 48 / nap 6).
# Each run writes ALL outputs into a per-run subdir OUT_DIR = EXP_DIR/RUN_TAG so
# concurrent runs never collide.
QKV_IN  = int(os.environ.get('QKV_IN', cfg.get('compress_inner_in', 48)))
QKV_TPH = int(os.environ.get('QKV_TPH', cfg.get('compress_tph', 32)))
# out_proj HELD. Overridable only so the pin itself is auditable from the log.
OUT_IN  = int(os.environ.get('OUT_IN', 48))
OUT_TPH = int(os.environ.get('OUT_TPH', 64))
RUN_TAG = os.environ.get('RUN_TAG', f'qkvin{QKV_IN}_tph{QKV_TPH}')
OUT_DIR = os.path.join(EXP_DIR, RUN_TAG)
os.makedirs(OUT_DIR, exist_ok=True)
if os.environ.get('N_STEPS'):
    cfg['n_steps'] = int(os.environ['N_STEPS'])
cfg['exp_name'] = f'exp_g_0017_sweep_qkv_{RUN_TAG}'
print(f'[sweep] SWEPT  q/k/v : inner_in={QKV_IN} tph={QKV_TPH} nap=6 '
      f'inner_out=-1 (multihead) heads={cfg["n_heads"]}')
print(f'[sweep] HELD  out_proj: inner_in={OUT_IN} tph={OUT_TPH} nap=6 '
      f'inner_out=48 heads={cfg.get("out_lut_heads", 8)}')
...
_C_IN   = QKV_IN                               # SWEPT: q/k/v LUT input width per head
_C_NAP  = cfg.get('compress_nap', 6)           # routing bits per table (2**nap cells); HELD at 6
_C_TPH  = QKV_TPH                              # SWEPT: q/k/v tables per head
