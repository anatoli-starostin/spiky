...
#   'decompress_only' -> CompressionMHL with inner_in_dim=-1 (NO compress: routes
#                    on the full n_embd input, verified has_compress=False and
#                    compress is nn.Identity) but inner_out_dim kept positive, so
#                    a real decompress Linear(n_heads*inner_out -> n_embd) stays.
#                    Table cells are inner_out wide instead of n_embd wide, which
#                    is what keeps the parameter count near exp_n_0045's.
FFN_LUT_KIND = cfg.get('ffn_lut_kind', 'compression')
assert FFN_LUT_KIND in ('compression', 'fastmhl_raw', 'decompress_only'), FFN_LUT_KIND
RAW_INNER_OUT = int(cfg.get('raw_inner_out', 48))   # decompress_only cell width
...
class MinimalBlock(nn.Module):
    def __init__(self, n_embd, n_head, layer_idx):
        ...
        if FFN_TYPE == 'dense':
            ...
            if FFN_LUT_KIND == 'decompress_only':
                # No compress (inner_in_dim=-1 -> routes on the full n_embd), but
                # KEEP decompress: cells emit RAW_INNER_OUT wide and a Linear
                # projects n_heads*RAW_INNER_OUT -> n_embd.
                self.ffn = CompressionMultiHeadLUT(
                    input_dim=n_embd, output_dim=n_embd,
                    inner_in_dim=-1, inner_out_dim=RAW_INNER_OUT,
                    nap=RAW_NAP, tph=RAW_TPH, n_heads=RAW_H,
