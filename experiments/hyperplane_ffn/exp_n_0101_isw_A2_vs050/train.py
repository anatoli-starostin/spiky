...
# max_train_steps: stop training early WITHOUT changing the LR schedule (which is
# keyed to N_STEPS). Lets sweep runs keep exp_n_0084's full 16k warmup/cosine so
# their first ~3600 steps are directly comparable to 0084's — then break.
MAX_STEPS = int(cfg.get('max_train_steps', cfg['n_steps']))
...
# V-LUT knobs: route the attention V projection through a CompressionMHL instead
# of the dense Linear V slice. Gated by cfg['v_lut']; q/k stay dense. The v_lut
# emits output_dim=n_embd (== n_head*d), reshaped to attention heads for SDPA.
# Its n_heads are ROUTING mini-heads (compress groups), independent of attn heads.
V_LUT_ENABLE = bool(cfg.get('v_lut', False))
V_LUT_IN     = cfg.get('v_lut_inner_in_dim', 24)
V_LUT_OUT    = cfg.get('v_lut_inner_out_dim', 24)
V_LUT_NAP    = cfg.get('v_lut_n_anchor_pairs', 6)
...
# optional separate AdamW LR for the v_lut TABLE cells (conditioning probe P1/P2).
# None => v_lut table stays in the shared nodecay group at the global lr.
V_LUT_TABLE_LR = cfg.get('v_lut_table_lr', None)
# optional init-(C): scale the v_lut decompress WEIGHT (bias already zeroed) to hit
# step-0 V RMS instead of a hot table init. None => no scaling.
V_LUT_DECOMP_SCALE = cfg.get('v_lut_decompress_weight_scale', None)
...
class MinimalAttention(nn.Module):
    def __init__(self, n_embd, n_head, layer_idx=0):
        super().__init__()
        self.n_head = n_head
        self.v_lut_enable = V_LUT_ENABLE
        if V_LUT_ENABLE:
            # q,k stay dense; V is routed through a CompressionMHL emitting n_embd.
            self.qk = nn.Linear(n_embd, 2 * n_embd, bias=False)
            self.v_lut = CompressionMultiHeadLUT(
                input_dim=n_embd, output_dim=n_embd,
                inner_in_dim=V_LUT_IN, inner_out_dim=V_LUT_OUT,
    ...
    def forward(self, x, cos, sin):
        B, T, C = x.size()
        if self.v_lut_enable:
            q, k = self.qk(x).split(C, dim=2)
            v = self.v_lut(x.reshape(B * T, C)).reshape(B, T, C).to(x.dtype)
        else:
    ...
    def __init__(self, vocab_size, n_embd, n_head, n_layer, seq_len):
        ...
        for block in self.blocks:
            ...
            if V_LUT_ENABLE and getattr(block.attn, 'v_lut', None) is not None \
                ...
                # init-(C): scale the v_lut decompress WEIGHT to reach step-0 V RMS
                if V_LUT_DECOMP_SCALE is not None:
                    with torch.no_grad():
                        block.attn.v_lut.decompress.weight.mul_(float(V_LUT_DECOMP_SCALE))
            if FFN_TYPE == 'dense':
...
def setup_optimizer(model, lr, weight_decay):
    ...
    # v_lut table params (probe P1/P2): optionally split into their own LR group.
    vlut_tbl_ids = set()
    if V_LUT_ENABLE and V_LUT_TABLE_LR is not None:
        for block in model.blocks:
            vl = getattr(block.attn, 'v_lut', None)
            if vl is not None and hasattr(vl, 'lut_batched'):
                vlut_tbl_ids.add(id(vl.lut_batched.weights))
    decay, nodecay, vlut_tbl = [], [], []
    for p in model.parameters():
        if not p.requires_grad:
            continue
        if id(p) in vlut_tbl_ids:
            vlut_tbl.append(p)
        elif id(p) in lut_ids or p.ndim < 2:
            nodecay.append(p)
        else:
            decay.append(p)
    ...
    if vlut_tbl:
        groups.append(dict(params=vlut_tbl, lr=float(V_LUT_TABLE_LR), betas=(0.9, 0.95), eps=1e-8, weight_decay=0.0))
    opt = torch.optim.AdamW(groups)
    for g in opt.param_groups:
        g['initial_lr'] = g['lr']
    n_lut = sum(m.weights.numel() for m in model.modules() if isinstance(m, FastMultiHeadLut))
    extra = (f" | v_lut_table_group={sum(p.numel() for p in vlut_tbl):,} lr={V_LUT_TABLE_LR} wd=0" if vlut_tbl else "")
    print(f"AdamW (0033 grouping) | decay(2-D weights)={sum(p.numel() for p in decay):,} wd={weight_decay} | "
          ...
          f"eps=1e-8 [LUT tables={n_lut:,}]{extra}")
    return opt
