...
class Model(nn.Module):
    ...
    def forward(self, tokens, targets=None, loss_reduction='mean'):
        ...
        # HyperplaneCodeLUT unembedder: [B*T, E] -> [B*T, VOCAB]
        logits = self.unembedder_lut(x_flat).float().view(B, T, VOCAB_SIZE)
...
print(f'exp019 SINGLE-STREAM E={E} (no residual_lut / emb_resid / D-stream); LayerNorm everywhere')
print(f'UNEMBEDDER HyperplaneCodeLUT: nap={cfg["code_unemb_nap"]} (V=2^nap={1 << cfg["code_unemb_nap"]}) T={cfg["code_unemb_tables"]} w_cell_init={cfg.get("code_unemb_wcell_init",0.02)}; input=E={E}; ln_final(E) KEPT before unembedder')
