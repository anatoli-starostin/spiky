...
_COMBINE = cfg.get('layer_combine', 'concat')
assert _COMBINE in ('concat', 'residual_sum'), _COMBINE
CONCAT_DIM = N_LAYERS * E

def _make_q(seed_offset):
    return CompressionMultiHeadLUTMH(
        input_dim=E, output_dim=d_qk, inner_in_dim=_QK_IN, inner_out_dim=-1,
...
def _make_v(seed_offset):
    return CompressionMultiHeadLUTMH(
        input_dim=E, output_dim=d_v, inner_in_dim=_V_IN, inner_out_dim=-1,
...
class LUTBlock(nn.Module):
    ...
    def forward(self, x, cos, sin):
        ...
        # exp_g_0018: 'concat' emits the block's OWN output (no skip) -- the next
        # block reads it directly and every layer output is kept for the final
        # concatenation. 'residual_sum' restores exp103's single residual stream.
        x_next = out_e if _COMBINE == 'concat' else x_flat + out_e
...
class Model(nn.Module):
    ...
    def forward(self, tokens, targets=None, loss_reduction='mean'):
        B, T = tokens.shape
        x = self.tok_emb_E(tokens)                       # [B,T,E] token embedding
        outs = []
        for layer in self.layers:
            x = layer(x, self.rope.cos, self.rope.sin)   # [B,T,E]; block reads the
            outs.append(x)                               # PREVIOUS layer's output
        # Concatenate every layer's output along the feature axis: [B,T,N_LAYERS*E].
        # Note the token embedding itself is NOT concatenated -- only the N_LAYERS
        # block outputs, so concat_dim is exactly N_LAYERS*E.
        x = torch.cat(outs, dim=-1)                      # [B,T,CONCAT_DIM]
        x_flat = self.ln_final(x.reshape(B * T, CONCAT_DIM))
...
if os.environ.get('SMOKE'):
    ...
    print(f'[SMOKE] per-site params (x{N_LAYERS} layers each for q/k/v/out): '
          f'q_lut={q_p:,} k_lut={k_p:,} v_lut={v_p:,} out_proj={o_p:,} | '
          f'rest(attn+emb+unembed+norms)={rest:,} | TOTAL={n_params:,}')
