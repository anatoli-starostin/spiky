...
def _ortho_init_compress_heads(ffn):
    """exp_n_0036's new ingredient: orthogonal per-head init of the compression projection.
    For the independent-per-head CompressionMHL, `compress.weight` is [n_heads*inner_in, input_dim]; each
    head's [inner_in, input_dim] row-block (rows h*inner_in : (h+1)*inner_in, matching forward's
    .view(N, n_heads, inner_in)) is initialised with nn.init.orthogonal_ so that head's inner_in output
    directions start orthonormal in the input space. For the joint mode the single shared compress is made
    orthonormal directly. No-op if the slot has no compress projection."""
    if not getattr(ffn, 'has_compress', False):
        return
    W = ffn.compress.weight                       # [n_heads*inner_in, input_dim] (indep) or [inner_in, input_dim] (joint)
    ...
    for h in range(H):
        ...
        torch.nn.init.orthogonal_(q)               # orthonormal rows (din < input_dim -> semi-orthogonal)
        Wv[h].copy_(q)
...
class MinimalGPT(nn.Module):
    def __init__(self, vocab_size, n_embd, n_head, n_layer, seq_len):
        ...
        if LUT_ORTHO and FFN_TYPE != 'dense':
            for block in self.blocks:                            # exp_n_0036: orthogonal per-head compress init
                _ortho_init_compress_heads(block.ffn)
        if TIE:
...
def setup_optimizers(model, lr, weight_decay):
    """AdamW EVERYWHERE (exp_n_0036 baseline): ONE standard AdamW over all params — NO Lion, no hybrid.
    Grouping is the usual weight-decay convention established in this experiment line (exp_n_0033's
    single-AdamW setup): the LUT table tensors + the 0-dim learnable temps + all 1-D params go to the
    nodecay group (wd 0); the 2-D weights (attn / compress / decompress / mlp / embedding) go to the decay
    group (wd 0.1). lr 3e-4, betas (0.9,0.95), eps 1e-8. Returns [adam] (a 1-element list so the training
    loop's `for o in optimizers` stays unchanged)."""
    lut_ids = {id(m.weights) for m in model.modules() if isinstance(m, FastMultiHeadLut)}
    decay, nodecay = [], []
    for p in model.parameters():
        if not p.requires_grad:
            continue
        if id(p) in lut_ids or p.ndim < 2:
            nodecay.append(p)          # LUT tables + learnable temps + 1-D params -> no weight decay
        else:
            decay.append(p)            # 2-D weights -> weight decay
    ...
    print(f"AdamW-everywhere (no Lion) | decay(2-D weights)={sum(p.numel() for p in decay):,} wd={weight_decay} | "
...
optimizers = setup_optimizers(model, LR, WD)
