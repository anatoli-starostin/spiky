...
class LUTBlock(nn.Module):
    def __init__(self, layer_idx):
        ...
        self.ln_pre  = nn.LayerNorm(E)   # exp018 A/B: was MeanAbsNorm(E)
        self.ln_resid = nn.LayerNorm(E)  # exp018 A/B: was MeanAbsNorm(E) (pre-norm before residual_lut)
...
class Model(nn.Module):
    def __init__(self):
        ...
        self.ln_emb_resid  = nn.LayerNorm(E)   # exp018 A/B: was MeanAbsNorm(E)
...
print(f'residual_lut TinyMHLut(soft) [per-layer, x{N_LAYERS}]: in_nap={cfg["residual_input_nap"]} tph={cfg["residual_tph"]} n_out=D={D}; LayerNorm(E) before each [exp018: was MeanAbsNorm]')
