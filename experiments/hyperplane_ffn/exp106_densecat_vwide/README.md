# exp106_densecat_vwide — exp105_densecat with v_lut widened (d_v 16 → 64)

**Single change vs exp105:** v_lut per-head output `d_v` 16 → 64 (back to full width). This
auto-grows out_proj input `H·d_v` from 6·16=96 to 6·64=384 (v_lut output_dim=d_v, out_proj
input_dim=H·d_v, forward reshape all derive from d_v). q/k unchanged (d_qk=64). Everything
else identical to exp105 (dense-concat, readout 448→384). **Total 63,759,666** (+7,741,440
vs exp105's 56,018,226: v_lut tables 4× wider +7,077,888; out_proj compress 96→384 input
+663,552). Verified via real forward (v out [N,6,64], out_proj [N,64], logits [B,512,32768]).

---
(exp105 architecture notes below still apply.)
