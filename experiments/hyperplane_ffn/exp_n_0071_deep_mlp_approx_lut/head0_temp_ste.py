"""exp_n_0071 (head-0, hard-forward + TRAINABLE-TEMPERATURE soft backward STE) —
the FastMHL-style trick: exact hard bit forward (starts in the 0.867 regime), but
gradients flow through a temperature-scaled sigmoid whose slope k is learnable.
Does the learned-temp backward beat plain STE's 0.867, or match it?

Front-end:
  head-0 exact 384 anchor pairs (A,B); d = x[:,A]-x[:,B] -> [.,384]
  hard = (d>0).float()          # {0,1} exact hard bit
  soft = sigmoid(k*d)           # k = per-feature trainable temp (nn.Parameter, len 384, init 1.0)
  b = soft + (hard - soft).detach()   # forward = hard {0,1}; backward = grad of sigmoid(k*d) (k trainable)
Body: SAME wide body as the STE run — Linear(384->16384)->GELU->Linear(16384->48),
output linear zero-init, first linear default init. Same seed/opt/steps (40k, val R2
every 2k). Same target/setup as prior head-0 runs. No shared-module edits.
"""
...
class TempSTENet(nn.Module):
    def __init__(self, A, B, nfeat, H, dout):
        super().__init__()
        self.register_buffer('A', A); self.register_buffer('B', B)
        self.k = nn.Parameter(torch.ones(nfeat))      # per-feature trainable temperature, init 1.0
        self.body = nn.Sequential(nn.Linear(nfeat, H), nn.GELU(), nn.Linear(H, dout))
        nn.init.zeros_(self.body[2].weight); nn.init.zeros_(self.body[2].bias)   # zero-init output
    def forward(self, x):
        d = x[:, self.A] - x[:, self.B]               # exact routing diffs [N,384]
        hard = (d > 0).float()                        # {0,1} exact hard bit
        soft = torch.sigmoid(self.k * d)              # temperature-scaled sigmoid (k trainable)
        b = soft + (hard - soft).detach()             # forward = hard; backward = grad of soft (k,x)
        return self.body(b)
...
net=TempSTENet(A,B_,NFEAT,HID,DOUT).to(DEV)
# verify forward is exactly {0,1}
with torch.no_grad():
    dd = Xv[:32,A]-Xv[:32,B_]; hard=(dd>0).float(); soft=torch.sigmoid(net.k*dd); bb=soft+(hard-soft)
    print(f'forward-bit uniques (should be {{0,1}}): {torch.unique(bb).tolist()}')
...
traj=[]; print(f'--- temp-STE (hard fwd, learned-temp bwd) + wide {NFEAT}->{HID}->{DOUT} params={sum(p.numel() for p in net.parameters()):,} | {FIT_STEPS} steps ---')
def kstats():
...
plt.plot([t['step'] for t in traj],[t['r2'] for t in traj],'o-',c='tab:brown',label='hard fwd + trainable-temp bwd STE')
plt.axhline(0.867, ls='--', c='tab:cyan', label='plain hard-sign STE wide 0.867')
plt.axhline(0.823, ls='--', c='tab:purple', label='soft-forward sigmoid (no STE) 0.823')
plt.axhline(1.0, ls=':', c='green', alpha=0.5)
plt.xlabel('training step'); plt.ylabel('val R2 (approx head0)'); plt.title('head0: hard-forward + trainable-temperature backward STE — R2 vs step (40k)')
plt.legend(fontsize=8); plt.grid(True,alpha=0.3); plt.tight_layout(); plt.savefig(os.path.join(EXP_DIR,'head0_temp_ste_curves.png'),dpi=120); plt.close()

summary={'exp_name':'exp_n_0071_head0_temp_ste_16384_40k','target':'exp_n_0052 block0 head0 (48->48)',
         'front_end':'hard fwd (d>0) + backward grad of sigmoid(k*d), k per-feature trainable temp (len 384, init 1.0)',
         'H':HID,'fit_steps':FIT_STEPS,'body_init':'first default; output zero-init',
         'refs':{'plain_hard_sign_ste_wide':0.867,'soft_forward_sigmoid':0.823},'learned_k_stats':ks,
         'final_r2':final['r2'],'peak_r2':peak['r2'],'peak_step':peak['step'],'final_val_mse':final['val_mse'],'trajectory':traj}
json.dump(summary,open(os.path.join(EXP_DIR,'head0_temp_ste_summary.json'),'w'),indent=2)
print('\n=== DONE ==='); print(f"final R2={final['r2']} peak={peak['r2']}@{peak['step']} | refs: plain-STE 0.867, soft-sigmoid 0.823 | k={ks}")
