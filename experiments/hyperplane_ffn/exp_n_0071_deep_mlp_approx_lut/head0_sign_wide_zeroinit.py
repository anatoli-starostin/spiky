"""exp_n_0071 (head-0, hard-sign STE {0,1} + wide 16384, ZERO-INIT output) — does
zero-initializing the output projection remove the warmup instability and/or reach
a higher final R2 than the default-init wide run (commit 37d32491, 0.867, noisy
warmup, still climbing at 40k)?

Identical to head0_sign_wide.py EXCEPT the explicit init below:
  - first Linear(384->16384): PyTorch DEFAULT (Kaiming-uniform weight, default bias) -- unchanged.
  - output Linear(16384->48): weight = zeros, bias = zeros  -> model starts at exactly 0 output.
Everything else identical (seed, optimizer, 40k steps, cosine+warmup+grad-clip, batch 8192).
Front-end = hard-sign STE {0,1} on head-0 exact 384 anchor pairs. No shared-module edits.
"""
...
# --- explicit init (ONLY change vs 37d32491) ---
# first linear net[0]: keep PyTorch DEFAULT Kaiming-uniform weight + default bias (unchanged).
# output linear net[2]: zero-init weight AND bias -> model starts at exactly 0 output.
nn.init.zeros_(net[2].weight)
nn.init.zeros_(net[2].bias)
print(f'init: net[0] default Kaiming-uniform (unchanged); net[2] weight/bias ZERO-init '
      f'(|W_out|={net[2].weight.abs().sum().item():.1f}, |b_out|={net[2].bias.abs().sum().item():.1f})')
...
traj=[]; print(f'--- sign-STE(0,1) wide ZERO-INIT out {DIN}->{NFEAT}(bits)->{HID}->{DOUT} params={sum(p.numel() for p in net.parameters()):,} | {FIT_STEPS} steps ---')
net.train()
...
plt.plot([t['step'] for t in traj],[t['r2'] for t in traj],'o-',c='tab:green',label='ZERO-init output (this run)')
plt.axhline(0.8672, ls='--', c='tab:cyan', label='DEFAULT-init wide 0.867 (37d32491)')
plt.axhline(0.867, ls=':', c='tab:red', label='sign-STE deep6 0.867')
plt.axhline(1.0, ls=':', c='green', alpha=0.5)
plt.xlabel('training step'); plt.ylabel('val R2 (approx head0)'); plt.title('head0: hard-sign STE {0,1} + wide 16384, ZERO-init output — R2 vs step (40k)')
plt.legend(fontsize=8); plt.grid(True,alpha=0.3); plt.tight_layout(); plt.savefig(os.path.join(EXP_DIR,'head0_sign_wide_zeroinit_curves.png'),dpi=120); plt.close()

summary={'exp_name':'exp_n_0071_head0_sign_wide01_16384_zeroinit_40k','target':'exp_n_0052 block0 head0 (48->48)',
         'front_end':'hard-sign STE of exact 384 anchor diffs, {0,1}','H':HID,'fit_steps':FIT_STEPS,
         'init':'net[0] default Kaiming-uniform; net[2] (output) weight=0 bias=0',
         'refs':{'default_init_wide_37d32491':0.8672,'sign_ste_deep6':0.867},
         'final_r2':final['r2'],'peak_r2':peak['r2'],'peak_step':peak['step'],'final_val_mse':final['val_mse'],'trajectory':traj}
json.dump(summary,open(os.path.join(EXP_DIR,'head0_sign_wide_zeroinit_summary.json'),'w'),indent=2)
print('\n=== DONE ==='); print(f"final R2={final['r2']} peak={peak['r2']}@{peak['step']} | ref default-init 0.8672")
