"""exp_n_0071 (head-0, OPTIMIZER SWEEP to push past the ~0.867 ceiling) — fixed best
architecture, tune batch/weight-decay/length. The wide config was still climbing at
40k, so train to 100k.

FIXED arch (identical across all runs): front-end = hard-sign STE {0,1} on head-0
exact 384 anchor pairs; body = Linear(384->16384)->GELU->Linear(16384->48), output
linear ZERO-init (as e4238dc8), first linear default init. Same target/data.

MATRIX (4 configs), each 100k steps, cosine LR to 100k + 5% warmup + grad-clip 1.0:
  1. batch  8192, wd 0.0, peak LR 1e-3   (base)
  2. batch  8192, wd 0.1, peak LR 1e-3
  3. batch 32768, wd 0.0, peak LR 4e-3   (4x batch -> ~4x peak LR, linear scaling)
  4. batch 32768, wd 0.1, peak LR 4e-3
Log val R2 every 4k. Reference: hard-sign STE wide @ batch8192/40k = 0.867. No shared edits.
"""
...
FIT_WARM = cfg['fit_warmup_frac']; TPH, NAP = cfg['lut_tables_per_head'], cfg['lut_n_anchor_pairs']
HID, FIT_STEPS, EVAL_EVERY = 16384, 100000, 4000
# (batch, weight_decay, peak_lr, label)
CONFIGS = [(8192,0.0,1e-3,'b8192_wd0.0'),(8192,0.1,1e-3,'b8192_wd0.1'),
           (32768,0.0,4e-3,'b32768_wd0.0'),(32768,0.1,4e-3,'b32768_wd0.1')]
if os.environ.get('SMOKE'):
    N_COLLECT = 24576; FIT_STEPS = 400; EVAL_EVERY = 200; CONFIGS = [(8192,0.0,1e-3,'b8192_wd0.0'),(32768,0.1,4e-3,'b32768_wd0.1')]; print('*** SMOKE ***')
...
def sign_ste01(x):
    d = x[:, A] - x[:, B_]; s = torch.sign(d)*0.5 + 0.5
    return d + (s - d).detach()
...
def lr_at(st, peak):
    ...
    if st<w: return peak*st/max(w,1)
    p=(st-w)/max(FIT_STEPS-w,1); return peak*(0.05+0.95*0.5*(1+math.cos(math.pi*p)))

def run(batch, wd, peak, label):
    torch.manual_seed(cfg['random_seed'])   # same init draw for every config (clean A/B)
    net=nn.Sequential(nn.Linear(NFEAT,HID),nn.GELU(),nn.Linear(HID,DOUT)).to(DEV)
    nn.init.zeros_(net[2].weight); nn.init.zeros_(net[2].bias)   # zero-init output
    opt=torch.optim.AdamW(net.parameters(),lr=peak,weight_decay=wd,betas=(0.9,0.95)); g=torch.Generator(device=DEV).manual_seed(0)
    traj=[]; print(f'--- {label}: batch={batch} wd={wd} peak_lr={peak:.0e} params={sum(p.numel() for p in net.parameters()):,} | {FIT_STEPS} steps ---')
    net.train()
    for st in range(1,FIT_STEPS+1):
        lr=lr_at(st,peak)
        for gp in opt.param_groups: gp['lr']=lr
        idx=torch.randint(0,ntr,(min(batch,ntr),),device=DEV,generator=g)
        ...
        if st%EVAL_EVERY==0 or st==FIT_STEPS:
            ...
            print(f'   {label} step {st:6d} | lr={lr:.2e} | val_mse={vm:.6f} R2={r2:.5f}')
    return traj

results={}
for (batch,wd,peak,label) in CONFIGS:
    results[label] = run(batch,wd,peak,label)

plt.figure(figsize=(10,6))
cols={'b8192_wd0.0':'tab:blue','b8192_wd0.1':'tab:cyan','b32768_wd0.0':'tab:red','b32768_wd0.1':'tab:orange'}
for (batch,wd,peak,label) in CONFIGS:
    tr=results[label]; plt.plot([t['step'] for t in tr],[t['r2'] for t in tr],'o-',ms=3,c=cols.get(label),label=f'{label} (peakLR {peak:.0e})')
plt.axhline(0.867, ls='--', c='k', label='hard-sign STE wide @b8192/40k = 0.867')
plt.axhline(1.0, ls=':', c='green', alpha=0.5)
plt.xlabel('training step'); plt.ylabel('val R2 (approx head0)'); plt.title('head0: optimizer sweep (batch x weight-decay), hard-sign STE wide, 100k')
plt.legend(fontsize=8); plt.grid(True,alpha=0.3); plt.tight_layout(); plt.savefig(os.path.join(EXP_DIR,'head0_opt_sweep_curves.png'),dpi=120); plt.close()

summary={'exp_name':'exp_n_0071_head0_opt_sweep_100k','target':'exp_n_0052 block0 head0 (48->48)',
         'fixed_arch':'hard-sign STE {0,1} + Linear(384->16384)->GELU->Linear(16384->48), output zero-init',
         'fit_steps':FIT_STEPS,'ref_b8192_40k':0.867,
         'configs':[{'label':l,'batch':b,'weight_decay':w,'peak_lr':p} for (b,w,p,l) in CONFIGS],
         'final_r2':{l:results[l][-1]['r2'] for (b,w,p,l) in CONFIGS},
         'best_r2':{l:max(t['r2'] for t in results[l]) for (b,w,p,l) in CONFIGS},
         'trajectories':results}
json.dump(summary,open(os.path.join(EXP_DIR,'head0_opt_sweep_summary.json'),'w'),indent=2)
print('\n=== DONE ==='); print('final R2:', summary['final_r2']); print('best  R2:', summary['best_r2'], '| ref 0.867')
