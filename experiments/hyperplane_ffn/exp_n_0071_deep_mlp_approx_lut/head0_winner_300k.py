"""exp_n_0071 (head-0, WINNING config extended to 300k) — the opt-sweep winner
(batch 32768, wd 0.1, peak LR 4e-3) reached R2=0.9232 at 100k STILL CLIMBING.
Extend to 300k to find where it actually plateaus.

Config (fixed): hard-sign STE {0,1} on head-0 exact 384 anchor bits ->
Linear(384->16384)->GELU->Linear(16384->48), output zero-init, first linear default.
batch 32768, weight_decay 0.1, peak LR 4e-3, AdamW, cosine (5% warmup), grad-clip 1.0.
300k steps, log val R2 every 10k. Same target/setup as prior head-0 runs. No shared edits.
"""
...
HID, FIT_STEPS, EVAL_EVERY = 16384, 300000, 10000
BATCH, WD, PEAK_LR = 32768, 0.1, 4e-3
if os.environ.get('SMOKE'):
    N_COLLECT = 24576; FIT_STEPS = 600; EVAL_EVERY = 300; print('*** SMOKE ***')
...
def lr_at(st):
    ...
    if st<w: return PEAK_LR*st/max(w,1)
    p=(st-w)/max(FIT_STEPS-w,1); return PEAK_LR*(0.05+0.95*0.5*(1+math.cos(math.pi*p)))
...
opt=torch.optim.AdamW(net.parameters(),lr=PEAK_LR,weight_decay=WD,betas=(0.9,0.95)); g=torch.Generator(device=DEV).manual_seed(0)
traj=[]; print(f'--- winner 300k: batch={BATCH} wd={WD} peakLR={PEAK_LR:.0e} params={sum(p.numel() for p in net.parameters()):,} | {FIT_STEPS} steps ---')
net.train()
for st in range(1,FIT_STEPS+1):
    ...
    if st%EVAL_EVERY==0 or st==FIT_STEPS:
        ...
        print(f'   step {st:7d} | lr={lr:.2e} | val_mse={vm:.6f} R2={r2:.5f}')
final=traj[-1]; peak=max(traj,key=lambda t:t['r2'])
print(f'final R2={final["r2"]} | peak R2={peak["r2"]}@{peak["step"]}')

plt.figure(figsize=(10,6))
plt.plot([t['step'] for t in traj],[t['r2'] for t in traj],'o-',ms=3,c='tab:orange',label='winner b32768/wd0.1/4e-3, 300k')
plt.axhline(0.9232, ls='--', c='tab:red', label='same config @100k = 0.9232')
plt.axhline(0.8762, ls='--', c='gray', label='b8192/wd0 @100k = 0.876')
plt.axhline(1.0, ls=':', c='green', alpha=0.5)
plt.xlabel('training step'); plt.ylabel('val R2 (approx head0)'); plt.title('head0: winning config extended to 300k — where does it plateau?')
plt.legend(fontsize=8); plt.grid(True,alpha=0.3); plt.tight_layout(); plt.savefig(os.path.join(EXP_DIR,'head0_winner_300k_curves.png'),dpi=120); plt.close()

summary={'exp_name':'exp_n_0071_head0_winner_300k','target':'exp_n_0052 block0 head0 (48->48)',
         'config':{'front_end':'hard-sign STE {0,1}','body':'Linear(384->16384)->GELU->Linear(16384->48) output zero-init',
                   'batch':BATCH,'weight_decay':WD,'peak_lr':PEAK_LR,'steps':FIT_STEPS},
         'refs':{'same_config_100k':0.9232,'b8192_wd0_100k':0.8762},
         'final_r2':final['r2'],'peak_r2':peak['r2'],'peak_step':peak['step'],'final_val_mse':final['val_mse'],'trajectory':traj}
json.dump(summary,open(os.path.join(EXP_DIR,'head0_winner_300k_summary.json'),'w'),indent=2)
print('\n=== DONE ==='); print(f"final R2={final['r2']} peak={peak['r2']}@{peak['step']} | ref same-config@100k 0.9232")
