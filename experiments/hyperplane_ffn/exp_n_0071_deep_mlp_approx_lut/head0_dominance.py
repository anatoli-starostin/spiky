"""exp_n_0071 (head-0, DOMINANCE feature map) — does exposing the raw pairwise
differences (the exact quantities the FastMHL sign-tests operate on) break past
the 0.7884 depth floor, ideally toward 1.0?

Fixed, non-learned feature map: given x[.,48], compute all pairwise signed diffs
x[:,i]-x[:,j] for i<j (48*47/2 = 1128 features) via precomputed triu index pairs.
Then a plain 2-layer MLP on top:
    x[.,48] --dominance--> [.,1128] -> Linear(1128->4096) -> GELU -> Linear(4096->48)
bias=True, no norm, no residual; dominance has NO learnable params. Same target/
setup as the head-0 runs (block0 head0 of exp_n_0052, input z[:,0,:], target
...
# --- dominance feature map: all pairwise signed diffs x[i]-x[j], i<j -> 1128 dims ---
II, JJ = torch.triu_indices(DIN, DIN, offset=1)   # 1128 index pairs, precomputed once
II, JJ = II.to(DEV), JJ.to(DEV); NFEAT = II.numel()
assert NFEAT == DIN*(DIN-1)//2 == 1128, NFEAT
def dominance(x): return x[:, II] - x[:, JJ]       # [N,48] -> [N,1128]
...
# verify dominance transform: x[:,i]-x[:,j] matches for a couple of pairs
with torch.no_grad():
    F0=dominance(X[:8])
    assert F0.shape==(8,1128)
    for (i,j),col in [((0,1),0),((0,47),46)]:
        assert torch.allclose(F0[:,col], X[:8,i]-X[:8,j]), (i,j,col)
print(f'head0 collected: X={tuple(X.shape)} Y={tuple(Y.shape)} | dominance feat dim={NFEAT} (verified)')
...
Fv=dominance(Xv)   # val features precomputed once

# plain 2-layer MLP on dominance features: Linear(1128->4096)->GELU->Linear(4096->48)
...
traj=[]; print(f'--- dominance {DIN}->{NFEAT}->{HID}->{DOUT} params={sum(p.numel() for p in net.parameters()):,} | {FIT_STEPS} steps ---')
net.train()
for st in range(1,FIT_STEPS+1):
    ...
    loss=F.mse_loss(net(dominance(Xtr[idx])),Ytr[idx]); opt.zero_grad(); loss.backward()
...
plt.plot([t['step'] for t in traj],[t['r2'] for t in traj],'o-',c='tab:green',label='dominance -> Linear(1128->4096)->GELU->Linear')
...
plt.xlabel('training step'); plt.ylabel('val R2 (approx head0)'); plt.title('head0: DOMINANCE (pairwise-diff) features -> MLP — R2 vs step (40k)')
plt.legend(fontsize=8); plt.grid(True,alpha=0.3); plt.tight_layout(); plt.savefig(os.path.join(EXP_DIR,'head0_dominance_curves.png'),dpi=120); plt.close()

summary={'exp_name':'exp_n_0071_head0_dominance_1128_4096_40k','target':'exp_n_0052 block0 head0 (48->48)',
         'arch':f'dominance({DIN}->{NFEAT}) -> Linear({NFEAT}->{HID})->GELU->Linear({HID}->{DOUT})','fit_steps':FIT_STEPS,
         'refs':{'shallow_depth2':0.71,'wide_H4096':0.752,'wide_H8192':0.759,'funnel_4096_512_peak':0.780,'deep_depth3_w32_H1536':0.7884},
         'final_r2':final['r2'],'peak_r2':peak['r2'],'peak_step':peak['step'],'final_val_mse':final['val_mse'],'trajectory':traj}
json.dump(summary,open(os.path.join(EXP_DIR,'head0_dominance_summary.json'),'w'),indent=2)
print('\n=== DONE ==='); print(f"final R2={final['r2']} peak={peak['r2']}@{peak['step']} | refs: funnel 0.780, depth3xw32 0.7884")
