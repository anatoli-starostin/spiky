...
def _make_unembedder(seed_offset):
    """exp016 unembedder = HyperplaneMultiHeadLUT mapping the dim-E hidden state
    DIRECTLY to VOCAB logits: `unemb_nap` learned sign-tests on the E-dim stream
    pick 1 of 2^nap rows per table, `unemb_tph` tables are summed, and n_heads=1
    with n_outputs=VOCAB gives the full logit vector. Always HyperplaneMultiHeadLUT
    (anchor-pairs init), regardless of lut_layer_type."""
    n_heads_u = cfg.get('unemb_n_heads', 1)
    return HyperplaneMultiHeadLUT(
        input_dim=E, n_heads=n_heads_u, n_outputs=VOCAB_SIZE // n_heads_u,
        n_anchor_pairs=cfg['unemb_nap'], tables_per_head=cfg['unemb_tph'],
        forward_mode=cfg.get('forward_mode', 'hard'),
        weight_dtype=_WEIGHT_DTYPE, hyperplane_dtype=_HYPER_DTYPE,
        hyperplane_init=cfg.get('hyperplane_init', 'anchor_pairs'),
...
class Model(nn.Module):
    ...
    def forward(self, tokens, targets=None, loss_reduction='mean'):
        ...
        logits = self.unembedder_lut(x_flat).squeeze(1).float().view(B, T, VOCAB_SIZE)
...
print(f'UNEMBEDDER HyperplaneMHL: in_nap={cfg["unemb_nap"]} tph={cfg["unemb_tph"]} n_heads={cfg.get("unemb_n_heads",1)} n_out=V={VOCAB_SIZE}; input=E={E}; tok_emb_E UNTIED; ln_final(E); RoPE base={_ROPE_BASE}')
