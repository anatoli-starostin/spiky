...
FFN_VARIANT = cfg.get('ffn_variant', 'standard')       # "standard" | "disjoint_compress_joint_decompress"
...
class MinimalBlock(nn.Module):
    def __init__(self, n_embd, n_head, layer_idx):
        ...
        elif FFN_VARIANT == 'disjoint_compress_joint_decompress':
            # DISJOINT per-head compress (joint_head_compression=False) + per-head LUT emitting
            # at inner-width (output_dim=inner_in, inner_out_dim=-1 -> heads summed in the
            # inner-width space), then ONE shared JOINT decompress Linear(inner_in -> n_embd).
            self.lin = None
            self.ffn = CompressionMultiHeadLUT(
                input_dim=n_embd, output_dim=LUT_IN,
                inner_in_dim=LUT_IN, inner_out_dim=-1,
                nap=LUT_NAP, tph=LUT_TPH, n_heads=LUT_HEADS,
                joint_head_compression=False, forward_mode=LUT_FWD,
                use_bf16=LUT_BF16, initial_weights_noise=LUT_NOISE,
                random_seed=LUT_SEED + layer_idx)
            self.joint_decompress = nn.Linear(LUT_IN, n_embd)   # the single shared decompress
        else:
    ...
    def forward(self, x, cos, sin):
        ...
        if self.ffn_type == 'dense':
            return x + self.mlp(h)
        B, T, C = h.shape
        y = self.ffn(h.reshape(B * T, C))                       # [B*T, C] or [B*T, inner_in]
        if FFN_VARIANT == 'disjoint_compress_joint_decompress':
            y = self.joint_decompress(y)                        # inner_in -> n_embd (shared)
        out = y.reshape(B, T, C).to(h.dtype)
    ...
    def __init__(self, vocab_size, n_embd, n_head, n_layer, seq_len):
        ...
        for block in self.blocks:
            ...
            if FFN_TYPE == 'dense':
                ...
                if getattr(block, 'joint_decompress', None) is not None:
                    nn.init.zeros_(block.joint_decompress.weight) # shared joint decompress (variant)
