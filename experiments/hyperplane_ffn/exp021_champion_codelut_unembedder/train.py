"""exp021 — champion exp010 with EXACTLY ONE change: Linear unembedder -> HyperplaneCodeLUT.

Byte-identical to exp010 (the E=384 dual-stream champion, best_val_bpb 1.1940) EXCEPT the
final nn.Linear(D=384 -> V=32768) unembedder is replaced by HyperplaneCodeLUT(input_dim=D=384,
nap=15 -> V=2^15=32768, T=16, anchor-pairs init). KEEP everything else from exp010: dual
stream (E-stream + D-stream), BOTH residual LUTs (per-layer residual_lut ×6 + emb_resid_lut),
MeanAbsNorm pre-norms (NOT LayerNorm), ln_final(D) feeding the unembedder (the code-LUT reads
ln_final(x_resid), the D-stream), LUT sites qk4/256 v6/256 out7/512 resid6/256 emb6/256,
seed 42, 16k recipe. Purpose: isolate whether the code-LUT unembedder underperforms at scale
INDEPENDENT of the single-stream reduction (cf. exp019/exp020 single-stream code-LUT).
...
def _make_unembedder(seed_offset):
    """exp021 unembedder = HyperplaneCodeLUT reading the D-stream (dim D=384) after
    ln_final, mapping DIRECTLY to V=2^nap logits (replaces the Linear unembedder).
    Per-code soft-sign scores gated by w_cell, voted over code_unemb_tables tables.
    hyperplane_weight/bias -> AdamW no-wd (by name); w_cell -> AdamW wd0.1."""
    return HyperplaneCodeLUT(
        input_dim=D, nap=cfg['code_unemb_nap'], n_tables=cfg['code_unemb_tables'],
...
class Model(nn.Module):
    def __init__(self):
        ...
        self.unembedder_lut = _make_unembedder(900)   # exp021: Linear -> HyperplaneCodeLUT
    ...
    def forward(self, tokens, targets=None, loss_reduction='mean'):
        ...
        # exp021: HyperplaneCodeLUT unembedder reads the D-stream [B*T, D] -> [B*T, V].
        logits = self.unembedder_lut(x_resid.reshape(B * T, D)).float().view(B, T, VOCAB_SIZE)
...
print(f'UNEMBEDDER HyperplaneCodeLUT: nap={cfg["code_unemb_nap"]} (V=2^nap={1 << cfg["code_unemb_nap"]}) T={cfg["code_unemb_tables"]} input=D={D}; tok_emb_E at E={E}; ln_final(D) -> code-LUT; RoPE base={_ROPE_BASE}')
