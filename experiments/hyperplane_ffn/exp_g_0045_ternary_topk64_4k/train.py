...
# exp_g_0042: with a target the penalty becomes lambda*relu(surrogate - target)
# instead of lambda*surrogate -- a hinge that releases once the router is sparse
# enough, rather than a one-way push toward an empty router. 0.0 disables it and
# restores the exp_g_0039..41 behaviour exactly.
LUT_TARGET_NZ = float(cfg.get('lut_target_nonzero_frac', 0.0))
# exp_g_0045/0046: hard per-hyperplane top-K on the routing. K > 0 pins EXACTLY
# K non-zeros per hyperplane in both the training forward and the hard eval, so
# what trains is what deploys. The straight-through gradient still reaches every
# component, including the masked-out ones, so the set can reshuffle. 0 disables
# it and restores the exp_g_0037..44 behaviour exactly.
LUT_TOPK = int(cfg.get('lut_topk_per_hyperplane', 0))
    ...
    def __init__(self, n_embd, n_head, layer_idx):
        ...
        if FFN_TYPE == 'dense':
            ...
            self.ffn = CompressionMultiHeadLUT(
                ...
                target_nonzero_frac=LUT_TARGET_NZ,
                topk_per_hyperplane=LUT_TOPK,
...
def _mask_all():
    if LUT_TOPK <= 0:
        return None
    return torch.cat([_m.topk_mask().reshape(-1) for _m in _tern_luts])


_mask_init = _mask_all()
_mask_prev = None if _mask_init is None else _mask_init.clone()
...
drift_w.writerow([
    ...
    # exp_g_0042: the hinge. slack = surrogate - target, so > 0 means it is still
    # pushing and <= 0 means it has released and contributes nothing. Logged
    # alongside frac_zero / mean_nonzero_count so the CALIBRATION between the soft
    # surrogate target and the hard non-zero count it actually produces is visible
    # rather than assumed -- they track each other but are not the same number.
    'target_nonzero_frac', 'sparsity_slack', 'hinge_active',
    # exp_g_0045/0046: with top-K the density is pinned, so nnz says nothing and
    # the interesting quantity is WHICH components are in the set. mask_churn is
    # the symmetric difference against the previous eval, mask_entered the half of
    # it that moved IN. If those fall to zero the set has frozen; if they stay
    # large it is thrashing, and both are failure modes worth seeing early.
    'topk_k', 'topk_mask_churn', 'topk_mask_entered', 'topk_mask_vs_init',
])
...
def _drift_row(step):
    ...
    _slack = sum(float(m_.sparsity_slack()) for m_ in _tern_luts) / len(_tern_luts)
    row += [f'{LUT_TARGET_NZ:.6f}', f'{_slack:+.6f}',
            str(int(LUT_TARGET_NZ > 0.0 and _slack > 0.0))]
    global _mask_prev
    _mk = _mask_all()
    if _mk is None:
        row += [str(LUT_TOPK), '', '', '']
        ...
        row += [str(LUT_TOPK),
                str(int((_mk != _mask_prev).sum())),
                str(int((_mk & ~_mask_prev).sum())),
                str(int((_mk != _mask_init).sum()))]
        _mask_prev = _mk.clone()
