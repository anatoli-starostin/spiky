...
from local_poe_decompress import attach_poe_decompress, poe_modules
...
# g-series short-run convention: the LR schedule stays anchored to the FULL length and
# the run is cut, so the early trajectory is comparable to a full-length run and to
# every other short run on this board. Defaults to n_steps -- a no-op for full runs.
...
# exp_g_0051 (Escape 1): read the per-head gather-SUMS as log-space and exponentiate
# at the decompress, so exp(sum_i log c_i) = prod_i c_i gives a product over the 128
# cells a head gathered -- for 192 exponentials per token instead of 24,576, because
# the decompress call site is already deferred past the sum. See
# local_poe_decompress.py. 'off' restores exp_n_0121 exactly.
POE_PLACEMENT = cfg.get('poe_placement', 'off')   # 'off' | 'a' | 'b'
POE_CLAMP = float(cfg.get('poe_clamp', 10.0))     # pre-exp argument clamp
POE_EXPM1 = bool(cfg.get('poe_expm1', False))     # placement 'a' only
...
if POE_PLACEMENT != 'off':
    _poe_slots = [b.ffn for b in model.blocks
                  if isinstance(b.ffn, CompressionMultiHeadLUT)]
    for _s in _poe_slots:
        attach_poe_decompress(_s, placement=POE_PLACEMENT, clamp=POE_CLAMP,
                              use_expm1=POE_EXPM1)
    print(f'PoE decompress: placement {POE_PLACEMENT!r} on {len(_poe_slots)} slots, '
          f'clamp +-{POE_CLAMP}, expm1={POE_EXPM1}')
...
poe_f = open(os.path.join(EXP_DIR, 'poe_gate.csv'), 'w', newline='')
poe_w = csv.writer(poe_f)
poe_w.writerow(['step', 'pre_min', 'pre_max', 'pre_absmean', 'clamp_frac'])
...
for step in range(1, N_STEPS + 1):
    ...
    if step % EVAL_EVERY == 0 or step == N_STEPS:
        ...
        if POE_PLACEMENT != 'off':
            _gs = [q.last_stats for q in poe_modules(model) if q.last_stats]
            if _gs:
                _lo = min(q['pre_min'] for q in _gs)
                _hi = max(q['pre_max'] for q in _gs)
                _am = sum(q['pre_absmean'] for q in _gs) / len(_gs)
                _cf = max(q['clamp_frac'] for q in _gs)
                print(f'[POE] step {step}: pre-exp {_lo:+.3f}..{_hi:+.3f} '
                      f'|mean| {_am:.4f} clamped {_cf * 100:.4f}% (worst slot)')
                poe_w.writerow([step, f'{_lo:.6f}', f'{_hi:.6f}',
                                f'{_am:.6f}', f'{_cf:.8f}'])
                poe_f.flush()
