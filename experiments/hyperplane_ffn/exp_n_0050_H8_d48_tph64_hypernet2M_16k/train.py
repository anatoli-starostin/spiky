...
# exp_n_0050: ~2M-param generator (vs 0049's 34K) — spend capacity on per-row identity (wider
# table/module/cluster embeddings) + a wider shared MLP. Input 262 = table_emb128 + module_emb64
# + cluster_emb64 + code6; MLP hidden 1152 -> lands ~2.09M generator params.
_HN_CODE, _HN_TE, _HN_ME, _HN_CE, _HN_HID = 6, 128, 64, 64, 1152


class HyperGen(nn.Module):
    """Shared generator: one wide MLP + table/module/cluster embeddings, reused across all FastMHL modules."""
    def __init__(self, n_modules, tph, n_outputs, n_clusters):
        super().__init__()
        self.table_emb = nn.Embedding(n_modules * tph, _HN_TE)   # per (module,table)
        self.module_emb = nn.Embedding(n_modules, _HN_ME)        # per module
        self.cluster_emb = nn.Embedding(n_clusters, _HN_CE)      # per cluster (shared across tables)
        self.mlp = nn.Sequential(
            nn.Linear(_HN_CODE + _HN_TE + _HN_ME + _HN_CE, _HN_HID), nn.GELU(),
...
class RowHypernet(nn.Module):
    ...
    def forward(self, W):
        ...
        te = gen.table_emb(gt)                                              # [T, te]
        me = gen.module_emb(torch.tensor(self.module_id, device=dev))      # [me]
        ce = gen.cluster_emb(torch.arange(K, device=dev))                  # [K, ce]
        code = self.codes.to(gen.mlp[0].weight.dtype)                      # [K, nap]
        code_b = code.unsqueeze(0).expand(T, -1, -1)                       # [T, K, nap]
        te_b = te.unsqueeze(1).expand(-1, K, -1)                           # [T, K, te]
        me_b = me.view(1, 1, -1).expand(T, K, -1)                          # [T, K, me]
        ce_b = ce.unsqueeze(0).expand(T, -1, -1)                           # [T, K, ce]
        rows = gen.mlp(torch.cat([code_b, te_b, me_b, ce_b], dim=-1))      # [T, K, n_outputs]
...
_NCLU = _fmhls[0].weights.shape[1]   # 2^nap clusters
_NOUT = _fmhls[0].weights.shape[2]
model.hypergen = HyperGen(_N_MOD, _TPH, _NOUT, _NCLU).to(DEVICE)
...
print(f"[HYPERNET] attached to {_N_MOD} FastMHL modules (tph={_TPH}, clusters={_NCLU}, n_out={_NOUT}) | "
      f"shared-gen params={_hn_params:,} (mlp {sum(p.numel() for p in model.hypergen.mlp.parameters()):,} "
      f"+ table_emb {model.hypergen.table_emb.weight.numel():,} + module_emb {model.hypergen.module_emb.weight.numel():,} "
      f"+ cluster_emb {model.hypergen.cluster_emb.weight.numel():,}) | frozen dead original rows={_orig_dead:,}")
# ============================================================================================
