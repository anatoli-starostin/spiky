...
# exp102 PER-SITE tuning (differs from exp101's shared config):
#   q_lut,k_lut: n_heads=6, inner_in=48, nap=4, tph=32
#   v_lut:       n_heads=6, inner_in=48, nap=6, tph=64
#   out_proj:    n_heads=8, inner_in=48, nap=6, tph=128 (inner_out held at 48)
_QK_IN, _QK_NAP, _QK_TPH = cfg.get('qk_inner_in', 48), cfg.get('qk_nap', 4), cfg.get('qk_tph', 32)
_V_IN,  _V_NAP,  _V_TPH  = cfg.get('v_inner_in', 48),  cfg.get('v_nap', 6),  cfg.get('v_tph', 64)
_O_IN,  _O_NAP,  _O_TPH  = cfg.get('out_inner_in', 48), cfg.get('out_nap', 6), cfg.get('out_tph', 128)
_O_INOUT = cfg.get('out_inner_out', 48)        # out_proj decompress width (held)
# exp103: DECOUPLE routing-head count from attention-head count. q/k/v use MANY routing
# mini-heads (each with its OWN learned compression W_c -> its own low-rank subspace); the
# ROUTING_H mini-head outputs are then SUMMED in groups of GROUP into the H attention heads.
_QKV_H = cfg.get('qkv_routing_heads', 48)      # routing mini-heads for q/k/v (was = H in exp102)
assert _QKV_H % H == 0, f'qkv_routing_heads {_QKV_H} must be divisible by n_head {H}'
_QKV_GROUP = _QKV_H // H                        # mini-heads summed into each attention head (8)
_OUT_H = cfg.get('out_lut_heads', 8)           # out_proj internal LUT head count

# exp_g_0018: how consecutive layers are combined.
#   'concat'       -> y_l = block_l(y_{l-1}); ALL y_l concatenated at the end.
#                     exp103's ONLY residual was `x_next = x_flat + out_e`, so this
#                     removes the residual path entirely -- 6 stacked attention
#                     blocks with no skip. Deliberate (that is what "replace
#                     summation with concatenation" means here), but it is the main
#                     trainability risk in this design.
#   'residual_sum' -> exp103 behaviour restored (x + out_e), still concatenated at
#                     the end. Kept as a one-flag ablation to separate "small E +
#                     concat readout" from "no residual".
# exp_g_0020: optional READOUT LUT between the two readout LayerNorms.
_READOUT_LUT = bool(cfg.get('readout_lut', False))
_RO_H    = cfg.get('readout_lut_heads', 8)
_RO_IN   = cfg.get('readout_lut_inner', 48)    # legacy inner_dim: sets inner_in AND inner_out
_RO_NAP  = cfg.get('readout_lut_nap', 6)
_RO_TPH  = cfg.get('readout_lut_tph', 64)

# exp105 DENSE-CONCAT: layer L input = (L+1)*E (running concat of emb + all prior layer
# outputs); each layer PRODUCES E; NO residual add (the concat IS the carry-forward). The
...
def _make_q(seed_offset, in_dim):
    return CompressionMultiHeadLUTMH(
        input_dim=in_dim, output_dim=d_qk, inner_in_dim=_QK_IN, inner_out_dim=-1,
        nap=_QK_NAP, tph=_QK_TPH, n_heads=_QKV_H, multihead_output=True,
...
def _make_k(seed_offset, in_dim):
    return CompressionMultiHeadLUTMH(
        input_dim=in_dim, output_dim=d_qk, inner_in_dim=_QK_IN, inner_out_dim=-1,
        nap=_QK_NAP, tph=_QK_TPH, n_heads=_QKV_H, multihead_output=True,
        batched_multi_head_input=True,
...
def _make_v(seed_offset, in_dim):
    return CompressionMultiHeadLUTMH(
        input_dim=in_dim, output_dim=d_v, inner_in_dim=_V_IN, inner_out_dim=-1,
        nap=_V_NAP, tph=_V_TPH, n_heads=_QKV_H, multihead_output=True,
        batched_multi_head_input=True,
        forward_mode="hard", use_bf16=True,
...
def _make_out(seed_offset):
    return CompressionMultiHeadLUT(
        input_dim=H * d_v, output_dim=E, inner_in_dim=_O_IN, inner_out_dim=_O_INOUT,
        nap=_O_NAP, tph=_O_TPH, n_heads=_OUT_H,
        batched_multi_head_input=True,
        forward_mode="hard", use_bf16=True,
...
class LUTBlock(nn.Module):
    ...
    def __init__(self, layer_idx):
        super().__init__()
        # exp105 DENSE-CONCAT: this layer reads the running stream of dim (layer_idx+1)*E.
        self.in_dim = (layer_idx + 1) * E
        # q/k/v LUTs are built PER-LAYER with input_dim = (layer_idx+1)*E (the growing stream).
        self.q_lut    = _make_q(layer_idx, self.in_dim)
        self.k_lut    = _make_k(100 + layer_idx, self.in_dim)
        self.v_lut    = _make_v(200 + layer_idx, self.in_dim)
        self.out_proj = _make_out(400 + layer_idx)   # input H*d_v (fixed), output E — unchanged

        self.q_norm = nn.LayerNorm(d_qk)
        self.k_norm = nn.LayerNorm(d_qk)
        # PRE-NORM (matching exp_g_0021): ln_pre normalizes the GROWN concatenated stream
        # of width (layer_idx+1)*E before this layer's q/k/v — i.e. the per-concat LayerNorm.
        self.ln_pre = nn.LayerNorm(self.in_dim)

    def forward(self, x, cos, sin):
        B, T, _ = x.shape
        x_flat = x.reshape(B * T, self.in_dim)

        x_pre = self.ln_pre(x_flat)

        # q/k/v_lut each return [N, ROUTING_H, d] (many routing mini-heads). GROUP them into
        # the H attention heads: [N, ROUTING_H, d] -> [N, H, GROUP, d] -> sum over GROUP -> [N, H, d].
        def _grp(t):
            return t.reshape(t.shape[0], H, _QKV_GROUP, t.shape[-1]).sum(dim=2)   # [N, H, d]
        q_vec = self.q_norm(_grp(self.q_lut(x_pre).float()))   # [N, H, d_qk]
        k_vec = self.k_norm(_grp(self.k_lut(x_pre).float()))
        q = q_vec.reshape(B, T, H, d_qk).permute(0, 2, 1, 3)
        k = k_vec.reshape(B, T, H, d_qk).permute(0, 2, 1, 3)
        q, k = apply_rope(q, k, cos[:T], sin[:T])

        v_vec = _grp(self.v_lut(x_pre).float())           # [N, ROUTING_H, d_v] -> [N, H, d_v]
        v = v_vec.reshape(B, T, H, d_v).permute(0, 2, 1, 3)

        attn = F.scaled_dot_product_attention(q, k, v, is_causal=True)
        out_in = attn.permute(0, 2, 1, 3).reshape(B * T, H * d_v)
        out_e = self.out_proj(out_in).squeeze(1).float()   # [N, E]

        # exp105 DENSE-CONCAT: return this layer's E-dim output ONLY. NO residual add
        # (no `x_flat + out_e`) — the Model concatenates out_e onto the running stream.
        return out_e.reshape(B, T, E)
...
class Model(nn.Module):
    ...
    def __init__(self):
        ...
        # exp_g_0018: readout sees the CONCATENATION of all layer outputs, so the
        # final norm and the unembedder are sized to CONCAT_DIM = N_LAYERS*E, not E.
        # At E=64 x 6 layers that is 384 -- exactly exp103's E, so the unembedder is
        # the same shape as exp103's despite the 6x narrower stream.
        self.ln_final = nn.LayerNorm(CONCAT_DIM)
        # exp_g_0020: readout LUT sitting BETWEEN two LayerNorms.
        #   concat [CONCAT_DIM] -> ln_final -> readout_lut -> ln_readout -> unembedder
        # Standard collapsed-output CompressionMultiHeadLUT (NOT the MH subclass):
        # output_dim is set, so heads are summed and decompressed back to CONCAT_DIM.
        # inner_dim=_RO_IN is the legacy shortcut that sets inner_in_dim AND
        # inner_out_dim, i.e. compress Linear(384 -> 8*48=384) -> per-head FastMHL
    ...
    def forward(self, tokens, targets=None, loss_reduction='mean'):
        B, T = tokens.shape
        x = self.tok_emb_E(tokens)                       # [B,T,E] running stream = emb
        for layer in self.layers:
            out = layer(x, self.rope.cos, self.rope.sin) # layer reads x=[B,T,(L+1)E] -> [B,T,E]
            x = torch.cat([x, out], dim=-1)              # GROW stream by E (DenseNet-style)
        # After all layers the running stream is [B,T,(N_LAYERS+1)*E] = emb + all outputs.
        x_flat = self.ln_final(x.reshape(B * T, READOUT_DIM))
        if self.readout_lut is not None:
            # LN -> readout LUT -> LN, then the unembedder.
            x_flat = self.ln_readout(self.readout_lut(x_flat).float())
...
if os.environ.get('SMOKE'):
    ...
    q_grp = q_dbg.reshape(4, H, _QKV_GROUP, d_qk).sum(dim=2)   # grouped -> [4, H, d_qk]
    v_grp = v_dbg.reshape(4, H, _QKV_GROUP, d_v).sum(dim=2)
    print(f'[SMOKE] q_lut RAW out shape = {tuple(q_dbg.shape)} (expect [4,{_QKV_H},{d_qk}]) -> GROUPED {tuple(q_grp.shape)} (expect [4,{H},{d_qk}])')
    print(f'[SMOKE] v_lut RAW out shape = {tuple(v_dbg.shape)} (expect [4,{_QKV_H},{d_v}]) -> GROUPED {tuple(v_grp.shape)} (expect [4,{H},{d_v}])')
    print(f'[SMOKE] out_proj out shape = {tuple(o_dbg.shape)} (expect [4,{E}])')
    assert tuple(q_dbg.shape) == (4, _QKV_H, d_qk) and tuple(q_grp.shape) == (4, H, d_qk)
    assert tuple(v_dbg.shape) == (4, _QKV_H, d_v) and tuple(v_grp.shape) == (4, H, d_v)
    assert tuple(o_dbg.shape) == (4, E)
    bflags = {n: getattr(m, 'batched_multi_head_input', None)
              for n, m in [('q_lut', blk.q_lut), ('k_lut', blk.k_lut), ('v_lut', blk.v_lut), ('out_proj', blk.out_proj)]}
    print(f'[SMOKE] batched_multi_head_input per site: {bflags}')
    assert all(bflags.values()), f'batched_multi_head_input not True everywhere: {bflags}'
    # DENSE-CONCAT: confirm the stream dim entering each layer grows E,2E,...,N_LAYERS*E,
    # and the readout reads the final (N_LAYERS+1)*E stream — via a REAL forward + hooks.
    print(f'[SMOKE] per-layer ln_pre / in_dim: {[lyr.in_dim for lyr in model.layers]} '
          f'(expect {[(i+1)*E for i in range(N_LAYERS)]})')
    print(f'[SMOKE] READOUT_DIM (final concat incl emb) = {READOUT_DIM} (expect {(N_LAYERS+1)*E})')
    _caps = []
    _hk = [lyr.register_forward_pre_hook(lambda m, inp: _caps.append(int(inp[0].shape[-1]))) for lyr in model.layers]
    with torch.no_grad():
        model(torch.randint(0, VOCAB_SIZE, (2, 8), device=DEVICE))
    for h in _hk:
        h.remove()
    print(f'[SMOKE] REAL-forward captured layer INPUT dims: {_caps}  (proves the growing stream)')
    assert _caps == [(i + 1) * E for i in range(N_LAYERS)], _caps
    ...
    # one forward+BACKWARD (grad path through the CompressionMHL + subclass)
    model.train()
    toks2 = torch.randint(0, VOCAB_SIZE, (2, CONTEXT_SIZE), device=DEVICE)
    tgt2 = torch.randint(0, VOCAB_SIZE, (2, CONTEXT_SIZE), device=DEVICE)
    loss_dbg = model(toks2, targets=tgt2)
    loss_dbg.backward()
    ngrad = sum(1 for p in model.parameters() if p.requires_grad and p.grad is not None)
    print(f'[SMOKE] forward+backward OK: loss={loss_dbg.item():.4f}, params_with_grad={ngrad}')
    model.zero_grad(set_to_none=True)
    # per-site param breakdown
    def _sp(sub):
        return sum(p.numel() for n, p in model.named_parameters() if sub in n)
    q_p, k_p, v_p, o_p = _sp('.q_lut.'), _sp('.k_lut.'), _sp('.v_lut.'), _sp('.out_proj.')
    rest = n_params - (q_p + k_p + v_p + o_p)
    print(f'[SMOKE] per-site TOTALS (summed over {N_LAYERS} layers): '
          f'q_lut={q_p:,} k_lut={k_p:,} v_lut={v_p:,} out_proj={o_p:,} | '
          f'rest(emb+readout+unembed+norms)={rest:,} | TOTAL={n_params:,}')
    # PER-LAYER breakdown — q/k/v GROW with layer index (input_dim (L+1)*E); out_proj flat.
    def _lp(L, site):
        return sum(p.numel() for n, p in model.named_parameters() if f'layers.{L}.{site}.' in n)
    for L in range(N_LAYERS):
        print(f'[SMOKE]   layer {L} (in_dim={(L+1)*E}): q={_lp(L,"q_lut"):,} k={_lp(L,"k_lut"):,} '
              f'v={_lp(L,"v_lut"):,} out_proj={_lp(L,"out_proj"):,}')
    _ro = sum(p.numel() for n, p in model.named_parameters() if 'readout_lut' in n)
    _un = sum(p.numel() for n, p in model.named_parameters() if 'unembedder' in n)
    _emb = sum(p.numel() for n, p in model.named_parameters() if 'tok_emb_E' in n)
    print(f'[SMOKE]   readout_lut={_ro:,}  unembedder={_un:,}  tok_emb={_emb:,}')
...
print(f'exp102 PER-SITE: q/k_lut = CompressionMHL(in={E}->out={d_qk}, inner_in={_QK_IN}, inner_out=-1, '
      f'nap={_QK_NAP}, tph={_QK_TPH}, n_heads={_QKV_H}) -> [N,{_QKV_H},{d_qk}]')
print(f'                v_lut   = CompressionMHL(in={E}->out={d_v}, inner_in={_V_IN}, inner_out=-1, '
      f'nap={_V_NAP}, tph={_V_TPH}, n_heads={_QKV_H}) -> [N,{_QKV_H},{d_v}]')
print(f'                out_proj= CompressionMHL(in={H*d_v}->out={E}, inner_in={_O_IN}, inner_out={_O_INOUT}, '
      f'nap={_O_NAP}, tph={_O_TPH}, n_heads={_OUT_H}) -> [N,{E}]')
