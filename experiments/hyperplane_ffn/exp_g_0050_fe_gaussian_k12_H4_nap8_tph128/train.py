...
from local_function_emitting_ffn import FunctionEmittingFastMHL
...
# g-series short-run convention: the LR schedule stays anchored to the FULL run length
# and the run is simply cut short, so the early trajectory is directly comparable to a
# full-length run of the same recipe. Defaults to n_steps, i.e. a no-op for full runs.
SCHED_STEPS = int(cfg.get('lr_schedule_steps', cfg['n_steps']))
...
# exp_g_0023: swap the FFN slot's CompressionMHL for a RAW FastMultiHeadLut.
#   'compression' -> exp_n_0045 behaviour, unchanged.
#   'fastmhl_raw' -> FastMultiHeadLut(input_dim=n_embd, n_heads=RAW_H,
#                    n_outputs=n_embd, nap=RAW_NAP, tph=RAW_TPH). No compress
#                    (routes on the full n_embd input) and no decompress (tables
#                    emit n_embd directly), so the slot's in/out widths are both
#                    n_embd and the surrounding block needs no reshaping beyond
#                    dropping FastMHL's head axis.
FFN_LUT_KIND = cfg.get('ffn_lut_kind', 'compression')
# exp_g_0048/0049: FUNCTION-EMITTING cells. A cell stores the parameters of a
# few basis functions over the output-index axis instead of one learnable value
# per output dimension; its 384-d contribution is those functions evaluated at
# every index. Routing is untouched -- only the contents of `weights` change,
# from a stored Parameter to a synthesised tensor. See
# local_function_emitting_ffn.py for the injection point and the caveat about
# the Fourier variant commuting with the gather-sum.
FE_BASIS = cfg.get('fe_basis', 'gaussian')
FE_K = int(cfg.get('fe_k', 24))                # bumps per cell / harmonics
FE_LEARN_FREQ = bool(cfg.get('fe_learn_freq', False))
FE_CHUNK = int(cfg.get('fe_chunk', 4096))      # cells per synthesis chunk
FE_COMPILE = bool(cfg.get('fe_compile', False))  # fuse the gaussian synthesis
assert FFN_LUT_KIND in ('compression', 'fastmhl_raw',
                        'fastmhl_function_emitting'), FFN_LUT_KIND
RAW_NAP = int(cfg.get('raw_nap', 7))
RAW_TPH = int(cfg.get('raw_tph', 2048))
RAW_H   = int(cfg.get('raw_n_heads', 1))
...
class MinimalBlock(nn.Module):
    def __init__(self, n_embd, n_head, layer_idx):
        ...
        if FFN_TYPE == 'dense':
            ...
            if FFN_LUT_KIND == 'fastmhl_function_emitting':
                # Same routing as fastmhl_raw; only cell EMISSION differs.
                self.ffn = FunctionEmittingFastMHL(
                    input_dim=n_embd, n_heads=RAW_H, n_outputs=n_embd,
                    n_anchor_pairs=RAW_NAP, tables_per_head=RAW_TPH,
                    forward_mode=LUT_FWD, use_bf16=LUT_BF16,
                    initial_weights_noise=LUT_NOISE,
                    learnable_temps=LUT_LEARNTEMP,
                    random_seed=LUT_SEED + layer_idx,
                    fe_basis=FE_BASIS, fe_k=FE_K,
                    fe_learn_freq=FE_LEARN_FREQ, fe_chunk=FE_CHUNK,
                    fe_compile=FE_COMPILE,
                    fe_seed=LUT_SEED + layer_idx)
            elif FFN_LUT_KIND == 'fastmhl_raw':
                # Raw FastMHL: no compress, no decompress. Routing anchors index
                # the full n_embd input; tables are n_outputs=n_embd wide, so the
                # slot maps n_embd -> n_embd with no projection either side.
    ...
    def forward(self, x, cos, sin):
        ...
        if self.ffn_type == 'dense':
            return x + self.mlp(h)
        B, T, C = h.shape
        _o = self.ffn(h.reshape(B * T, C))
        # CompressionMHL returns [N, C]; raw FastMHL returns [N, n_heads, C].
        # Sum the head axis (n_heads=1 here, so this is just a squeeze) so both
        # paths hand the block the same [N, C].
        if _o.dim() == 3:
            _o = _o.sum(dim=1)
        out = _o.reshape(B, T, C).to(h.dtype)
...
def setup_optimizer(model, lr, weight_decay):
    # recurse=True, not False: the function-emitting variant keeps its basis
    # parameters in a child module (self.cells), and recurse=False would miss
    # them -- they would land in the decay group and weight-decay the bump
    # means and log-sigmas, which the LUT table was explicitly never subject to.
    # For the raw/compression paths this is a no-op: they have no param-bearing
    # children.
    ...
    for g in opt.param_groups:
        g['initial_lr'] = g['lr']
    # count the STORED parameters, not m.weights -- under the function-emitting
    # variant reading m.weights would synthesise all 302M elements just to call
    # .numel() on them.
    n_lut = sum(p.numel() for m in model.modules()
                if isinstance(m, FastMultiHeadLut)
                for p in m.parameters(recurse=True) if p.ndim >= 2)
