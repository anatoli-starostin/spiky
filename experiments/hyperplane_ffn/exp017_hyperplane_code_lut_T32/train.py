"""exp017 — dim-96 single-stream LUT-GPT with a HyperplaneCodeLUT (T=32) UNEMBEDDER.

Full 16000-step run. Same backbone as exp016 (dim96, 6 layers, 6 heads x16, LUT
qk/v/out projections, single stream, no residual/emb_resid) but the unembedder is
HyperplaneCodeLUT (nap=15 -> V=2^15=32768, n_tables T=32): E-dim hidden -> per-code
soft-sign scores gated by w_cell, voted over 32 tables (no stored per-cell rows).
Motivation: the 2000-step T-sweep found T=32 the quality/compute sweet spot — it beat
the best LUT-unembedder (val_bpb 1.8942 @ 67M unemb params) with only ~1.1M unemb params.

ORIGINAL header follows:
================================================================================
exp016 — dim-64 single-stream LUT-GPT with a HyperplaneMultiHeadLUT UNEMBEDDER.

Tests staying in "ranking space" all the way to the logits: no ranking->Euclidean
transition before unembedding. Architecture (derived from exp014's train.py):
  * dim E=64 (very small), single residual stream, UNTIED token embeddings, 6 layers.
  * Standard Euclidean softmax attention (the one allowed exception), but q/k/v and
    the output projection are HyperplaneMultiHeadLUTs (hyperplane, anchor_pairs, hard).
    out_proj doubles as unify-heads + FFN nonlinearity — NO separate MLP.
  * REMOVED entirely: residual_lut, emb_resid_lut, and all dual-stream machinery.
  * UNEMBEDDER: a HyperplaneMultiHeadLUT maps the dim-64 hidden DIRECTLY to VOCAB
    logits (replacing the linear unembedder).
Recipe identical to exp014 (16000 steps, Lion 2e-4 tables / AdamW 3e-4 no-wd
hyperplanes by name, grad clip 1.0, warmup 0.1 cosine, bf16 tables + fp32 master).

ORIGINAL exp010 docstring follows:
================================================================================
...
from spiky.lutorch.hyperplane_code_lut import HyperplaneCodeLUT  # smoke: code-LUT unembedder
import matplotlib
matplotlib.use('Agg')
...
def _make_emb_resid_lut(seed_offset):
    """UNUSED in exp016 (kept for lineage). Was the D-stream embedding residual."""
    return _LUT_CLS(
        input_dim=E, n_heads=1, n_outputs=D,
...
def _make_unembedder(seed_offset):
    """SMOKE: unembedder = HyperplaneCodeLUT mapping the E-dim hidden DIRECTLY to
    V=2^nap logits (no stored per-cell rows; per-code soft-sign scores gated by
    w_cell, voted over n_tables). nap must satisfy 2^nap == VOCAB_SIZE."""
...
class LUTBlock(nn.Module):
    """exp016 SINGLE-STREAM block: standard Euclidean softmax attention whose
    q/k/v and output projections are HyperplaneMultiHeadLUTs. out_proj does
    double duty (unify heads + the FFN/nonlinearity role) and its output is the
    ONLY residual contribution. No residual_lut, no D-stream (both removed)."""
    ...
    def forward(self, x, cos, sin):
        ...
        # Standard Euclidean softmax attention, but q/k/v come from LUT projections.
        ...
        # out_proj LUT: unify heads (H*d_v -> E) AND serve the FFN role. No MLP.
        out_e = self.out_proj(out_in).squeeze(1).float()

        # Single residual stream (no separate D-stream / residual_lut).
        x_next = x_flat + out_e
        return x_next.reshape(B, T, E)


class Model(nn.Module):
    """exp016: single residual stream (dim E=64). No emb_resid_lut, no D-stream.
    The unembedder is a HyperplaneMultiHeadLUT mapping the dim-64 hidden DIRECTLY
    to VOCAB logits (sign-tests on the 64-dim stream -> index bits -> LUT rows ->
    logits), replacing the linear unembedder."""
    def __init__(self):
        super().__init__()
        ...
        self.unembedder_lut = _make_unembedder(900)
    ...
    def forward(self, tokens, targets=None, loss_reduction='mean'):
        ...
        x_flat = self.ln_final(x.reshape(B * T, E))      # LayerNorm(E)
        # LUT unembedder: [B*T, E] -> [B*T, 1, VOCAB] -> [B*T, VOCAB]
        logits = self.unembedder_lut(x_flat).float().view(B, T, VOCAB_SIZE)  # code-LUT: [N,V] direct
...
print(f'exp016 SINGLE-STREAM dim E={E}, H={H}, d_qk={d_qk}, d_v={d_v}, L={N_LAYERS} (NO residual_lut / emb_resid / D-stream)')
print(f'qk_lut   HyperplaneMHL: in_nap={cfg["qkv_input_nap"]} tph={cfg["qkv_tph"]} n_out=2*d_qk={2*d_qk}')
print(f'v_lut    HyperplaneMHL: in_nap={cfg["v_input_nap"]} tph={cfg["v_tph"]} d_v={d_v}')
print(f'out_proj HyperplaneMHL: in_nap={cfg["out_input_nap"]} tph={cfg["out_tph"]} n_out=E={E} (unify heads + FFN role)')
print(f'UNEMBEDDER HyperplaneCodeLUT: nap={cfg["code_unemb_nap"]} (V=2^nap={1 << cfg["code_unemb_nap"]}) n_tables(T)={cfg["code_unemb_tables"]} w_cell_init={cfg.get("code_unemb_wcell_init",0.02)}; input=E={E}; tok_emb_E UNTIED; ln_final(E); RoPE base={_ROPE_BASE}')
