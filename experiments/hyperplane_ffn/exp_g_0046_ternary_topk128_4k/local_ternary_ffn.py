...
class PureTernaryHyperplaneMHL(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # ---- per-head OUTPUT decompression, by analogy with CompressionMultiHeadLUT
        # Reference (compression_mhl.py:184 and :226):
        #     self.decompress = nn.Linear(n_heads * out_raw, output_dim, device=device)
        #     return self.decompress(torch.cat(parts, dim=-1))
        # Each head sums its OWN tables into an inner_out vector, the heads are
        # CONCATENATED (not summed), and one learned Linear projects the concatenation
        # back to output_dim. That concatenation is exactly what makes the heads
        # functionally distinct -- with the summed path they are a pure partition of
        # the table axis and carry no function.
        self.decompress_heads = bool(decompress_heads)
        self.inner_out = int(inner_out) if decompress_heads else None
        # has_decompress drives train.py's zero-init branch, matching the sandwich
        # convention: the slot's output projection starts at zero, so the FFN sub-layer
        # is the identity at step 0.
        self.has_decompress = self.decompress_heads
        if self.decompress_heads:
            self.decompress = nn.Linear(n_heads * self.inner_out, output_dim)
        else:
            self.decompress = nn.Identity()
        _lut_out = self.inner_out if self.decompress_heads else output_dim
        self.lut = TernaryHyperplaneMultiHeadLUT(
            input_dim=input_dim, n_heads=n_heads, n_outputs=_lut_out,
            ...
            nonzero_penalty_weight=nonzero_penalty_weight,
            target_nonzero_frac=target_nonzero_frac,
            topk_per_hyperplane=topk_per_hyperplane,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        y = self.lut(x)                                    # [N, H, inner_out or out]
        if self.decompress_heads:
            # CONCATENATE the per-head outputs, then project. reshape of [N, H, K] is
            # exactly torch.cat of the H slices, which is what CompressionMHL's batched
            # path does (compression_mhl.py:214).
            N = y.shape[0]
            return self.decompress(y.reshape(N, self.n_heads * self.inner_out)).to(x.dtype)
        return y.sum(dim=1).to(x.dtype)                     # [N, H, out] -> [N, out]
