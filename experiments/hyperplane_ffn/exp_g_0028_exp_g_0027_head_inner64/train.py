...
head_lut=True adds ONE extra CompressionMHL sub-layer at the end of the trunk, right
before the unembedder: x = x + head_ffn(head_ln(x)), then logits = head(ln_f(x)). It uses
the same pre-norm + residual shape as an FFN sub-layer, and its decompress is
zero-initialized so the layer is the identity at step 0. Registered LAST so every
pre-existing module keeps its original init RNG stream.

By default the head slot copies the FFN slots' LUT hyperparameters exactly. The optional
head_lut_* config keys (inner_in_dim, inner_out_dim, n_anchor_pairs, tables_per_head,
n_heads) size that ONE slot independently; each falls back to the corresponding lut_*
value, so omitting them all reproduces exp_g_0027 exactly. The 6 FFN slots always read
lut_* and are never affected by a head_lut_* override.
...
HEAD_LUT = bool(cfg.get('head_lut', False))            # extra CompressionMHL before the unembedder
...
# Pre-unembedder slot knobs. Each falls back to the FFN-slot value, so omitting all of
# them makes the head slot an exact copy of an FFN slot (that is exp_g_0027). Setting
# one sizes the head slot INDEPENDENTLY of the 6 FFN slots, which stay untouched.
HEAD_IN    = cfg.get('head_lut_inner_in_dim', LUT_IN)
HEAD_OUT   = cfg.get('head_lut_inner_out_dim', LUT_OUT)
HEAD_NAP   = cfg.get('head_lut_n_anchor_pairs', LUT_NAP)
HEAD_TPH   = cfg.get('head_lut_tables_per_head', LUT_TPH)
HEAD_HEADS = cfg.get('head_lut_n_heads', LUT_HEADS)
...
class MinimalGPT(nn.Module):
    def __init__(self, vocab_size, n_embd, n_head, n_layer, seq_len):
        ...
        # Extra CompressionMHL sub-layer immediately before the unembedder. Same LUT
        # hyperparameters as the FFN slots; registered AFTER head so tok_emb/blocks/ln_f/head
        # all consume the exact same init RNG stream as the baseline.
        if HEAD_LUT:
            self.head_ln = nn.LayerNorm(n_embd)
            self.head_ffn = CompressionMultiHeadLUT(
                input_dim=n_embd, output_dim=n_embd,
                inner_in_dim=HEAD_IN, inner_out_dim=HEAD_OUT,
                nap=HEAD_NAP, tph=HEAD_TPH, n_heads=HEAD_HEADS,
                joint_head_compression=LUT_JOINT, forward_mode=LUT_FWD,
                use_bf16=LUT_BF16, initial_weights_noise=LUT_NOISE,
                learnable_temps=LUT_LEARNTEMP,
                random_seed=LUT_SEED + n_layer)      # distinct from the 6 FFN-slot seeds
        else:
            self.head_ln, self.head_ffn = None, None
        ...
        if self.head_ffn is not None and self.head_ffn.has_decompress:
            nn.init.zeros_(self.head_ffn.decompress.weight)       # identity at step 0
        if TIE:
    ...
    def forward(self, idx, targets=None, loss_reduction='mean'):
        ...
        if self.head_ffn is not None:
            h = self.head_ln(x)
            B, T, C = h.shape
            x = x + self.head_ffn(h.reshape(B * T, C)).reshape(B, T, C).to(h.dtype)
...
print(f'MinimalGPT: depth={DEPTH}, dim={N_EMBD}, heads={N_HEAD}, seq_len={SEQ_LEN} | ffn={FFN_TYPE} tie={TIE} gamma={GAMMA} head_lut={HEAD_LUT}')
if HEAD_LUT:
    print(f'  FFN slots  : inner_in={LUT_IN} inner_out={LUT_OUT} nap={LUT_NAP} tph={LUT_TPH} n_heads={LUT_HEADS}')
    print(f'  head slot  : inner_in={HEAD_IN} inner_out={HEAD_OUT} nap={HEAD_NAP} tph={HEAD_TPH} n_heads={HEAD_HEADS}')
