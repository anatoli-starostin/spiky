...
class MinimalBlock(nn.Module):
    def __init__(self, n_embd, n_head, layer_idx):
        ...
        if FFN_TYPE == 'dense':
            ...
            # exp_n_0038: SEQUENTIAL two half-size CompressionMHL sub-blocks stacked inside the
            # block, each with its OWN pre-LayerNorm (ln2_a / ln2_b) and its OWN residual add:
            #   h   = x + ffn_a(ln2_a(x))
            #   out = h + ffn_b(ln2_b(h))
            def _mk(seed):
                return CompressionMultiHeadLUT(
                    ...
                    learnable_temps=LUT_LEARNTEMP, random_seed=seed)
            self.ln2_a = nn.LayerNorm(n_embd)
            self.ffn_a = _mk(LUT_SEED + 2 * layer_idx)
            self.ln2_b = nn.LayerNorm(n_embd)
            self.ffn_b = _mk(LUT_SEED + 2 * layer_idx + 1)
    ...
    def forward(self, x, cos, sin):
        ...
        B, T, C = x.shape
        # sub-block 1: h = x + ffn_a(ln2_a(x))
        ha = self.ln2_a(x)
        out_a = self.ffn_a(ha.reshape(B * T, C)).reshape(B, T, C).to(ha.dtype)
        h = x + out_a
        # sub-block 2: out = h + ffn_b(ln2_b(h))
        hb = self.ln2_b(h)
    ...
    def __init__(self, vocab_size, n_embd, n_head, n_layer, seq_len):
        ...
        for block in self.blocks:
            ...
            if FFN_TYPE == 'dense':
                ...
                for sub in (block.ffn_a, block.ffn_b):           # exp_n_0038: zero both sub-blocks' output proj
                    if sub.has_decompress:
                        nn.init.zeros_(sub.decompress.weight)
        if TIE:
