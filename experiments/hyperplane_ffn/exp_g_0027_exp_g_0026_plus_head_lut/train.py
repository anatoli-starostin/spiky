...
before the unembedder: x = x + head_ffn(head_ln(x)), then logits = head(ln_f(x)). It is
built with exactly the same LUT hyperparameters (lut_*) as the FFN slots, uses the same
pre-norm + residual shape as an FFN sub-layer, and its decompress is zero-initialized so
the layer is the identity at step 0. Registered LAST so every pre-existing module keeps
its original init RNG stream.
