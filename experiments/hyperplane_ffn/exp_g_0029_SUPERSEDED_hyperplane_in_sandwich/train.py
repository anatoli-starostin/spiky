...
# The local wrapper lives beside this file. `python train.py` would put it on sys.path
...
# exp_g_0029: the FFN slots use HyperplaneMHL instead of CompressionMHL's FastMHL.
# HyperplaneMHL generalizes FastMultiHeadLut, not CompressionMHL, so the local wrapper
# keeps CompressionMHL's compress/per-head-LUT/decompress sandwich and swaps only the
# filling -- see local_hyperplane_compression.py. Shared src/spiky/lutorch/ untouched.
from local_hyperplane_compression import HyperplaneCompressionMHL as CompressionMultiHeadLUT
...
# Length of the LR schedule, decoupled from how many steps we actually run. exp_g_0029
# is a 4,000-step SCREEN of exp_n_0121's 16,000-step run, and its whole readout is the
# step-for-step delta against that curve. Recomputing the schedule for 4,000 steps
# (warmup 400, cosine finishing at 4,000) would make the LR differ at every single
# step, so each aligned comparison would confound the module swap with a different
# optimizer trajectory. Holding the schedule at 16,000 -- warmup 1,600, cosine over
# 16,000, exactly as exp_n_0121 -- makes step k here and step k there identical in
# everything except the LUT module, which is the one variable under test.
# Consequence, stated plainly: this run stops mid-anneal at LR ~0.86 of peak, so its
# final number is a trajectory probe, not a converged result.
...
# "anchor_pairs" reproduces each fixed anchor pair EXACTLY at init (w = e_a - e_b,
# b = 0, no noise), so step 0 is bit-for-bit the FastMHL exp_n_0121 started from.
...
# The LUT module types whose OWN parameters go in the no-decay group. exp_n_0121's
# rule was "the LUT module's parameters are not weight-decayed" (tables + temps);
# HyperplaneMHL is the same module role, so its parameters -- tables, temps, and the
# hyperplane w/b that replace 0121's fixed anchor buffers -- follow the same rule.
# Letting w/b fall into the wd=0.1 group would shrink them over training: harmless to
# the hard forward (sign(<w,x>+b) is scale-invariant) but it steadily softens the
# temperature-scaled soft backward, which would be a second variable on top of the
# module swap.
...
print(f'FFN LUT module: {type(model.blocks[0].ffn).__name__} '
      f'(inner {type(model.blocks[0].ffn.luts[0]).__name__}, '
      f'hyperplane_init={model.blocks[0].ffn.hyperplane_init})')
print(f'Run length {N_STEPS:,} steps on a {SCHED_STEPS:,}-step LR schedule '
      f'(warmup {int(WARMUP_FRAC * SCHED_STEPS):,}) -- step-aligned with exp_n_0121')
