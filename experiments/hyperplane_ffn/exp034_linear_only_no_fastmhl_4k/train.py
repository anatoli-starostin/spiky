...
class MinimalBlock(nn.Module):
    """exp034 LINEAR-ONLY control: the dense 384->1536->384 GELU FFN is replaced by a
    SINGLE plain nn.Linear(n_embd->n_embd) with bias -- no GELU, no hidden expansion, and
    NO FastMHL. x = x + linear(ln2(x)). This is the linear-only ablation of exp033 (which
    adds a parallel hard-FastMHL path on the same input); it isolates FastMHL's
    contribution. The Linear weight is zero-init'd (residual-identity start, as vanilla did
    for the FFN output). Attention, LayerNorms and residual structure are byte-identical to
    exp032/exp033.
    """
    def __init__(self, n_embd, n_head):
        super().__init__()
        ...
        self.lin  = nn.Linear(n_embd, n_embd, bias=True)   # the only FFN-slot module

    def forward(self, x, cos, sin):
        x = x + self.attn(self.ln1(x), cos, sin)
        x = x + self.lin(self.ln2(x))
        return x
...
class MinimalGPT(nn.Module):
    def __init__(self, vocab_size, n_embd, n_head, n_layer, seq_len):
        ...
        for block in self.blocks:
            ...
            nn.init.zeros_(block.lin.weight)   # FFN-slot Linear: residual-identity start
