...
class MinimalBlock(nn.Module):
    """exp035 PURE-FastMHL block: the dense 384->1536->384 GELU FFN is replaced by ONLY a
    hard FastMultiHeadLUT -- NO parallel linear, NO GELU, NO dense FFN:

        h = ln2(x);  x = x + fastmhl(h)

    The plain fixed-anchor-pair FastMultiHeadLUT (hard forward, full soft-surrogate backward
    -> grads reach its tables AND flow densely back into h). This is the opposite control to
    exp034 (linear-only): here the FFN slot is entirely the LUT. Attention sub-block,
    LayerNorms and residual structure are byte-identical to exp032/exp002. LUT tables are
    ~zero init (init_weights_noise=1e-3), so the FFN slot ~= 0 at step 0 and grows on merit.
    """
    def __init__(self, n_embd, n_head, layer_idx):
        ...
        # The ONLY FFN-slot module: a plain fixed-anchor-pair FastMHL. Per-layer seed so
        # anchor pairs and table inits are decorrelated across depth. Built on CPU here;
        # the model-level .to(DEVICE) moves its Parameter + buffers onto the GPU.
    ...
    def forward(self, x, cos, sin):
        ...
        lut_out = lut_out.reshape(B, T, C).to(h.dtype)         # heads*nout == C
        x = x + lut_out                                        # FastMHL is the whole FFN slot
        return x
...
class MinimalGPT(nn.Module):
    def __init__(self, vocab_size, n_embd, n_head, n_layer, seq_len):
        ...
        for block in self.blocks:
            ...
            # (no FFN-slot linear to zero-init; the FastMHL tables are already near-zero init)
