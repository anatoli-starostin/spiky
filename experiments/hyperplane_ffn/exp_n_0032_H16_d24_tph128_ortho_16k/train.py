...
LUT_ORTHO  = bool(cfg.get('lut_compress_orthogonal', False))  # orthogonal-across-heads compress init
    ...
    def __init__(self, vocab_size, n_embd, n_head, n_layer, seq_len):
        ...
        for block in self.blocks:
            ...
            if FFN_TYPE == 'dense':
                ...
                if LUT_ORTHO and block.ffn.has_compress:
                    # orthogonal-across-heads compress init: the n_heads*d rows of the [n_heads*d, 384]
                    # compress weight become mutually orthonormal (so per-head d-dim row-subspaces are
                    # orthogonal & tile R^384), scaled to match the N(0,0.02^2) Frobenius norm.
                    w = block.ffn.compress.weight
                    nn.init.orthogonal_(w, gain=0.02 * math.sqrt(w.shape[1]))
        if TIE:
