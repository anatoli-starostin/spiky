...
# Length of the LR schedule, decoupled from how many steps we run -- the g-series
# convention. exp_n_0121 originally ran 16,000 steps with warmup 1,600 and cosine over
# 16,000; this re-run keeps that schedule EXACTLY and only stops early at 4,000, so
# every step is directly comparable with the g-series board.
...
print(f'Run length {N_STEPS:,} steps on a {SCHED_STEPS:,}-step LR schedule '
      f'(warmup {int(WARMUP_FRAC * SCHED_STEPS):,}) -- the g-series held schedule')
assert SCHED_STEPS == 16000 and int(WARMUP_FRAC * SCHED_STEPS) == 1600, \
    'schedule is not the g-series held one'
_f0 = model.blocks[0].ffn
print(f'FFN slot: {type(_f0).__name__} | compress={_f0.has_compress} '
      f'decompress={_f0.has_decompress} | inner_in={_f0.inner_in_dim} '
      f'inner_out={_f0.inner_out_dim} | n_heads={_f0.n_heads} nap={_f0.nap} tph={_f0.tph}')
