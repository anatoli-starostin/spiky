"""exp013 — exp012 with NEAR-ZERO hyperplane init (std=1e-4): does sparse structure emerge from ~nothing?

Identical to exp012 (random hyperplane init) EXCEPT hyperplane_init_scale = 0.0001
instead of 0.05, so the hyperplane normals start at ~0 and must grow from nothing.
Tests whether the network wakes up a SPARSE coefficient set (like exp010's anchor
geometry) or a DENSE one (like exp012's random-0.05). Only config diff vs exp012 is
hyperplane_init_scale.

ORIGINAL exp012 docstring follows:
================================================================================
exp012 — exp010 with RANDOM hyperplane init (the from-scratch arm of the A/B).

IDENTICAL to exp010 (full ~325M HyperplaneMultiHeadLUT model, exp752 geometry +
recipe, 16000 steps) EXCEPT hyperplane_init="random" instead of "anchor_pairs".
exp010 starts bit-for-bit as exp752's fixed-anchor front-end and learns from there;
exp012 instead initializes every hyperplane normal randomly (Gaussian rows,
std=hyperplane_init_scale ~= 1/sqrt(input_dim) so <w,x> is O(1)), bias 0 — a genuine
from-scratch front-end with NO anchor prior. Tests whether the learned-hyperplane
win in exp010 depended on the anchor-pair warm start or holds from random init.
Everything else (geometry, optimizer routing, bf16 LUT + fp32 master Lion, grad clip,
seed) is identical to exp010.
...
if _LUT_LAYER == 'hyperplane':
    ...
    _LUT_KWARGS = dict(
        ...
        # Only used when hyperplane_init="random": std of the Gaussian hyperplane
        # rows. None -> module falls back to initial_weights_noise. exp012 sets it
        # ~1/sqrt(input_dim) so the random sign tests have O(1) pre-activations.
