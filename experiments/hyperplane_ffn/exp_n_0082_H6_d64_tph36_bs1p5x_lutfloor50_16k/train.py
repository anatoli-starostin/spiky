...
def setup_optimizer(model, lr, weight_decay, base_floor=0.1, lut_floor=0.1):
    # exp_n_0082: DECOUPLED LR floors. The LUT TABLE weights (FastMHL .weights) get their
    # OWN param group with a higher cosine floor (min_scale=lut_floor); everything else
    # (2-D decay weights, and nodecay = temps + 1-D biases/norms) keeps the base floor.
    lut_ids = {id(p) for m in model.modules() if isinstance(m, FastMultiHeadLut)
               for p in m.parameters(recurse=False)}
    lut_table_ids = {id(m.weights) for m in model.modules() if isinstance(m, FastMultiHeadLut)}
    decay, nodecay, lut_tables = [], [], []
    for p in model.parameters():
        if not p.requires_grad:
            continue
        if id(p) in lut_table_ids:
            lut_tables.append(p)                      # LUT table weights -> higher-floor group
        elif id(p) in lut_ids or p.ndim < 2:
            nodecay.append(p)                         # temps + 1-D (base floor, wd 0)
        else:
            decay.append(p)                           # 2-D weights (base floor, wd)
    groups = [dict(params=decay,      lr=lr, betas=(0.9, 0.95), eps=1e-8, weight_decay=weight_decay, min_scale=base_floor, gname='decay'),
              dict(params=nodecay,    lr=lr, betas=(0.9, 0.95), eps=1e-8, weight_decay=0.0,          min_scale=base_floor, gname='nodecay'),
              dict(params=lut_tables, lr=lr, betas=(0.9, 0.95), eps=1e-8, weight_decay=0.0,          min_scale=lut_floor,  gname='lut_tables')]
    opt = torch.optim.AdamW(groups)
    for g in opt.param_groups:
        g['initial_lr'] = g['lr']
    print(f"AdamW DECOUPLED-FLOOR | decay(2-D)={sum(p.numel() for p in decay):,} floor={base_floor} wd={weight_decay} | "
          f"nodecay(temps+1-D)={sum(p.numel() for p in nodecay):,} floor={base_floor} wd=0 | "
          f"LUT-tables={sum(p.numel() for p in lut_tables):,} floor={lut_floor} wd=0 | "
          f"lr={lr} -> min_lr: base={lr*base_floor:.2e} lut={lr*lut_floor:.2e}")
    return opt


def get_lr_scale(step, n_steps, warmup_frac, floor=0.1):
    w = int(warmup_frac * n_steps)
    if step < w:
        return step / max(w, 1)                       # warmup identical for all groups
    progress = (step - w) / max(n_steps - w, 1)
    return floor + (1.0 - floor) * 0.5 * (1 + math.cos(math.pi * progress))  # cosine to `floor`
...
BASE_FLOOR = cfg.get('base_lr_floor', 0.1)     # non-LUT cosine floor (exp_n_0081's 0.1)
LUT_FLOOR  = cfg.get('lut_lr_floor', 0.5)       # LUT-table cosine floor (0.5 -> min_lr 1.5e-4)
optimizer = setup_optimizer(model, lr=LR, weight_decay=WD, base_floor=BASE_FLOOR, lut_floor=LUT_FLOOR)
...
for step in range(1, N_STEPS + 1):
    for g in optimizer.param_groups:                 # DECOUPLED: per-group floor
        g['lr'] = g['initial_lr'] * get_lr_scale(step, N_STEPS, WARMUP_FRAC, g['min_scale'])
    ...
    if step % 100 == 0 or step == 1:
        lr_base = optimizer.param_groups[0]['lr']    # decay group (base floor)
        lr_lut = next(g['lr'] for g in optimizer.param_groups if g.get('gname') == 'lut_tables')
        print(f'step {step:6d} | loss={ema:.4f} | lr_base={lr_base:.3e} lr_lut={lr_lut:.3e}')
