"""exp006_hyperplane_mhl_ffn — exp001 (MinimalGPT + RoPE, untied head) with the
dense MLP FFN in every block replaced by a single-head HyperplaneMultiHeadLUT.
...
class HyperplaneFFN(nn.Module):
    """FFN = single-head HyperplaneMultiHeadLUT operating per token.

    x: [B, T, C] -> flatten to [B*T, C] -> HyperplaneMHL -> [B*T, 1, C]
    -> squeeze head -> .float() (LUT output is bf16) -> [B, T, C].
    n_heads=1 & n_outputs=C map straight to d_model, no reshape/projection.
    """
    def __init__(self, n_embd, layer_idx):
        ...
        self.n_embd = n_embd
        self.lut = HyperplaneMultiHeadLUT(
            input_dim=n_embd,
            ...
            # distinct, reproducible init per layer
            random_seed=cfg['random_seed'] + layer_idx + 1,
            device=DEVICE,
        )

    def forward(self, x):
        B, T, C = x.shape
        y = self.lut(x.reshape(B * T, C))          # [B*T, 1, C]
        y = y.reshape(B * T, C).float()            # squeeze head + bf16->fp32
...
print(f'MinimalGPT + HyperplaneMHL FFN: depth={DEPTH}, dim={N_EMBD}, heads={N_HEAD}, seq_len={SEQ_LEN}')
