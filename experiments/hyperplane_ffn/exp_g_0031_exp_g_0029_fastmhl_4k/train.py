...
# exp_g_0031: the FFN slot is the STOCK FastMultiHeadLut at full model dim -- fixed
# random anchor-pair routing, anchors held as buffers and never learned. Same pure
# topology as exp_g_0029 (no compress, no decompress, 384-dim cells). This is the
# fixed-routing baseline for the learned-routing runs. See local_fastmhl_ffn.py;
# shared src/spiky/lutorch/ untouched.
from local_fastmhl_ffn import PureFastMHL as CompressionMultiHeadLUT
...
# The seed for the FIXED anchor pairs. train.py passes LUT_SEED + layer_idx, so the
# six slots draw different routings from 1000..1005 -- the same seed convention every
# other experiment on this branch uses.
ANCHOR_SEED_BASE = LUT_SEED
...
print(f'  FIXED anchor pairs (buffers, never learned): '
      f'a {tuple(_f0.lut.soft_anchor_a_long.shape)} b {tuple(_f0.lut.soft_anchor_b_long.shape)}'
      f' drawn from seed base {ANCHOR_SEED_BASE} (+layer_idx)')
print(f'  routing learnable params: '
      f'{sum(p.numel() for nm, p in _f0.lut.named_parameters() if "weights" not in nm and "temp" not in nm)}'
      f' (0 expected -- anchors are buffers)')
