...
# unconditional per-site breakdown (on record in the launch log)
def _site_p(sub):
    return sum(p.numel() for n, p in model.named_parameters() if sub in n)
_qP, _kP, _vP, _oP = _site_p('.q_lut.'), _site_p('.k_lut.'), _site_p('.v_lut.'), _site_p('.out_proj.')
print(f'PER-SITE params (x{N_LAYERS} layers): q_lut={_qP:,} k_lut={_kP:,} v_lut={_vP:,} out_proj={_oP:,} | '
      f'rest(non-LUT)={n_params-(_qP+_kP+_vP+_oP):,}')
...
if os.environ.get('SMOKE'):
    ...
    # routing/compress vs table split per site
    def _spx(site, kind):  # kind uses leading/trailing dots so '.compress.' != '.decompress.'
        return sum(p.numel() for n, p in model.named_parameters() if site in n and kind in n)
    A_tot = B_tot = 0
    for site in ('.q_lut.', '.k_lut.', '.v_lut.', '.out_proj.'):
        comp = _spx(site, '.compress.') + _spx(site, '.decompress.')
        tab = _spx(site, 'lut_batched.weights')
        A_tot += comp; B_tot += tab
        print(f'[SMOKE]   {site:11s} routing(compress+decompress)={comp:,}  table(cells)={tab:,}')
    print(f'[SMOKE] LUT split: A routing/compress={A_tot:,}  B table={B_tot:,}  A:B={A_tot/B_tot:.3f}')
