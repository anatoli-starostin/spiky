"""Distill the frozen int4 Walker2d LUT actor into CompetitiveBucketLIFMHL (post-hoc column WTA).

Same protocol as distill_walker2d_bucket.py (constant LR 3e-3, mode='st' train / 'hard' eval, seeds/batch/
steps matched). The 16-row-per-neuron tables are learned from scratch. Reference: hard R2 ~0.096 at the
default heads=4 (needs ~heads=16 to reach ~0.229).

Run: PYTHONPATH=<spiky>/src python experiments/lif_detectors_mhl/distill_walker2d_competitive.py [--heads 4] [--steps 6000]
"""
...
def main():
    ...
    s = CompetitiveBucketLIFMHL(in_dim=N, out_dim=6, n_heads=a.heads, neurons_per_head=a.neurons, buckets=a.buckets)
    print(f"CompetitiveBucketLIFMHL params: {s.param_count()} (heads={a.heads} N={a.neurons} M={a.buckets})", flush=True)
