"""exp_c01 — SAC baseline on Walker2d-v5 (issue #75, step 1 of the roadmap).
...
Hyperparameters are the issue's spec verbatim:
  ...
  (target -6 = -|A|), 1M timesteps.

Deliverable: the trained actor checkpoint, reused as the distillation teacher for the
LUT policy (roadmap step 2).

Usage:
    python train_sac.py --seed 0 [--steps 1000000] [--device cpu]
"""
...
# The issue's sample-efficiency checkpoints: return @ 100k / 300k / 1M.
...
def main():
    ...
    model = SAC(
        ...
        seed=a.seed,
        device=a.device,
        tensorboard_log=os.path.join(run, "tb"),
    )
    ...
    print("wrote", os.path.join(run, "summary.json"))
