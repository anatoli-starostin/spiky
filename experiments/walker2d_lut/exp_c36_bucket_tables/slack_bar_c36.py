"""exp_c36 — self-updating Slack bar for the 3-seed Bucket-LIF SAC sweep (#75).
...
Exits once SWEEP_DONE_C36 exists or --max-hours passes. Deliberately not `pgrep -f`:
...
SEEDS = (0, 1, 2)
TOTAL = len(SEEDS)
SENTINEL = "SWEEP_DONE_C36"
REF = ("_*SCALING TABLES, NOT BUCKETS* — 16 buckets × *128 tables* (c32b was 16 × 32). "
       "This is the sharper test of the expressivity hypothesis: the action is the SUM "
       "over tables, so 128 tables give 128 *independent* addresses of 16 ordered "
       "outcomes each, where c33's 64 buckets only stretched ONE ordered scalar further. "
       "The bucket line so far: capacity, resolution and boundary placement all moved the "
       "addressing statistics and left return flat (best seed 3234/3166/3245, spread 79), "
       "and *freezing* the boundaries was the only change that mattered — downward "
       "(0/300 full). If independence is the missing ingredient, this is where it shows. "
       "*31,360 params* = 6,784 front-end + 24,576 table (the chapter-standard table); "
       "111.9% of the 28,032 baseline. Parity at T=128: 40/40. Anchors: exp_c18 4308 ± "
       "500; c32b 16×32 2041 ± 1230; c33 64×32 1536 ± 1417; c34 quantile 1662 ± 1375._")
...
def main():
    ...
    h = a.handle or progress.progress_start(
        "exp_c36 — Bucket-LIF, 16 buckets × 128 TABLES, 3 seeds (#75)",
        task=a.task, style="emoji", width=10)
    open(os.path.join(HERE, ".slack_c36.handle"), "w").write(h)
