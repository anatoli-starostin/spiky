"""exp_c27 — quantization-aware training of the LUT-SAC torque outputs (#75).

FORK of exp_c09_lut_sac/lut_sac.py. Forked rather than flagged because exp_c09's
trainer produced the published c11-c22 numbers and must keep running bit-identically.

exp_c26 quantized a FINISHED policy's outputs and found the knee at K = 7 levels per
joint: K = 5 kept the mean but started costing episodes, K = 3 broke the gait. The
question here is whether training with the grid in the loop rescues the coarse grids
that post-hoc rounding could not.

Straight-through: forward uses the grid value, backward passes the continuous
gradient. The quantized action goes to the env AND to the critic, so the critic
learns Q of what is actually executed. The entropy/log-prob terms stay on the
continuous tanh-Gaussian sample -- see actor_sample.
...
# -----------------------------------------------------------------------------
# Output quantization, straight-through
# -----------------------------------------------------------------------------
# K uniformly spaced levels across [-1, 1], the SAME grid on every joint. exp_c26
# measured every joint's observed action range at within 0.008 of [-1, +1] over
# 100,000 samples -- this policy drives all six joints across essentially the full
# torque range -- so a fixed shared grid is not an approximation here, it is what the
# per-joint measurement reduces to.
#
# K = 0 disables quantization and reproduces the unforked trainer.

def quantize_action(a, K):
    """Midtread uniform on [-1, 1], K levels. K=3 -> {-1, 0, +1}."""
    return jnp.rint((a + 1.0) * 0.5 * (K - 1)) / (K - 1) * 2.0 - 1.0


def ste(a, K):
    """q = a + stopgrad(quantize(a) - a): exact value forward, identity backward.

    The straight-through estimator is what makes this trainable at all. The rounding
    has zero gradient almost everywhere, so a faithful backward pass would deliver no
    signal to the actor whatsoever; STE hands the actor the gradient it would have
    received from the continuous action while the critic and the environment see the
    grid value.
    """
    if not K:
        return a
    return a + jax.lax.stop_gradient(quantize_action(a, K) - a)


def actor_sample(p, obs, key):
    """Reparameterised tanh-Gaussian: returns (action, logp, mu_tanh).

    The action is quantized; the LOG-PROB IS NOT. logp is computed from the
    continuous tanh-squashed sample, exactly as in the unforked trainer. Folding the
    grid into the density would be a different algorithm -- the tanh-Gaussian's
    density is not the density of its rounded image, and the entropy target
    (--target-entropy -6) is calibrated against the continuous one. Quantization here
    is a property of the action VALUE, not of the policy distribution.
    """
    ...
    K = CFG["action_levels"]
    return ste(a, K), logp, ste(jnp.tanh(mu), K)
...
def main():
    ...
    ap.add_argument("--action-levels", type=int, default=0, metavar="K",
                    help="quantize the policy OUTPUT to K uniformly spaced levels "
                         "per joint across [-1, 1], straight-through, in both the "
                         "env step and the critic input. 0 = off, reproducing the "
                         "unforked trainer. Written into the checkpoint so the CPU "
                         "eval cannot be run off-grid.")
    ap.add_argument("--quant-penalty", type=float, default=0.0, metavar="LAMBDA",
                    help="SOFT alternative to --action-levels: leave the action "
                         "continuous everywhere (env, critic, eval) and instead add "
                         "LAMBDA * mean((a - nearest_level)^2) to the ACTOR loss, so "
                         "the raw output distribution is pulled onto the grid on its "
                         "own. 0 = off. Mutually exclusive with --action-levels.")
    ap.add_argument("--penalty-levels", type=int, default=5, metavar="K",
                    help="grid the --quant-penalty term snaps to (default 5).")
    ...
    a = ap.parse_args()
    if a.action_levels and a.quant_penalty:
        raise SystemExit(
            "--action-levels and --quant-penalty are two different answers to "
            "the same question (hard snap with a straight-through gradient vs a "
            "soft pull toward the grid). Running both would measure neither. "
            "Pick one.")
    ...
    CFG.update(n_heads=a.heads, tph=a.tph, obs_mean=obs_mean, obs_std=obs_std,
               apply=X.apply(a.forward_mode), action_levels=a.action_levels)
    ...
    print("  output quantization: "
          + (f"K = {a.action_levels} levels/joint on [-1, 1], straight-through, "
             f"applied to env step AND critic input"
             if a.action_levels else "OFF (continuous actions)"), flush=True)
    if a.quant_penalty:
        print(f"  soft quantization penalty: lambda = {a.quant_penalty} on "
              f"mean((a - nearest of {a.penalty_levels} levels)^2), added to the "
              f"ACTOR loss only; actions stay continuous everywhere", flush=True)
    ...
    def rollout(ap_, st, key, random_actions):
        def one(carry, _):
            ...
            # Warm-up exploration is quantized too, so EVERY action in the replay
            # buffer lies on the grid. A critic trained partly on off-grid actions
            # would be answering a question the deployed policy never asks.
            a_rand = ste(jax.random.uniform(k1, (a.envs, ACT), minval=-1.0, maxval=1.0),
                         a.action_levels)
    ...
    def update(ap_, qp, qt, log_alpha, os_a, os_q, os_al, batch, key):
        ...
        def a_loss(ap_):
            ...
            sac = (alpha * logp - q).mean()
            # Soft grid pull. `an` is the CONTINUOUS tanh action here (this branch is
            # only reachable with --action-levels 0), and the target level is held
            # constant: d/da of (a - round(a))^2 must be 2(a - round(a)), not zero.
            # stop_gradient makes that explicit rather than relying on rint's a.e.
            # derivative happening to be 0.
            pen = jnp.mean(jnp.square(
                an - jax.lax.stop_gradient(quantize_action(an, a.penalty_levels))))
            return sac + a.quant_penalty * pen, (logp, sac, pen)
        (al, (logp, sac_l, pen_l)), ga = jax.value_and_grad(a_loss, has_aux=True)(ap_)
        ...
        return (ap_, qp, qt, log_alpha, os_a, os_q, os_al, key,
                dict(q_loss=ql, a_loss=al, alpha=alpha, logp=logp.mean(),
                     td=td.mean(), rows=rows, sac_loss=sac_l, pen=pen_l))

    @jax.jit
    def det_action(ap_, obs):
        # Quantized here too: the in-training MJX eval must measure the deployment
        # condition, otherwise the progress log would report a continuous policy that
        # is never the thing being trained or scored.
        mu, _ = actor_out(ap_, obs)
        return ste(jnp.tanh(mu), a.action_levels)
    ...
    for it in range(a.iters):
        ...
        if (it + 1) % a.eval_every == 0 or it == a.iters - 1:
            ...
            extra = ""
            if a.quant_penalty and it >= a.warmup:
                extra = (f" | sac {float(info['sac_loss']):8.1f} "
                         f"pen {float(info['pen']):.5f} "
                         f"({a.quant_penalty * float(info['pen']):.4f})")
            ...
            np.savez(os.path.join(HERE, f"{out_name}_actor.npz"),
                     **{k: np.asarray(v) for k, v in ap_.items()
                     ...
                     action_levels=np.int32(a.action_levels),
                     quant_penalty=np.float32(a.quant_penalty),
                     penalty_levels=np.int32(a.penalty_levels))
