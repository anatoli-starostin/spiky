#!/usr/bin/env bash
# exp_c34 — 3 seeds of Bucket-LIF SAC, CONCURRENTLY on the one 5090.
#
...
cell () {
  local S=$1 TAG="_c34_s$1"
  echo "=== seed $S: training ($(date -u +%H:%M:%SZ)) ==="
  $PY -u bucket_sac.py --seed "$S" --tag "$TAG" --boundary-init quantile > "cell_s${S}.log" 2>&1
