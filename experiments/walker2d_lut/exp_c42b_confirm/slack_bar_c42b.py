"""exp_c42b — self-updating Slack bar for the 3-seed MHL-LIF SAC sweep.
...
Exits once SWEEP_DONE_C42B exists or --max-hours passes. Deliberately not `pgrep -f`:
exp_c25's refresher matched its own command line and posted a frozen number for nine hours.

Usage:
  python slack_bar_c42b.py --task <BODY_TASK id> [--handle <existing>] [--interval 150]
"""
import argparse
import json
import os
...
SEEDS = (3, 4, 5, 6, 7, 8)
TOTAL = len(SEEDS)
SENTINEL = "SWEEP_DONE_C42B"
REF = ("_*exp_c42b — STRUCTURED BOUNDARY INIT.* The exp_c39 config (1 head × 32 tables × 3 "
...
def read(seed):
    ...
    cpu = os.path.join(HERE, f"mhl_sac_c42b_s{seed}_cpueval.json")
...
def render():
    ...
    if n_cpu:
        vals = [json.load(open(os.path.join(
            HERE, f"mhl_sac_c42b_s{s}_cpueval.json")))["cpu_reference_mean"]
...
def main():
    ...
    h = a.handle or progress.progress_start(
        "exp_c42b — structured BOUNDARY init: 32 tables × 3 detectors × 4 buckets, 3 seeds",
        task=a.task, style="emoji", width=10)
    open(os.path.join(HERE, ".slack_c42b.handle"), "w").write(h)
    ...
    while True:
        ...
        if finished or timeout:
            ...
            progress.progress_done(
                ...
                final_text=(f"exp_c42b MHL-LIF sweep "
