...
BATCH = 24
DELAY_INIT_STD = 0.0          # zero delays, c36s setting
DELAY_MIN = float("-inf")     # exp_c50: NO lower clamp bound (upper t_window cap kept)
DELAY_INIT_CONST = 0.0        # exp_c50: init at 0, exactly as c36 did
SPIKE_FORM = "detach_hard"    # exp_c53: t_soft := t_hard; T_cross unused
...
def main():
    ...
    print(f"run   cfg {RUN}  freeze_temperature=False  delay_init_std={DELAY_INIT_STD}  boundary_offset={BOUNDARY_OFFSET} table_init_std={TABLE_INIT_STD} share_betas={SHARE_BETAS} spike_form={SPIKE_FORM} delay_min={DELAY_MIN} delay_init_const={DELAY_INIT_CONST}")
    one_case("run", LIFMultiHeadLUT(**RUN, freeze_temperature=False,
                                    delay_init_std=DELAY_INIT_STD, table_init_std=TABLE_INIT_STD, share_betas=SHARE_BETAS, delay_min=DELAY_MIN, delay_init_const=DELAY_INIT_CONST, spike_form=SPIKE_FORM),
             RUN, 11, dump)
    # exp_c50/c51: `delay_min` must be passed to the perturbed and alt cases too. The JAX
    # side carries it as a MODULE constant, so it applies to every case; leaving these two
    # on upstream's 0.0 default would compare a no-floor port against a floored reference
    # and fail for the right reason in the most confusing possible place. It also makes
    # `perturbed` the case that actually exercises the change: its delays are 3.0*randn,
    # so roughly half are negative, plus one pinned at -5.0 and one above the ceiling.
    print(f"perturbed cfg {RUN}  freeze_temperature=False  delay_min={DELAY_MIN}")
    one_case("perturbed", perturb(LIFMultiHeadLUT(**RUN, delay_min=DELAY_MIN, spike_form=SPIKE_FORM), 77),
             RUN, 13, dump)
    print(f"alt   cfg {ALT}  freeze_temperature=False  delay_min={DELAY_MIN}")
    one_case("alt", perturb(LIFMultiHeadLUT(**ALT, delay_min=DELAY_MIN, spike_form=SPIKE_FORM), 99),
             ALT, 17, dump)
    ...
    m = LIFMultiHeadLUT(**RUN, freeze_temperature=False, delay_init_std=DELAY_INIT_STD,
                        table_init_std=TABLE_INIT_STD, share_betas=SHARE_BETAS, delay_min=DELAY_MIN, delay_init_const=DELAY_INIT_CONST, spike_form=SPIKE_FORM)
