...
# ---------------------------------------------------------------------------------------
# exp_c53: WHERE THE SPIKE TIME COMES FROM.
#
#   "soft"         t_soft is the T_cross-weighted expectation of the arrival time over ALL
#                  N arrivals: c = sigmoid((V - theta)/T_cross), survival cumprod, then
#                  sum(prob * a_srt) + surv[-1]*t_window. This is what c36-c52 ran, and the
#                  reference's own formulation.
#   "detach_hard"  t_soft := t_hard, the arrival at the FIRST index whose membrane crosses
#                  theta. The index is an argmax over a boolean, so the choice of WHICH
#                  arrival spikes carries no gradient; the arrival VALUE still does.
#
# WHAT THIS DOES AND DOES NOT DETACH, since the distinction is the whole experiment. The
# reorder decision -- the permutation itself -- was ALREADY non-differentiable in every
# SORT_FORM: `rank` builds its one-hot from integer comparisons, `argsort` from integer
# indices, and both give bit-identical gradients (exp_c52's sort_equivalence.py measured
# 0.000e+00 on all 8 parameters, including on inputs where 100% of arrival pairs are tied).
# So "stop_gradient on the ordering" alone is a no-op here. What "detach_hard" actually
# removes is the SOFT CROSSING: the T_cross sigmoid-survival average that let gradient
# reach every arrival in proportion to how nearly it spiked, rather than only the one that
# did. The delay gradient still flows -- through the membrane kernel and through the
# gathered t_hard into the soft bucket partition, which remains soft under T_bkt.
#
# `log_T_cross` becomes UNUSED under "detach_hard". It stays in the pytree so checkpoints,
# the parity harness and the parameter count keep one shape across variants; its gradient
# is then exactly zero, which the parity gate asserts rather than assumes.
SPIKE_FORM = "detach_hard"  # "soft" | "detach_hard"
...
def _clamp_like_torch(x, lo, hi):
    """clamp(x, lo, hi) with TORCH's backward, not JAX's.

    WHY THIS EXISTS, and it is not pedantry. `jnp.clip` lowers to
    `minimum(maximum(x, lo), hi)`, and JAX defines the gradient of `maximum` AT AN EXACT
    TIE as an even 0.5/0.5 split between the operands. `torch.clamp`'s backward is
    `grad * ((x >= min) & (x <= max))` -- a full 1.0 at the boundary. So for any delay
    sitting EXACTLY at 0 the two frameworks disagree by a factor of two on that
    parameter's gradient.

    Every configuration from exp_c38 to exp_c47 used `delay_init_std=4`, whose half-normal
    draws are strictly positive, so the clamp was never active and the discrepancy could
    not appear. exp_c48 is the first run to use `delay_init_std=0` -- c36's setting -- and
    the parity gate caught it immediately: the reference's delay gradient was exactly
    2.000000x ours on all 2,176 entries.

    The forward is unchanged; only the boundary subgradient is corrected to match the
    reference. `x * mask` carries the gradient, and the stop_gradient term restores the
    clamped value without contributing any.
    """
    y = jnp.clip(x, lo, hi)
...
def membrane(p, x, t_window=T_WINDOW, form="linear"):
    ...
    # exp_c50: DELAY_MIN replaces the hard 0.0 floor. Upstream's floor enforces causality
    # but is a one-way trap -- below it the clamp zeroes both the value AND the gradient,
    # so exp_c49 measured 95-97% of delays permanently dead when they start ON the floor.
    # Setting DELAY_MIN = -inf removes the floor (reproducing the old
    # BucketLIFDetectorsMHL forward `a = t + delay`) while KEEPING the upper t_window cap,
    # which is what protects the reference's cumsum membrane from exp(a/tau) overflow.
    d = _clamp_like_torch(p["delay"], DELAY_MIN, t_window)           # (T,D,N)
...
def first_spike(p, x, t_window=T_WINDOW, form="linear"):
    ...
    crossed = V >= THETA_MEM
    # stop_gradient on the ORDERING decision. `argmax` over a boolean already yields an
    # integer index that carries no cotangent, so this is an assertion of intent rather
    # than a change in behaviour -- and exp_c53's `detach_ordering_check.py` verifies that
    # it changes nothing, which is the point: the reorder decision was never
    # differentiable in this module, in any SORT_FORM.
    kstar = jax.lax.stop_gradient(jnp.argmax(crossed.astype(a_srt.dtype), axis=-1))
    t_hard = jnp.take_along_axis(a_srt, kstar[..., None], axis=-1)[..., 0]
    t_hard = jnp.where(crossed.any(-1), t_hard, t_window)            # (B,T,D)

    if SPIKE_FORM == "detach_hard":
        # exp_c53. WHICH arrival is the spike is decided hard and carries no gradient:
        # `kstar` is an argmax over a boolean, so it is an integer index and the
        # `stop_gradient` below is documentation, not machinery -- it makes the intent
        # checkable rather than inferred. The VALUE still carries gradient: the gather
        # passes the cotangent to `a_srt[kstar]`, and from there to the delay, the
        # synaptic weight and tau exactly as before. So the delay gradient still flows
        # through the membrane kernel; what disappears is the T_cross-weighted average
        # over ALL N arrivals, and with it the whole cumprod-survival block below.
        #
        # `t_soft := t_hard` therefore feeds the SOFT bucket partition (still governed by
        # T_bkt) with the true first-crossing time instead of an expectation over
        # crossings. `log_T_cross` becomes an unused parameter in this variant -- it is
        # left in the pytree so checkpoints and the parity harness keep one shape, but
        # nothing reads it and its gradient is exactly zero by construction.
        return t_hard, t_hard
...
def structured_delay(delay, delay_offset, n_det, t_window=T_WINDOW):
    """Add a per-DETECTOR additive delay bias, shared across that detector's synapses.

    THE PROBLEM THIS ADDRESSES. `delay` is (T, D, N) -- per synapse per detector -- and at
    stock init every one of those T*D*N scalars is an independent half-normal draw. The D
    detectors of a table therefore differ only by noise, and they all read the same
    latency-coded input against the same initial boundaries (8, 16, 24 for every detector
    of every table). Measured consequence: the within-table pairwise agreement of the hard
    digits is 0.62-0.64 at init, against 0.25 for independent uniform digits at 4 buckets.
    The "three independent detectors" premise is barely realised at the start, so the
    mixed-radix product over 4**3 = 64 cells carries far less information than 64 cells'
    worth, and exp_c39's transplant showed it is exactly this front-end draw that decides
    whether a seed takes off.

    THE CHANGE. Detector d gets `d * delay_offset` added to all N of its delays. The
    per-synapse i.i.d. jitter is kept -- this biases the detectors apart, it does not
    replace the randomness -- and the sum is clamped to the same [0, t_window] the forward
    uses, so delays stay non-negative and arrivals stay inside [0, 2*t_window].

    `delay_offset = 0.0` reproduces the stock init EXACTLY, and consumes no RNG, so it is
    byte-identical to the previous behaviour rather than merely equivalent.
    """
    if float(delay_offset) == 0.0:
        return delay
    bias = float(delay_offset) * jnp.arange(n_det, dtype=delay.dtype)   # (D,)
    return jnp.clip(delay + bias[None, :, None], 0.0, t_window)


def structured_boundaries(beta_base, boundary_offset, n_det):
    """Add a per-DETECTOR additive offset to the boundary base, shifting where in time
    that detector quantises WITHOUT touching whether or when it fires.

    WHY THIS AND NOT A DELAY OFFSET. exp_c40 biased the delays instead, and the sweep
    showed why that cannot work: a per-detector delay bias is shared across all N synapses,
    so it TRANSLATES the detector's whole arrival pattern later rather than changing what
    the detector reads. Past a few units the membrane stops crossing theta_mem at all, the
    detector folds into the last bucket as a constant digit, and pairwise agreement goes UP
    (34 of 96 detectors dead at offset 5.33; agreement 0.634 -> 0.670).

    The boundaries are a different lever entirely. The digit is #{boundaries <= t*}, and
    `boundaries = beta_base + cumsum(softplus(beta_raw))`, so adding d*offset to
    `beta_base[:, d]` slides detector d's whole ladder along the time axis. The membrane,
    the arrival times and the spike time t* are untouched, so:

      * NO DETECTOR CAN BE KILLED by this. Firing is unchanged by construction; only the
        readout of the firing time moves.
      * The three detectors of a table quantise three DIFFERENT regions of the window,
        which is what "read different structure" actually requires.

    There is still a degenerate limit -- push the ladder entirely past the spike-time
    distribution and every sample lands in bucket 0 (or all in the last bucket the other
    way), which is a constant digit again. But it saturates the QUANTISER rather than
    killing the neuron, and the sweep locates it by measurement rather than assumption.

    `boundary_offset = 0.0` returns the input untouched and consumes no RNG, so the default
    is byte-identical to the stock init.
    """
    if float(boundary_offset) == 0.0:
        return beta_base
    bias = float(boundary_offset) * jnp.arange(n_det, dtype=beta_base.dtype)   # (D,)
    return beta_base + bias[None, :, None]


TABLE_INIT_STD = 0.1     # the reference's hard-coded constant; see init()

# exp_c50: lower bound of the delay clamp in `membrane`. 0.0 reproduces upstream --
# causal, but a ONE-WAY TRAP, since below the floor the clamp zeroes both the value and
# the gradient. Set to -inf to remove the floor while keeping the upper t_window cap.
DELAY_MIN = float("-inf")   # exp_c50: NO lower bound (upper t_window cap kept)


def init(key, n_buckets, n_det, tph, n_heads, input_dim, n_outputs,
         delay_init_std=0.0, delay_offset=0.0, boundary_offset=0.0, share_betas=False,
         delay_init_const=0.0,
         table_init_std=TABLE_INIT_STD):
    ...
    # exp_c45: `share_betas` ties the bucket ladder across every (table, detector) — one
    # global beta_base scalar and one global beta_raw vector, broadcast everywhere. The
    # rest of the model is untouched: `boundaries()` returns (1, 1, M-1) instead of
    # (T, D, M-1), and every consumer already indexes it with a leading batch axis and
    # broadcasts, so no forward code changes. The gradient sums over all tables and
    # detectors automatically, which is what "tied" means.
    bsh = (1, 1) if share_betas else (n_tables, n_det)
    ...
    # exp_c51: deterministic constant shift, so delays start in the INTERIOR of the
    # valid window with gradient headroom in both directions. Consumes no RNG.
    if float(delay_init_const) != 0.0:
        delay = delay + float(delay_init_const)
    delay = structured_delay(delay, delay_offset, n_det, T_WINDOW)
    return dict(
        delay=delay,
        w_raw=(W_RAW_MEAN + W_RAW_STD * jax.random.normal(kw, dsh, jnp.float32)),
        tau_raw=jnp.ones((n_tables, n_det), jnp.float32),
        beta_base=structured_boundaries(
            jnp.zeros(bsh + (1,), jnp.float32),
            0.0 if share_betas else boundary_offset, n_det),
        beta_raw=jnp.full(bsh + (n_buckets - 1,), inv_softplus_step, jnp.float32),
        log_T_cross=jnp.zeros((n_tables,), jnp.float32),
        log_T_bkt=jnp.zeros((n_tables,), jnp.float32),
        # exp_c42: `table_init_std` replaces the reference's hard-coded 0.1.
        #
        # WHY THE CONSTANT IS A PROBLEM. A row is read ONE-HOT and then SUMMED over the
        # `tph` tables of a head, so the head's output at init is a sum of tph independent
        # draws and its std grows as sqrt(tph) * table_init_std -- NOT as a constant. At
        # tph=32 that is 0.58; at exp_c36's tph=128 the identical code gives 1.13. The
        # initial policy is therefore twice as spread in one configuration as in another
        # purely because of a missing fan-in correction, which is an unintended difference
        # between experiments in this chapter rather than a modelling choice.
        #
        # 0.1/sqrt(tph) makes the summed mu-head output std ~0.1 at any table count, which
        # is what "small, smooth initial actions" asks for and what fan-in scaling means
        # here. The trainer's separate log-sigma bias of -1/(heads*tph) is UNCHANGED, so
        # the initial sigma is still exp(-1) ~ 0.368 regardless of this std.
        table=jax.random.normal(kt, (n_tables, cells, n_outputs),
                                jnp.float32) * float(table_init_std),
    )
