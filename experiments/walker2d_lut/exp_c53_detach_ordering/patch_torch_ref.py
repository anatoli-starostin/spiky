...
INIT_NEW = ("        # exp_c51 SCRATCH: deterministic constant delay init. Consumes no "
            ...
            "        self.spike_form = str(spike_form)")
...
SPIKE_OLD = ("        a_srt, V = self._membrane(x)\n"
             "        t_hard = self._t_hard(a_srt, V)\n"
             "        Tc = self.T_cross.view(1, self.n_tables, 1, 1)")
SPIKE_NEW = ("        a_srt, V = self._membrane(x)\n"
             "        t_hard = self._t_hard(a_srt, V)\n"
             "        # exp_c53 SCRATCH: the DETACHED-HARD spike time. `t_soft := t_hard`,\n"
             "        # so the soft bucket partition is fed the actual first-crossing\n"
             "        # arrival instead of the T_cross-weighted expectation over all N.\n"
             "        # The crossing INDEX carries no gradient (it is an argmax over a\n"
             "        # bool); the arrival VALUE still does, so delay/w/tau keep learning\n"
             "        # through the membrane kernel. T_cross goes unused.\n"
             "        if self.spike_form == 'detach_hard':\n"
...
def main():
    ...
    for old, new, what in ((SIG_OLD, SIG_NEW, "constructor signature"),
                           ...
                           (SPIKE_OLD, SPIKE_NEW, "_first_spike head"),
