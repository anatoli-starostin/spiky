...
def actor_init(key, n_buckets, n_det, tph, heads, delay_init_std, delay_offset=0.0):
    p = LIF.init(key, n_buckets, n_det, tph, heads, OBS, 2 * ACT,
                 delay_init_std=delay_init_std, delay_offset=delay_offset)
...
def main():
    ...
    ap.add_argument("--delay-offset", type=float, default=2.0,
                    help="exp_c40: per-detector additive delay bias (detector d gets "
                         "d*offset added to all its synapse delays, on top of the i.i.d. "
                         "half-normal jitter). 0.0 reproduces the stock exp_c39 init "
                         "byte-for-byte.")
    ...
    print(f"MHL-LIF SAC actor heads={a.heads} tph={a.tph} n_det={a.ndet} "
          ...
          f"delay_offset={a.delay_offset} | temperatures "
    ...
    for it in range(a.iters):
        ...
        if (it + 1) % a.eval_every == 0 or it == a.iters - 1:
            ...
            ck = dict({k: np.asarray(v) for k, v in ap_.items()},
                      ...
                      delay_offset=np.float32(a.delay_offset))
