"""exp_c40 — self-updating Slack bar for the 3-seed MHL-LIF SAC sweep.
...
Exits once SWEEP_DONE_C40 exists or --max-hours passes. Deliberately not `pgrep -f`:
exp_c25's refresher matched its own command line and posted a frozen number for nine hours.

Usage:
  python slack_bar_c40.py --task <BODY_TASK id> [--handle <existing>] [--interval 150]
"""
import argparse
import json
import os
...
SEEDS = (0, 1, 2)
TOTAL = len(SEEDS)
SENTINEL = "SWEEP_DONE_C40"
REF = ("_*exp_c40 — STRUCTURED DELAY INIT.* The exp_c39 config (1 head × 32 tables × 3 LIF "
       "detectors × 4 buckets, freeze_temperature=True, delay_init_std=4) with one change: "
       "detector d gets *d × 2.0* added to all 17 of its synapse delays, on top of the "
       "i.i.d. half-normal jitter. An INIT change only — params unchanged at *28,384* "
       "(101.3% of the hyperplane baseline). Parity with the structured init on: *83/83*, "
       "including two assertions that the bias is really present on both sides.\n"
       "⚠️ _The init-only offset sweep predicts this will HURT: pairwise digit agreement "
       "RISES with offset (0.634 → 0.652 at 2.0) and detector entropy falls (0.937 → "
       "0.857). It is run anyway because the exp_c39 diagnosis established that init "
       "aggregates do NOT predict takeoff — vetoing on an aggregate would contradict the "
       "finding this experiment came from._\n"
       "_Success metric: takeoff count against stock c39 (*1/3*, 2030 ± 1895) and c38 "
       "(*2/3*, 3214 ± 1526). Anchors: exp_c18 hyperplane *4308 ± 500* (6 seeds); c36 "
       "bucket 16×128 4246 ± 298, the only baseline-matching config._")


def read(seed):
    log = os.path.join(HERE, f"cell_s{seed}.log")
    cpu = os.path.join(HERE, f"mhl_sac_c40_s{seed}_cpueval.json")
...
def render():
    ...
    if n_cpu:
        vals = [json.load(open(os.path.join(
            HERE, f"mhl_sac_c40_s{s}_cpueval.json")))["cpu_reference_mean"]
...
def main():
    ...
    h = a.handle or progress.progress_start(
        "exp_c40 — LIFMultiHeadLUT: 32 tables × 3 LIF detectors × 4 buckets, 3 seeds",
        task=a.task, style="emoji", width=10)
    open(os.path.join(HERE, ".slack_c40.handle"), "w").write(h)
    ...
    while True:
        ...
        if finished or timeout:
            ...
            progress.progress_done(
                ...
                final_text=(f"exp_c40 MHL-LIF sweep "
