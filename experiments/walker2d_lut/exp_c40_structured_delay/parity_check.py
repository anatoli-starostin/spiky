...
def main():
    ...
    for case in CASES:
        ...
        if case == "run" and nd > 1:
            dly = np.asarray(p["delay"])                     # (T, D, N)
            per_det_min = dly.min(axis=(0, 2))               # (D,)
            steps = np.diff(per_det_min)
            check_bool(f"{case}: torch delays carry the per-detector bias",
                       bool(np.all(steps > 0.5)),
                       f"per-detector min {np.round(per_det_min, 3).tolist()}, "
                       f"steps {np.round(steps, 3).tolist()}")
            base = jnp.zeros((4, nd, 5))
            got = np.asarray(M.structured_delay(base, 2.0, nd))
            want = 2.0 * np.arange(nd)[None, :, None] * np.ones((4, nd, 5))
            check_bool(f"{case}: jax structured_delay applies d*offset",
                       bool(np.allclose(got, want)),
                       f"detector biases {np.round(got[0, :, 0], 3).tolist()}")
