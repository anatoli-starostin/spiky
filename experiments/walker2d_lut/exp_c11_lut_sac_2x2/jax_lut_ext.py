...
def _smooth_out(x, w, b, weights, log_T_soft, log_T_sel, n_heads, tph):
    ...
    return blended.reshape(x.shape[0], n_heads, tph, -1).sum(2), main
...
def _fwd(x, w, b, weights, log_T_soft, log_T_sel, n_heads, tph):
    out, main = _smooth_out(x, w, b, weights, log_T_soft, log_T_sel, n_heads, tph)
    return out, (x, w, b, weights, log_T_soft, log_T_sel, main)


def _bwd(n_heads, tph, res, g):
    # Same full-K softmax surrogate as the hard forward: torch shares the surrogate
    # across forward modes too, so the verified gradient path is reused unchanged.
    return L._bwd(n_heads, tph, res, g)
