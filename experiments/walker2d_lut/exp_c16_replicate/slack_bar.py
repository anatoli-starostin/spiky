"""exp_c16 — self-updating Slack bar for the replicate test (#75).
...
REF = ("exp_c15 seed 0 (3-concurrent) = 4152.7 · exp_c15 across-seed sd = 178 · "
       "unexplained exp_c11-vs-exp_c14 same-seed gap = 952")
...
def read(rep):
    import json
    ev = os.path.join(C09, f"lut_sac_c16_rep_{rep}_cpueval.json")
    ...
    if st["eval_done"]:
        try:
            ...
            st["eval_done"] = False
    p = os.path.join(HERE, f"cell_rep_{rep}.log")
    try:
...
def render():
    ...
    if all(s["eval_done"] for s in sts):
        ...
        lines.append(f"\n*|A − B| = {d:.1f}* — pure run-to-run nondeterminism "
                     f"(the seed is identical)")
...
def main():
    ...
    a = ap.parse_args()
    h = progress.progress_start("replicate test — same seed twice, sequential (#75)",
                                task=a.task, style="emoji", width=10)
    open(os.path.join(HERE, ".slack_c16.handle"), "w").write(h)
