"""exp_c33 — collect the CPU-reference results for the Bucket-LIF actor.
...
def main():
    rows = {}
    for s in SEEDS:
        p = os.path.join(HERE, f"bucket_sac_c33_s{s}_cpueval.json")
        rows[s] = json.load(open(p)) if os.path.exists(p) else None

    print("=== exp_c33 — Bucket-LIF actor, 64 buckets x 32 tables, 3 seeds ===")
