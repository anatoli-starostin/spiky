"""exp_c33 — self-updating Slack bar for the 3-seed Bucket-LIF SAC sweep (#75).
...
Exits once SWEEP_DONE_C33 exists or --max-hours passes. Deliberately not `pgrep -f`:
...
REF = ("_*64 BUCKETS — the decisive experiment of this line.* exp_c32b (16 buckets) "
       "changed the addressing scheme AND cut the table 4×, so it cannot separate "
       "'bucket addressing is worse' from '16 rows is too small'. This restores the "
       "chapter's standard 64 rows/table and lands *param-matched to the baseline*: "
       "*27,808 total* = 3,232 front-end + 24,576 table — **99.2% of exp_c18's 28,032**, "
       "front-end 93.5% of its 3,456. Same corrected model, same SAC recipe; only "
       "`n_buckets` differs from c32b. Parity at 64 buckets: 40/40. Anchors: *exp_c18 "
       "hyperplane 4308 ± 500 (6 seeds)*; c32b 16-bucket; c32 BROKEN 1174 ± 517; "
       "c31 PureLIF 2951 ± 2109; c30b 4087 ± 991._")


def read(seed):
    log = os.path.join(HERE, f"cell_s{seed}.log")
    cpu = os.path.join(HERE, f"bucket_sac_c33_s{seed}_cpueval.json")
...
def render():
    ...
    if n_cpu:
        vals = [json.load(open(os.path.join(
            HERE, f"bucket_sac_c33_s{s}_cpueval.json")))["cpu_reference_mean"]
        ...
        lines += ["", f"*mean over {len(vals)} seeds: {m:.0f}" +
                  ...
                  "  ·  c18 hyperplane 4308 ± 500 at 1.008× the params"]
    lines += ["", "_bare number = 100-ep deterministic CPU reference (the result); "
                  ...
                  "out of 64; starts ~32 (non-firing folds into the last bucket) and "
                  "should FALL while the spread RISES. At init only 21 of 64 buckets are "
                  "reachable — whether training widens that is the thing to watch._"]
...
def main():
    ...
    h = a.handle or progress.progress_start(
        "exp_c33 — Bucket-LIF at 64 BUCKETS × 32 tables, param-matched, 3 seeds (#75)",
        task=a.task, style="emoji", width=10)
    open(os.path.join(HERE, ".slack_c33.handle"), "w").write(h)
