...
def main():
    ...
    print(f"run   cfg {RUN}  freeze_temperature=False  delay_init_std={DELAY_INIT_STD}  boundary_offset={BOUNDARY_OFFSET} table_init_std={TABLE_INIT_STD} share_betas={SHARE_BETAS} delay_min={DELAY_MIN} delay_init_const={DELAY_INIT_CONST}")
    one_case("run", LIFMultiHeadLUT(**RUN, freeze_temperature=False,
                                    delay_init_std=DELAY_INIT_STD, table_init_std=TABLE_INIT_STD, share_betas=SHARE_BETAS, delay_min=DELAY_MIN, delay_init_const=DELAY_INIT_CONST),
             RUN, 11, dump)
    ...
    one_case("perturbed", perturb(LIFMultiHeadLUT(**RUN, delay_min=DELAY_MIN), 77),
             RUN, 13, dump)
    print(f"alt   cfg {ALT}  freeze_temperature=False  delay_min={DELAY_MIN}")
    one_case("alt", perturb(LIFMultiHeadLUT(**ALT, delay_min=DELAY_MIN), 99),
             ALT, 17, dump)
    ...
    m = LIFMultiHeadLUT(**RUN, freeze_temperature=False, delay_init_std=DELAY_INIT_STD,
                        table_init_std=TABLE_INIT_STD, share_betas=SHARE_BETAS, delay_min=DELAY_MIN, delay_init_const=DELAY_INIT_CONST)
