...
def main():
    ...
    ap.add_argument("--ndet", type=int, default=3)
    ...
    ap.add_argument("--table-seed", type=int, default=None,
                    help="Take the TABLE init from this seed while the FRONT-END init "
                         "(delay, w_raw) comes from --actor-seed. Splits the actor init "
                         "into its two halves so they can be transplanted separately.")
    ap.add_argument("--actor-seed", type=int, default=None,
                    help="Draw the ACTOR INIT from this seed's key while every other "
                         "random quantity -- critic init, env resets, action noise, "
                         "replay sampling -- still comes from --seed. Omit (or set equal "
                         "to --seed) and the run is bit-identical to mhl_sac.py.")
    ...
    # THE TRANSPLANT. `ka` is the only key the actor's init consumes, and `key` carries on
    # from the same split regardless -- so swapping ka alone exchanges the STARTING
    # PARAMETERS between two runs while leaving every other random stream (critic init,
    # env resets, exploration noise, replay indices) attached to --seed. That isolates
    # "was it the init?" from "was it the RL trajectory?", which no amount of staring at
    # the finished checkpoints can.
    if a.actor_seed is not None and a.actor_seed != a.seed:
        ka = jax.random.split(jax.random.PRNGKey(a.actor_seed), 4)[1]
        print(f"TRANSPLANT: actor init from seed {a.actor_seed}, all other randomness "
              f"from seed {a.seed}", flush=True)
    ...
    # SPLIT THE INIT. The transplant above showed the outcome follows the actor init
    # wholesale; this separates the init's two halves so we can see WHICH half carries it.
    # The front-end init (delay, w_raw -- what the addressing does) stays with --actor-seed;
    # only `table` (the initial action values, 0.1*randn) is taken from --table-seed.
    if a.table_seed is not None:
        kt = jax.random.split(jax.random.PRNGKey(a.table_seed), 4)[1]
        ap_ = dict(ap_, table=actor_init(kt, a.buckets, a.ndet, a.tph, a.heads,
                                         a.delay_init_std)["table"])
        print(f"TABLE SPLIT: front-end init from seed "
              f"{a.actor_seed if a.actor_seed is not None else a.seed}, "
              f"TABLE init from seed {a.table_seed}", flush=True)
