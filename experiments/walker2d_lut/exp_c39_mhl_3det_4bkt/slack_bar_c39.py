"""exp_c39 — self-updating Slack bar for the 3-seed MHL-LIF SAC sweep.
...
Exits once SWEEP_DONE_C39 exists or --max-hours passes. Deliberately not `pgrep -f`:
exp_c25's refresher matched its own command line and posted a frozen number for nine hours.

Usage:
  python slack_bar_c39.py --task <BODY_TASK id> [--handle <existing>] [--interval 150]
"""
import argparse
import json
import os
...
SEEDS = (0, 1, 2)
TOTAL = len(SEEDS)
SENTINEL = "SWEEP_DONE_C39"
REF = ("_*LIFMultiHeadLUT* (branch exp/lif-detectors-mhl @ 24c0e60a). Config: 1 head, "
       "*32 tables × 3 LIF detectors × 4 buckets* = 4³ = 64 mixed-radix cells per table, "
       "temperatures FROZEN at 1.0, delay_init_std=4. *28,384 params* (28,320 trainable) "
       "= 3,808 front-end + 24,576 table = *101.3%* of the 28,032 hyperplane baseline — "
       "the closest param match of ANY model in this chapter, and it lands there without "
       "being tuned for it. THE CONTROL: same 64 rows, same 24,576 table and same 32 "
       "tables as c38 and c31, so c38-vs-c39 is a clean digit-WIDTH (4 buckets) against "
       "digit-COUNT (6 bits) trade at fixed capacity — and c39 does it on HALF the "
       "front-end, since 3 detectors carry half the delays and synapses of 6. Anchors: "
       "*exp_c18 hyperplane 4308 ± 500* (6 seeds); c36 bucket 16×128 4246 ± 298 (the only "
       "baseline-matching config); c38 mhl 6det×2bkt 3214 ± 1526; c31 PureLIF 2951 ± 2109; "
       "c37 bucket 32×64 2531 ± 1266; c32b bucket 16×32 2041 ± 1230._")


def read(seed):
    log = os.path.join(HERE, f"cell_s{seed}.log")
    cpu = os.path.join(HERE, f"mhl_sac_c39_s{seed}_cpueval.json")
...
def render():
    ...
    if n_cpu:
        vals = [json.load(open(os.path.join(
            HERE, f"mhl_sac_c39_s{s}_cpueval.json")))["cpu_reference_mean"]
...
def main():
    ...
    h = a.handle or progress.progress_start(
        "exp_c39 — LIFMultiHeadLUT: 32 tables × 3 LIF detectors × 4 buckets, 3 seeds",
        task=a.task, style="emoji", width=10)
    open(os.path.join(HERE, ".slack_c39.handle"), "w").write(h)
    ...
    while True:
        ...
        if finished or timeout:
            ...
            progress.progress_done(
                ...
                final_text=(f"exp_c39 MHL-LIF sweep "
