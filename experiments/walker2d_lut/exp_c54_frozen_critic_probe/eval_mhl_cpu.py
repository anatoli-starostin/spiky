...
def load_actor(path):
    ...
    if p["tau_raw"].shape != (n_tables, nd):
        raise ValueError(f"{base}: tau_raw is {p['tau_raw'].shape}, expected "
                         f"{(n_tables, nd)}")
    # exp_c45: the betas may be SHARED -- one global ladder of shape (1, 1, ·) broadcast
    # to every (table, detector) -- or per-(table, detector) as upstream. Both are valid
    # checkpoints; anything else means the file does not match its own recorded config.
    for k, tail in (("beta_base", 1), ("beta_raw", nb - 1)):
        if p[k].shape not in ((n_tables, nd, tail), (1, 1, tail)):
            raise ValueError(f"{base}: {k} is {p[k].shape}, expected "
                             f"{(n_tables, nd, tail)} (per detector) or "
                             f"{(1, 1, tail)} (shared ladder)")
    if p["beta_raw"].shape[0] == 1 and n_tables > 1:
        print(f"  note: SHARED bucket ladder — one beta_base scalar and one "
              f"{nb-1}-vector broadcast to all {n_tables} tables", flush=True)
