...
def actor_init(key, n_buckets, n_det, tph, heads, delay_init_std, boundary_offset=0.0,
               table_init_std=0.1, share_betas=False, delay_init_const=0.0):
    p = LIF.init(key, n_buckets, n_det, tph, heads, OBS, 2 * ACT,
                 ...
                 delay_init_const=delay_init_const)
...
def main():
    ...
    ap.add_argument("--delay-init-const", type=float, default=0.0,
                    help="exp_c51: deterministic positive delay init, so delays start in "
                         "the INTERIOR of the clamp window with gradient headroom in both "
                         "directions instead of pinned on the floor. Consumes no RNG.")
    ...
    ap.add_argument("--freeze-critic", type=int, default=0,
                    help="exp_c54: 1 = the critic and its target are held fixed. The "
                         "actor still forwards through the critic to compute its own "
                         "loss; what is skipped is the twin-Q loss, its gradient, the "
                         "critic optimiser step and the target EMA.")
    ap.add_argument("--profile-iters", type=int, default=0,
                    help="exp_c54: run this many post-warmup iterations timing the "
                         "rollout and the fused 32-update block separately, print the "
                         "attribution, and exit without training.")
    ...
    ap_ = actor_init(ka, a.buckets, a.ndet, a.tph, a.heads, a.delay_init_std,
                     a.boundary_offset, a.table_init_std, bool(a.share_betas), a.delay_init_const)
    ...
    print(f"MHL-LIF SAC actor heads={a.heads} tph={a.tph} n_det={a.ndet} "
          ...
          f"share_betas={bool(a.share_betas)} "
          f"delay_const={a.delay_init_const} delay_min={LIF.DELAY_MIN} | temperatures "
    ...
    def update(ap_, qp, qt, log_alpha, os_a, os_q, os_al, batch, key):
        ...
        # exp_c54: with the critic frozen, EVERYTHING in this block disappears -- the
        # target-network forward passes, the twin-Q loss, its gradient, and the optimiser
        # step. The actor loss below still forwards through `qp`, so the critic is still
        # evaluated once per update; what goes away is differentiating and updating it.
        # That is exactly the cost the frozen-critic idea proposes to save, which is why
        # the split is drawn here.
        if a.freeze_critic:
            ql, td = jnp.float32(0.0), jnp.zeros_like(r)
        else:
            key, k1 = jax.random.split(key)
        ...
        # The target EMA is also critic work: with the critic frozen there is nothing for
        # the target network to track, so it too is skipped.
        if not a.freeze_critic:
    ...
    # ---- exp_c54: per-iteration cost attribution --------------------------------------
    # Answers "what is the ceiling on the speedup from dropping critic learning" without
    # needing a trained critic, which is the one thing this chapter never saved. The two
    # phases are timed separately, each with block_until_ready so async dispatch cannot
    # smear them into each other. Warmup iterations run first so the buffer is populated
    # and both phases are compiled before any timing starts.
    if a.profile_iters:
        for it in range(a.warmup):
            key, kro = jax.random.split(key)
            st, kro, tr = rollout(ap_, st, kro, True)
            s_, a_, r_, s2_, d_ = [np.asarray(x).reshape((-1,) + x.shape[2:])
                                   for x in tr]
            n = len(s_)
            buf = insert(buf, jnp.asarray((ptr + np.arange(n)) % N),
                         s_, a_, r_, s2_, d_)
            ptr = int((ptr + n) % N)
            size = min(size + n, N)
        args = (ap_, qp, qt, log_alpha, os_a, os_q, os_al, key, row_updates, buf, size)
        jax.block_until_ready(update_block(*args))          # compile
        t_ro = t_up = 0.0
        for it in range(a.profile_iters):
            key, kro = jax.random.split(key)
            tA = time.perf_counter()
            st, kro, tr = rollout(ap_, st, kro, False)
            ...
            out = jax.block_until_ready(update_block(
                ap_, qp, qt, log_alpha, os_a, os_q, os_al, key, row_updates, buf, size))
            (ap_, qp, qt, log_alpha, os_a, os_q, os_al, key, row_updates,
             last_alpha) = out
            t_ro += tB - tA
            t_up += time.perf_counter() - tB
        m = a.profile_iters
        tot = (t_ro + t_up) / m
        print(f"PROFILE freeze_critic={bool(a.freeze_critic)}  {m} iters, "
              f"{a.updates} updates/iter, batch {a.batch}", flush=True)
        print(f"  rollout (env + buffer insert)  {1e3*t_ro/m:8.2f} ms/iter "
              f"({100*t_ro/(t_ro+t_up):5.1f}%)", flush=True)
        print(f"  update block ({a.updates} updates) {1e3*t_up/m:8.2f} ms/iter "
              f"({100*t_up/(t_ro+t_up):5.1f}%)", flush=True)
        print(f"  TOTAL                          {1e3*tot:8.2f} ms/iter "
              f"= {tot:.4f} s/iter", flush=True)
        return
    ...
    for it in range(a.iters):
        ...
        if (it + 1) % a.eval_every == 0 or it == a.iters - 1:
            ...
            ck = dict({k: np.asarray(v) for k, v in ap_.items()},
                      ...
                      delay_init_const=np.float32(a.delay_init_const))
