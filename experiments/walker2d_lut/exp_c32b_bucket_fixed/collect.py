"""exp_c32b — collect the CPU-reference results for the Bucket-LIF actor.
...
def main():
    rows = {}
    for s in SEEDS:
        p = os.path.join(HERE, f"bucket_sac_c32b_s{s}_cpueval.json")
        rows[s] = json.load(open(p)) if os.path.exists(p) else None

    print("=== exp_c32b — Bucket-LIF actor, 16 buckets x 32 tables, 3 seeds ===")
