"""exp_c32b — the Bucket-LIF actor against the four anchors of this line.
...
def main():
    hist = {s: json.load(open(os.path.join(HERE, f"bucket_sac_c32b_s{s}.json")))
            ["history"] for s in SEEDS}
    cpu = [json.load(open(os.path.join(HERE, f"bucket_sac_c32b_s{s}_cpueval.json")))
    ...
    fig.suptitle("exp_c32b — bucket-addressed LIF: one neuron per table, row = which of 16 "
    ...
    out = os.path.join(HERE, "c32b_bucket_result.png")
