...
def main():
    ...
    # =========================================================================
    # The exp_profiling optimisations. Measured 1.45x, verified BIT-EXACT.
    # =========================================================================
    # The baseline inner loop ran `a.updates` Python iterations, each dispatching ~8 host
    # operations: a key split, five separate buffer gathers, the jitted update, and a
    # device->host sync plus np.bincount for row coverage. That last one dominated -- it
    # was 71% of the total speedup on its own, because each sync stalls the pipeline until
    # the GPU drains.
    #
    # Fusing the whole loop into one `lax.scan` requires the scan body to perform exactly
    # the same operations in exactly the same order on exactly the same RNG stream, or the
    # run is no longer the same experiment. `check_equivalence.py` asserts that rather
    ...
    def update_block(ap_, qp, qt, log_alpha, os_a, os_q, os_al, key, cov, buf, size):
        def one(carry, _):
            ...
            # Coverage on device: the same count np.bincount produced, as a scatter-add
            # that never leaves the GPU. Read once per eval instead of a.updates times
            # per iteration.
    ...
    last_alpha = None          # set by update_block; only used for logging
    total_steps = 0
