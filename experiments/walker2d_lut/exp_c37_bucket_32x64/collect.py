"""exp_c37 — collect the CPU-reference results for the Bucket-LIF actor.
...
ANCHORS = [("exp_c18 hyperplane", 4308.0, 500.1, 6, 28032, 3456),
           ("exp_c32b bucket 16x32", 2041.2, 1230.1, 3, 7840, 1696),
           ("exp_c33 bucket 64x32", 1536.2, 1416.8, 3, 27808, 3232),
           ("exp_c36 bucket 16x128", 4246.1, 298.4, 3, 31360, 6784),
           ("exp_c32 BROKEN bucket (free-signed w)", 1174.4, 517.2, 3, 7840, 1696),
...
def main():
    rows = {}
    for s in SEEDS:
        p = os.path.join(HERE, f"bucket_sac_c37_s{s}_cpueval.json")
        rows[s] = json.load(open(p)) if os.path.exists(p) else None

    print("=== exp_c37 — Bucket-LIF actor, 32 buckets x 64 tables, 3 seeds ===")
