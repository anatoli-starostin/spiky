"""exp_c37 — self-updating Slack bar for the 3-seed Bucket-LIF SAC sweep (#75).
...
Exits once SWEEP_DONE_C37 exists or --max-hours passes. Deliberately not `pgrep -f`:
...
def read(seed):
    ...
    cpu = os.path.join(HERE, f"bucket_sac_c37_s{seed}_cpueval.json")
...
def render():
    ...
    if n_cpu:
        vals = [json.load(open(os.path.join(
            HERE, f"bucket_sac_c37_s{s}_cpueval.json")))["cpu_reference_mean"]
...
def main():
    ...
    h = a.handle or progress.progress_start(
        "exp_c37 — Bucket-LIF 32 buckets × 64 tables, param-matched, 3 seeds (#75)",
        task=a.task, style="emoji", width=10)
    open(os.path.join(HERE, ".slack_c37.handle"), "w").write(h)
