...
def main():
    ...
    for case in CASES:
        ...
        if case == "run" and nd == 1:
            ...
            # exp_c45: the SHARED ladder. beta_base/beta_raw must have collapsed to a
            # single global copy, and the ladder every table sees must be byte-identical.
            shared = p["beta_raw"].shape[0] == 1
            check_bool(f"{case}: betas are SHARED (one global ladder)", shared,
                       f"beta_base {tuple(p['beta_base'].shape)}, "
                       f"beta_raw {tuple(p['beta_raw'].shape)} "
                       f"(unshared would be ({n_tables}, {nd}, 1) / "
                       f"({n_tables}, {nd}, {nb-1}))")
            if shared:
                # Broadcast to every table exactly as the forward does, then assert all
                # 64 ladders are identical to the last bit -- not merely close.
                per_table = np.broadcast_to(bb, (n_tables, nd, nb - 1))
                allsame = bool(np.all(per_table == per_table[0:1]))
                check_bool(f"{case}: every table sees a BYTE-IDENTICAL ladder", allsame,
                           f"{n_tables} tables x {nd} det, max spread "
                           f"{float(np.abs(per_table - per_table[0:1]).max()):.3e}")
                # And it must actually reach the forward: digits for a spike time equal to
                # a boundary must agree across tables when the input does.
                bd = np.asarray(M.bucket(p, jnp.full((3, n_tables, nd), 16.0),
                                         jnp.full((3, n_tables, nd), 16.0))[0])
                check_bool(f"{case}: shared ladder reaches the forward",
                           bool(np.all(bd == bd[:, 0:1, :])),
                           f"identical spike time -> identical digit in all "
                           f"{n_tables} tables (got {sorted(set(bd.ravel().tolist()))})")
