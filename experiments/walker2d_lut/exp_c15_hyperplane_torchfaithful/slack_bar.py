"""exp_c15 — self-updating Slack bar for the 3-seed hyperplane reference (#75)."""
...
SEEDS = (0, 1, 2)
REF = ("exp_c14 legacy-init hyperplane = 3968 ± 1187 · best anchors 4678 ± 474 "
...
def read(seed):
    p = os.path.join(HERE, f"cell_hp_anchorinit_s{seed}.log")
    try:
...
def main():
    ...
    a = ap.parse_args()
    h = progress.progress_start("hyperplane×hard TORCH-FAITHFUL init — 3 seeds (#75)",
                                task=a.task, style="emoji", width=10)
    open(os.path.join(HERE, ".slack_c15.handle"), "w").write(h)
