...
SEEDS = (0, 1, 2)
OLD_SINGLE_SEED = 3968.3        # exp_c14 legacy-init 3-seed mean, the A/B comparator
...
def main():
    ...
    for i, s in enumerate(SEEDS, 1):
        actor = f"lut_sac_c15_hp_anchorinit_s{s}_actor.npz"
        log = os.path.join(HERE, f"cell_hp_anchorinit_s{s}.log")
        ...
        if not os.path.exists(ev):
            if not run_eval(actor, f"hp-anchorinit/s{s}", i, len(SEEDS)):
                continue
        e = json.load(open(ev))
    ...
    print(f"\n=== hyperplane x hard, TORCH-FAITHFUL anchor_pairs init, {len(got)} seeds ===")
    for g in got:
    ...
    print(f"  (exp_c14 legacy-init 3-seed mean (A/B comparator): {OLD_SINGLE_SEED})")
