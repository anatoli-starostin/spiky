...
BATCH = 24
TABLE_INIT_STD = 0.0176777   # exp_c42: 0.1/sqrt(tph=32), the fan-in correction
...
def main():
    ...
    print(f"run   cfg {RUN}  freeze_temperature=True  delay_init_std=4  boundary_offset={BOUNDARY_OFFSET} table_init_std={TABLE_INIT_STD}")
    one_case("run", LIFMultiHeadLUT(**RUN, freeze_temperature=True,
                                    delay_init_std=4.0, table_init_std=TABLE_INIT_STD),
             RUN, 11, dump)
