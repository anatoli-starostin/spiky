...
def main():
    ...
    ap.add_argument("--boundary-init", default="uniform",
                    choices=["uniform", "quantile"],
                    help="uniform = the module's own even spacing over (0, t_window). "
                         "quantile = equal-mass cuts placed at the empirical first-spike "
                         "quantiles, measured on a short random-action rollout BEFORE "
                         "training. See jax_bucket_lif.quantile_boundaries.")
    ap.add_argument("--freeze-boundaries", action="store_true",
                    help="zero the gradient on beta_base/beta_raw, pinning the bucket "
                         "boundaries at their init for the whole run. THE POINT: exp_c34 "
                         "started at 7.8 effective buckets (quantile init) and TRAINED "
                         "DOWN to 3.1 -- worse than the uniform-init run it was meant to "
                         "beat. So the addressing collapse is an attractor of the "
                         "training dynamics, not an initialisation artefact. Freezing "
                         "asks the causal question: if the collapse is what limits "
                         "return, preventing it should help; if return is unchanged, "
                         "bucket occupancy is not the binding constraint and the readout "
                         "itself is simply weak.")
    ap.add_argument("--boundary-probe-steps", type=int, default=64,
                    help="random-action env steps used to measure the init spike "
                         "distribution for --boundary-init quantile")
    ...
    # Done AFTER the env exists and BEFORE any gradient step, using a short
    # random-action rollout: the boundaries must be placed on the state distribution the
    # trainer actually starts from, and during warmup that is the random policy. Nothing
    # outside this process is consulted -- no trained checkpoint, no external dataset --
    # so the run stays self-contained and reproducible from its seed alone.
    if a.boundary_init == "quantile":
        kb0, key = jax.random.split(key)

        # Jitted scan, NOT a Python loop calling v_step: MJX stepping in eager mode is
        # thousands of small dispatches per step and is pathologically slow -- the first
        # version of this probe failed to reach iteration 200 in 15 minutes.
        @jax.jit
        def probe(st0, k):
            def one(c, _):
                s_, kk = c
                kk, k1 = jax.random.split(kk)
                act_ = jax.random.uniform(k1, (a.envs, ACT), minval=-1.0, maxval=1.0)
                return (v_step(s_, act_), kk), s_.obs
            _, obs = jax.lax.scan(one, (st0, k), None,
                                  length=a.boundary_probe_steps)
            return obs.reshape(-1, OBS)

        obs_probe = probe(st, kb0)
        xo = (obs_probe - obs_mean) / (obs_std + 1e-6)
        t_hard_0, _ = LIF.first_spike(ap_, xo, n_tables)
        b_before = np.asarray(LIF.boundaries(ap_))
        ap_ = LIF.set_boundaries(ap_, LIF.quantile_boundaries(t_hard_0, a.buckets))
        b_after = np.asarray(LIF.boundaries(ap_))
        addr0 = np.asarray(actor_rows(ap_, obs_probe, a.eval_eps))
        occ = np.stack([np.bincount(addr0[:, t], minlength=K) for t in range(n_tables)])
        pr = occ / np.maximum(occ.sum(1, keepdims=True), 1)
        ent = float((-(np.where(pr > 0, pr * np.log2(np.maximum(pr, 1e-12)), 0)).sum(1)
                     ).mean())
        print(f"  boundary-init=quantile: probed {xo.shape[0]:,} random-policy states | "
              f"boundaries [{b_before.min():.2f},{b_before.max():.2f}] -> "
              f"[{b_after.min():.2f},{b_after.max():.2f}] | "
              f"init occupancy entropy {ent:.2f} of {np.log2(K):.2f} bits "
              f"(= {2**ent:.1f} of {K} effective buckets)", flush=True)
    ...
    def update(ap_, qp, qt, log_alpha, os_a, os_q, os_al, batch, key):
        ...
        if a.freeze_boundaries:
            # Zero the gradient rather than excluding the leaves from the optimiser: the
            # pytree structure stays identical, so the optax state, the checkpoint schema
            # and eval_bucket_cpu.py all keep working untouched. Adam sees a zero
            # gradient forever, so the boundaries never move from their init.
            ga = dict(ga, beta_base=jnp.zeros_like(ga["beta_base"]),
                      beta_raw=jnp.zeros_like(ga["beta_raw"]))
