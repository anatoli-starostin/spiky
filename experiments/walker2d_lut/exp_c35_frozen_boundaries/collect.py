"""exp_c35 — collect the CPU-reference results for the Bucket-LIF actor.
...
ANCHORS = [("exp_c18 hyperplane", 4308.0, 500.1, 6, 28032, 3456),
           ("exp_c32b bucket x16 UNIFORM", 2041.2, 1230.1, 3, 7840, 1696),
           ("exp_c33 bucket x64 uniform", 1536.2, 1416.8, 3, 27808, 3232),
           ("exp_c34 quantile, FREE boundaries", 1661.8, 1375.0, 3, 7840, 1696),
...
def main():
    rows = {}
    for s in SEEDS:
        p = os.path.join(HERE, f"bucket_sac_c35_s{s}_cpueval.json")
        rows[s] = json.load(open(p)) if os.path.exists(p) else None

    print("=== exp_c35 — Bucket-LIF actor, 16 buckets x 32 tables, 3 seeds ===")
