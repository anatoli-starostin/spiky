"""exp_c35 — self-updating Slack bar for the 3-seed Bucket-LIF SAC sweep (#75).
...
Exits once SWEEP_DONE_C35 exists or --max-hours passes. Deliberately not `pgrep -f`:
...
SEEDS = (0, 1, 2)
TOTAL = len(SEEDS)
SENTINEL = "SWEEP_DONE_C35"
REF = ("_*QUANTILE BOUNDARIES* — same model and same 16 buckets as exp_c32b; the ONLY "
       "difference is where the bucket boundaries start. Measured on a trained c32b "
       "actor: the middle 50% of first-spike times spans *3.41 of 32 time units (11% of "
       "the window)*, so uniform cuts spend ~89% of their resolution on empty time and "
       "only *4.7 of 16 buckets* are effectively used (entropy 2.06 of 4.00 bits). This "
       "places the cuts at equal-mass quantiles of the spike distribution, measured on a "
       "random-action rollout inside the trainer before any gradient step. Measured at "
       "init: boundaries [2.0,30.0] → [13.5,32.6], entropy *2.97 of 4.00 bits = 7.8 "
       "effective buckets* (1.7× c32b's trained value, at step 0). Params identical: "
       "*7,840*. Anchors: c32b 16-bucket uniform; exp_c18 hyperplane 4308 ± 500; "
       "c32 BROKEN 1174 ± 517._")

REF_OLD = ("_*CORRECTED* BucketLIFDetectorsMHL (branch @ 2a2795e0). Three fixes vs the "
...
def read(seed):
    ...
    cpu = os.path.join(HERE, f"bucket_sac_c35_s{seed}_cpueval.json")
...
def render():
    ...
    if n_cpu:
        vals = [json.load(open(os.path.join(
            HERE, f"bucket_sac_c35_s{s}_cpueval.json")))["cpu_reference_mean"]
    ...
    lines += ["", "_bare number = 100-ep deterministic CPU reference (the result); "
                  ...
                  "Unlike c32b this does NOT start pinned near 15 — the quantile cuts put "
                  "it near the middle from step 0 (smoke: 7.1±5.6 by iter 250, vs c32b's "
                  "11.0±3.6). Watch the SPREAD: wider = more of the table addressable._"]
...
def main():
    ...
    h = a.handle or progress.progress_start(
        "exp_c35 — QUANTILE boundaries, 16 buckets × 32 tables, 3 seeds (#75)",
        task=a.task, style="emoji", width=10)
    open(os.path.join(HERE, ".slack_c35.handle"), "w").write(h)
