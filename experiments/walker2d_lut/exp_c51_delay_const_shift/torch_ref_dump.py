...
BATCH = 24
DELAY_INIT_STD = 0.0          # no random draw
DELAY_MIN = 0.0               # exp_c51: KEEP the causal floor
DELAY_INIT_CONST = 3.2        # exp_c51: constant interior init (= mean of the std-4 half-normal)
...
def main():
    ...
    one_case("run", LIFMultiHeadLUT(**RUN, freeze_temperature=False,
             ...
             RUN, 11, dump)
    # exp_c51 keeps upstream's floor (delay_min=0.0), so these two are already on the
    # right setting -- passed explicitly anyway so the c50/c51 pair is symmetric and a
    # future edit to DELAY_MIN cannot silently desynchronise the cases. `perturb` writes
    # the delays directly, so delay_init_const does not apply to them.
