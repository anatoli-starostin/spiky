...
BATCH = 24
TABLE_INIT_STD = 0.0125       # 0.1/sqrt(tph=64) — the fan-in correction
SHARE_BETAS = True            # exp_c45: ONE bucket ladder shared by every (table, detector)
BOUNDARY_OFFSET = 0.0        # stock c39 boundaries — this experiment isolates the table std
INPUT_DIM = 17
N_OUT = 12

# The shipped configuration. Anatoli's spec.
RUN = dict(input_dim=INPUT_DIM, n_heads=1, n_outputs=N_OUT, tables_per_head=64,
           n_det=1, n_buckets=32)
...
def main():
    ...
    print(f"run   cfg {RUN}  freeze_temperature=True  delay_init_std=4  boundary_offset={BOUNDARY_OFFSET} table_init_std={TABLE_INIT_STD} share_betas={SHARE_BETAS}")
    one_case("run", LIFMultiHeadLUT(**RUN, freeze_temperature=True,
                                    delay_init_std=4.0, table_init_std=TABLE_INIT_STD, share_betas=SHARE_BETAS),
             RUN, 11, dump)
