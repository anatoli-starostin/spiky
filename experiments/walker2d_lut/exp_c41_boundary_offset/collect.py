"""exp_c41 — collect the CPU-reference results for the MHL-LIF actor.
...
def main():
    rows = {}
    for s in SEEDS:
        p = os.path.join(HERE, f"mhl_sac_c41_s{s}_cpueval.json")
        rows[s] = json.load(open(p)) if os.path.exists(p) else None

    print("=== exp_c41 — LIFMultiHeadLUT actor, 32 tables x 3 detectors x 4 buckets, "
          "3 seeds ===")
