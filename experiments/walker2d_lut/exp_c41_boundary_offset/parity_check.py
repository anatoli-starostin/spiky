...
def main():
    ...
    for case in CASES:
        ...
        if case == "run" and nd > 1:
            # The reference's own boundaries must carry the per-detector offset...
            bnp = np.asarray(M.boundaries(p))                # (T, D, M-1)
            per_det = bnp.mean(axis=(0, 2))                  # (D,) mean ladder position
            steps = np.diff(per_det)
            check_bool(f"{case}: torch boundaries carry the per-detector offset",
                       bool(np.all(steps > 0.5)),
                       f"mean ladder position per detector "
                       f"{np.round(per_det, 3).tolist()}, steps "
                       f"{np.round(steps, 3).tolist()}")
            # ...and the SPACING within a detector must be UNCHANGED: the offset slides
            # the ladder, it does not stretch it. beta_raw is untouched, so every
            # detector's internal gaps must be identical.
            gaps = np.diff(bnp, axis=-1)                     # (T, D, M-2)
            gap_spread = float(gaps.max() - gaps.min())
            check_bool(f"{case}: boundary SPACING unchanged by the offset",
                       gap_spread < 1e-4,
                       f"gap range across all detectors {gap_spread:.2e}")
            # ...and our own init must impose the same structure.
            base = jnp.zeros((4, nd, 1))
            got = np.asarray(M.structured_boundaries(base, 6.0, nd))
            want = 6.0 * np.arange(nd)[None, :, None] * np.ones((4, nd, 1))
            check_bool(f"{case}: jax structured_boundaries applies d*offset",
                       bool(np.allclose(got, want)),
                       f"detector offsets {np.round(got[0, :, 0], 3).tolist()}")
            # The structural claim: a boundary shift cannot change WHETHER a detector
            # fires. t_hard depends only on the membrane, so it must be identical with
            # and without the offset.
            p_no = dict(p, beta_base=jnp.zeros_like(p["beta_base"]))
            th_off, _ = M.first_spike(p, x)
            th_no, _ = M.first_spike(p_no, x)
            d_th = float(np.abs(np.asarray(th_off) - np.asarray(th_no)).max())
            check_bool(f"{case}: offset does NOT change firing (t_hard)", d_th == 0.0,
                       f"max|Δt_hard| {d_th:.3e} — firing is independent of boundaries")
