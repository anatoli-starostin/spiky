...
BATCH = 24
BOUNDARY_OFFSET = 6.0     # exp_c41: per-detector additive BOUNDARY offset (see offset_sweep)
INPUT_DIM = 17
N_OUT = 12
...
def main():
    ...
    print(f"run   cfg {RUN}  freeze_temperature=True  delay_init_std=4  boundary_offset={BOUNDARY_OFFSET}")
    one_case("run", LIFMultiHeadLUT(**RUN, freeze_temperature=True,
                                    delay_init_std=4.0, boundary_offset=BOUNDARY_OFFSET),
             RUN, 11, dump)
