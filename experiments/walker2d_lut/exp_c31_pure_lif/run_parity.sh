#!/usr/bin/env bash
# exp_c31 — the two-venv parity test for PureLIFDetectorsMHL, end to end.
#
...
# drift from the thing we claim parity with. Nothing on that branch is modified.
set -eu
cd "$(dirname "$0")"

REF_BRANCH=912d3f99   # PINNED: was a moving branch name; see note below
REF_SRC=src/spiky/lutorch/pure_lif_detectors_mhl.py
STAGE=/tmp/pure_lif_ref
NPZ=/tmp/pure_lif_ref/torch_reference.npz
...
git -C "$HOME/projects/spiky" show "$REF_BRANCH:$REF_SRC" \
    > "$STAGE/spiky/lutorch/pure_lif_detectors_mhl.py"
echo "staged $REF_BRANCH:$REF_SRC -> $STAGE  ($(wc -l < "$STAGE/spiky/lutorch/pure_lif_detectors_mhl.py") lines)"
