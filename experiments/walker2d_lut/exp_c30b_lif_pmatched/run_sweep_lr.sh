...
for seed in $SEEDS; do
  ...
  nohup $PY -u "$TRAIN" --seed "$seed" $COMMON --tag "_c30b_s${seed}" \
        > "cell_s${seed}.log" 2>&1 &
  pids+=($!)
  sleep 25
done
...
wait
echo "ALL 3 PARAM-MATCHED LIF RUNS DONE $(date -u +%FT%TZ)"

for seed in $SEEDS; do
  A="lif_sac_c30b_s${seed}_actor.npz"
