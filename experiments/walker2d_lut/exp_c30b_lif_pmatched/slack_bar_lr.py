...
Exits once SWEEP_DONE_LR exists or --max-hours passes. Deliberately not `pgrep -f`:
...
SEEDS = (0, 1, 2)
TOTAL = len(SEEDS)
SENTINEL = "SWEEP_DONE_LR"
REF = ("_PARAM-MATCHED LIF-detector actor, nap6/tph32, 48,193 params (-1.95% vs the "
       "49,152-param LUT actors; exp_c30 dense-P was 87,361). Anchors: exp_c18 "
       "hyperplane 4308 +/- 500 (6 seeds); exp_c30 dense-P LIF 3931 +/- 586._")


def read(seed):
    """(state, pct, text) for one cell."""
    log = os.path.join(HERE, f"cell_s{seed}.log")
    cpu = os.path.join(HERE, f"lif_sac_c30b_s{seed}_cpueval.json")
...
def render():
    ...
    if n_cpu:
        vals = [json.load(open(os.path.join(
            HERE, f"lif_sac_c30b_s{s}_cpueval.json")))["cpu_reference_mean"]
        ...
        lines += ["", f"*mean over {len(vals)} seeds: {m:.0f}" +
                  ...
                  f"  ·  exp_c18 hyperplane 4308 ± 500  ·  exp_c30 dense-P 3931 ± 586"]
...
def main():
    ...
    h = a.handle or progress.progress_start(
        "exp_c30b — PARAM-MATCHED LIF-detector actor, 3 seeds (#75)",
    ...
    while True:
        ...
        if finished or timeout:
            ...
            progress.progress_done(
                ...
                final_text=(f"exp_c30b param-matched LIF sweep "
