"""exp_c49 — self-updating Slack bar for the 3-seed MHL-LIF SAC sweep.
...
Exits once SWEEP_DONE_C49 exists or --max-hours passes. Deliberately not `pgrep -f`:
exp_c25's refresher matched its own command line and posted a frozen number for nine hours.

Usage:
  python slack_bar_c49.py --task <BODY_TASK id> [--handle <existing>] [--interval 150]
"""
import argparse
import json
import os
...
SEEDS = (0, 1, 2)
TOTAL = len(SEEDS)
SENTINEL = "SWEEP_DONE_C49"
REF = ("_*exp_c49 — c48 with temperatures UNFROZEN. The decisive isolation.* Identical to "
       "exp_c48 in every respect — current unified `LIFMultiHeadLUT`, 1 head × *128 "
       "tables × 1 LIF detector × 16 buckets*, per-table betas, stock table init 0.1, "
       "zero delays, SORT_FORM='rank', seeds 0/1/2 — with ONE change: "
       "*freeze_temperature=False*, so `log_T_cross` and `log_T_bkt` are live parameters "
       "and free to anneal, exactly as c36 had them.\n"
       "_*31,360 params, now ALL 31,360 trainable* (c48 had 256 frozen) = 6,784 front-end "
       "+ 24,576 table. Parity *99/99*, with the freeze checks INVERTED: the reference "
       "reports both temperatures `requires_grad=True`, both carry a live gradient "
       "(|grad|max 0.82 and 2.26), and our port matches those gradients to rel ~8e-07 — "
       "so a silently-zeroed temperature gradient cannot masquerade as an unfrozen run._\n"
       "_*The verdict this settles.* c48 (*2608 ± 1114*, 2/3) reproduced c36's two init "
       "settings on the current module and came in *1,638 below* c36 (*4246 ± 298*, 3/3: "
       "4527.5 / 3933.2 / 4277.6). But c48 differed from c36 in TWO ways — the module AND "
       "the temperature freeze. c49 removes the second. So:_\n"
       "_• *c49 ≈ 4246* → the temperature FREEZE caused the gap; the refactor is clean, "
       "and `freeze_temperature=True` is a real functional handicap on this addressing "
       "machinery._\n"
       "_• *c49 ≈ 2600* → the freeze is NOT the cause; the module becomes the prime "
       "suspect and a component-level bisect is warranted._\n"
       "_*Watch the temperature trajectory*: c36 annealed T_bkt 1.000 → *0.018* (55× "
       "sharper) and T_cross → 0.436. Does c49 reproduce that? `Tbkt`/`Tcr` are logged "
       "every eval and should now MOVE rather than sit pinned at 1.000._\n"
       "⚠️ _n=3 cannot resolve takeoff-rate differences (the c42b lesson) — read the MEAN._")


def read(seed):
    log = os.path.join(HERE, f"cell_s{seed}.log")
    cpu = os.path.join(HERE, f"mhl_sac_c49_s{seed}_cpueval.json")
...
def render():
    ...
    if n_cpu:
        vals = [json.load(open(os.path.join(
            HERE, f"mhl_sac_c49_s{s}_cpueval.json")))["cpu_reference_mean"]
...
def main():
    ...
    h = a.handle or progress.progress_start(
        "exp_c49 — c48 with temperatures UNFROZEN: the decisive isolation, 3 seeds",
        task=a.task, style="emoji", width=10)
    open(os.path.join(HERE, ".slack_c49.handle"), "w").write(h)
    ...
    while True:
        ...
        if finished or timeout:
            ...
            progress.progress_done(
                ...
                final_text=(f"exp_c49 MHL-LIF sweep "
