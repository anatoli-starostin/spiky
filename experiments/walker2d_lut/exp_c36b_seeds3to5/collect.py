"""exp_c36 — collect the CPU-reference results for the Bucket-LIF actor.
...
def main():
    rows = {}
    for s in SEEDS:
        p = os.path.join(HERE, f"bucket_sac_c36_s{s}_cpueval.json")
        rows[s] = json.load(open(p)) if os.path.exists(p) else None

    print("=== exp_c36 — Bucket-LIF actor, 16 buckets x 128 TABLES, 3 seeds ===")
