...
CFG = dict(input_dim=17, n_heads=1, n_outputs=12, tables_per_head=128)
N_BUCKETS = 16
BATCH = 24
...
def perturb(m, seed):
    ...
    with torch.no_grad():
        ...
        # w_raw, not w -- `w` is now a read-only property (w_max * sigmoid(w_raw)). Spread
        # around the hot init so the effective weights land across (0, w_max) rather than
...
def one_case(name, m, seed, dump):
    ...
    addr = m.address(x)
    # The headline diagnostic for this re-run: what fraction of (sample, table) pairs land
    # in the LAST bucket, i.e. never spiked. exp_c32's broken version sat near 0.97 at init
    # and the whole failure followed from it.
    nospike = float((addr == N_BUCKETS - 1).float().mean())
    ...
    print(f"          partition sums to 1: max|Σg - 1| = "
          ...
          f"boundaries increasing: {bool((bnd[:, 1:] > bnd[:, :-1]).all())}")
    print(f"          NO-SPIKE MASS (last bucket): {nospike:.3f}   "
          f"mean bucket {float(addr.float().mean()):.2f}   "
          f"effective w in [{float(m.w.min()):.3f}, {float(m.w.max()):.3f}], "
          f"mean {float(m.w.mean()):.3f}   tau init {float(m.tau.mean()):.3f}")
