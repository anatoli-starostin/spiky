"""exp_c38 — dump the TORCH reference for LIFMultiHeadLUT.
...
THREE CASES, and each one exists to catch a specific class of port bug:
  ...
  run        The exact shipped configuration -- heads=1, tph=32, n_det=6, n_buckets=2,
  ...
  radix      A different shape entirely: heads=2, tph=3, n_det=2, n_buckets=8. At M=2 the
             soft partition's middle term S[..., :-1] - S[..., 1:] is EMPTY and the
             mixed-radix weights are the powers of two, which is the one arrangement where
             several plausible indexing mistakes coincide with the right answer. M=8 makes
             the middle term real and the radix [8, 1] rather than a bit-shift; n_heads=2
             checks the head/tph reshape is not silently transposed.
...
RUN = dict(input_dim=INPUT_DIM, n_heads=1, n_outputs=N_OUT, tables_per_head=32,
           n_det=6, n_buckets=2)
RADIX = dict(input_dim=INPUT_DIM, n_heads=2, n_outputs=N_OUT, tables_per_head=3,
             n_det=2, n_buckets=8)
...
def main():
    ...
    print(f"run   cfg {RUN}  freeze_temperature=True  delay_init_std=4")
    one_case("run", LIFMultiHeadLUT(**RUN, freeze_temperature=True,
                                    delay_init_std=4.0), RUN, 11, dump)
    print(f"perturbed cfg {RUN}  freeze_temperature=False")
    one_case("perturbed", perturb(LIFMultiHeadLUT(**RUN), 77), RUN, 13, dump)
    print(f"radix cfg {RADIX}  freeze_temperature=False")
    one_case("radix", perturb(LIFMultiHeadLUT(**RADIX), 99), RADIX, 17, dump)

    # The parameter budget of the shipped config, reported the way the chapter reports it.
    m = LIFMultiHeadLUT(**RUN, freeze_temperature=True, delay_init_std=4.0)
