"""exp_c38 — LIFMultiHeadLUT SAC: the exp_c09 LUT-SAC trainer with a multi-detector actor.

A fork of exp_c37/bucket_sac.py with the front-end module swapped. Everything else is
...
difference is attributable to the front-end. It carries the exp_profiling optimisations
(on-device coverage, jitted buffer insert, fused 32-update lax.scan) that exp_c37 introduced.

THE CONFIGURATION: 1 head, 32 tables, 6 LIF detectors per table, 2 buckets per detector,
frozen temperatures, delay_init_std=4.

PARAMETER COUNT: 31,744 total = 7,168 front-end + 24,576 table, of which 31,680 are
trainable (the 64 frozen temperature scalars are counted but never move).
...
WHERE IT SITS IN THE LINE. 2**6 = 64 cells per table over 32 tables is exactly exp_c31's
row layout and within 1.2% of exp_c36's parameter count, but it gets there a third way:

    c31  ONE LIF per table; its single spike time compared against 6 learned deadlines.
         The 6 bits are 6 views of one scalar, so they cannot be independent.
    c36  ONE LIF per table, 16 ordered buckets, 128 tables. Capacity from TABLE COUNT --
         the finding of that experiment was that per-table addressing entropy predicts
         nothing and the number of independent indices summed predicts everything.
    c38  SIX LIFs per table, each with its own delays, synapses and tau, each thresholded
         against its own boundary. 32 tables. The 6 bits are genuinely independent
         detectors, and the 64 cells are their mixed-radix product.

So c38 asks the question c36 left open from the other side: c36 bought independent indices
by adding TABLES (which also adds table rows, and sums their outputs); c38 buys them by
adding DETECTORS INSIDE a table (which multiplies the row count and does NOT add a summand).
If the c36 finding is really about the number of independent summands, c38 should land near
the 32-table runs (~2,000) despite having 128-table parameter count. If it is about the
number of independent BINARY DECISIONS available to the addressing, c38 should reach the
band. The two hypotheses make opposite predictions here, which is why this configuration
is worth the GPU.
...
COLD START. At init the single boundary per detector sits at 16.0, the middle of the
window, so each detector is an unbiased soft bit -- but the membrane must still cross
theta_mem = 1.0 to spike at all, and a non-firing detector folds into bucket 1 by
construction. The parity dump measures 20.2% no-spike mass at this init. `cell-cov`,
`eff-cells` and `bit1` are logged so a run that never leaves that corner is visible at the
first eval.
"""
...
def actor_init(key, n_buckets, n_det, tph, heads, delay_init_std):
...
def main():
    ...
    ap.add_argument("--ndet", type=int, default=6)
    ap.add_argument("--tph", type=int, default=32)
    ap.add_argument("--heads", type=int, default=1)
    ap.add_argument("--delay-init-std", type=float, default=4.0)
    ap.add_argument("--freeze-temperature", type=int, default=1,
    ...
    a = ap.parse_args()
    out_name = a.out or f"mhl_sac{a.tag}"
    ...
    print(f"MHL-LIF SAC actor heads={a.heads} tph={a.tph} n_det={a.ndet} "
          ...
          f"{n_tables*K:,} | delay_init_std={a.delay_init_std} | temperatures "
    ...
    def addr_stats(ap_, obs):
        """(bit-1 rate, no-spike rate, mean effective cells per table).

        bit1     mean over (sample, table, detector) of the hard digit. At 2 buckets this
                 is the fraction of detector tests firing "late". 0 or 1 means the
                 detectors have collapsed to a constant and the table is 1 row.
    ...
    for it in range(a.iters):
        ...
        if (it + 1) % a.eval_every == 0 or it == a.iters - 1:
            ...
            bit1, nosp, eff = [float(v) for v in addr_stats(ap_, st.obs)]
            ...
            rows_log.append(dict(iter=it + 1, env_steps=total_steps, mjx_return=ret,
                                 row_coverage=cov, bit1=bit1, nospike=nosp,
            ...
            print(f"[{it+1:>5}/{a.iters}] steps {total_steps:>9,} | MJX ret {ret:8.1f} "
                  f"| cell-cov {cov*100:5.1f}% | bit1 {bit1:5.3f} | nosp {nosp:5.3f} "
