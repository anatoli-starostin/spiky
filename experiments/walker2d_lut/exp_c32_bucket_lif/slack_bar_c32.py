"""exp_c32 — self-updating Slack bar for the 3-seed Bucket-LIF SAC sweep (#75).
...
Exits once SWEEP_DONE_C32 exists or --max-hours passes. Deliberately not `pgrep -f`:
...
SEEDS = (0, 1, 2)
TOTAL = len(SEEDS)
SENTINEL = "SWEEP_DONE_C32"
REF = ("_BucketLIFDetectorsMHL — ONE LIF neuron per table, row = which of 16 TIME BUCKETS "
       "its first spike lands in. No anchor pairs, no bit vector: the address is a "
       "monotone quantisation of one scalar, with trainable sorted boundaries. "
       "*7,840 params* = 1,696 front-end + 6,144 table — the smallest actor in the "
       "chapter, *28% of the 28,032 baseline* (front-end 49% of its 3,456, table 25% of "
       "its 24,576 since rows are 16 not 64). Parity vs torch: 40/40. Anchors: exp_c18 "
       "hyperplane 4308 ± 500 (6 seeds); c31 PureLIF 2951 ± 2109 (2 of 3 seeds ~4200); "
       "c30b 4087 ± 991; c30 3931 ± 586._")


def read(seed):
    log = os.path.join(HERE, f"cell_s{seed}.log")
    cpu = os.path.join(HERE, f"bucket_sac_c32_s{seed}_cpueval.json")
...
def render():
    ...
    if n_cpu:
        vals = [json.load(open(os.path.join(
            HERE, f"bucket_sac_c32_s{s}_cpueval.json")))["cpu_reference_mean"]
...
def main():
    ...
    h = a.handle or progress.progress_start(
        "exp_c32 — Bucket-LIF actor, 16 buckets × 32 tables, 3 seeds (#75)",
        task=a.task, style="emoji", width=10)
    open(os.path.join(HERE, ".slack_c32.handle"), "w").write(h)
