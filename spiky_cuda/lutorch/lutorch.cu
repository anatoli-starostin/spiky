...
__global__ void anchor_pairs_lookup_forward_na2_kernel(
    ...
    int64_t lookup_idx = 0;
    scalar_t min1_abs_delta = static_cast<scalar_t>(0);
    scalar_t min2_abs_delta = static_cast<scalar_t>(0);
    ...
    for (int64_t p = 0; p < n_anchor_pairs; ++p) {
        ...
        scalar_t abs_delta = lutorch_abs(delta);
        if ((p == 0) || (abs_delta < min1_abs_delta)) {
            ...
            min1_bit_pos = p;
        } else if ((p == 1) || (abs_delta < min2_abs_delta)) {
...
template <typename scalar_t>
__global__ void anchor_pairs_lookup_eval_forward_na2_kernel(
    const scalar_t* x_ptr,
    int64_t batch_size,
    int64_t x_stride0,
    int64_t x_stride1,
...
public:
    ...
    torch::Tensor
    anchor_pairs_lookup_eval_forward_na2(
        const torch::Tensor& x,
        const torch::Tensor& anchor_pairs_a,
        const torch::Tensor& anchor_pairs_b,
        double cmp_eps,
        ...
        AT_DISPATCH_FLOATING_TYPES(x.scalar_type(), "anchor_pairs_lookup_eval_forward_na2_kernel", [&] {
            anchor_pairs_lookup_eval_forward_na2_kernel<scalar_t><<<blocks, threads>>>(
                reinterpret_cast<const scalar_t*>(x.data_ptr()),
                batch_size,
                x_stride0,
                x_stride1,
...
void PB_LUTorchManager(py::module& m) {
    py::class_<LUTorchManager>(m, "LUTorchManager")
        ...
        .def(
            "anchor_pairs_lookup_eval_forward_na2",
            &LUTorchManager::anchor_pairs_lookup_eval_forward_na2,
            py::arg("x"),
            py::arg("anchor_pairs_a"),
            py::arg("anchor_pairs_b"),
            py::arg("cmp_eps"),
