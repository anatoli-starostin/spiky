...
class __attribute__((visibility("hidden"))) DenseToCOOConverterNative {
public:
    DenseToCOOConverterNative() 
        #ifdef ENABLE_PROFILING
        : profiler(N_TORCH_UTILS_PROFILER_OPS)
        #endif
    {
        #ifdef ENABLE_PROFILING
        profiler.register_operation_type(TORCH_UTILS_DENSE_TO_COO_PROFILER_OP, "torch_utils::dense_to_coo_32");
        #endif
    }
    ...
    void dense_to_coo_32(
        ...
        PROF_START(TORCH_UTILS_DENSE_TO_COO_PROFILER_OP);
        ...
        PROF_END(TORCH_UTILS_DENSE_TO_COO_PROFILER_OP);
    }
...
void PB_DenseToCOOConverter(py::module& m) {
    py::class_<DenseToCOOConverterNative>(m, "DenseToCOOConverterNative")
        .def(py::init<>())
        .def("dense_to_coo_32", &DenseToCOOConverterNative::dense_to_coo_32,
            "Convert dense 32-bit tensor to sparse COO format",
            py::arg("source"),
            py::arg("target_indices"),
            py::arg("target_values"),
            py::arg("counter_buffer"),
            py::arg("erase_input") = false)
        .def("get_profiling_stats", &DenseToCOOConverterNative::get_profiling_stats);
}
